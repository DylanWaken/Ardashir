...
def integration_chapters(api_link) -> dict[Path, str]:
    source = (ROOT / "Source/ArdaBackend/Tests/ArdaCudaTests.cpp").read_text(encoding="utf-8")
    def region(first: str, last: str) -> str:
    ...
    ptx = region('    const char* AddPtx', '    class FCudaDiagnostics')
    operand = region('    class FAddOperand', '    class FSelectionOperand')
    usage = region('    TEST_P(ArdaCudaGpu, OrderedKernels', '    TEST_P(ArdaCudaGpu, SurfaceWrites')
    mixed = region('    TEST_P(ArdaCudaGpu, GraphicsFallback', '    TEST_P(ArdaCudaGpu, RejectsInvalid')
    selection = region('    class FSelectionOperand', '    TEST_P(ArdaCudaGpu, CapabilityAndResource')
    fixture = region('    class FCudaDiagnostics', '    class FAddOperand')
    surface = region('    TEST_P(ArdaCudaGpu, SurfaceWrites', '    TEST_P(ArdaCudaGpu, TextureFormat')
    sections = [
        ("model", "One allocation, ordered graphics and CUDA access", '<p>A CUDA-compatible buffer or texture can have two API representations of the same allocation. Representation availability is an allocation fact. Read/write states and queue ordering are separate facts. All current CUDA work is recorded in the graphics API command stream; there is no CUDA-only queue owner to toggle.</p><p>Buffers supply a private linear device address, with view offset already applied. Textures supply a surface handle for one mip, including its array layers or depth. The public API deliberately supplies neither a general CUDA-runtime pointer conversion nor an OptiX handle conversion.</p><p>Start with ' + api_link('FArdaCudaCapabilities') + ' and inspect ' + api_link('FArdaCudaResourceInfo') + ' for each resource. CUDA device support does not imply that an ordinary graphics allocation was created for sharing.</p>' + code('const auto Cuda = Device->GetCudaCapabilities();\nconst auto Representation = Buffer->GetCudaResourceInfo();\nif (!Cuda || !Representation.mbSharingEnabled) {\n    // Choose a registered graphics implementation or report Unsupported.\n}')),
        ("build", "Enable CUDA or build without it", '<p>The default build excludes the CUDA provider. To enable D3D12 CiG, supply CUDA 13.3 or newer headers. Driver loading remains dynamic; installing headers does not prove that the running adapter supports CiG. Vulkan native kernel launch uses Vulkan extension declarations, not a CUDA runtime library.</p>' + code('cmake -S . -B build/cuda-on -DARDASHIR_ENABLE_CUDA=ON -DARDASHIR_CUDA_INCLUDE_DIR="C:/CUDA/include"\ncmake --build build/cuda-on --config Debug --target ArdaBackendTests\ncmake -S . -B build/cuda-off -DARDASHIR_ENABLE_CUDA=OFF', 'shell') + '<p>Use actual local SDK/build paths. Public RHI headers compile with either setting. An enabled build with missing/old required headers fails explicitly; an unavailable runtime device reports its reason through GetCudaCapabilities. A CUDA-only operand then returns Unsupported. Registered graphics alternatives remain usable in both builds.</p>'),
        ("resources", "Create buffers and qualify textures", '<p>Set mByteSize and usage before allocating a buffer, then set mbCudaInterop=true to request sharing. A request can fail: do not allocate an ordinary buffer and later change a flag. Both providers admit dedicated, nonempty device-local buffers. CPU-visible, sparse/tiled, virtual/placed, versioned and acceleration-structure storage buffers are rejected.</p>' + code('FArdaRHIBufferDesc Desc;\nDesc.mByteSize = 1024;\nDesc.mUsage = EArdaRHIBufferUsage::ShaderResource | EArdaRHIBufferUsage::UnorderedAccess;\nDesc.mbCudaInterop = true;\nauto Shared = Device->CreateBuffer(Desc);\nif (!Shared) {\n    // Inspect Shared.mStatus; choose a graphics allocation/alternative if appropriate.\n}') + '<p>Surfaces support raw R/RG/RGBA integer channels at 8/16/32 bits and floating-point channels at 16/32 bits: 24 formats. Qualified dimensions are 1D, 1D array, 2D, 2D array and 3D. Normalized/sRGB/BGRA/packed/compressed/depth, cubemap and multisample images are rejected. Native imports cannot request sharing without provider-owned export/mapping lifetime. All other <a href="resource-recipes.html">resource kinds</a>, including views, AS, micromaps, feedback objects, samplers and pipelines, remain non-CUDA objects.</p>'),
        ("packing", "Understand surface addressing and tensor packing", '<p>Surface instructions access typed storage; they do not sample/filter a texture and do not perform normalization, swizzling, sRGB conversion or tensor layout conversion. Use byte offsets in the surface x coordinate where the PTX instruction requires them. Mip dimensions, array layer indexing and 3D depth must match the selected binding.</p><p>A kernel expecting contiguous NCHW/NHWC tensors cannot consume an opaque tiled image as a flat pointer. Use an explicit surface-to-buffer packing operator and record it before the tensor operator. Keep format/layout conversion in that operator so the generic binding contract stays independent of the tensor library.</p><p>The source-backed surface example below writes a nonzero mip and checks it through graphics readback:</p><details><summary>Surface kernel, binding and readback example</summary>' + code(surface) + '</details>'),
        ("operand", "Register one or several kernels", '<p>Inherit ' + api_link('FArdaComputeOperand') + '. Declare ordered named ports once, including buffer/surface kind, access, minimum buffer size and offset alignment. Optional surface constraints select format/dimension. The base class applies the port access to a copy of each invocation, then validates it before calling either implementation.</p><p>The sample registers two ordered CUDA launches with ' + api_link('FArdaComputeOperand::RegisterCuda') + '. The first adds 7 to the input; the second adds 11 in place, so the result is input + 18. The builder owns PTX and copied argument bytes. Its SM bounds use major × 10 + minor; SM 12.0 is 120. Register preferred implementations first.</p>' + code(operand) + '<details><summary>PTX source used by the operand</summary>' + code(ptx) + '</details><p>The resource parameters are 64-bit binding values; scalar parameters must have exactly the PTX-declared size/layout. Value&lt;T&gt; copies trivially copyable bytes; it does not marshal strings, C++ objects or arbitrary library descriptors.</p>'),
        ("dispatch", "Record, submit and check results", '<p>' + api_link('FArdaComputeInvocation') + ' supplies one binding per port, logical extent and application-defined parameter byte arrays. Override ValidateInvocation for exact shape, dtype and parameter schema rules. Generic range checks cannot prove that arbitrary PTX stays inside a buffer.</p><p>Dispatch returns the selected variant name or a status. It records work on an open list; Close and ExecuteCommandList submit it. The example uses offset views, non-multiple work sizes and guard bytes, then checks each returned element:</p>' + code(usage) + '<details><summary>Fixture initialization and BufferDesc helper</summary>' + code(fixture) + '</details>'),
        ("fallback", "Provide the compute-shader alternative", '<p>' + api_link('FArdaComputeOperand::RegisterGraphics') + ' stores a callback that creates its binding set, sets resource states, binds a compute pipeline and records Dispatch. The same validated ports/parameters reach the callback. It must implement the same operation semantics as the CUDA sequence.</p><p>Auto chooses the first eligible registration. RequireGraphics excludes CUDA; RequireCuda excludes graphics. Ordinary nonshared buffers exclude CUDA even on an enabled device. A pinned variant must still satisfy architecture, resource and queue checks. An error after selecting/recording an implementation is propagated: automatic retry could duplicate partially recorded writes.</p><details><summary>Complete graphics → CUDA/graphics → graphics example</summary>' + code(mixed) + '</details><p>The checked result is input + 54 after three +18 operations. The source HLSL alternative is:</p>' + code((ROOT / 'Source/ArdaBackend/Tests/ArdaCudaFallback.hlsl').read_text(encoding='utf-8'), 'hlsl')),
        ("tuning", "Select variants without mutating live inputs", '<p>Override ' + api_link('FArdaComputeOperand::SelectVariant') + ' to choose an index from the supplied eligible list, using a device/workload tuning cache. The default preserves registration order. Out-of-set indices are rejected. Benchmark with scratch resources before normal dispatch; selection itself must not launch candidate kernels on live outputs.</p><p>Construct and register before publishing the operand. Concurrent Dispatch calls require independently synchronized command lists and thread-safe callbacks. The selection example deliberately rejects duplicate names and an invalid tuning result:</p>' + code(selection)),
        ("native", "Native execution and lifetime limits", '<p><strong>D3D12:</strong> the provider matches the adapter LUID and CUDA device, creates a CiG context on the graphics queue, imports shared committed memory and captures kernels with cuStreamBeginCaptureToCig/cuStreamEndCaptureToCig. UAV barriers surround capture. Streams, mappings and executable modules are retained until graphics-fence retirement. Submit or discard a captured list before recording another on the same context; captured lists are single-use.</p><p><strong>Vulkan:</strong> VK_NV_cuda_kernel_launch records kernels directly on a graphics/compute command buffer. Buffer device addresses are used only by this native launch path; they are not advertised as arbitrary CUDA-runtime pointers. VK_NVX_image_view_handle supplies 64-bit storage-image surface handles. Memory barriers order kernels and graphics consumers, and existing submission retention owns resources/modules until completion. Copy queues are unsupported.</p><p>D3D12 checks kernel parameter metadata. This Vulkan path has no equivalent parameter-metadata query; custom PTX authors remain responsible for matching argument ABI. Both providers validate declared ranges, dimensions and launch limits.</p>'),
        ("qualification", "Verified support and failure diagnosis", '<p>Qualification used Windows, RTX PRO 6000 Blackwell (SM 120), driver 610.62 / CUDA driver 13.3 and CUDA 13.3.29 headers. Buffer kernels and mixed graphics/CUDA work passed on both APIs. Vulkan surface reads/writes passed all 24 formats × 5 dimensions, plus nonuniform 2D addressing and allocation variations.</p><p><strong>D3D12 CiG surfaces are unavailable on this tested driver:</strong> image import succeeds but mapping to a mipmapped array fails in a CiG context. The shipping provider probes this and disables surface admission with a reason; graphics texture alternatives remain usable. This is not claimed as a working D3D12 image path. Other OS/GPU/driver combinations and independent CUDA streams are not qualified by these results.</p><dl><dt>No eligible variant</dt><dd>Inspect policy/pinning, SM bounds, queue type and each allocation’s sharing state; register or choose an explicit alternative.</dd><dt>InvalidArgument before recording</dt><dd>Check port order, buffer ranges, offset alignment, extent, scalar ABI and launch dimensions.</dd><dt>Capture failure or replay rejection</dt><dd>Discard/reset the failed recording and obey CiG capture/submission order; do not retry inside the partially recorded list.</dd><dt>GPU test skipped</dt><dd>Distinguish missing validation from unavailable CUDA capability. See <a href="validation.html">validation setup</a>; a skip is not successful GPU execution.</dd></dl><p>Run <code>ArdaBackendTests --gtest_filter=*ArdaCudaGpu*</code> in CUDA-on and CUDA-off builds. RDG node registration and scheduling integration remains a separate next step.</p>'),
    ]
    cuda_page = page("CUDA and graphics compute", "Register reusable operands with CUDA kernels and compute-shader alternatives while preserving resource ownership and graphics submission ordering.", sections, "cuda-command-stream.svg")
    validation_sections = [
