...
namespace arda
{
	namespace
	{
		FArdaShaderDirectoryContext& GetRegistry()
		{
			static FArdaShaderDirectoryContext Registry;
			return Registry;
		}
		...
		FArdaShaderDirectoryStatus& Publish(FArdaShaderDirectoryContext& Registry, FArdaShaderDirectoryStatus Status)
		{
		...
		FArdaShaderDirectoryStatus RegistryMutationStatus(FArdaShaderDirectoryContext& Registry)
		{
		...
		FArdaShaderDirectoryStatus AddDirectory(const std::filesystem::path& RealDirectory,
		    ...
		    bool Exclusive)
		{
			FArdaShaderDirectoryContext& Registry = GetRegistry();
			...
			for (const auto& Existing : Registry.mDirectories)
			{
				...
				const bool PhysicalOverlap = fileops::IsPathContainedBy(CanonicalDirectory, Existing.mPhysicalDirectory, true) ||
				    fileops::IsPathContainedBy(Existing.mPhysicalDirectory, CanonicalDirectory, true);
				...
				while (Iterator != End)
				{
					...
					else if (Entry.is_regular_file(Error))
					{
						...
						if (HasAcceptedExtension(ToEastl(Relative.generic_string())))
						{
							...
							if (!fileops::IsPathContainedBy(CanonicalFile, Directory.mPhysicalDirectory, false))
							{
