...
namespace arda
{
	namespace
	{
		FArdaGlobalShaderMapDiagnostic MakeDiagnostic(EArdaGlobalShaderMapError Code,
		...
		void AttachSource(FArdaGlobalShaderMapDiagnostic& Diagnostic, const FArdaShaderType& Type)
		{
	...
	const char* GetShaderArtifactExtension(const char* BackendName) noexcept
	{
		...
		return Module ? Module->GetDescriptor().mShaderArtifactExtension.c_str() : "";
	}

	FArdaShaderBytecodeResult LoadShaderBytecode(const std::filesystem::path& Path)
	{
		FArdaShaderBytecodeResult Result;
		const auto ReadResult = fileops::ReadBinaryFile(Path, Result.mBytecode);
		if (ReadResult == fileops::EArdaFileReadResult::OpenFailed)
		{
			Result.mDiagnostic = MakeDiagnostic(EArdaGlobalShaderMapError::BytecodeMissing,
			    nullptr, Path, eastl::string("Unable to open shader artifact: ") + ToEastlString(Path));
		}
		else if (ReadResult == fileops::EArdaFileReadResult::ReadFailed)
		{
			Result.mDiagnostic = MakeDiagnostic(EArdaGlobalShaderMapError::BytecodeReadFailed,
			    nullptr, Path, eastl::string("Unable to read shader artifact: ") + ToEastlString(Path));
		}
		else if (Result.mBytecode.empty())
		{
			Result.mDiagnostic = MakeDiagnostic(EArdaGlobalShaderMapError::BytecodeEmpty,
			    nullptr, Path, eastl::string("Shader artifact is empty: ") + ToEastlString(Path));
		}
		return Result;
	}

	bool FArdaGlobalShaderMap::Initialize(arda::FArdaRHIDeviceRef Device, const std::filesystem::path& ShaderDirectory)
	{
		std::lock_guard<std::mutex> Lock(mLoadMutex);
		std::error_code DirectoryError;
		const std::filesystem::path ResolvedDirectory = ShaderDirectory.empty()
		    ? std::filesystem::path{}
		    : std::filesystem::absolute(ShaderDirectory, DirectoryError).lexically_normal();
		if (DirectoryError || ResolvedDirectory.empty())
		{
			mDiagnostics.clear();
			mDiagnostics.push_back(MakeDiagnostic(EArdaGlobalShaderMapError::BytecodeMissing,
			    nullptr,
			    ShaderDirectory,
		...
		if (mbInitialized && mDevice == Device && mTarget.mBackendName == Configuration.mBackendName &&
		...
		if (mbInitialized)
		{
			mDiagnostics.push_back(MakeDiagnostic(EArdaGlobalShaderMapError::ResetRequired,
			    nullptr,
			    ResolvedDirectory,
		...
		if (!Device)
		{
			mDiagnostics.push_back(MakeDiagnostic(EArdaGlobalShaderMapError::InvalidDevice,
			    nullptr,
			    {},
		...
		if (!Registration)
		{
			mDiagnostics.push_back(
			    MakeDiagnostic(EArdaGlobalShaderMapError::RegistrationFailed, nullptr, {}, Registration.mMessage));
			return false;
		}
		...
		if (!bResolvedTarget)
		{
			mDiagnostics.push_back(MakeDiagnostic(EArdaGlobalShaderMapError::RegistrationFailed,
			    nullptr,
			    {},
			    "The device backend module has no registered shader target."));
			return false;
		}
	...
	bool FArdaGlobalShaderMap::Initialize(arda::FArdaRHIDeviceRef Device)
	{
		return Initialize(eastl::move(Device), GetBackendConfiguration().mShaderCacheDirectory);
	}
	...
	bool FArdaGlobalShaderMap::EnsureSlotLoadedLocked(size_t Index) const
	{
		...
		if (mMode != EArdaShaderCompilationMode::LoadOnly)
		{
			const FArdaShaderCompileResult CompileResult =
			    EnsureRegisteredShaderArtifact(Type, mTarget.mBackendName.c_str(), Shader.mPermutationId, mDirectory);
			if (!CompileResult)
			{
				const FArdaShaderCompileDiagnostic& CompilerDiagnostic = CompileResult.mDiagnostics.front();
				const bool Missing = CompilerDiagnostic.mCode == EArdaShaderCompileError::ArtifactMissing ||
				    CompilerDiagnostic.mCode == EArdaShaderCompileError::ArtifactOutdated;
				auto Diagnostic = MakeDiagnostic(Missing ? EArdaGlobalShaderMapError::BytecodeMissing
				                                         : EArdaGlobalShaderMapError::ArtifactCompileFailed,
				    Type.GetName(),
				    Path,
				    CompilerDiagnostic.mMessage);
				if (!CompilerDiagnostic.mSourcePath.empty())
				{
					Diagnostic.mPhysicalSource = ToEastlString(CompilerDiagnostic.mSourcePath);
				}
		...
		if (!ShaderResult)
		{
			auto Diagnostic = MakeDiagnostic(EArdaGlobalShaderMapError::ShaderCreationFailed,
			    Type.GetName(),
			    Path,
		...
		if (const FArdaShaderParameterMetadata* Metadata = Type.GetParameterMetadata())
		{
			eastl::vector<arda::FArdaRHIBindingLayoutDesc> LayoutDescs;
			const FArdaShaderStructStatus LayoutStatus = Metadata->BuildBindingLayoutDescs(LayoutDescs);
			if (!LayoutStatus)
			{
				auto Diagnostic = MakeDiagnostic(EArdaGlobalShaderMapError::LayoutCreationFailed,
				    Type.GetName(),
				    Path,
	...
	const FArdaGlobalShaderInstance* FArdaGlobalShaderMap::Find(const FArdaShaderType& Type,
	...
	const FArdaGlobalShaderInstance* FArdaGlobalShaderMap::Find(const eastl::string& Name, uint32_t PermutationId) const
	{
		std::lock_guard<std::mutex> Lock(mLoadMutex);
		for (size_t Index = 0; Index < mShaders.size(); ++Index)
		{
			const FArdaGlobalShaderInstance& Shader = mShaders[Index];
			if (Name == Shader.GetType().GetName() && Shader.GetPermutationId() == PermutationId)
			{
				return EnsureSlotLoadedLocked(Index) ? &mShaders[Index] : nullptr;
			}
		}
	...
	eastl::vector<FArdaGlobalShaderInstance> FArdaGlobalShaderMap::Enumerate() const
	{
		std::lock_guard<std::mutex> Lock(mLoadMutex);
		for (size_t Index = 0; Index < mShaders.size(); ++Index)
		{
	...
	eastl::vector<FArdaGlobalShaderMapDiagnostic> FArdaGlobalShaderMap::GetDiagnostics() const
	{
		std::lock_guard<std::mutex> Lock(mLoadMutex);
		return mDiagnostics;
	}
