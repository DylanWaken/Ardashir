/** Shared facade casts, state tracking and binding validation. */
#pragma once
...
namespace arda::detail
{
	...
	inline size_t TextureStateIndex(const FArdaRHITextureDesc& Desc,
	    uint32_t MipLevel,
	    uint32_t ArraySlice,
	...
	inline void StoreFacadeTextureState(eastl::vector<EArdaRHIResourceState>& States,
	...
	inline TArdaRHIResult<EArdaRHIResourceState> LoadFacadeTextureState(const eastl::vector<EArdaRHIResourceState>& States,
	    const FArdaRHITextureDesc& Desc,
	    const FArdaRHITextureSubresourceRange& InputRange)
	{
		const auto Range = InputRange.Resolve(Desc);
	...
	inline FArdaProviderObjectRef GetNativeObject(IArdaRHIResource* Resource)
	{
	...
	inline FArdaRHIStatus ResolveBindingItem(const FArdaRHIBindingLayoutDesc& Layout,
	...
	inline size_t PushConstantCapacity(const eastl::vector<FArdaRHIBindingLayoutRef>& Layouts)
	{
	...
	inline TArdaRHIResult<FArdaRHIBindingItem> MakeCollectionBinding(const FArdaRHIResourceCollectionItem& Item,
