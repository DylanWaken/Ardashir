...
namespace arda
{
	namespace
	{
		...
		bool IsExternal(const FArdaMemoryRequest& Request)
		{
			return Request.mExternalBuffer || Request.mExternalTexture || Request.mExternalAccelerationStructure;
		}

		const void* ExternalIdentity(const FArdaMemoryRequest& Request)
		{
		...
		FArdaRHIMemoryAllocationInfo ExternalAllocationInfo(const FArdaMemoryRequest& Request)
		{
		...
		FArdaRHIStatus CountExternalAllocations(const eastl::vector<FArdaMemoryRequest>& Requests,
		    bool bRequireKnown,
		    uint64_t& Total)
		{
			struct FArdaCounted
			...
			eastl::vector<FArdaCounted> Counted;
			Total = 0;
			...
			for (const auto& Request : Requests)
			{
				...
				const auto Kind = Request.mKind == EArdaMemoryKind::Buffer ? EArdaRHIResourceType::Buffer
				    : Request.mKind == EArdaMemoryKind::Texture            ? EArdaRHIResourceType::Texture
		...
		bool PlacementDescriptorAllowed(const FArdaMemoryRequest& Request)
		{
			if (!Request.mbAllowHeapPlacement || Request.mbPersistent || IsExternal(Request))
			{
				return false;
			}
			if (Request.mKind == EArdaMemoryKind::Buffer)
			{
		...
		bool Compatible(const FArdaMemoryRequest& Left, const FArdaMemoryRequest& Right)
		{
			if (Left.mKind != Right.mKind || !SameRequirements(Left.mRequirements, Right.mRequirements))
			{
				return false;
			}
			if (Left.mKind == EArdaMemoryKind::AccelerationStructure)
			{
				return ExternalIdentity(Left) == ExternalIdentity(Right);
			}
			return Left.mKind == EArdaMemoryKind::Buffer
			    ? StorageDesc(Left.mBufferDesc) == StorageDesc(Right.mBufferDesc)
			    : StorageDesc(Left.mTextureDesc) == StorageDesc(Right.mTextureDesc);
		}

		FArdaRHIStatus ValidateRequests(const eastl::vector<FArdaMemoryRequest>& Requests, bool bRequireSizes)
		{
			...
			for (const auto& Request : Requests)
			{
				...
				if ((Request.mKind != EArdaMemoryKind::Buffer && Request.mKind != EArdaMemoryKind::Texture &&
				        Request.mKind != EArdaMemoryKind::AccelerationStructure) ||
				    (Request.mKind == EArdaMemoryKind::Buffer &&
				...
				if ((Request.mKind == EArdaMemoryKind::Buffer &&
				    ...
				    (Request.mKind == EArdaMemoryKind::Texture && Request.mTextureDesc.mbTiled))
				{
					return FArdaRHIStatus::Error(EArdaRHIResult::Unsupported,
		...
		bool CanOrderReuse(const FArdaMemoryRequest& Previous,
		    const FArdaMemoryRequest& Next,
		...
		void AddAliasEdges(FArdaMemoryPlan& Plan, const FArdaMemoryRequest& Previous, const FArdaMemoryRequest& Next)
		{
			for (uint32_t Producer : Previous.mLastUseNodes)
			{
				for (uint32_t Consumer : Next.mFirstUseNodes)
	...
	FArdaRHIStatus DescribeArdaMemory(IArdaRHIDevice& Device, eastl::vector<FArdaMemoryRequest>& Requests)
	{
		...
		for (auto& Request : Described)
		{
			...
			else if (Request.mKind == EArdaMemoryKind::Buffer)
			{
				Requirements =
	...
	TArdaRHIResult<FArdaMemoryPlan> PlanArdaMemory(const eastl::vector<FArdaMemoryRequest>& Requests,
	    const FArdaMemoryOptions& Options)
	{
		...
		for (uint32_t Identifier : Order)
		{
			...
			if (Request.mbUseHeapPlacement)
			{
				...
				if (BestHeap == UINT32_MAX)
				{
					...
					Heap.mDesc.mMemoryTypeBits = Request.mRequirements.mMemoryTypeBits;
					Heap.mDesc.mDebugName = "Allocator frame heap";
				...
				for (uint32_t Prior : Heap.mResources)
				{
					        ...
					        Plan.mResourceBytes[Prior]))
					{
						...
						if (Request.mKind == EArdaMemoryKind::Texture)
						{
				...
				if (Request.mKind == EArdaMemoryKind::Buffer || Request.mFirstUseNodes.size() == 1)
				{
	...
	TArdaRHIResult<FArdaMemoryResources> MaterializeArdaMemory(IArdaRHIDevice& Device,
	    const FArdaMemoryPlan& Plan,
	    const FArdaMemoryResources* Shared)
	{
		...
		FArdaMemoryResources Resources;
		...
		for (const auto& Slot : Plan.mSlots)
		{
			...
			if (!Buffer && !Texture && !AccelerationStructure)
			{
				if (Request.mKind == EArdaMemoryKind::Buffer)
				{
					const auto Desc = StorageDesc(Request.mBufferDesc, Request.mbUseHeapPlacement);
					auto Created = Request.mbUseHeapPlacement
					    ? Device.CreatePlacedBuffer(Desc, Resources.mHeaps[Slot.mHeapIndex], Slot.mOffset)
				...
				{
					const auto Desc = StorageDesc(Request.mTextureDesc, Request.mbUseHeapPlacement);
					auto Created = Request.mbUseHeapPlacement
					    ? Device.CreatePlacedTexture(Desc, Resources.mHeaps[Slot.mHeapIndex], Slot.mOffset)
