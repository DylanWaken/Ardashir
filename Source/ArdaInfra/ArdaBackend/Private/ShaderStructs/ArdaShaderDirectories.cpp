...
namespace arda
{
	namespace
	{
		...
		FArdaDirectoryRegistry& GetRegistry()
		{
			static FArdaDirectoryRegistry Registry;
			return Registry;
		}
		...
		FArdaShaderDirectoryStatus MakeStatus(EArdaShaderDirectoryError Code,
		...
		FArdaShaderDirectoryStatus& Publish(FArdaDirectoryRegistry& Registry, FArdaShaderDirectoryStatus Status)
		{
		...
		FArdaShaderDirectoryStatus ValidateVirtualPath(const eastl::string& Path, bool IsRoot)
		{
			    ...
			    (Value.size() > 1 && Value.back() == '/'))
			{
				return MakeStatus(EArdaShaderDirectoryError::InvalidVirtualPath,
				    eastl::string("Virtual shader path must be normalized, absolute, and use forward slashes: ") + Path,
				    Path);
			}
			...
			while (Start <= Value.size())
			{
				...
				if (Component == "." || Component == ".." || Component.empty())
				{
					...
					{
						return MakeStatus(EArdaShaderDirectoryError::InvalidVirtualPath,
						    eastl::string("Virtual shader path contains an invalid component: ") + Path,
						    Path);
					}
				}
				for (const unsigned char Character : Component)
				{
					if (!IsPortableVirtualCharacter(Character))
					{
						return MakeStatus(EArdaShaderDirectoryError::InvalidVirtualPath,
						    eastl::string(
						        "Virtual shader path contains a non-portable character; use ASCII letters, digits, '_', '-', or '.': ") +
						        Path,
						    Path);
					}
				}
			...
			{
				return MakeStatus(EArdaShaderDirectoryError::UnsupportedExtension,
				    eastl::string("Virtual shader source has an unsupported extension: ") + Path,
				    Path);
			}
			return {};
		}
		...
		{
#if defined(_WIN32)
			return PortableKey(Left.generic_string()) == PortableKey(Right.generic_string());
#else
			return Left == Right;
#endif
		}
		    ...
		    const FArdaShaderSourceDirectory& Child)
		{
			...
			if (!IsPathContainedBy(Child.mPhysicalDirectory, Parent.mPhysicalDirectory, false) ||
		...
		FArdaShaderDirectoryStatus RegistryMutationStatus(FArdaDirectoryRegistry& Registry)
		{
			if (Registry.mbRegistryInUse)
			{
				return MakeStatus(EArdaShaderDirectoryError::RegistryInUse,
				    "Shader source directory registry is in use by backend initialization.");
			}
			if (Registry.mbBackendInitialized)
			{
				return MakeStatus(EArdaShaderDirectoryError::BackendInitialized,
				    "Shader source directories cannot be changed while the backend is initialized.");
			}
			return {};
		}

		FArdaShaderDirectoryStatus CanonicalizeDirectory(const std::filesystem::path& Input,
		    std::filesystem::path& Output)
		{
			...
			if (Error)
			{
				return MakeStatus(EArdaShaderDirectoryError::InvalidPhysicalDirectory,
			...
			{
				return MakeStatus(EArdaShaderDirectoryError::InvalidPhysicalDirectory,
				    ToEastl("Shader source directory does not exist or is not a directory: " + Absolute.string()));
			}
			return {};
		}
		...
		FArdaShaderDirectoryStatus AddDirectory(const std::filesystem::path& RealDirectory,
		    ...
		    bool Exclusive)
		{
			FArdaDirectoryRegistry& Registry = GetRegistry();
			std::lock_guard<std::mutex> Lock(Registry.mMutex);
			FArdaShaderDirectoryStatus MutationStatus = RegistryMutationStatus(Registry);
			if (!MutationStatus)
			{
			...
			if (Registry.mbFrozen)
			{
				return Publish(Registry,
				    MakeStatus(EArdaShaderDirectoryError::Frozen,
				        "Shader source directories are frozen until backend shutdown."));
			}

			FArdaShaderDirectoryStatus VirtualStatus = ValidateVirtualPath(VirtualRoot, true);
			if (!VirtualStatus)
			{
			...
			FArdaShaderDirectoryStatus PhysicalStatus = CanonicalizeDirectory(RealDirectory, CanonicalDirectory);
			if (!PhysicalStatus)
			{
			...
			for (const auto& Existing : Registry.mDirectories)
			{
				...
				const bool SamePhysical = IsPathContainedBy(CanonicalDirectory, Existing.mPhysicalDirectory, true) &&
				    IsPathContainedBy(Existing.mPhysicalDirectory, CanonicalDirectory, true);
				...
				{
					auto Status = MakeStatus(EArdaShaderDirectoryError::DuplicateMappingRoot,
					    eastl::string("Duplicate exclusive shader mapping root: ") + VirtualRoot,
					    VirtualRoot);
					Status.mPhysicalDirectories.push_back(Existing.mPhysicalDirectory);
					Status.mPhysicalDirectories.push_back(CanonicalDirectory);
					return Publish(Registry, eastl::move(Status));
				}

				const bool PhysicalOverlap = IsPathContainedBy(CanonicalDirectory, Existing.mPhysicalDirectory, true) ||
				    IsPathContainedBy(Existing.mPhysicalDirectory, CanonicalDirectory, true);
				if (PhysicalOverlap && !IsProperlyNestedRegistration(Existing, Candidate) &&
	...
	FArdaShaderDirectoryStatus AddShaderSourceDirectory(const std::filesystem::path& RealDirectory,
	    const eastl::string& VirtualRoot)
	{
		return AddDirectory(RealDirectory, VirtualRoot, false);
	}

	FArdaShaderDirectoryStatus AddShaderSourceDirectoryMapping(const eastl::string& VirtualRoot,
	...
	namespace
	{
		FArdaShaderDirectoryStatus ScanAndFreezeImpl(bool BackendAccess)
		{
			...
			{
				std::error_code Error;
				std::filesystem::recursive_directory_iterator Iterator(Directory.mPhysicalDirectory,
				...
				if (Error)
				{
					return Publish(Registry,
					    MakeStatus(EArdaShaderDirectoryError::ScanFailed,
					        ToEastl(
					            "Unable to scan shader source directory: " + Directory.mPhysicalDirectory.string())));
				}
				...
				while (Iterator != End)
				{
					...
					if (Error)
					{
						return Publish(Registry,
						    MakeStatus(EArdaShaderDirectoryError::ScanFailed,
						        ToEastl("Unable to inspect shader source path: " + Entry.path().string())));
					}
					const bool IsSymlink = std::filesystem::is_symlink(SymlinkStatus);
					...
					else if (Entry.is_regular_file(Error))
					{
						if (Error)
						{
							return Publish(Registry,
							    MakeStatus(EArdaShaderDirectoryError::ScanFailed,
							        ToEastl("Unable to inspect shader source file: " + Entry.path().string())));
						}
						const std::filesystem::path Relative =
						    Entry.path().lexically_relative(Directory.mPhysicalDirectory);
						if (Relative.empty())
						{
							return Publish(Registry,
							    MakeStatus(EArdaShaderDirectoryError::ScanFailed,
						...
						if (HasAcceptedExtension(Relative.generic_string()))
						{
							...
							if (IsShadowedByNestedExclusiveRoot(Directory, Registry.mDirectories, ToStd(VirtualPath)))
							{
								...
								if (Error)
								{
									return Publish(Registry,
									    MakeStatus(EArdaShaderDirectoryError::ScanFailed,
									        ToEastl("Unable to continue scanning shader source directory: " +
									            Directory.mPhysicalDirectory.string())));
								}
								continue;
							}
							const std::filesystem::path CanonicalFile = std::filesystem::canonical(Entry.path(), Error);
							if (Error)
							{
								return Publish(Registry,
								    MakeStatus(EArdaShaderDirectoryError::ScanFailed,
								        ToEastl("Unable to canonicalize shader source file: " + Entry.path().string()),
								        VirtualPath));
							}
							if (!IsPathContainedBy(CanonicalFile, Directory.mPhysicalDirectory, false))
							{
								auto Status = MakeStatus(EArdaShaderDirectoryError::EscapesPhysicalDirectory,
								    ToEastl("Shader source symlink escapes its registered physical directory: " +
								        Entry.path().string() + " -> " + CanonicalFile.string()),
								    VirtualPath);
								Status.mPhysicalPaths.push_back(Entry.path());
								Status.mPhysicalPaths.push_back(CanonicalFile);
								Status.mPhysicalDirectories.push_back(Directory.mPhysicalDirectory);
								return Publish(Registry, eastl::move(Status));
							}
							Candidates.push_back({VirtualPath, CanonicalFile, Directory.mPhysicalDirectory});
						}
					}
					else if (Error)
					{
			...
			for (size_t Index = 0; Index < Candidates.size(); ++Index)
			{
				...
				if (VirtualExisting != VirtualFiles.end())
				{
					...
					auto Status = MakeStatus(EArdaShaderDirectoryError::DuplicateVirtualPath,
					    eastl::string("Duplicate virtual shader source '") + Candidate.mVirtualPath + "' from '" +
					...
					Status.mPhysicalDirectories.push_back(Existing.mPhysicalDirectory);
					Status.mPhysicalDirectories.push_back(Candidate.mPhysicalDirectory);
					return Publish(Registry, eastl::move(Status));
				}
				    ...
				    PortableKey(ToStd(Candidates[PhysicalExisting->second].mVirtualPath)) != VirtualKey)
				{
					...
					auto Status = MakeStatus(EArdaShaderDirectoryError::AmbiguousPhysicalFile,
					    eastl::string("Physical shader source is mounted at both '") + Existing.mVirtualPath +
					        "' and '" + Candidate.mVirtualPath + "': " + ToEastl(Candidate.mPhysicalPath.string()),
					    Candidate.mVirtualPath);
					Status.mPhysicalPaths.push_back(Candidate.mPhysicalPath);
					return Publish(Registry, eastl::move(Status));
				}
	...
	eastl::vector<FArdaShaderSourceDirectory> EnumerateShaderSourceDirectories()
	{
		FArdaDirectoryRegistry& Registry = GetRegistry();
		std::lock_guard<std::mutex> Lock(Registry.mMutex);
		...
		eastl::sort(Result.begin(),
		    ...
		    [](const auto& Left, const auto& Right)
		    {
			    ...
			    return Left.mPhysicalDirectory.generic_string() < Right.mPhysicalDirectory.generic_string();
		    });
		return Result;
	}
	...
	FArdaShaderDirectoryStatus ResolveVirtualShaderSource(const eastl::string& VirtualPath,
	    std::filesystem::path& OutPhysicalPath)
	{
		OutPhysicalPath.clear();
		FArdaDirectoryRegistry& Registry = GetRegistry();
		std::lock_guard<std::mutex> Lock(Registry.mMutex);
		const FArdaShaderDirectoryStatus Validation = ValidateVirtualPath(VirtualPath, false);
		if (!Validation)
		{
		...
		{
			return Publish(Registry,
			    MakeStatus(EArdaShaderDirectoryError::NotFrozen,
		...
		return Publish(Registry,
		    MakeStatus(EArdaShaderDirectoryError::MissingVirtualSource,
		        eastl::string("Virtual shader source is not present in the frozen manifest: ") + VirtualPath,
		        VirtualPath));
	}
	...
	FArdaShaderDirectoryStatus ScanAndFreezeShaderSourceDirectoriesForBackend()
	{
		FArdaDirectoryRegistry& Registry = GetRegistry();
		{
			std::lock_guard<std::mutex> Lock(Registry.mMutex);
			...
			{
				return Publish(Registry,
				    MakeStatus(EArdaShaderDirectoryError::RegistryInUse,
