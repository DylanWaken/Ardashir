...
namespace arda
{
	namespace
	{
		...
		FArdaShaderRegistry& GetRegistry()
		{
			static FArdaShaderRegistry Registry;
			return Registry;
		}
		...
		uint64_t HashIdentity(const FArdaShaderType& Type)
		{
			...
			const auto Append = [&Hash](const char* Text)
			{
				...
				AppendArdaFnv1a64(Hash, &Delimiter, sizeof(Delimiter));
			};
		...
		FArdaShaderRegistrationStatus Error(EArdaShaderRegistrationError Code, const eastl::string& Message)
		{
			return {Code, Message};
		}

		bool DefaultShouldCompilePermutation(const FArdaShaderPermutationParameters&)
		{
			return true;
		}

		void DefaultModifyCompilationEnvironment(const FArdaShaderPermutationParameters&,
		...
		bool IsPortableArtifactStem(const char* Stem)
		{
			...
			if (Stem == nullptr || Stem[0] == '\0' || Stem[0] == '.' ||
			    !(IsAsciiAlphaNumeric(static_cast<unsigned char>(Stem[0])) || Stem[0] == '_'))
			{
				return false;
			}
	...
	{
		return mParameterMetadataFunction != nullptr ? mParameterMetadataFunction() : nullptr;
	}

	bool FArdaShaderType::ShouldCompilePermutation(const FArdaShaderTarget& Target, uint32_t PermutationId) const
	{
		if (PermutationId >= mPermutationCount || mShouldCompilePermutationFunction == nullptr)
		{
			return false;
		}
	...
	FArdaShaderCompileEnvironment FArdaShaderType::BuildCompilationEnvironment(const FArdaShaderTarget& Target,
	    uint32_t PermutationId) const
	{
		FArdaShaderCompileEnvironment Environment;
		if (PermutationId < mPermutationCount && mModifyCompilationEnvironmentFunction != nullptr)
		{
			FArdaShaderPermutationParameters Parameters;
			Parameters.mType = this;
			Parameters.mBackendName = Target.mBackendName;
	...
	eastl::string FArdaShaderType::GetPermutationArtifactStem(uint32_t PermutationId) const
	{
	...
	FArdaShaderTypeRegistration::FArdaShaderTypeRegistration(const char* Name,
	    const char* SourceStem,
	    const char* OutputStem,
	    const char* EntryPoint,
	    arda::EArdaRHIShaderStage Stage,
	    FArdaShaderType::FArdaParameterMetadataFunction ParameterMetadataFunction,
	    ...
	    FArdaShaderType::FArdaModifyCompilationEnvironmentFunction ModifyCompilationEnvironmentFunction)
	{
		...
		mType->mbPermutationCallbacksValid = PermutationCount == 1 ||
		    (ShouldCompilePermutationFunction != nullptr && ModifyCompilationEnvironmentFunction != nullptr);
		mType->mShouldCompilePermutationFunction = ShouldCompilePermutationFunction != nullptr
		    ? ShouldCompilePermutationFunction
		    : &DefaultShouldCompilePermutation;
		mType->mModifyCompilationEnvironmentFunction = ModifyCompilationEnvironmentFunction != nullptr
		    ? ModifyCompilationEnvironmentFunction
		    : &DefaultModifyCompilationEnvironment;
		mType->mIdentityHash = HashIdentity(*mType);

		FArdaShaderRegistry& Registry = GetRegistry();
	...
	FArdaShaderTypeRegistration::~FArdaShaderTypeRegistration()
	{
		FArdaShaderRegistry& Registry = GetRegistry();
		std::lock_guard<std::mutex> Lock(Registry.mMutex);
		Registry.mNodes.erase(eastl::remove(Registry.mNodes.begin(), Registry.mNodes.end(), mType),
		    Registry.mNodes.end());
		Registry.mCommitted.erase(eastl::remove(Registry.mCommitted.begin(), Registry.mCommitted.end(), mType),
	...
	FArdaShaderRegistrationStatus FArdaShaderTypeRegistration::CommitAll()
	{
		...
		for (uint32_t Attempt = 0; Attempt < MaxAttempts; ++Attempt)
		{
			...
			for (size_t Left = 0; Left < Candidates.size(); ++Left)
			{
				...
				{
					if (eastl::string(Candidates[Left]->GetName()) == Candidates[Right]->GetName())
					{
						return Error(EArdaShaderRegistrationError::DuplicateTypeName,
						    eastl::string("Duplicate global shader type name: ") + Candidates[Left]->GetName());
					}
					if (Candidates[Left]->GetIdentityHash() == Candidates[Right]->GetIdentityHash())
					{
						return Error(EArdaShaderRegistrationError::DuplicateIdentity,
						    eastl::string("Duplicate global shader artifact identity: ") + Candidates[Left]->GetName() +
						        " and " + Candidates[Right]->GetName());
					}
				}
			}
			...
			for (const auto& Candidate : Candidates)
			{
				...
				if (*Type->GetName() == '\0' || *Type->GetSourceStem() == '\0' || *Type->GetEntryPoint() == '\0' ||
				    Type->GetStage() == arda::EArdaRHIShaderStage::None ||
				    !IsPortableArtifactStem(Type->GetOutputStem()))
				{
					return Error(EArdaShaderRegistrationError::InvalidType,
					    eastl::string("Global shader type '") + Type->GetName() +
					        "' has an invalid identity or non-portable output stem.");
				}
				if (Type->GetPermutationCount() == 0 || Type->GetPermutationCount() > 65536 ||
				    !Type->mbPermutationCallbacksValid || Type->mShouldCompilePermutationFunction == nullptr ||
				    Type->mModifyCompilationEnvironmentFunction == nullptr)
				{
					return Error(EArdaShaderRegistrationError::InvalidPermutation,
					    eastl::string("Global shader type '") + Type->GetName() +
				...
				if (Type->GetSourceStem()[0] == '/')
				{
					...
					if (!SourceStatus)
					{
						...
						return Error(Code,
						    eastl::string("Global shader type '") + Type->GetName() + "' has invalid virtual source '" +
						        Type->GetSourceStem() + "': " + SourceStatus.mMessage);
					}
				}
				...
				if (Metadata != nullptr)
				{
					...
					const FArdaShaderStructStatus LayoutStatus = Metadata->BuildBindingLayoutDescs(Layouts);
					if (!LayoutStatus)
					{
						return Error(EArdaShaderRegistrationError::InvalidParameterMetadata, LayoutStatus.mMessage);
					}
					for (const auto& Layout : Layouts)
					{
						if ((static_cast<uint16_t>(Layout.mVisibility) & static_cast<uint16_t>(Type->GetStage())) == 0)
						{
							return Error(EArdaShaderRegistrationError::InvalidParameterMetadata,
				...
				for (uint32_t Id = 0; Id < Type->GetPermutationCount(); ++Id)
				{
					...
					{
						return Error(EArdaShaderRegistrationError::ArtifactStemCollision,
						    eastl::string("Global shader artifact stem collision between ") + Existing->second +
						        " and " + Type->GetName() + ": " + Stem);
					}
		...
		return Error(EArdaShaderRegistrationError::RegistryMutation,
		    "Shader registration changed repeatedly during validation; retry after registrations settle.");
	}

	const FArdaShaderType* FArdaShaderTypeRegistration::Find(const eastl::string& Name)
	{
	...
	eastl::vector<FArdaShaderType> FArdaShaderTypeRegistration::EnumerateSnapshots()
	{
		const auto Status = CommitAll();
		if (!Status)
		{
