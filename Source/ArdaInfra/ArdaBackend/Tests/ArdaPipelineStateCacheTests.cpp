...
namespace
{
	class FArdaCapturingDiagnosticCallback final : public arda::IArdaDiagnosticCallback
	{
	public:
		void Message(arda::EArdaDiagnosticSeverity, const char* Text) override
		{
		...
		{
			return std::any_of(mMessages.begin(),
			    mMessages.end(),
	...
	class FArdaTestTexture final : public arda::IArdaRHITexture
	{
	public:
		explicit FArdaTestTexture(arda::FArdaRHITextureDesc Desc)
	...
	class FArdaTestFramebuffer final : public arda::IArdaRHIFramebuffer
	{
	public:
		explicit FArdaTestFramebuffer(arda::FArdaRHIFramebufferDesc Desc)
		    : mDesc(eastl::move(Desc))
		{
		}
	...
	arda::TArdaRHIResult<arda::FArdaRHIShaderRef> CreateTestShader(arda::IArdaRHIDevice& Device,
	    ...
	    arda::EArdaRHIShaderStage Stage)
	{
		...
		const std::filesystem::path Path = std::filesystem::path(ARDA_BACKEND_TEST_SHADER_DIR) /
		    (eastl::string(Artifact) + arda::GetShaderArtifactExtension(BackendName)).c_str();
		auto Bytecode = arda::LoadShaderBytecode(Path);
		if (!Bytecode)
		{
			return {{},
			    arda::FArdaRHIStatus::Error(arda::EArdaRHIResult::InvalidState, Bytecode.mDiagnostic.mMessage.c_str())};
		}
	...
	arda::FArdaRHIStatus CreatePersistentTestPipelines(const char* BackendName,
	    ...
	    uint64_t* OutMeshletKey = nullptr)
	{
		...
		auto ComputeShader = CreateTestShader(*Device,
		    BackendName,
		...
		if (auto Status = Cache.GetOrCreateCompute(ComputeInitializer, ComputePipeline); !Status)
		{
			return Status;
		}
		OutComputeKey = ComputePipeline->GetDesc().mPersistentCacheKey;

		auto VertexShader = CreateTestShader(*Device,
		    BackendName,
		...
		auto PixelShader = CreateTestShader(*Device,
		    BackendName,
		...
		GraphicsInitializer.mDesc.mColorFormats.push_back(EArdaRHIFormat::RGBA8UNorm);
		...
		if (auto Status = Cache.GetOrCreateGraphics(GraphicsInitializer, {}, GraphicsPipeline); !Status)
		{
			return Status;
		}
		...
		if (OutMeshletKey != nullptr)
		{
			*OutMeshletKey = 0;
			...
			{
				auto MeshShader = CreateTestShader(*Device,
				    BackendName,
				...
				auto MeshPixelShader = CreateTestShader(*Device,
				    BackendName,
				...
				MeshletInitializer.mDesc.mColorFormats.push_back(EArdaRHIFormat::RGBA8UNorm);
				MeshletInitializer.mDesc.mSampleCount = 1;
				MeshletInitializer.mDesc.mRasterState.mCullMode = EArdaRHICullMode::None;
				...
				if (auto Status = Cache.GetOrCreateMeshlet(MeshletInitializer, {}, MeshletPipeline); !Status)
				{
					return Status;
				}
				*OutMeshletKey = MeshletPipeline->GetDesc().mPersistentCacheKey;
			}
		}
		return {};
	}
...
TEST(ArdaPipelineStateCache, PersistsReloadsAndRejectsCorruptD3D12Blobs)
{
	...
	const std::filesystem::path Directory =
	    std::filesystem::temp_directory_path() / ("arda-pso-cache-" + std::to_string(Unique));
	std::error_code Error;
	std::filesystem::remove_all(Directory, Error);

	FArdaBackendConfiguration Configuration = arda::MakeArdaTestBackendConfiguration();
	FArdaCapturingDiagnosticCallback Diagnostics;
	...
	const auto FirstCreateStatus =
	    CreatePersistentTestPipelines("native-d3d12", FirstComputeKey, FirstGraphicsKey, &FirstMeshletKey);
	ASSERT_TRUE(FirstCreateStatus) << FirstCreateStatus.mMessage.c_str();
	...
	const auto ReloadedCreateStatus =
	    CreatePersistentTestPipelines("native-d3d12", ReloadedComputeKey, ReloadedGraphicsKey, &ReloadedMeshletKey);
	EXPECT_TRUE(ReloadedCreateStatus) << ReloadedCreateStatus.mMessage.c_str();
...

	{
		...
		const std::string Actual{std::istreambuf_iterator<char>(Verify), std::istreambuf_iterator<char>()};
		EXPECT_EQ(Actual, NewerContents);
	}

	{
		std::fstream InvalidHeader(CacheFile, std::ios::binary | std::ios::in | std::ios::out);
		ASSERT_TRUE(InvalidHeader);
		constexpr uint32_t NonZeroReserved = 1;
		InvalidHeader.seekp(12);
		InvalidHeader.write(reinterpret_cast<const char*>(&NonZeroReserved), sizeof(NonZeroReserved));
		ASSERT_TRUE(InvalidHeader);
	}
	ASSERT_TRUE(ConfigureBackend(Configuration));
	ARDA_REQUIRE_BACKEND() << GetBackendError().c_str();
	...
	const auto WrongBackendCreateStatus =
	    CreatePersistentTestPipelines("native-d3d12", WrongBackendComputeKey, WrongBackendGraphicsKey);
	EXPECT_TRUE(WrongBackendCreateStatus) << WrongBackendCreateStatus.mMessage.c_str();
	ShutdownBackend();
	...
	const auto CorruptCreateStatus =
	    CreatePersistentTestPipelines("native-d3d12", CorruptComputeKey, CorruptGraphicsKey);
	EXPECT_TRUE(CorruptCreateStatus) << CorruptCreateStatus.mMessage.c_str();
...
TEST(ArdaPipelineStateCache, PersistsAndReloadsVulkanBlobsWhenAvailable)
{
	...
	const auto Unique = std::chrono::steady_clock::now().time_since_epoch().count();
	const std::filesystem::path Directory =
	    std::filesystem::temp_directory_path() / ("arda-vulkan-pso-cache-" + std::to_string(Unique));
	std::error_code Error;
	std::filesystem::remove_all(Directory, Error);
	...
	uint64_t FirstMeshletKey = 0;
	const auto FirstCreateStatus =
	    CreatePersistentTestPipelines("native-vulkan", FirstComputeKey, FirstGraphicsKey, &FirstMeshletKey);
	ASSERT_TRUE(FirstCreateStatus) << FirstCreateStatus.mMessage.c_str();
	ShutdownBackend();
	...
	ASSERT_TRUE(ConfigureBackend(Configuration));
	ARDA_REQUIRE_BACKEND() << GetBackendError().c_str();
	EXPECT_TRUE(Diagnostics.Contains("Vulkan persistent pipeline cache data was accepted"));
	uint64_t ReloadedComputeKey = 0;
	uint64_t ReloadedGraphicsKey = 0;
	uint64_t ReloadedMeshletKey = 0;
	const auto ReloadedCreateStatus =
	    CreatePersistentTestPipelines("native-vulkan", ReloadedComputeKey, ReloadedGraphicsKey, &ReloadedMeshletKey);
	EXPECT_TRUE(ReloadedCreateStatus) << ReloadedCreateStatus.mMessage.c_str();
	EXPECT_EQ(ReloadedComputeKey, FirstComputeKey);
	EXPECT_EQ(ReloadedGraphicsKey, FirstGraphicsKey);
	EXPECT_EQ(ReloadedMeshletKey, FirstMeshletKey);
...
TEST(ArdaPipelineStateCache, CachesPrecachesEvictsAndReportsFailures)
{
	...
	ShutdownBackend();
	FArdaBackendConfiguration BackendConfiguration = arda::MakeArdaTestBackendConfiguration();
...

	{
		...
		ASSERT_TRUE(Device);
		auto ShaderA = CreateTestShader(*Device,
		    GetBackendConfiguration().mBackendName.c_str(),
		    "ArdaShaderStructTest",
		    "ShaderStructTestCS",
		    EArdaRHIShaderStage::Compute);
		auto ShaderB = CreateTestShader(*Device,
		    GetBackendConfiguration().mBackendName.c_str(),
		    "ArdaShaderStructTest",
		    "ShaderStructTestCS",
		    EArdaRHIShaderStage::Compute);
		auto ShaderC = CreateTestShader(*Device,
		    GetBackendConfiguration().mBackendName.c_str(),
		    "ArdaShaderStructTest",
		    "ShaderStructTestCS",
		    EArdaRHIShaderStage::Compute);
		...
		const auto WrongDevice = Cache.PrecacheCompute(A, reinterpret_cast<const IArdaRHIDevice*>(uintptr_t{1}));
		...
		EXPECT_EQ(Cache.GetDiagnostics().back().mCode, EArdaRHIResult::InvalidArgument);
	}
...
TEST(ArdaPipelineStateCache, ResolvesFramebufferFormatsAndRejectsMismatches)
{
...

	{
		...
		FArdaRHITextureRef Color(new FArdaTestTexture(ColorDesc));
		...
		FArdaRHITextureRef Depth(new FArdaTestTexture(DepthDesc));
		...
		FArdaRHIFramebufferRef Framebuffer(new FArdaTestFramebuffer(FramebufferDesc));
		...
		ASSERT_TRUE(Cache.GetOrCreateGraphics(Initializer, Framebuffer, Pipeline));
		ASSERT_TRUE(Pipeline);
		ASSERT_EQ(Pipeline->GetDesc().mColorFormats.size(), 1u);
		EXPECT_EQ(Pipeline->GetDesc().mColorFormats[0], EArdaRHIFormat::RGBA8UNorm);
		...
		ASSERT_TRUE(Cache.GetOrCreateGraphics(Relabeled, Framebuffer, Reused));
		...
		auto NativeFramebuffer = Device->CreateFramebuffer(NativeFramebufferDesc);
		ASSERT_TRUE(NativeFramebuffer) << NativeFramebuffer.mStatus.mMessage.c_str();
		...
		ASSERT_TRUE(Cache.PrecacheGraphics(NativeInitializer, NativeFramebuffer.mValue));
		EXPECT_EQ(Cache.GetStats().mMisses, MissesBeforePrecache + 1);
		const uint64_t HitsBeforeBind = Cache.GetStats().mHits;
		auto CommandList = Device->CreateCommandList(EArdaRHIQueueType::Graphics);
		ASSERT_TRUE(CommandList);
		ASSERT_TRUE(CommandList.mValue->Open());
		...
		ASSERT_TRUE(Cache.SetGraphicsPipelineState(*CommandList.mValue, NativeInitializer, eastl::move(DynamicState)));
		EXPECT_EQ(Cache.GetStats().mHits, HitsBeforeBind + 1);
		EXPECT_TRUE(CommandList.mValue->Close());

		FArdaRHIFramebufferDesc EmptyFramebufferDesc;
		auto EmptyFramebuffer = Device->CreateFramebuffer(EmptyFramebufferDesc);
		EXPECT_FALSE(EmptyFramebuffer);
		auto BadBindCommandList = Device->CreateCommandList(EArdaRHIQueueType::Graphics);
		...
		EXPECT_EQ(
		    Cache.SetGraphicsPipelineState(*BadBindCommandList.mValue, Initializer, eastl::move(BadBindState)).mCode,
		    EArdaRHIResult::WrongDevice);
		EXPECT_TRUE(BadBindCommandList.mValue->Close());

		Cache.Clear();
		FArdaRHIGraphicsPipelineRef AfterClear;
		ASSERT_TRUE(Cache.GetOrCreateGraphics(Initializer, Framebuffer, AfterClear));
		EXPECT_NE(AfterClear.Get(), Pipeline.Get());

		auto MismatchedFormat = Initializer;
		MismatchedFormat.mDesc.mColorFormats.push_back(EArdaRHIFormat::BGRA8UNorm);
		EXPECT_EQ(Cache.GetOrCreateGraphics(MismatchedFormat, Framebuffer, Reused).mCode,
		    EArdaRHIResult::InvalidArgument);

		auto MismatchedSamples = Initializer;
		MismatchedSamples.mDesc.mSampleCount = 2;
		EXPECT_EQ(Cache.GetOrCreateGraphics(MismatchedSamples, Framebuffer, Reused).mCode,
		    EArdaRHIResult::InvalidArgument);
	}
...
TEST(ArdaPipelineStateCache, CachesPrecachesBindsAndEvictsMeshletPipelines)
{
...

	{
		...
		ASSERT_TRUE(Cache.GetOrCreateMeshlet(Initializer, Framebuffer.mValue, First));
		ASSERT_TRUE(First);
		EXPECT_EQ(Cache.GetStats().mHits, 1u);
		ASSERT_EQ(First->GetDesc().mColorFormats.size(), 1u);
		EXPECT_EQ(First->GetDesc().mColorFormats[0], EArdaRHIFormat::RGBA8UNorm);
		...
		ASSERT_TRUE(Cache.GetOrCreateMeshlet(Relabeled, Framebuffer.mValue, Reused));
		...
		EXPECT_TRUE(Cache.SetMeshletPipelineState(*CommandList.mValue, Initializer, eastl::move(State)));
		EXPECT_TRUE(CommandList.mValue->Close());

		Cache.Trim(128, 128, 0);
		EXPECT_EQ(Cache.GetStats().mMeshletEntries, 0u);
		FArdaRHIMeshletPipelineRef AfterTrim;
		ASSERT_TRUE(Cache.GetOrCreateMeshlet(Initializer, Framebuffer.mValue, AfterTrim));
		EXPECT_NE(AfterTrim.Get(), First.Get());

		auto Mismatched = Initializer;
		Mismatched.mDesc.mColorFormats.push_back(EArdaRHIFormat::BGRA8UNorm);
		EXPECT_EQ(Cache.GetOrCreateMeshlet(Mismatched, Framebuffer.mValue, Reused).mCode,
		    EArdaRHIResult::InvalidArgument);
...
TEST(ArdaPipelineStateCache, CachesRayTracingAndWorkGraphPipelines)
{
	...
	bool bTestedPipelineKind = false;
	{
		...
		FArdaPipelineStateCache Cache(Device, Configuration);
		const char* BackendName = GetBackendConfiguration().mBackendName.c_str();

		if (Device->GetCapabilities().mRayTracing.mbPipelineShaders)
		{
			bTestedPipelineKind = true;
			auto RayGeneration = CreateTestShader(*Device,
			    BackendName,
			    "ArdaRayTracingTest",
			    "RayGen",
			...
			Initializer.mDesc.mShaders.push_back({"RayGen", RayGeneration.mValue, {}});
			...
			const auto PrecacheStatus = Cache.PrecacheRayTracing(Initializer);
			ASSERT_TRUE(PrecacheStatus) << PrecacheStatus.mMessage.c_str();
			...
			ASSERT_FALSE(Cache.GetDiagnostics().empty());
			EXPECT_EQ(Cache.GetDiagnostics().back().mKind, EArdaPipelineStateKind::RayTracing);
		}

		if (Device->GetCapabilities().mWorkGraphTier != EArdaRHIWorkGraphTier::None)
		{
			bTestedPipelineKind = true;
			auto WorkGraphShader = CreateTestShader(*Device,
			    BackendName,
			    "ArdaWorkGraphTest",
			    "WorkGraphMain",
			    EArdaRHIShaderStage::WorkGraph);
			ASSERT_TRUE(WorkGraphShader) << WorkGraphShader.mStatus.mMessage.c_str();
			FArdaRHIBindingLayoutDesc LayoutDesc;
			...
			EXPECT_EQ(InvalidStatus.mCode, EArdaRHIResult::InvalidArgument);
			ASSERT_FALSE(Cache.GetDiagnostics().empty());
			EXPECT_EQ(Cache.GetDiagnostics().back().mKind, EArdaPipelineStateKind::WorkGraph);
		}
...
TEST(ArdaPipelineStateCache, DeterministicLruKeepsMostRecentlyUsedEntry)
{
...

	{
		...
		{
			auto Shader = CreateTestShader(*Device,
			    GetBackendConfiguration().mBackendName.c_str(),
			    "ArdaShaderStructTest",
			    "ShaderStructTestCS",
			    EArdaRHIShaderStage::Compute);
		...
		ASSERT_TRUE(Cache.GetOrCreateCompute(Initializers[0], AStillCached));
		EXPECT_EQ(AStillCached.Get(), A.Get());
		FArdaRHIComputePipelineRef BRecreated;
		ASSERT_TRUE(Cache.GetOrCreateCompute(Initializers[1], BRecreated));
		EXPECT_NE(BRecreated.Get(), B.Get());
		EXPECT_EQ(Cache.GetStats().mComputeEntries, 2u);
		ASSERT_NE(A->GetDesc().mPersistentCacheKey, 0u);
		EXPECT_EQ(A->GetDesc().mPersistentCacheKey, BRecreated->GetDesc().mPersistentCacheKey);

		FArdaRHIBindingLayoutDesc ChangedLayoutDesc = LayoutDesc;
		ChangedLayoutDesc.mItems.push_back({1, 1, EArdaRHIBindingType::StructuredBufferUAV});
		...
		EXPECT_NE(A->GetDesc().mPersistentCacheKey, ChangedPipeline->GetDesc().mPersistentCacheKey);
	}
		...
		for (size_t Index = 0; Index < ThreadCount; ++Index)
		{
			Threads.emplace_back(
			    [&, Index]
			    {
				    ...
				    Statuses[Index] = Cache.GetOrCreateCompute(Initializer, Pipelines[Index]);
			    });
		}
