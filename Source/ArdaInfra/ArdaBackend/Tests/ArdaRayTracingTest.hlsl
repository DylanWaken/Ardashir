...
ConstantBuffer<FArdaLocalRecord> LocalRecord;
#else
...
[shader("miss")]
void KnownMiss(inout FArdaKnownRayPayload payload)
{
	payload.Value = 0xB055u ^ (ARDA_RECORD_VALUE(0x22222222u) ^ 0x22222222u);
}

[shader("closesthit")]
void KnownClosestHit(inout FArdaKnownRayPayload payload, BuiltInTriangleIntersectionAttributes attributes)
{
	payload.Value =
	    (0xC105E57u ^ (ARDA_RECORD_VALUE(0x33333333u) ^ 0x33333333u)) + (attributes.barycentrics.x >= 0.0 ? 0u : 1u);
}
