...
namespace
{
	class FArdaStateBackendCleanup final
	{
	public:
		explicit FArdaStateBackendCleanup(const arda::FArdaTestDiagnosticCallback& Diagnostics)
		    : mDiagnostics(Diagnostics)
		{
		}

		~FArdaStateBackendCleanup()
		{
			const bool MissingValidationSkip = testing::Test::IsSkipped() && arda::ArdaTestValidationEnabled &&
			    arda::GetBackendInitializeResult() == arda::EArdaInitializeResult::ValidationUnavailable;
			// Include shutdown diagnostics while the callback still exists, including early assertion exits.
	...
	private:
		const arda::FArdaTestDiagnosticCallback& mDiagnostics;
	};

	void VerifyNativeResourceStateConformance(const char* BackendName,
	    ...
	    arda::EArdaRHINativeResourceType BufferNativeType)
	{
		...
		ShutdownBackend();
		FArdaTestDiagnosticCallback Diagnostics;
		FArdaStateBackendCleanup Cleanup(Diagnostics);
		...
		BufferDesc.mUsage = EArdaRHIBufferUsage::ShaderResource | EArdaRHIBufferUsage::UnorderedAccess;
		...
		auto InitialTexture = Commands.mValue->QueryTextureState(*Texture.mValue, {});
		ASSERT_TRUE(InitialTexture);
		EXPECT_TRUE(InitialTexture.mValue.IsConsistent());
		EXPECT_EQ(InitialTexture.mValue.mFacadeState, EArdaRHIResourceState::Common);
		EXPECT_EQ(InitialTexture.mValue.mNative.mNativeType, TextureNativeType);

		ASSERT_TRUE(
		    Commands.mValue->SetTextureState(*Texture.mValue, Mip0Slice0, EArdaRHIResourceState::UnorderedAccess));
		auto UavTexture = Commands.mValue->QueryTextureState(*Texture.mValue, Mip0Slice0);
		ASSERT_TRUE(UavTexture);
		EXPECT_TRUE(UavTexture.mValue.IsConsistent());
		EXPECT_EQ(UavTexture.mValue.mNative.mState, EArdaRHIResourceState::UnorderedAccess);
		EXPECT_NE(UavTexture.mValue.mNative.mPrimaryState, InitialTexture.mValue.mNative.mPrimaryState);
		EXPECT_TRUE(
		    Commands.mValue->AssertTextureState(*Texture.mValue, Mip0Slice0, EArdaRHIResourceState::UnorderedAccess));

		auto UntouchedTexture = Commands.mValue->QueryTextureState(*Texture.mValue, Mip1Slice1);
		ASSERT_TRUE(UntouchedTexture);
		EXPECT_TRUE(UntouchedTexture.mValue.IsConsistent());
		EXPECT_EQ(UntouchedTexture.mValue.mFacadeState, EArdaRHIResourceState::Common);
		EXPECT_EQ(UntouchedTexture.mValue.mNative.mPrimaryState, InitialTexture.mValue.mNative.mPrimaryState);
		const auto MixedTexture = Commands.mValue->QueryTextureState(*Texture.mValue, {});
		EXPECT_FALSE(MixedTexture);
		EXPECT_EQ(MixedTexture.mStatus.mCode, EArdaRHIResult::InvalidState);

		const FArdaRHIStatus WrongStart =
		    Commands.mValue->AssertTextureState(*Texture.mValue, Mip0Slice0, EArdaRHIResourceState::CopyDest);
		...
		EXPECT_EQ(CopyBuffer.mValue.mFacadeState, EArdaRHIResourceState::CopyDest);

		ASSERT_TRUE(Commands.mValue->SetBufferState(*Buffer.mValue, EArdaRHIResourceState::ShaderResource));
		...
		EXPECT_NE(ReadBuffer.mValue.mNative.mPrimaryState | ReadBuffer.mValue.mNative.mPipelineStageMask |
		        ReadBuffer.mValue.mNative.mAccessMask,
		    CopyBuffer.mValue.mNative.mPrimaryState | CopyBuffer.mValue.mNative.mPipelineStageMask |
		...
		auto NextCommands = Device->CreateCommandList(EArdaRHIQueueType::Graphics);
		ASSERT_TRUE(NextCommands);
		ASSERT_TRUE(NextCommands.mValue->Open());
		auto PersistedTexture = NextCommands.mValue->QueryTextureState(*Texture.mValue, Mip0Slice0);
		auto PersistedBuffer = NextCommands.mValue->QueryBufferState(*Buffer.mValue);
		...
		EXPECT_EQ(PersistedTexture.mValue.mFacadeState, EArdaRHIResourceState::UnorderedAccess);
		EXPECT_EQ(PersistedBuffer.mValue.mFacadeState, EArdaRHIResourceState::ShaderResource);
		ASSERT_TRUE(NextCommands.mValue->Close());
		ASSERT_TRUE(Device->ExecuteCommandList(NextCommands.mValue));
		ASSERT_TRUE(Device->WaitForIdle());

		auto CopyCommands = Device->CreateCommandList(EArdaRHIQueueType::Graphics);
		ASSERT_TRUE(CopyCommands);
		ASSERT_TRUE(CopyCommands.mValue->Open());
		const uint32_t Pattern = 0x1234abcd;
		ASSERT_TRUE(CopyCommands.mValue->WriteBuffer(*Buffer.mValue, &Pattern, sizeof(Pattern)));
		auto RestoredAfterWrite = CopyCommands.mValue->QueryBufferState(*Buffer.mValue);
		ASSERT_TRUE(RestoredAfterWrite);
		EXPECT_TRUE(RestoredAfterWrite.mValue.IsConsistent());
		EXPECT_EQ(RestoredAfterWrite.mValue.mFacadeState, EArdaRHIResourceState::ShaderResource);

		eastl::vector<uint8_t> Readback;
		ASSERT_TRUE(CopyCommands.mValue->CopyBufferDeviceToHost(*Buffer.mValue, Readback, 0, sizeof(Pattern)));
		auto ReadbackSource = CopyCommands.mValue->QueryBufferState(*Buffer.mValue);
		ASSERT_TRUE(ReadbackSource);
		EXPECT_TRUE(ReadbackSource.mValue.IsConsistent());
		EXPECT_EQ(ReadbackSource.mValue.mFacadeState, EArdaRHIResourceState::CopySource);
		...
		auto WrongStartCommands = Device->CreateCommandList(EArdaRHIQueueType::Graphics);
		...
		ASSERT_TRUE(WrongStartCommands.mValue->BeginTrackingTextureState(*Texture.mValue,
		    Mip0Slice0,
		    EArdaRHIResourceState::Common));
		ASSERT_TRUE(WrongStartCommands.mValue->SetTextureState(*Texture.mValue,
		    Mip0Slice0,
		    EArdaRHIResourceState::ShaderResource));
		ASSERT_TRUE(WrongStartCommands.mValue->Close());
		const auto RejectedSubmission = Device->ExecuteCommandList(WrongStartCommands.mValue);
		EXPECT_FALSE(RejectedSubmission);
		EXPECT_EQ(RejectedSubmission.mStatus.mCode, EArdaRHIResult::InvalidState);

		auto VerificationCommands = Device->CreateCommandList(EArdaRHIQueueType::Graphics);
		ASSERT_TRUE(VerificationCommands);
		ASSERT_TRUE(VerificationCommands.mValue->Open());
		auto UnchangedAfterReject = VerificationCommands.mValue->QueryTextureState(*Texture.mValue, Mip0Slice0);
		auto PersistedReadbackSource = VerificationCommands.mValue->QueryBufferState(*Buffer.mValue);
		ASSERT_TRUE(UnchangedAfterReject);
		ASSERT_TRUE(PersistedReadbackSource);
		EXPECT_EQ(UnchangedAfterReject.mValue.mFacadeState, EArdaRHIResourceState::UnorderedAccess);
		EXPECT_TRUE(UnchangedAfterReject.mValue.IsConsistent());
		EXPECT_TRUE(PersistedReadbackSource.mValue.IsConsistent());
		EXPECT_EQ(PersistedReadbackSource.mValue.mFacadeState, EArdaRHIResourceState::CopySource);
...
{
	VerifyNativeResourceStateConformance("native-d3d12",
...
{
	VerifyNativeResourceStateConformance("native-vulkan",
