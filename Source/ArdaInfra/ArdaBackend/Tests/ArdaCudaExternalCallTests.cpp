...
#define ARDA_EXTERNAL_SCALAR_FIELDS(V, B, S) V(uint32_t, mValue)
	ARDA_CUDA_PARAMETER_STRUCT(FArdaExternalScalarParameters, ARDA_EXTERNAL_SCALAR_FIELDS)

	struct FArdaPreparedNoop final : IArdaCudaPreparedCall
	{
	...
	struct FArdaExternalScalarCall : TArdaCudaExternalCall<FArdaExternalScalarParameters>
	{
		...
		TArdaRHIResult<eastl::unique_ptr<IArdaCudaPreparedCall>> PrepareCall(
		    ...
		    const FArdaExternalScalarParameters::FArdaCuda& P) const override
		{
			mValue = P.mValue;
			mbAligned = reinterpret_cast<uintptr_t>(&P) % alignof(FArdaExternalScalarParameters::FArdaCuda) == 0;
			mContext = Context.mContext;
			return {eastl::unique_ptr<IArdaCudaPreparedCall>(new FArdaPreparedNoop), {}};
		}
	};

	struct FArdaThrowingSupportCall final : FArdaExternalScalarCall
	{
	...
	struct FArdaWrongExternalSignature final : IArdaCudaExternalCall
	{
	...
	struct FArdaExternalTestEntry final : IArdaCudaKernelEntry
	{
		FArdaCudaBuildInfo mBuild{"external-test", "external-test-build", {{120, false}}, false};

		FArdaCudaKernelSignature GetSignature() const noexcept override
		{
			return FArdaExternalScalarParameters::GetCudaMetadata().GetSignature();
		}
	...
	{
		using FArdaRegistry = TArdaCudaKernelRegistry<FArdaExternalScalarParameters, uint32_t>;
		auto Call = eastl::make_shared<FArdaExternalScalarCall>();
		FArdaRegistry Registry;
		...
		FArdaRegistry Wrong;
		EXPECT_FALSE(Wrong.Add(
		    BindArdaCudaExternalCall<uint32_t>("wrong", 0, eastl::make_shared<FArdaWrongExternalSignature>())));
	...
	{
		TArdaCudaKernelRegistry<FArdaExternalScalarParameters, uint32_t> Registry;
		auto Call = eastl::make_shared<FArdaExternalScalarCall>();
		...
		TArdaCudaKernelRegistry<FArdaExternalScalarParameters, uint32_t> Restricted;
	...
	{
		FArdaExternalScalarCall Call;
		FArdaExternalScalarParameters::FArdaCuda P;
		P.mValue = 43;
	...
	{
		auto Call = eastl::make_shared<FArdaThrowingSupportCall>();
		TArdaCudaKernelRegistry<FArdaExternalScalarParameters, uint32_t> Registry;
		...
		K.mParameters.resize(sizeof(FArdaExternalScalarParameters::FArdaCuda));
	...
	{
		FArdaCudaKernel K;
		K.mExternalCall = eastl::make_shared<FArdaExternalScalarCall>();
		K.mParameters.resize(sizeof(FArdaExternalScalarParameters::FArdaCuda));
		...
		auto Invalid = K;
		Invalid.mEntry = eastl::make_shared<FArdaExternalTestEntry>();
	...
	{
#if defined(_WIN32)
		using FArdaGetCurrent = int(__stdcall*)(void**);
		const auto Module = GetModuleHandleW(L"nvcuda.dll");
		const auto GetCurrent =
		    Module ? reinterpret_cast<FArdaGetCurrent>(GetProcAddress(Module, "cuCtxGetCurrent")) : nullptr;
	...
	struct FArdaExternalTrace
	{
		...
		eastl::vector<FArdaAddParameters::FArdaCuda> mPatchedParameters;
	...
	struct FArdaExternalCachedState final : IArdaCudaExternalCallState
	{
		eastl::shared_ptr<FArdaExternalTrace> mTrace;

		~FArdaExternalCachedState() override
		{
	...
	enum class EArdaExternalFailure
	{
		None,
		PrepareStatus,
		PrepareException,
	...
	struct FArdaPreparedExternalAdd final : IArdaCudaPreparedCall
	{
		eastl::shared_ptr<const IArdaCudaKernelEntry> mEntry;
		eastl::shared_ptr<FArdaExternalTrace> mTrace;
		FArdaAddParameters::FArdaCuda mParameters;
		EArdaExternalFailure mFailure = EArdaExternalFailure::None;
		uint32_t mInternalLaunchCount = 1;

		~FArdaPreparedExternalAdd() override
		{
		...
		FArdaRHIStatus Enqueue(void* Stream) override
		{
			...
			if (mFailure == EArdaExternalFailure::EnqueueException)
			{
	...
	struct FArdaExternalAddCall final : TArdaCudaExternalCall<FArdaAddParameters>
	{
		eastl::shared_ptr<const IArdaCudaKernelEntry> mEntry;
		eastl::shared_ptr<FArdaExternalTrace> mTrace;
		EArdaExternalFailure mFailure = EArdaExternalFailure::None;
		    ...
		    const FArdaCudaExternalCallContext&) const override
		{
			...
			if (mFailure == EArdaExternalFailure::CreateStateStatus)
			{
				return {{}, FArdaRHIStatus::Error(EArdaRHIResult::InvalidArgument, "external state status")};
			}
			if (mFailure == EArdaExternalFailure::CreateStateException)
			{
			...
			auto State = eastl::unique_ptr<FArdaExternalCachedState>(new FArdaExternalCachedState);
			State->mTrace = mTrace;
		...
		TArdaRHIResult<eastl::unique_ptr<IArdaCudaPreparedCall>> PrepareCall(
		    const FArdaCudaExternalCallContext& Context,
		    const FArdaAddParameters::FArdaCuda& P) const override
		{
			...
			if (mFailure == EArdaExternalFailure::PrepareStatus)
			{
				return {{}, FArdaRHIStatus::Error(EArdaRHIResult::InvalidArgument, "external prepare status")};
			}
			if (mFailure == EArdaExternalFailure::PrepareException)
			{
				throw std::runtime_error("external prepare exception");
			}
			if (mFailure == EArdaExternalFailure::PrepareNull)
			{
				return {{}, {}};
			}
			auto Prepared = eastl::unique_ptr<FArdaPreparedExternalAdd>(new FArdaPreparedExternalAdd);
	...
	class FArdaExternalAddOperand final : public TArdaComputeOperand<FArdaAddParameters, uint32_t>
	{
	public:
		FArdaExternalAddOperand(FArdaRHIDeviceRef Device, eastl::shared_ptr<const IArdaCudaExternalCall> Call)
		...
		void BindKernelVariants(FArdaRegistry& Registry) const override
		{
			Registry.Add(BindArdaCudaExternalCall<uint32_t>("external-add", 0, mCall));
		}

		TArdaRHIResult<FArdaCudaKernelSelection> SelectKernel(const FArdaParameters& P,
		    const FArdaCudaSelectionContext&,
		    const FArdaVariants& Candidates) const override
		{
	...
	class FArdaCudaExternalGpu : public testing::TestWithParam<const char*>
	{
	protected:
		...
		void SetUp() override
		{
			...
			C.mbEnableValidation = arda::ArdaTestValidationEnabled;
			...
			// A broken fixture registration is a test failure; only native compatibility may skip below.
			ASSERT_TRUE(Registry) << Registry.mStatus.mMessage.c_str();
		...
		void TearDown() override
		{
			...
			mDevice.Reset();
			ShutdownBackend();
			// Missing validation is an initialization skip; initialized workloads still check shutdown errors.
		...
		eastl::shared_ptr<FArdaExternalAddCall> MakeCall(const eastl::shared_ptr<FArdaExternalTrace>& Trace,
		    EArdaExternalFailure Failure = EArdaExternalFailure::None)
		{
			auto Call = eastl::make_shared<FArdaExternalAddCall>();
			Call->mEntry = mEntry;
			Call->mTrace = Trace;
		...
		void ExpectContexts(const FArdaExternalTrace& Trace)
		{
	...
	TEST_P(FArdaCudaExternalGpu, RetainedGraphRequiresOptInAndRetainsPreparedStateAcrossReplay)
	{
		...
		auto Trace = eastl::make_shared<FArdaExternalTrace>();
		auto Call = MakeCall(Trace);
		Call->mbCacheState = true;
		FArdaExternalAddOperand Operand(mDevice, Call);
	...
	TEST_P(FArdaCudaExternalGpu, TimingRegionIncludesAllLibraryLaunchesAndReplaysWithoutPreparation)
	{
		...
		FArdaExternalAddOperand External(mDevice, Call);
		FArdaAddOperand Compiled(mDevice);
		FArdaAddParameters P;
		P.mCount = Count;
		P.mBias = 2;
	...
	TEST_P(FArdaCudaExternalGpu, CaptureOperationFailureEndsCaptureWithoutRetryingTheAdapter)
	{
		...
		P.mBias = 7;
		P.mInput.mBuffer = P.mOutput.mBuffer = Buffer.mValue;
		auto FailedTrace = eastl::make_shared<FArdaExternalTrace>();
		{
			auto Call = MakeCall(FailedTrace, EArdaExternalFailure::EnqueueException);
			Call->mbGraphCaptureSafe = true;
			FArdaExternalAddOperand Operand(mDevice, Call);
			FArdaCudaSequence Sequence(mDevice, EArdaRHIQueueType::Graphics, Cache);
			ASSERT_TRUE(Sequence.Add(Operand, P));
	...
	TEST_P(FArdaCudaExternalGpu, StandaloneDispatchRetainsPatchedValuesUntilRetirement)
	{
		...
		auto Trace = eastl::make_shared<FArdaExternalTrace>();
		{
			FArdaExternalAddOperand Operand(mDevice, MakeCall(Trace));
	...
	TEST_P(FArdaCudaExternalGpu, OneExternalOperationMayEnqueueMultipleInternalKernels)
	{
	...
	TEST_P(FArdaCudaExternalGpu, CachedStateIsReusedAcrossSubmissionsAndDestroyedAfterPreparedCalls)
	{
		auto Trace = eastl::make_shared<FArdaExternalTrace>();
		{
			auto Buffer = mDevice->CreateBuffer(BufferDesc(128));
	...
	TEST_P(FArdaCudaExternalGpu, OutstandingRecordingsShareContextStateAndRetainSeparatePreparedCalls)
	{
		auto Buffer = mDevice->CreateBuffer(BufferDesc(128));
		ASSERT_TRUE(Buffer);
		auto Upload = mDevice->CreateCommandList(EArdaRHIQueueType::Graphics);
	...
	TEST_P(FArdaCudaExternalGpu, AbandonedRecordingReleasesPreparedCallsWithoutExecutingWork)
	{
		auto Buffer = mDevice->CreateBuffer(BufferDesc(128));
		ASSERT_TRUE(Buffer);
		auto Upload = mDevice->CreateCommandList(EArdaRHIQueueType::Graphics);
	...
	TEST_P(FArdaCudaExternalGpu, ContextStateFailuresLeaveLaterRecordingUsable)
	{
		auto Buffer = mDevice->CreateBuffer(BufferDesc(128));
		ASSERT_TRUE(Buffer);
		auto Upload = mDevice->CreateCommandList(EArdaRHIQueueType::Graphics);
		...
		P.mCount = 128;
		P.mBias = 23;
		for (auto Failure : {EArdaExternalFailure::CreateStateStatus, EArdaExternalFailure::CreateStateException})
		{
			auto GoodTrace = eastl::make_shared<FArdaExternalTrace>();
			auto BadTrace = eastl::make_shared<FArdaExternalTrace>();
			FArdaExternalAddOperand Good(mDevice, MakeCall(GoodTrace));
			{
				FArdaExternalAddOperand Bad(mDevice, MakeCall(BadTrace, Failure));
				FArdaCudaSequence Sequence(mDevice);
	...
	TEST_P(FArdaCudaExternalGpu, MixedSequencePreservesOrderingOffsetsAndOneStream)
	{
		constexpr uint32_t Count = 257;
		...
		auto Trace = eastl::make_shared<FArdaExternalTrace>();
		{
			FArdaExternalAddOperand External(mDevice, MakeCall(Trace));
			FArdaAddOperand Compiled(mDevice);
			FArdaCudaSequence Sequence(mDevice);
			FArdaAddParameters P;
	...
	TEST_P(FArdaCudaExternalGpu, LatePreparationFailuresNeverEnqueueAndReleaseInContext)
	{
		auto Buffer = mDevice->CreateBuffer(BufferDesc(128));
		ASSERT_TRUE(Buffer);
		for (auto Failure : {EArdaExternalFailure::PrepareStatus,
		         EArdaExternalFailure::PrepareException,
		         EArdaExternalFailure::PrepareNull})
		{
			auto Trace = eastl::make_shared<FArdaExternalTrace>();
			{
				FArdaExternalAddOperand Good(mDevice, MakeCall(Trace));
				FArdaExternalAddOperand Bad(mDevice, MakeCall(Trace, Failure));
				FArdaAddParameters P;
				P.mInput.mBuffer = P.mOutput.mBuffer = Buffer.mValue;
				P.mCount = 128;
				P.mBias = 7;
	...
	TEST_P(FArdaCudaExternalGpu, EnqueueExceptionsReturnStatusAndReleaseInContext)
	{
		auto Buffer = mDevice->CreateBuffer(BufferDesc(128));
		ASSERT_TRUE(Buffer);
		auto Trace = eastl::make_shared<FArdaExternalTrace>();
		{
			FArdaExternalAddOperand Operand(mDevice, MakeCall(Trace, EArdaExternalFailure::EnqueueException));
			FArdaAddParameters P;
			P.mInput.mBuffer = P.mOutput.mBuffer = Buffer.mValue;
			P.mCount = 128;
			P.mBias = 7;
	...
	// External library calls and compiled kernels share the same graph node contract.
	struct FArdaPersistentExternalAddNode
	    : TArdaCudaOperandNode<FArdaPersistentExternalAddNode, FArdaExternalAddOperand>
	{
		static FArdaDependencyNodeMetadata GetMetadata()
		{
			return {"test.cuda.persistent-external-add", 1};
		}
	};

	struct FArdaPersistentCompiledAddNode : TArdaCudaOperandNode<FArdaPersistentCompiledAddNode, FArdaAddOperand>
	{
		static FArdaDependencyNodeMetadata GetMetadata()
		{
			return {"test.cuda.persistent-compiled-add", 1};
		}
	};

	TEST_P(FArdaCudaExternalGpu, PersistentGraphMixesExternalAndCompiledCudaNodes)
	{
		ASSERT_TRUE(RegisterArdaBuiltinNodes());
		auto Trace = eastl::make_shared<FArdaExternalTrace>();
		{
			auto External = eastl::make_shared<FArdaExternalAddOperand>(mDevice, MakeCall(Trace));
			auto Compiled = eastl::make_shared<FArdaAddOperand>(mDevice);
			const auto ExternalDefinition = FArdaPersistentExternalAddNode::GetMetadata().mName;
			const auto CompiledDefinition = FArdaPersistentCompiledAddNode::GetMetadata().mName;
			ASSERT_TRUE(FArdaPersistentExternalAddNode::Register());

			struct FArdaRegistrationScope
			{
				eastl::string mName;

				~FArdaRegistrationScope()
				{
					EXPECT_TRUE(FArdaNodeRegistry::Get().Unregister(mName));
				}
			} ExternalRegistration{ExternalDefinition};

			ASSERT_TRUE(FArdaPersistentCompiledAddNode::Register());
			FArdaRegistrationScope CompiledRegistration{CompiledDefinition};
			...
			P.mCount = 128;
			P.mBias = 7;
			auto First = Graph.AttachOrFind<FArdaPersistentExternalAddNode>("external first", {External, P});
			ASSERT_TRUE(First) << First.mStatus.mMessage.c_str();
			P.mInput.mResource = Middle.mValue;
			P.mOutput.mResource = CompiledOutput.mValue;
			P.mBias = 11;
			auto Second = Graph.AttachOrFind<FArdaPersistentCompiledAddNode>("compiled middle", {Compiled, P});
			ASSERT_TRUE(Second);
			P.mInput.mResource = CompiledOutput.mValue;
			P.mOutput.mResource = Output.mValue;
			P.mBias = 13;
			auto Third = Graph.AttachOrFind<FArdaPersistentExternalAddNode>("external last", {External, P});
			ASSERT_TRUE(Third);
