...
namespace
{
	...
	ARDA_BEGIN_COMPUTE_PARAMETER_STRUCT(FArdaNestedOperandParameters)
		ARDA_COMPUTE_BUFFER(mInput, EArdaComputeAccess::Read)
		ARDA_COMPUTE_PARAMETER(float, mScale)
	ARDA_END_COMPUTE_PARAMETER_STRUCT()

	ARDA_BEGIN_COMPUTE_PARAMETER_STRUCT(FArdaOperandParameters)
		ARDA_COMPUTE_PARAMETER_STRUCT_ARRAY(FArdaNestedOperandParameters, mLayers, 2)
	...
	ARDA_BEGIN_COMPUTE_PARAMETER_STRUCT(FArdaScalarOperandParameters)
		ARDA_COMPUTE_PARAMETER(uint32_t, mValue)
		ARDA_COMPUTE_PARAMETER_STRUCT(FArdaNestedOperandParameters, mNested)
	ARDA_END_COMPUTE_PARAMETER_STRUCT()

	class FArdaParameterTestBuffer final : public IArdaRHIBuffer
	{
	public:
		int mReferences = 0;
	...
	class FArdaParameterTestTexture final : public IArdaRHITexture
	{
	public:
		int mReferences = 0;
	...
	{
		FArdaParameterTestBuffer Buffer;
		FArdaParameterTestTexture Texture;
		eastl::vector<FArdaComputeResourceAccess> Accesses;
		{
			FArdaOperandParameters P;
			...
			P.mDimensions = {8, 4, 2};
			P.mHostPlan = {5, 9, 7};
			const auto& Metadata = FArdaOperandParameters::GetStaticMetadata();
			...
			ASSERT_TRUE(Metadata.Enumerate(&P,
			    [&](const FArdaComputeParameter& Leaf)
			    {
				    Paths.push_back(Leaf.mPath);
				    EXPECT_EQ(static_cast<const uint8_t*>(Leaf.mValue) - reinterpret_cast<const uint8_t*>(&P),
				        Leaf.mAbsoluteOffset);
				    if (Leaf.mPath == "mHostPlan")
				    {
					    EXPECT_EQ(*static_cast<const eastl::vector<uint32_t>*>(Leaf.mValue), P.mHostPlan);
				    }
				    if (Leaf.mPath == "mDimensions[2]")
				    {
					    EXPECT_EQ(*static_cast<const uint32_t*>(Leaf.mValue), 2u);
				    }
			    }));
	...
	{
		const auto& Good = FArdaScalarOperandParameters::GetStaticMetadata();
		FArdaScalarOperandParameters P;
		...
		{
			FArdaComputeParameterMetadata Bad("bad",
			    sizeof(P),
			    alignof(FArdaScalarOperandParameters),
			    eastl::move(Members));
			EXPECT_FALSE(Bad.GetStatus());
			bool Visited = false;
			EXPECT_FALSE(Bad.Enumerate(&P,
			    [&](const auto&)
			    {
				    Visited = true;
			    }));
		...
		Members[0].mOffset = sizeof(P);
		CheckBad(Members);
		Members = Good.GetMembers();
		Members[0].mElementCount = SIZE_MAX;
		CheckBad(Members);
		Members = Good.GetMembers();
		Members[1].mNestedMetadata = nullptr;
		CheckBad(Members);
		Members = Good.GetMembers();
		Members[1].mName = Members[0].mName;
		CheckBad(Members);
		Members = Good.GetMembers();
		Members[1].mOffset = 0;
		CheckBad(Members);
		Members = Good.GetMembers();
		Members[0].mKind = EArdaComputeParameterKind::Buffer;
		CheckBad(Members);
		Members = Good.GetMembers();
		Members[0].mAlignment = 3;
		CheckBad(Members);
		FArdaComputeParameterMetadata BadStruct("bad", 4, 0, {});
		EXPECT_FALSE(BadStruct.GetStatus());
		EXPECT_FALSE(Good.Enumerate(reinterpret_cast<const uint8_t*>(&P) + 1,
		    [](const auto&)
		    {
		    }));
	}
