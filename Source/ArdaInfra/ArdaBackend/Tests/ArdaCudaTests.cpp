...
namespace
{
	...
	class FArdaCudaGpu : public testing::TestWithParam<const char*>
	{
	protected:
		...
		void SetUp() override
		{
			...
			// Metadata, registration and internal failures must not masquerade as unsupported hardware.
	...
	TEST_P(FArdaCudaGpu, SequenceUsesOneStreamAndRetainsPingPongViews)
	{
	...
	TEST_P(FArdaCudaGpu, SequenceImmediateReplayNoWorkAndStickyErrors)
	{
	...
	TEST_P(FArdaCudaGpu, RetainedGraphReplaysAndRebuildsForArgumentsAndResources)
	{
	...
	TEST_P(FArdaCudaGpu, TimingRegionsPreserveBatchReplayAndUnsampledVariants)
	{
		constexpr uint32_t Count = 4097;
		auto Buffer = mDevice->CreateBuffer(BufferDesc(Count * 4));
		ASSERT_TRUE(Buffer);
	...
	TEST_P(FArdaCudaGpu, TimingQueryCancelsUnsubmittedRecordingsAndRejectsOverwritingASample)
	{
		auto Query = eastl::make_shared<FArdaCudaTimingQuery>();
	...
	TEST_P(FArdaCudaGpu, SequenceRejectsLateInvalidStepBeforeAnyLaunch)
	{
	...
	TEST_P(FArdaCudaGpu, PrecompiledDeferredAndImmediateUseFrozenSingleKernelPlans)
	{
		for (uint32_t Count : {32u, 128u})
		{
			...
			if (Count == 128)
			{
				...
				FArdaAddOperand::FArdaVariants Portable, Fast;
	...
	TEST_P(FArdaCudaGpu, MultipleRecordedDispatchesRetainIndependentParameters)
	{
		constexpr uint32_t Count = 65;
	...
	TEST_P(FArdaCudaGpu, ResourceChecksAreRuntimeAndBufferOffsetsPreserveGuards)
	{
		FArdaAddOperand Operand(mDevice);
	...
	TEST_P(FArdaCudaGpu, TextureBufferCopiesAcceptOffsetsAndMinimalFinalRow)
	{
	...
	TEST_P(FArdaCudaGpu, TextureBufferSequencePreservesRegionsMipsAndLayers)
	{
	...
	TEST_P(FArdaCudaGpu, SurfaceConversionAndMipLifetime)
	{
		const auto Caps = mDevice->GetCudaCapabilities();
	...
	TEST_P(FArdaCudaGpu, SequenceMixesBufferAndSurfaceSchemas)
	{
		const auto Caps = mDevice->GetCudaCapabilities();
		if (!Caps.mbSurfaceAccess)
		{
	...
	struct FArdaScopedPersistentCudaRegistration
	{
		eastl::string mName;

		~FArdaScopedPersistentCudaRegistration()
		{
	...
	// A single registered node type adapts the compiled add operand for all persistent graph scenarios.
	struct FArdaPersistentAddNode : TArdaCudaOperandNode<FArdaPersistentAddNode, FArdaAddOperand>
	{
		static FArdaDependencyNodeMetadata GetMetadata()
		{
			return {"test.cuda.persistent-add", 1};
		}
	};

	TEST_P(FArdaCudaGpu, PersistentGraphDerivesCudaDependenciesBeforeExecution)
	{
		ASSERT_TRUE(RegisterArdaBuiltinNodes());
		auto Operand = eastl::make_shared<FArdaAddOperand>(mDevice);
		const auto Definition = FArdaPersistentAddNode::GetMetadata().mName;
		ASSERT_TRUE(FArdaPersistentAddNode::Register());
		FArdaScopedPersistentCudaRegistration Registration{Definition};
		...
		P.mCount = 128;
		P.mBias = 7;
		auto First = Graph.AttachOrFind<FArdaPersistentAddNode>("add seven", {Operand, P});
		ASSERT_TRUE(First) << First.mStatus.mMessage.c_str();
		P.mInput.mResource = Middle.mValue;
		P.mOutput.mResource = Output.mValue;
		P.mBias = 11;
		auto Second = Graph.AttachOrFind<FArdaPersistentAddNode>("add eleven", {Operand, P});
	...
	TEST_P(FArdaCudaGpu, PersistentGraphCoalescesCudaNodesAndCullsUnusedWork)
	{
		ASSERT_TRUE(RegisterArdaBuiltinNodes());
		auto Operand = eastl::make_shared<FArdaAddOperand>(mDevice);
		const auto Definition = FArdaPersistentAddNode::GetMetadata().mName;
		...
		P.mCount = 128;
		P.mBias = 7;
		auto First = Graph.AttachOrFind<FArdaPersistentAddNode>("first CUDA kernel", {Operand, P});
		ASSERT_TRUE(First);
		P.mInput.mResource = Scratch.mValue;
		P.mOutput.mResource = Output.mValue;
		P.mBias = 11;
		auto Second = Graph.AttachOrFind<FArdaPersistentAddNode>("second CUDA kernel", {Operand, P});
		ASSERT_TRUE(Second);
		P.mBias = 999; // Mutating the source must not change either stored node snapshot.
		P.mInput.mResource = Input.mValue;
		P.mOutput.mResource = DeadOutput.mValue;
		auto Dead = Graph.AttachOrFind<FArdaPersistentAddNode>("unused CUDA work", {Operand, P});
		ASSERT_TRUE(Dead);
		auto Bytes = eastl::make_shared<eastl::vector<uint8_t>>();
		ASSERT_TRUE(
		    Graph.AttachOrFind("readback", "arda.readback", FArdaGraphReadbackParameters{Output.mValue, Bytes}));
	...
	TEST_P(FArdaCudaGpu, PersistentGraphTextureCudaNodesPreserveCropAndPadding)
	{
		FArdaRHITextureDesc Desc;
		Desc.mWidth = 75;
		Desc.mHeight = 31;
		...
		ASSERT_TRUE(Output);
		auto Download = Graph.AttachOrFind<FArdaGraphTextureToBufferNode>("download crop",
		    {Texture.mValue, Crop, Intermediate.mValue, TransferLayout});
		...
		P.mBias = 17;
		auto First = Graph.AttachOrFind<FArdaPersistentAddNode>("add seventeen", {Operand, P});
		ASSERT_TRUE(First);
		P.mInput.mResource = FirstOutput.mValue;
		P.mOutput.mResource = SecondOutput.mValue;
		P.mBias = 23;
		auto Second = Graph.AttachOrFind<FArdaPersistentAddNode>("add twenty-three", {Operand, P});
		ASSERT_TRUE(Second);
		auto Upload = Graph.AttachOrFind<FArdaGraphBufferToTextureNode>("upload crop",
		    {UpdatedTexture.mValue, Crop, SecondOutput.mValue, TransferLayout});
		ASSERT_TRUE(Upload) << Upload.mStatus.mMessage.c_str();
		auto CopyForReadback = Graph.AttachOrFind<FArdaGraphTextureToBufferNode>("copy for readback",
		    {UpdatedTexture.mValue, {}, Output.mValue, ReadbackLayout});
	...
	TEST_P(FArdaCudaGpu, PersistentGraphRejectsForeignResourcesAndLatePreparationFailure)
	{
		ASSERT_TRUE(RegisterArdaBuiltinNodes());
		const auto Definition = FArdaPersistentAddNode::GetMetadata().mName;
		auto Operand = eastl::make_shared<FArdaAddOperand>(mDevice);
		...
		for (const bool Shared : {true, false})
		{
			...
			P.mCount = 128;
			P.mBias = 7;
			EXPECT_FALSE(Graph.AttachOrFind<FArdaPersistentAddNode>("foreign resource", {Operand, P}));
			P.mInput.mResource = Input.mValue;
			ASSERT_TRUE(Graph.AttachOrFind<FArdaPersistentAddNode>("first CUDA node", {Operand, P}));
			P.mInput.mResource = Middle.mValue;
			P.mOutput.mResource = Output.mValue;
			P.mCount = Shared
			    ? 127
			    : 128; // Selection fails after the first plan prepares, or the first input is not shareable.
			ASSERT_TRUE(Graph.AttachOrFind<FArdaPersistentAddNode>("invalid CUDA node", {Operand, P}));
			auto Bytes = eastl::make_shared<eastl::vector<uint8_t>>();
			ASSERT_TRUE(
			    Graph.AttachOrFind("readback", "arda.readback", FArdaGraphReadbackParameters{Output.mValue, Bytes}));
			auto Compiled = Graph.EndGraphEdit();
	...
	TEST_P(FArdaCudaGpu, PersistentInductorCompilesReorderedCudaNodesAndReusesFrameStorage)
	{
		ASSERT_TRUE(RegisterArdaBuiltinNodes());
		const auto Definition = FArdaPersistentAddNode::GetMetadata().mName;
		auto Operand = eastl::make_shared<FArdaAddOperand>(mDevice);
		...
		P.mCount = Count;
		P.mBias = 11;
		auto Last = Graph.AttachOrFind<FArdaPersistentAddNode>("add eleven", {Operand, P});
		ASSERT_TRUE(Last);
		P.mInput.mResource = Input.mValue;
		P.mOutput.mResource = Middle.mValue;
		P.mBias = 7;
