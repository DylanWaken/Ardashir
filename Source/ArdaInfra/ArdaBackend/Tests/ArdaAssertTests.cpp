...
namespace
{
	...
	protected:
		void SetUp() override
		{
			arda::SetEnsureBehavior(arda::EArdaEnsureBehavior::Log);
		}
	};
}
...
{
	EXPECT_DEATH(ARDA_CHECK(false), "Assertion failed: false");
}

TEST(ArdaAssertDeathTest, CheckfIncludesFormattedMessage)
{
	EXPECT_DEATH(ARDA_CHECKF(false, "Bad value %d", 13), "Bad value 13");
}

TEST(ArdaAssertDeathTest, CheckMessageTerminatesWithFormattedMessage)
{
	EXPECT_DEATH(ARDA_CHECK_MSG("Unconditional failure %d", 19), "Unconditional failure 19");
}
#endif
...
{
	EXPECT_DEATH((void)ARDA_VERIFY(false), "Assertion failed: false");
}
#endif
