...
namespace
{
	class FArdaExtendedDiagnosticCallback final : public arda::IArdaDiagnosticCallback
	{
	public:
		void Message(arda::EArdaDiagnosticSeverity Severity, const char* Message) override
		{
			if (Severity == arda::EArdaDiagnosticSeverity::Error || Severity == arda::EArdaDiagnosticSeverity::Fatal)
			{
	...
	class FArdaExtendedBackendCleanup final
	{
	public:
		~FArdaExtendedBackendCleanup()
		{
			...
			static_cast<void>(arda::ConfigureBackend(arda::MakeArdaTestBackendConfiguration()));
		}
	};

#if defined(_WIN32)
	class FArdaWin32TestSurface final : public arda::IArdaWindowSurface
	{
	public:
		FArdaWin32TestSurface()
		{
			mWindow = CreateWindowExW(0,
			    L"STATIC",
			    L"Arda custom-present conformance",
			    WS_OVERLAPPED,
			    CW_USEDEFAULT,
			    CW_USEDEFAULT,
			    64,
		...
		~FArdaWin32TestSurface() override
		{
			if (mWindow)
			{
		...
		[[nodiscard]] arda::FArdaNativeObject CreateVulkanSurface(arda::FArdaNativeObject NativeInstance,
		    eastl::string& OutError) override
		{
			...
			const VkResult Result = CreateSurface(Instance, &Info, nullptr, &Surface);
			...
			OutError.clear();
			return arda::FArdaNativeObject(reinterpret_cast<uintptr_t>(Surface));
#else
	...
	class FArdaCustomPresentTracker final : public arda::IArdaCustomPresent
	{
	public:
		...
		[[nodiscard]] bool Present(arda::FArdaNativeObject BackBuffer, uint32_t Width, uint32_t Height) override
		{
			++mPresentCount;
	...
	{
		const std::string Path = std::string(ARDA_BACKEND_TEST_SHADER_DIR) + "/" + FileName;
	...
	arda::TArdaRHIResult<arda::FArdaRHIShaderRef> CreateExtendedShader(arda::IArdaRHIDevice& Device,
	    ...
	    arda::EArdaRHIShaderStage Stage)
	{
		...
		const eastl::string FileName = eastl::string(Artifact) + arda::GetShaderArtifactExtension(BackendName);
		const std::vector<uint8_t> Bytecode = LoadExtendedShaderArtifact(FileName.c_str());
		if (Bytecode.empty())
		{
			return {{},
	...
	arda::TArdaRHIResult<arda::FArdaRHIShaderRef> CreateExtendedComputeShader(arda::IArdaRHIDevice& Device,
	    const char* BackendName)
	{
		return CreateExtendedShader(Device,
		    BackendName,
	...
	void ExpectTextureState(arda::IArdaRHICommandList& Commands,
	...
	void ExpectBufferState(arda::IArdaRHICommandList& Commands,
	...
	void ExpectSamplerFeedbackState(arda::IArdaRHICommandList& Commands,
	    arda::IArdaRHISamplerFeedbackTexture& Texture,
	    arda::EArdaRHIResourceState Expected)
	{
		const auto Snapshot = Commands.QuerySamplerFeedbackTextureState(Texture);
		ASSERT_TRUE(Snapshot) << Snapshot.mStatus.mMessage.c_str();
		EXPECT_TRUE(Snapshot.mValue.IsConsistent());
		EXPECT_TRUE(Snapshot.mValue.mNative.mbNativeCompatible);
		EXPECT_EQ(Snapshot.mValue.mFacadeState, Expected);
	...
	void VerifySamplerFeedbackStateParity(bool bMipRegionUsed = false)
	{
		...
		ASSERT_TRUE(Device);
		if (Device->GetCapabilities().mSamplerFeedbackTier == EArdaRHISamplerFeedbackTier::None)
		{
		...
		FeedbackDesc.mFormat = bMipRegionUsed ? EArdaRHISamplerFeedbackFormat::MipRegionUsedOpaque
		...
		FeedbackDesc.mInitialState = EArdaRHIResourceState::UnorderedAccess;
		auto Feedback = Device->CreateSamplerFeedbackTexture(Paired.mValue, FeedbackDesc);
		...
		DecodedDesc.mWidth = (PairedDesc.mWidth + FeedbackDesc.mMipRegionX - 1u) / FeedbackDesc.mMipRegionX;
		DecodedDesc.mHeight = (PairedDesc.mHeight + FeedbackDesc.mMipRegionY - 1u) / FeedbackDesc.mMipRegionY;
		...
		ExpectSamplerFeedbackState(*Commands.mValue, *Feedback.mValue, EArdaRHIResourceState::UnorderedAccess);
		ASSERT_TRUE(Commands.mValue->ClearSamplerFeedbackTexture(*Feedback.mValue));
		ExpectSamplerFeedbackState(*Commands.mValue, *Feedback.mValue, EArdaRHIResourceState::UnorderedAccess);
		...
		auto Shader = CreateExtendedShader(*Device,
		    "native-d3d12",
		...
		SetDesc.mItems = {{0, 0, EArdaRHIBindingType::TextureSRV, FArdaRHIResourceRef(Paired.mValue.Get()), {}},
		    ...
		    {bMipRegionUsed ? 2u : 0u,
		        0,
		        EArdaRHIBindingType::SamplerFeedbackTextureUAV,
		        FArdaRHIResourceRef(Feedback.mValue.Get()),
		        {}},
		...
		ASSERT_TRUE(
		    Commands.mValue->DecodeSamplerFeedbackTexture(*Decoded.mValue, *Feedback.mValue, EArdaRHIFormat::R8UInt));
		ExpectSamplerFeedbackState(*Commands.mValue, *Feedback.mValue, EArdaRHIResourceState::ResolveSource);
		ExpectTextureState(*Commands.mValue, *Decoded.mValue, {}, EArdaRHIResourceState::ResolveDest);
		...
		ASSERT_TRUE(Commands.mValue->TransitionTexture(*Decoded.mValue, ToReadback));
		ExpectTextureState(*Commands.mValue, *Decoded.mValue, {}, EArdaRHIResourceState::CopySource);
		FArdaRHITextureSlice Slice;
		Slice.mMipLevel = bMipRegionUsed ? 2u : 0u;
		Slice.mWidth = ReadbackWidth;
		Slice.mHeight = ReadbackHeight;
		Slice.mDepth = 1;
		ASSERT_TRUE(Commands.mValue->CopyTextureToStaging(*Readback.mValue, Slice, *Decoded.mValue, Slice));
		ASSERT_TRUE(
		    Commands.mValue->CopyBufferDeviceToHost(*Metadata.mValue, MetadataReadback, 0, MetadataDesc.mByteSize));
	...
	void VerifyExtendedCommands(const char* BackendName)
	{
		using namespace arda;
		...
		WholeSlice.mDepth = 1;
		auto UploadMapping = Device->MapStagingTexture(Upload.mValue, WholeSlice, EArdaRHICpuAccess::Write);
		ASSERT_TRUE(UploadMapping);
		ASSERT_NE(UploadMapping.mValue.mData, nullptr);
		ASSERT_GE(UploadMapping.mValue.mRowPitch, 16u);
		for (uint32_t Y = 0; Y < 4; ++Y)
		{
			auto* Row = static_cast<uint8_t*>(UploadMapping.mValue.mData) + Y * UploadMapping.mValue.mRowPitch;
			for (uint32_t X = 0; X < 4; ++X)
			{
				const uint8_t Pixel[4] = {static_cast<uint8_t>(17 + X), static_cast<uint8_t>(91 + Y), 203, 255};
		...
		OutputBinding.mResource = TArdaRHIRef<IArdaRHIResource>(Output.mValue.Get());
		...
		ExpectTextureState(*Commands.mValue, *Source.mValue, WholeRange, EArdaRHIResourceState::Common);
		ExpectTextureState(*Commands.mValue, *Destination.mValue, WholeRange, EArdaRHIResourceState::Common);

		ASSERT_TRUE(
		    Commands.mValue->CopyTextureFromStaging(*Source.mValue, TextureRegion, *Upload.mValue, StagingRegion));
		ExpectTextureState(*Commands.mValue, *Source.mValue, WholeRange, EArdaRHIResourceState::Common);
		ASSERT_TRUE(Commands.mValue->CopyTexture(*Destination.mValue, TextureRegion, *Source.mValue, TextureRegion));
		ExpectTextureState(*Commands.mValue, *Source.mValue, WholeRange, EArdaRHIResourceState::Common);
		ExpectTextureState(*Commands.mValue, *Destination.mValue, WholeRange, EArdaRHIResourceState::Common);
		ASSERT_TRUE(
		    Commands.mValue->CopyTextureToStaging(*Readback.mValue, StagingRegion, *Destination.mValue, TextureRegion));
		ExpectTextureState(*Commands.mValue, *Destination.mValue, WholeRange, EArdaRHIResourceState::Common);
		...
		ASSERT_TRUE(Commands.mValue->TransitionTexture(*Destination.mValue, TextureTransition));
		ExpectTextureState(*Commands.mValue, *Destination.mValue, WholeRange, EArdaRHIResourceState::CopyDest);
		...
		const FArdaRHIStatus Rejected = Commands.mValue->TransitionTexture(*Destination.mValue, BadBefore);
		...
		ASSERT_TRUE(Commands.mValue->TransitionTexture(*Destination.mValue, SplitTransition));
		ExpectTextureState(*Commands.mValue, *Destination.mValue, WholeRange, EArdaRHIResourceState::CopyDest);
		SplitTransition.mFlags = EArdaRHITransitionFlags::EndOnly;
		ASSERT_TRUE(Commands.mValue->TransitionTexture(*Destination.mValue, SplitTransition));
		ExpectTextureState(*Commands.mValue, *Destination.mValue, WholeRange, EArdaRHIResourceState::ShaderResource);

		const uint32_t DispatchArguments[3] = {1, 1, 1};
		ASSERT_TRUE(Commands.mValue->WriteBuffer(*Arguments.mValue, DispatchArguments, sizeof(DispatchArguments)));
		ExpectBufferState(*Commands.mValue, *Arguments.mValue, EArdaRHIResourceState::Common);
		FArdaRHIBufferTransitionDesc ArgumentTransition;
		ArgumentTransition.mStateBefore = EArdaRHIResourceState::Common;
		ArgumentTransition.mStateAfter = EArdaRHIResourceState::IndirectArgument;
		...
		ASSERT_TRUE(Commands.mValue->TransitionBuffer(*Arguments.mValue, ArgumentTransition));
		ExpectBufferState(*Commands.mValue, *Arguments.mValue, EArdaRHIResourceState::IndirectArgument);
		ASSERT_TRUE(Commands.mValue->SetBufferState(*Output.mValue, EArdaRHIResourceState::UnorderedAccess));
		ExpectBufferState(*Commands.mValue, *Output.mValue, EArdaRHIResourceState::UnorderedAccess);
		...
		ExpectBufferState(*Commands.mValue, *Arguments.mValue, EArdaRHIResourceState::IndirectArgument);
		const FArdaRHIStatus OutOfBounds = Commands.mValue->DispatchIndirect(*Arguments.mValue, 4);
		EXPECT_FALSE(OutOfBounds);
		EXPECT_EQ(OutOfBounds.mCode, EArdaRHIResult::InvalidArgument);

		eastl::vector<uint8_t> OutputReadback;
		ASSERT_TRUE(Commands.mValue->CopyBufferDeviceToHost(*Output.mValue, OutputReadback, 0, sizeof(uint32_t)));
		ExpectBufferState(*Commands.mValue, *Output.mValue, EArdaRHIResourceState::CopySource);
		ASSERT_TRUE(Commands.mValue->Close());
		ASSERT_TRUE(Device->ExecuteCommandList(Commands.mValue));
		ASSERT_TRUE(Device->WaitForIdle());

		auto ReadbackMapping = Device->MapStagingTexture(Readback.mValue, WholeSlice, EArdaRHICpuAccess::Read);
		ASSERT_TRUE(ReadbackMapping) << ReadbackMapping.mStatus.mMessage.c_str();
		ASSERT_NE(ReadbackMapping.mValue.mData, nullptr);
		ASSERT_GE(ReadbackMapping.mValue.mRowPitch, 16u);
		for (uint32_t Y = 0; Y < 2; ++Y)
		{
			const auto* Row = static_cast<const uint8_t*>(ReadbackMapping.mValue.mData) +
			    (Y + StagingRegion.mY) * ReadbackMapping.mValue.mRowPitch;
			for (uint32_t X = 0; X < 2; ++X)
			{
				const uint8_t ExpectedPixel[4] = {static_cast<uint8_t>(17 + X + StagingRegion.mX),
				    static_cast<uint8_t>(91 + Y + StagingRegion.mY),
				    203,
				    255};
				EXPECT_EQ(0, std::memcmp(Row + (X + StagingRegion.mX) * 4, ExpectedPixel, sizeof(ExpectedPixel)));
			}
		}
		ASSERT_TRUE(Device->UnmapStagingTexture(Readback.mValue));
		ASSERT_EQ(OutputReadback.size(), sizeof(uint32_t));
		uint32_t OutputValue = 0;
		std::memcpy(&OutputValue, OutputReadback.data(), sizeof(OutputValue));
		EXPECT_EQ(OutputValue, 0xA2DAu);

		auto Persisted = Device->CreateCommandList(EArdaRHIQueueType::Graphics);
		ASSERT_TRUE(Persisted);
		ASSERT_TRUE(Persisted.mValue->Open());
		ExpectTextureState(*Persisted.mValue, *Destination.mValue, WholeRange, EArdaRHIResourceState::ShaderResource);
		ExpectBufferState(*Persisted.mValue, *Arguments.mValue, EArdaRHIResourceState::IndirectArgument);
		ExpectBufferState(*Persisted.mValue, *Output.mValue, EArdaRHIResourceState::CopySource);
	...
	void VerifyResolveAndPlaneTracking(const char* BackendName)
	{
		using namespace arda;
		...
		ASSERT_TRUE(Commands.mValue->Open());
		const FArdaRHITextureSubresourceRange WholeRange{};
		ASSERT_TRUE(Commands.mValue->SetTextureState(*Source.mValue, WholeRange, EArdaRHIResourceState::RenderTarget));
		ExpectTextureState(*Commands.mValue, *Source.mValue, WholeRange, EArdaRHIResourceState::RenderTarget);
		ASSERT_TRUE(Commands.mValue->ClearTexture(*Source.mValue, WholeRange, {0.25f, 0.5f, 0.75f, 1.0f}));
		ExpectTextureState(*Commands.mValue, *Source.mValue, WholeRange, EArdaRHIResourceState::RenderTarget);
		...
		Slice.mHeight = 4;
		Slice.mDepth = 1;
		ASSERT_TRUE(Commands.mValue->ResolveTexture(*Destination.mValue, Slice, *Source.mValue, Slice));
		ExpectTextureState(*Commands.mValue, *Source.mValue, WholeRange, EArdaRHIResourceState::RenderTarget);
		ExpectTextureState(*Commands.mValue, *Destination.mValue, WholeRange, EArdaRHIResourceState::Common);
		ASSERT_TRUE(Commands.mValue->CopyTextureToStaging(*Readback.mValue, Slice, *Destination.mValue, Slice));
		ExpectTextureState(*Commands.mValue, *Destination.mValue, WholeRange, EArdaRHIResourceState::Common);

		const FArdaRHITextureSubresourceRange DepthPlane{0, 1, 0, 1, 0, 1};
		const FArdaRHITextureSubresourceRange StencilPlane{0, 1, 0, 1, 1, 1};
		ASSERT_TRUE(
		    Commands.mValue->SetTextureState(*DepthStencil.mValue, DepthPlane, EArdaRHIResourceState::DepthWrite));
		ExpectTextureState(*Commands.mValue, *DepthStencil.mValue, DepthPlane, EArdaRHIResourceState::DepthWrite);
		ExpectTextureState(*Commands.mValue, *DepthStencil.mValue, StencilPlane, EArdaRHIResourceState::Common);
		const auto MixedPlanes = Commands.mValue->QueryTextureState(*DepthStencil.mValue, WholeRange);
	...
	void VerifyExplicitHeapAliasing(const char* BackendName)
	{
		using namespace arda;
		...
		ASSERT_TRUE(First);
		ASSERT_TRUE(Second);
		auto FirstRequirements = Device->GetBufferMemoryRequirements(First.mValue);
		auto SecondRequirements = Device->GetBufferMemoryRequirements(Second.mValue);
		...
		EXPECT_EQ(FirstRequirements.mValue.mSize, SecondRequirements.mValue.mSize);
		EXPECT_EQ(FirstRequirements.mValue.mAlignment, SecondRequirements.mValue.mAlignment);

		FArdaRHIHeapDesc HeapDesc;
		HeapDesc.mDebugName = "Explicit overlapping buffer heap";
		HeapDesc.mCapacity = eastl::max(FirstRequirements.mValue.mSize, SecondRequirements.mValue.mSize);
		HeapDesc.mMemoryTypeBits = FirstRequirements.mValue.mMemoryTypeBits & SecondRequirements.mValue.mMemoryTypeBits;
		...
		const FArdaRHIStatus Rebind = Device->BindBufferMemory(First.mValue, Heap.mValue, 0);
		EXPECT_FALSE(Rebind);
		...
		ASSERT_TRUE(Commands);
		ASSERT_TRUE(Commands.mValue->Open());
		ExpectBufferState(*Commands.mValue, *First.mValue, EArdaRHIResourceState::Common);
		ExpectBufferState(*Commands.mValue, *Second.mValue, EArdaRHIResourceState::Common);
		const uint32_t FirstPattern = 0x11111111u;
		const uint32_t SecondPattern = 0x6a7b8c9du;
		ASSERT_TRUE(Commands.mValue->WriteBuffer(*First.mValue, &FirstPattern, sizeof(FirstPattern)));
		ExpectBufferState(*Commands.mValue, *First.mValue, EArdaRHIResourceState::Common);
		ASSERT_TRUE(Commands.mValue->AliasingBarrier(First.mValue.Get(), Second.mValue.Get()));
		ExpectBufferState(*Commands.mValue, *Second.mValue, EArdaRHIResourceState::Common);
		ASSERT_TRUE(Commands.mValue->WriteBuffer(*Second.mValue, &SecondPattern, sizeof(SecondPattern)));
		ExpectBufferState(*Commands.mValue, *Second.mValue, EArdaRHIResourceState::Common);
		eastl::vector<uint8_t> Readback;
		ASSERT_TRUE(Commands.mValue->CopyBufferDeviceToHost(*Second.mValue, Readback, 0, sizeof(SecondPattern)));
		ExpectBufferState(*Commands.mValue, *Second.mValue, EArdaRHIResourceState::CopySource);
		ASSERT_TRUE(Commands.mValue->Close());
		ASSERT_TRUE(Device->ExecuteCommandList(Commands.mValue));
		ASSERT_TRUE(Device->WaitForIdle());
		ASSERT_EQ(Readback.size(), sizeof(SecondPattern));
		uint32_t ActualPattern = 0;
		std::memcpy(&ActualPattern, Readback.data(), sizeof(ActualPattern));
	...
	void VerifyComputeArithmetic(const char* BackendName, const char* Artifact, const char* Entry)
	{
		...
		ASSERT_TRUE(Commands);
		ASSERT_TRUE(Commands.mValue->Open());
		const uint32_t Data[] = {bFloat16 ? 0x3fc00000u : 0xff030201u, bFloat16 ? 0x40200000u : 0x04030201u, 0, 0};
	...
	void VerifyRuntimeDescriptorVersions(const char* BackendName)
	{
		...
		auto Shader = CreateExtendedShader(*Device,
		    BackendName,
		    "ArdaRuntimeDescriptors",
		    "RuntimeDescriptorsCS",
		    EArdaRHIShaderStage::Compute);
		ASSERT_TRUE(Shader);
		FArdaRHIComputePipelineDesc PipelineDesc;
		PipelineDesc.mComputeShader = Shader.mValue;
	...
	void VerifyBindlessDescriptorTable(const char* BackendName,
	    ...
	    bool bDescriptorBuffer = false)
	{
		...
		if (std::strcmp(BackendName, "native-d3d12") == 0)
		{
			FArdaRHIFeatureRequirements Requirements;
			Requirements.mbRequireDescriptorHeap = true;
			const auto Admission = Device->RequireFeatures(Requirements);
			ASSERT_TRUE(Admission) << Admission.mMessage.c_str();
		}
		if (bDirectHeapIndexing && !Device->GetCapabilities().mDescriptors.mbDirectResourceHeapIndexing)
		{
		...
		LayoutDesc.mLayoutType = EArdaRHIBindlessLayoutType::MutableSrvUavCbv;
		...
		LayoutDesc.mRegisterSpaces.push_back({0, 1, EArdaRHIBindingType::StructuredBufferUAV});
		...
		for (uint32_t Element = 0; Element < 4; ++Element)
		{
			...
			Item.mType = EArdaRHIBindingType::StructuredBufferUAV;
			Item.mResource = TArdaRHIRef<IArdaRHIResource>(Output.mValue.Get());
		...
		FArdaRHIComputeState State;
		State.mPipeline = Pipeline.mValue;
		State.mBindings.push_back(FArdaRHIBindingSetRef(Table.mValue.Get()));
		ASSERT_TRUE(Commands.mValue->SetComputeState(State));
		Commands.mValue->Dispatch(1, 1, 1);
		eastl::vector<uint8_t> Readback;
		ASSERT_TRUE(Commands.mValue->CopyBufferDeviceToHost(*Output.mValue, Readback, 0, sizeof(uint32_t)));
		ExpectBufferState(*Commands.mValue, *Output.mValue, EArdaRHIResourceState::CopySource);
		ASSERT_TRUE(Commands.mValue->Close());
		ASSERT_TRUE(Device->ExecuteCommandList(Commands.mValue));
		ASSERT_TRUE(Device->WaitForIdle());
		...
		InvalidItem.mResource = TArdaRHIRef<IArdaRHIResource>(Output.mValue.Get());
		const FArdaRHIStatus InvalidWrite = Device->WriteDescriptorTable(Table.mValue, InvalidItem);
		EXPECT_FALSE(InvalidWrite);
		EXPECT_EQ(InvalidWrite.mCode, EArdaRHIResult::InvalidArgument);
		EXPECT_EQ(Diagnostics.GetErrorCount(), 0u);
	}

	void VerifyDirectResourceAndSamplerHeapIndexing(const char* BackendName, bool bDescriptorBuffer = false)
	{
		using namespace arda;
		...
		auto Device = GetDevice();
		ASSERT_TRUE(Device);
		const auto& DescriptorCaps = Device->GetCapabilities().mDescriptors;
		if (bDescriptorBuffer
		        ? !DescriptorCaps.mbDescriptorBuffer
		        : (!DescriptorCaps.mbDirectResourceHeapIndexing || !DescriptorCaps.mbDirectSamplerHeapIndexing))
		{
		...
		TextureDesc.mFormat = EArdaRHIFormat::RGBA8UNorm;
		TextureDesc.mUsage = EArdaRHITextureUsage::ShaderResource | EArdaRHITextureUsage::RenderTarget;
		...
		ResourceLayoutDesc.mLayoutType = EArdaRHIBindlessLayoutType::MutableSrvUavCbv;
		ResourceLayoutDesc.mRegisterSpaces.push_back({0, 1, EArdaRHIBindingType::TextureSRV});
		ResourceLayoutDesc.mRegisterSpaces.push_back({0, 1, EArdaRHIBindingType::StructuredBufferUAV});
		ResourceLayoutDesc.mDebugName = "Direct resource heap layout";
		auto ResourceLayout = Device->CreateBindlessLayout(ResourceLayoutDesc);
		ASSERT_TRUE(ResourceLayout) << ResourceLayout.mStatus.mMessage.c_str();
		auto ResourceTable = Device->CreateDescriptorTable(ResourceLayout.mValue);
		ASSERT_TRUE(ResourceTable) << ResourceTable.mStatus.mMessage.c_str();
		...
		ASSERT_TRUE(Device->WriteDescriptorTable(ResourceTable.mValue, TextureBinding));
		FArdaRHIBindingItem OutputBinding;
		OutputBinding.mType = EArdaRHIBindingType::StructuredBufferUAV;
		OutputBinding.mResource = FArdaRHIResourceRef(Output.mValue.Get());
		ASSERT_TRUE(Device->WriteDescriptorTable(ResourceTable.mValue, OutputBinding));
		...
		SamplerLayoutDesc.mLayoutType = EArdaRHIBindlessLayoutType::MutableSampler;
		SamplerLayoutDesc.mRegisterSpaces.push_back({0, 1, EArdaRHIBindingType::Sampler});
		SamplerLayoutDesc.mDebugName = "Direct sampler heap layout";
		auto SamplerLayout = Device->CreateBindlessLayout(SamplerLayoutDesc);
		ASSERT_TRUE(SamplerLayout) << SamplerLayout.mStatus.mMessage.c_str();
		auto SamplerTable = Device->CreateDescriptorTable(SamplerLayout.mValue);
		ASSERT_TRUE(SamplerTable) << SamplerTable.mStatus.mMessage.c_str();
		...
		ASSERT_TRUE(Device->WriteDescriptorTable(SamplerTable.mValue, SamplerBinding));

		auto Shader = CreateExtendedShader(*Device,
		    BackendName,
		    "ArdaDirectHeapSamplerTest",
		    "DirectHeapSamplerTestCS",
		    EArdaRHIShaderStage::Compute);
		ASSERT_TRUE(Shader) << Shader.mStatus.mMessage.c_str();
		FArdaRHIComputePipelineDesc PipelineDesc;
		PipelineDesc.mComputeShader = Shader.mValue;
		PipelineDesc.mBindingLayouts = {ResourceLayout.mValue, SamplerLayout.mValue};
		auto Pipeline = Device->CreateComputePipeline(PipelineDesc);
		ASSERT_TRUE(Pipeline) << Pipeline.mStatus.mMessage.c_str();
		...
		ASSERT_TRUE(Commands);
		ASSERT_TRUE(Commands.mValue->Open());
		ASSERT_TRUE(Commands.mValue->ClearTexture(*Texture.mValue, {}, {0.25f, 0.5f, 0.75f, 1.f}));
		ASSERT_TRUE(Commands.mValue->SetTextureState(*Texture.mValue, {}, EArdaRHIResourceState::ShaderResource));
		ASSERT_TRUE(Commands.mValue->SetBufferState(*Output.mValue, EArdaRHIResourceState::UnorderedAccess));
		FArdaRHIComputeState State;
		State.mPipeline = Pipeline.mValue;
		State.mBindings = {FArdaRHIBindingSetRef(ResourceTable.mValue.Get()),
	...
	void VerifyShaderBundleExecution(const char* BackendName)
	{
		using namespace arda;
		ShutdownBackend();
		FArdaExtendedDiagnosticCallback Diagnostics;
		...
		ASSERT_TRUE(Device->SetShaderBundleRecords(Bundle.mValue, {RecordA, RecordB}));
		...
		ExpectBufferState(*Commands.mValue, *OutputA.mValue, EArdaRHIResourceState::UnorderedAccess);
		ExpectBufferState(*Commands.mValue, *OutputB.mValue, EArdaRHIResourceState::UnorderedAccess);
	...
	void VerifyD3D12WorkGraphExecution()
	{
		...
		auto Device = GetDevice();
		ASSERT_TRUE(Device);
		if (Device->GetCapabilities().mWorkGraphTier == EArdaRHIWorkGraphTier::None)
		{
		...
		ASSERT_TRUE(Commands.mValue->SetBufferState(*Output.mValue, EArdaRHIResourceState::UnorderedAccess));
		ASSERT_TRUE(
		    Commands.mValue->DispatchWorkGraph(*Pipeline.mValue, &InputRecord, 1, sizeof(InputRecord), {Set.mValue}));
		ExpectBufferState(*Commands.mValue, *Output.mValue, EArdaRHIResourceState::UnorderedAccess);
		eastl::vector<uint8_t> Readback;
		ASSERT_TRUE(Commands.mValue->CopyBufferDeviceToHost(*Output.mValue, Readback, 0, sizeof(uint32_t)));
		ASSERT_TRUE(Commands.mValue->Close());
	    ...
	    bool bIndirectFirstInstance = false)
	{
		...
		auto Vertex = CreateExtendedShader(*Device,
		    BackendName,
		    "ArdaRasterStageVS",
		    "RasterStageVS",
		    EArdaRHIShaderStage::Vertex);
		auto Pixel = CreateExtendedShader(*Device,
		    BackendName,
		    "ArdaRasterStagePS",
		    "RasterStagePS",
		    EArdaRHIShaderStage::Pixel);
		ASSERT_TRUE(Vertex) << Vertex.mStatus.mMessage.c_str();
		ASSERT_TRUE(Pixel) << Pixel.mStatus.mMessage.c_str();
		FArdaRHIGraphicsPipelineDesc PipelineDesc;
		PipelineDesc.mDebugName =
		    bTessellation ? "Hull/domain capability conformance" : "Geometry capability conformance";
		...
		if (bTessellation)
		{
			auto Hull = CreateExtendedShader(*Device,
			    BackendName,
			    "ArdaRasterStageHS",
			    "RasterStageHS",
			    EArdaRHIShaderStage::Hull);
			auto Domain = CreateExtendedShader(*Device,
			    BackendName,
			    "ArdaRasterStageDS",
			    "RasterStageDS",
			    EArdaRHIShaderStage::Domain);
			ASSERT_TRUE(Hull) << Hull.mStatus.mMessage.c_str();
			ASSERT_TRUE(Domain) << Domain.mStatus.mMessage.c_str();
			PipelineDesc.mHullShader = Hull.mValue;
			PipelineDesc.mDomainShader = Domain.mValue;
		...
		else if (!bIndirectFirstInstance)
		{
			auto Geometry = CreateExtendedShader(*Device,
			    ...
			    "ArdaRasterStageGS",
			    "RasterStageGS",
			    EArdaRHIShaderStage::Geometry);
			ASSERT_TRUE(Geometry) << Geometry.mStatus.mMessage.c_str();
			PipelineDesc.mGeometryShader = Geometry.mValue;
		}
		auto Pipeline = Device->CreateGraphicsPipeline(PipelineDesc);
		ASSERT_TRUE(Pipeline) << Pipeline.mStatus.mMessage.c_str();

		FArdaRHITextureDesc TargetDesc;
		TargetDesc.mDebugName = "Raster stage native render target";
		TargetDesc.mWidth = TargetDesc.mHeight = 4;
		...
		ASSERT_TRUE(Commands);
		ASSERT_TRUE(Commands.mValue->Open());
		ASSERT_TRUE(Commands.mValue->ClearTexture(*Target.mValue, {}, {0, 0, 0, 1}));
		...
		Slice.mDepth = 1;
		ASSERT_TRUE(Commands.mValue->CopyTextureToStaging(*Readback.mValue, Slice, *Target.mValue, Slice));
		ASSERT_TRUE(Commands.mValue->Close());
		ASSERT_TRUE(Device->ExecuteCommandList(Commands.mValue));
		ASSERT_TRUE(Device->WaitForIdle());
	...
	void VerifyMeshPipelineCapabilityAndExecution(const char* BackendName, bool bBundle = false)
	{
		using namespace arda;
		...
		auto MeshShader = CreateExtendedShader(*Device,
		    BackendName,
		    "ArdaMeshPipelineTestMS",
		    "MeshPipelineTestMS",
		    EArdaRHIShaderStage::Mesh);
		auto PixelShader = CreateExtendedShader(*Device,
		    BackendName,
		...
		if (Device->GetCapabilities().mMeshShaderTier == EArdaRHIMeshShaderTier::None)
		{
		...
		ASSERT_TRUE(
		    Commands.mValue->BeginTrackingTextureState(*Target.mValue, {}, EArdaRHIResourceState::RenderTarget));
		ExpectTextureState(*Commands.mValue, *Target.mValue, {}, EArdaRHIResourceState::RenderTarget);
		ASSERT_TRUE(Commands.mValue->ClearTexture(*Target.mValue, {}, {0.f, 0.f, 0.f, 1.f}));
		ExpectTextureState(*Commands.mValue, *Target.mValue, {}, EArdaRHIResourceState::RenderTarget);
		...
		ToReadback.mDestinationPipelines = EArdaRHIPipeline::Copy;
		ASSERT_TRUE(Commands.mValue->TransitionTexture(*Target.mValue, ToReadback));
		ExpectTextureState(*Commands.mValue, *Target.mValue, {}, EArdaRHIResourceState::CopySource);
		FArdaRHITextureSlice Slice;
		Slice.mWidth = 4;
		Slice.mHeight = 4;
		Slice.mDepth = 1;
		...
		ASSERT_GE(Mapping.mValue.mRowPitch, 16u);
		const auto* Center =
		    static_cast<const uint8_t*>(Mapping.mValue.mData) + 2u * Mapping.mValue.mRowPitch + 2u * 4u;
	...
	void VerifyQueueBreadth(const char* BackendName)
	{
		using namespace arda;
		...
		const FArdaRHICapabilities& Capabilities = Device->GetCapabilities();
		const bool bD3D12 = std::strcmp(BackendName, "native-d3d12") == 0;
		if (bD3D12)
		{
			FArdaRHIFeatureRequirements Requirements;
			Requirements.mbRequireTimelineSynchronization = true;
		...
		if (Capabilities.mQueues.mbCopy)
		{
			...
			auto CopyCommands = Device->CreateCommandList(EArdaRHIQueueType::Copy);
			ASSERT_TRUE(CopyCommands);
			ASSERT_TRUE(CopyCommands.mValue->Open());
			const uint32_t Expected = 0xC0A1C0A1u;
			ASSERT_TRUE(CopyCommands.mValue->WriteBuffer(*Source.mValue, &Expected, sizeof(Expected)));
			ExpectBufferState(*CopyCommands.mValue, *Source.mValue, EArdaRHIResourceState::Common);
			...
			ASSERT_TRUE(CopyCommands.mValue->TransitionBuffer(*Source.mValue, SourceTransition));
			...
			ASSERT_TRUE(CopyCommands.mValue->TransitionBuffer(*Destination.mValue, DestinationTransition));
			ExpectBufferState(*CopyCommands.mValue, *Source.mValue, EArdaRHIResourceState::CopySource);
			ExpectBufferState(*CopyCommands.mValue, *Destination.mValue, EArdaRHIResourceState::CopyDest);
			ASSERT_TRUE(CopyCommands.mValue->CopyBuffer(*Destination.mValue, 0, *Source.mValue, 0, sizeof(Expected)));
			ExpectBufferState(*CopyCommands.mValue, *Source.mValue, EArdaRHIResourceState::CopySource);
			ExpectBufferState(*CopyCommands.mValue, *Destination.mValue, EArdaRHIResourceState::CopyDest);
			ASSERT_TRUE(CopyCommands.mValue->Close());
			auto Submission = Device->ExecuteCommandList(CopyCommands.mValue);
			ASSERT_TRUE(Submission);
			ASSERT_TRUE(Device->QueueWait(EArdaRHIQueueType::Graphics, EArdaRHIQueueType::Copy, Submission.mValue));

			auto ReadbackCommands = Device->CreateCommandList(EArdaRHIQueueType::Graphics);
			ASSERT_TRUE(ReadbackCommands);
			ASSERT_TRUE(ReadbackCommands.mValue->Open());
			eastl::vector<uint8_t> Readback;
			ASSERT_TRUE(
			    ReadbackCommands.mValue->CopyBufferDeviceToHost(*Destination.mValue, Readback, 0, sizeof(Expected)));
			ExpectBufferState(*ReadbackCommands.mValue, *Destination.mValue, EArdaRHIResourceState::CopySource);
			...
			if (bD3D12)
			{
				// The existing graphics readback completed after its GPU wait on this copy timeline.
				const auto Completed = Device->PollSubmission(Submission.mValue);
				ASSERT_TRUE(Completed) << Completed.mStatus.mMessage.c_str();
				EXPECT_TRUE(Completed.mValue);
			}
		}
		else
		{
		...
		if (Capabilities.mQueues.mbCompute)
		{
			...
			auto ComputeCommands = Device->CreateCommandList(EArdaRHIQueueType::Compute);
			ASSERT_TRUE(ComputeCommands);
			ASSERT_TRUE(ComputeCommands.mValue->Open());
			ASSERT_TRUE(ComputeCommands.mValue->SetBufferState(*Output.mValue, EArdaRHIResourceState::UnorderedAccess));
			ExpectBufferState(*ComputeCommands.mValue, *Output.mValue, EArdaRHIResourceState::UnorderedAccess);
			FArdaRHIComputeState State;
			State.mPipeline = Pipeline.mValue;
			...
			eastl::vector<uint8_t> Readback;
			ASSERT_TRUE(ComputeCommands.mValue->CopyBufferDeviceToHost(*Output.mValue, Readback, 0, sizeof(uint32_t)));
			ExpectBufferState(*ComputeCommands.mValue, *Output.mValue, EArdaRHIResourceState::CopySource);
	...
	void VerifyRayTracingPipelineCapabilityAndExecution(const char* BackendName, bool bLocalDescriptors = false)
	{
		using namespace arda;
		...
		auto RayGeneration = CreateExtendedShader(*Device,
		    BackendName,
		    "ArdaRayTracingTest",
		    "RayGen",
		    EArdaRHIShaderStage::RayGeneration);
		ASSERT_TRUE(RayGeneration) << RayGeneration.mStatus.mMessage.c_str();
		FArdaRHIBindingLayoutDesc LayoutDesc;
		...
		EXPECT_GT(Device->GetCapabilities().mRayTracing.mMaxRayDispatchInvocations, 0u);
		ASSERT_TRUE(Pipeline) << Pipeline.mStatus.mMessage.c_str();
		...
		auto ShaderTable = Device->CreateShaderTable(Pipeline.mValue, TableDesc);
		ASSERT_TRUE(ShaderTable) << ShaderTable.mStatus.mMessage.c_str();
		ASSERT_TRUE(Device->SetShaderTableRayGeneration(ShaderTable.mValue,
		    "RayGen",
		    bLocalDescriptors ? BindingSet.mValue : FArdaRHIBindingSetRef{}));
		...
		const FArdaRHIStatus OversizedDispatch = Commands.mValue->DispatchRays(UINT32_MAX, 2, 1);
		EXPECT_FALSE(OversizedDispatch);
		EXPECT_EQ(OversizedDispatch.mCode, EArdaRHIResult::InvalidArgument);
	...
	void VerifyAccelerationStructureLifecycleAndStateParity(const char* BackendName)
	{
		using namespace arda;
		...
		BlasDesc.mBuildFlags = EArdaRHIAccelStructBuildFlags::AllowUpdate |
		...
		const auto BlasRequirements = Device->GetAccelStructBuildMemoryRequirements(BlasDesc);
		ASSERT_TRUE(BlasRequirements) << BlasRequirements.mStatus.mMessage.c_str();
		...
		EXPECT_EQ(Blas.mValue->GetBuildState(), EArdaRHIAccelStructBuildState::Unbuilt);
		...
		ASSERT_TRUE(Build.mValue->WriteBuffer(*VertexBuffer.mValue, Vertices, sizeof(Vertices), 0));
		ASSERT_TRUE(Build.mValue->BuildBottomLevelAccelStruct(*Blas.mValue, {Geometry}, BlasDesc.mBuildFlags));
		...
		EXPECT_EQ(BlasState.mValue.mFacadeState, EArdaRHIResourceState::AccelStructRead);
		FArdaRHIRayTracingInstanceDesc Instance;
		Instance.mBottomLevelAccelStruct = Blas.mValue;
		ASSERT_TRUE(Build.mValue->BuildTopLevelAccelStruct(*Tlas.mValue, {Instance}, TlasDesc.mBuildFlags));
		...
		ASSERT_TRUE(BuiltSubmission) << BuiltSubmission.mStatus.mMessage.c_str();
		ASSERT_TRUE(Device->WaitForIdle());
		EXPECT_EQ(Blas.mValue->GetBuildState(), EArdaRHIAccelStructBuildState::Built);
		EXPECT_EQ(Tlas.mValue->GetBuildState(), EArdaRHIAccelStructBuildState::Built);

		auto Update = Device->CreateCommandList(EArdaRHIQueueType::Graphics);
		ASSERT_TRUE(Update);
		ASSERT_TRUE(Update.mValue->Open());
		ASSERT_TRUE(Update.mValue->BuildBottomLevelAccelStruct(*Blas.mValue,
		    {Geometry},
		    BlasDesc.mBuildFlags | EArdaRHIAccelStructBuildFlags::PerformUpdate));
		...
		ASSERT_TRUE(Device->WaitForIdle());
		EXPECT_EQ(Blas.mValue->GetBuildState(), EArdaRHIAccelStructBuildState::Updated);

		const auto CompactedSize = Device->GetAccelStructCompactedSize(Blas.mValue);
		ASSERT_TRUE(CompactedSize) << CompactedSize.mStatus.mMessage.c_str();
		EXPECT_GT(CompactedSize.mValue, 0u);
		EXPECT_LE(CompactedSize.mValue, BlasRequirements.mValue.mResultSize);
		...
		ASSERT_TRUE(CompactBlas) << CompactBlas.mStatus.mMessage.c_str();
		auto Compact = Device->CreateCommandList(EArdaRHIQueueType::Graphics);
		ASSERT_TRUE(Compact);
		ASSERT_TRUE(Compact.mValue->Open());
		ASSERT_TRUE(Compact.mValue->CompactAccelStruct(*CompactBlas.mValue, *Blas.mValue));
		const auto CompactState = Compact.mValue->QueryAccelStructState(*CompactBlas.mValue);
		ASSERT_TRUE(CompactState);
		EXPECT_TRUE(CompactState.mValue.IsConsistent());
		EXPECT_EQ(CompactState.mValue.mFacadeState, EArdaRHIResourceState::AccelStructRead);
		...
		EXPECT_EQ(CompactBlas.mValue->GetBuildState(), EArdaRHIAccelStructBuildState::Compacted);
		EXPECT_EQ(Diagnostics.GetErrorCount(), 0u);
	}

	void VerifyRayTracingSceneHitGroupsLocalArgumentsAndIndirect(const char* BackendName,
	    bool bInlineQuery = false,
	    bool bInstanceBuffer = false)
	{
		...
		ASSERT_TRUE(Device);
		const auto& Ray = Device->GetCapabilities().mRayTracing;
		if (!Ray.mbPipelineShaders || !Ray.mbAccelerationStructures || !Ray.mbIndirectDispatch ||
		    !Ray.mbLocalShaderTableArguments)
		{
			GTEST_SKIP() << "Native scene tracing, indirect dispatch, or local SBT arguments are unavailable.";
		}

		auto RayGeneration = CreateExtendedShader(*Device,
		    BackendName,
		    "ArdaLocalRayTracingTest",
		    "KnownSceneRayGen",
		    EArdaRHIShaderStage::RayGeneration);
		auto Miss = CreateExtendedShader(*Device,
		    BackendName,
		    "ArdaLocalRayTracingTest",
		    "KnownMiss",
		    EArdaRHIShaderStage::Miss);
		auto ClosestHit = CreateExtendedShader(*Device,
		    BackendName,
		    "ArdaLocalRayTracingTest",
		    "KnownClosestHit",
		...
		LayoutDesc.mItems.push_back({0, 1, EArdaRHIBindingType::RayTracingAccelStruct});
		LayoutDesc.mItems.push_back({0, 1, EArdaRHIBindingType::StructuredBufferUAV});
		...
		FArdaRHIRayTracingPipelineDesc PipelineDesc;
		PipelineDesc.mDebugName = "Known-result hit/miss ray tracing pipeline";
		PipelineDesc.mShaders.push_back({"KnownSceneRayGen", RayGeneration.mValue, LocalLayout.mValue});
		PipelineDesc.mShaders.push_back({"KnownMiss", Miss.mValue, LocalLayout.mValue});
		...
		SceneItem.mResource = TArdaRHIRef<IArdaRHIResource>(Tlas.mValue.Get());
		SetDesc.mItems.push_back(SceneItem);
		FArdaRHIBindingItem OutputItem;
		OutputItem.mSlot = 0;
		OutputItem.mType = EArdaRHIBindingType::StructuredBufferUAV;
		OutputItem.mResource = TArdaRHIRef<IArdaRHIResource>(Output.mValue.Get());
		...
		TArdaRHIResult<FArdaRHIBindingSetRef> BindingSet{FArdaRHIBindingSetRef(DescriptorTable.mValue.Get()), {}};
		ASSERT_TRUE(BindingSet) << BindingSet.mStatus.mMessage.c_str();
		...
		TableDesc.mDebugName = "Known-result ray-scene shader table";
		auto ShaderTable = Device->CreateShaderTable(Pipeline.mValue, TableDesc);
		ASSERT_TRUE(ShaderTable) << ShaderTable.mStatus.mMessage.c_str();
		const auto WriteRecord = [&Device, &ShaderTable](uint32_t Index,
		                             ...
		                             uint32_t LocalValue)
		{
			...
			std::memcpy(Record.mLocalArguments.data(), &LocalValue, sizeof(LocalValue));
			return Device->SetShaderTableRecord(ShaderTable.mValue, Record);
		};
		ASSERT_TRUE(WriteRecord(0, EArdaRHIShaderTableRecordType::RayGeneration, "KnownSceneRayGen", 0x11111111u));
		ASSERT_TRUE(WriteRecord(1, EArdaRHIShaderTableRecordType::Miss, "KnownMiss", 0x22222222u));
		ASSERT_TRUE(WriteRecord(2, EArdaRHIShaderTableRecordType::HitGroup, "KnownHitGroup", 0x33333333u));
		...
		const auto OversizedStatus = Device->SetShaderTableRecord(ShaderTable.mValue, OversizedRecord);
		...
		ASSERT_TRUE(Commands);
		ASSERT_TRUE(Commands.mValue->Open());
		ASSERT_TRUE(Commands.mValue->WriteBuffer(*VertexBuffer.mValue, Vertices, sizeof(Vertices), 0));
		ASSERT_TRUE(Commands.mValue->BuildBottomLevelAccelStruct(*Blas.mValue, {Geometry}, BlasDesc.mBuildFlags));
		FArdaRHIRayTracingInstanceDesc Instance;
		Instance.mBottomLevelAccelStruct = Blas.mValue;
		if (bInstanceBuffer)
		{
			...
			ASSERT_TRUE(
			    Commands.mValue->SetBufferState(*Instances.mValue, EArdaRHIResourceState::AccelStructBuildInput));
			ASSERT_TRUE(Commands.mValue
			        ->BuildTopLevelAccelStructFromBuffer(*Tlas.mValue, *Instances.mValue, 0, 1, TlasDesc.mBuildFlags));
			EXPECT_EQ(
			    Commands.mValue
			        ->BuildTopLevelAccelStructFromBuffer(*Tlas.mValue, *Instances.mValue, 16, 1, TlasDesc.mBuildFlags)
			        .mCode,
			    EArdaRHIResult::InvalidArgument);
		}
		else
		{
			ASSERT_TRUE(Commands.mValue->BuildTopLevelAccelStruct(*Tlas.mValue, {Instance}, TlasDesc.mBuildFlags));
		}
		const auto BlasState = Commands.mValue->QueryAccelStructState(*Blas.mValue);
		const auto TlasState = Commands.mValue->QueryAccelStructState(*Tlas.mValue);
		ASSERT_TRUE(BlasState);
		ASSERT_TRUE(TlasState);
		EXPECT_TRUE(BlasState.mValue.IsConsistent());
		EXPECT_TRUE(TlasState.mValue.IsConsistent());
		EXPECT_EQ(BlasState.mValue.mFacadeState, EArdaRHIResourceState::AccelStructRead);
		EXPECT_EQ(TlasState.mValue.mFacadeState, EArdaRHIResourceState::AccelStructRead);
		ASSERT_TRUE(Commands.mValue->SetBufferState(*Output.mValue, EArdaRHIResourceState::UnorderedAccess));
		ASSERT_TRUE(Commands.mValue->WriteBuffer(*Indirect.mValue, Dimensions, sizeof(Dimensions), 0));
		ASSERT_TRUE(Commands.mValue->SetBufferState(*Indirect.mValue, EArdaRHIResourceState::IndirectArgument));
		ExpectBufferState(*Commands.mValue, *Indirect.mValue, EArdaRHIResourceState::IndirectArgument);
		FArdaRHIRayTracingState State;
		State.mShaderTable = ShaderTable.mValue;
		State.mBindings.push_back(BindingSet.mValue);
		if (bInlineQuery)
	...
	void VerifyVulkanOpacityMicromapLifecycleAndStateParity()
	{
		...
		InputDesc.mUsage = EArdaRHIBufferUsage::OpacityMicromapBuildInput;
		InputDesc.mCpuAccess = EArdaRHICpuAccess::Write;
		InputDesc.mInitialState = EArdaRHIResourceState::OpacityMicromapBuildInput;
		...
		Desc.mCounts.push_back({1, 0, EArdaRHIOpacityMicromapFormat::TwoState});
		Desc.mInputBuffer = Input.mValue;
		Desc.mPerMicromapDescBuffer = Triangles.mValue;
		Desc.mFlags = EArdaRHIOpacityMicromapBuildFlags::FastTrace | EArdaRHIOpacityMicromapBuildFlags::AllowCompaction;
		...
		EXPECT_EQ(Micromap.mValue->GetBuildState(), EArdaRHIAccelStructBuildState::Unbuilt);

		struct FArdaMicromapTriangle
		{
		...
		static_assert(sizeof(FArdaMicromapTriangle) == 8);
		const uint8_t EncodedOpacity = 0xff;
		const FArdaMicromapTriangle Triangle = {0, 0, static_cast<uint16_t>(EArdaRHIOpacityMicromapFormat::TwoState)};
		...
		FArdaRHIAccelStructDesc BlasDesc;
		BlasDesc.mBottomLevelGeometries.push_back(Geometry);
		BlasDesc.mBuildFlags = EArdaRHIAccelStructBuildFlags::PreferFastTrace;
		...
		ASSERT_TRUE(Commands);
		ASSERT_TRUE(Commands.mValue->Open());
		ASSERT_TRUE(Commands.mValue->WriteBuffer(*Input.mValue, &EncodedOpacity, sizeof(EncodedOpacity)));
		ASSERT_TRUE(Commands.mValue->WriteBuffer(*Triangles.mValue, &Triangle, sizeof(Triangle)));
		ASSERT_TRUE(Commands.mValue->WriteBuffer(*VertexBuffer.mValue, Vertices, sizeof(Vertices)));
		auto Before = Commands.mValue->QueryOpacityMicromapState(*Micromap.mValue);
		ASSERT_TRUE(Before) << Before.mStatus.mMessage.c_str();
		EXPECT_TRUE(Before.mValue.IsConsistent());
		EXPECT_EQ(Before.mValue.mFacadeState, EArdaRHIResourceState::OpacityMicromapWrite);
		ASSERT_TRUE(Commands.mValue->BuildOpacityMicromap(*Micromap.mValue));
		const auto Built = Commands.mValue->QueryOpacityMicromapState(*Micromap.mValue);
		...
		EXPECT_EQ(Built.mValue.mNative.mNativeType, EArdaRHINativeResourceType::VulkanOpacityMicromap);
		EXPECT_EQ(Built.mValue.mFacadeState, EArdaRHIResourceState::OpacityMicromapBuildInput);
		ASSERT_TRUE(Commands.mValue->BuildBottomLevelAccelStruct(*Blas.mValue, {Geometry}, BlasDesc.mBuildFlags));
		const auto BlasState = Commands.mValue->QueryAccelStructState(*Blas.mValue);
		ASSERT_TRUE(BlasState) << BlasState.mStatus.mMessage.c_str();
		EXPECT_TRUE(BlasState.mValue.IsConsistent());
		...
		ASSERT_TRUE(Submission) << Submission.mStatus.mMessage.c_str();
		ASSERT_TRUE(Device->WaitForIdle());
		EXPECT_EQ(Micromap.mValue->GetBuildState(), EArdaRHIAccelStructBuildState::Built);
		EXPECT_EQ(Blas.mValue->GetBuildState(), EArdaRHIAccelStructBuildState::Built);

		const auto CompactedSize = Device->GetOpacityMicromapCompactedSize(Micromap.mValue);
		ASSERT_TRUE(CompactedSize) << CompactedSize.mStatus.mMessage.c_str();
		...
		ASSERT_TRUE(CompactMicromap) << CompactMicromap.mStatus.mMessage.c_str();
		...
		TlasDesc.mbTopLevel = true;
		TlasDesc.mTopLevelMaxInstances = 1;
		TlasDesc.mBuildFlags = EArdaRHIAccelStructBuildFlags::PreferFastTrace;
		...
		ASSERT_TRUE(Layout) << Layout.mStatus.mMessage.c_str();
		FArdaRHIRayTracingPipelineDesc PipelineDesc;
		PipelineDesc.mShaders.push_back({"KnownSceneRayGen", RayGeneration.mValue, {}});
		PipelineDesc.mShaders.push_back({"KnownMiss", Miss.mValue, {}});
		FArdaRHIRayTracingHitGroupDesc HitGroup;
		HitGroup.mExportName = "KnownHitGroup";
		HitGroup.mClosestHitShader = ClosestHit.mValue;
		PipelineDesc.mHitGroups.push_back(HitGroup);
		...
		auto ShaderTable = Device->CreateShaderTable(Pipeline.mValue, TableDesc);
		ASSERT_TRUE(ShaderTable) << ShaderTable.mStatus.mMessage.c_str();
		ASSERT_TRUE(Device->SetShaderTableRayGeneration(ShaderTable.mValue, "KnownSceneRayGen"));
		ASSERT_TRUE(Device->AddShaderTableMiss(ShaderTable.mValue, "KnownMiss"));
		ASSERT_TRUE(Device->AddShaderTableHitGroup(ShaderTable.mValue, "KnownHitGroup"));

		auto CompactCommands = Device->CreateCommandList(EArdaRHIQueueType::Graphics);
		ASSERT_TRUE(CompactCommands);
		ASSERT_TRUE(CompactCommands.mValue->Open());
		ASSERT_TRUE(CompactCommands.mValue->CompactOpacityMicromap(*CompactMicromap.mValue, *Micromap.mValue));
		const auto CompactState = CompactCommands.mValue->QueryOpacityMicromapState(*CompactMicromap.mValue);
		ASSERT_TRUE(CompactState) << CompactState.mStatus.mMessage.c_str();
		EXPECT_TRUE(CompactState.mValue.IsConsistent());
		EXPECT_TRUE(CompactState.mValue.mNative.mbNativeCompatible);
		EXPECT_EQ(CompactState.mValue.mFacadeState, EArdaRHIResourceState::OpacityMicromapBuildInput);
		ASSERT_TRUE(CompactCommands.mValue->BuildBottomLevelAccelStruct(*CompactBlas.mValue,
		    {CompactGeometry},
		    CompactBlasDesc.mBuildFlags));
		FArdaRHIRayTracingInstanceDesc Instance;
		Instance.mBottomLevelAccelStruct = CompactBlas.mValue;
		ASSERT_TRUE(CompactCommands.mValue->BuildTopLevelAccelStruct(*Tlas.mValue, {Instance}, TlasDesc.mBuildFlags));
		const auto TlasState = CompactCommands.mValue->QueryAccelStructState(*Tlas.mValue);
		ASSERT_TRUE(TlasState) << TlasState.mStatus.mMessage.c_str();
		EXPECT_TRUE(TlasState.mValue.IsConsistent());
		EXPECT_EQ(TlasState.mValue.mFacadeState, EArdaRHIResourceState::AccelStructRead);
		ASSERT_TRUE(CompactCommands.mValue->SetBufferState(*Output.mValue, EArdaRHIResourceState::UnorderedAccess));
		...
		ASSERT_TRUE(
		    CompactCommands.mValue->CopyBufferDeviceToHost(*Output.mValue, TraversalReadback, 0, OutputDesc.mByteSize));
		ExpectBufferState(*CompactCommands.mValue, *Output.mValue, EArdaRHIResourceState::CopySource);
		ASSERT_TRUE(CompactCommands.mValue->Close());
		const auto CompactSubmission = Device->ExecuteCommandList(CompactCommands.mValue);
		ASSERT_TRUE(CompactSubmission) << CompactSubmission.mStatus.mMessage.c_str();
		ASSERT_TRUE(Device->WaitForIdle());
		EXPECT_TRUE(CompactMicromap.mValue->IsCompacted());
		EXPECT_EQ(CompactMicromap.mValue->GetBuildState(), EArdaRHIAccelStructBuildState::Compacted);
		EXPECT_EQ(CompactBlas.mValue->GetBuildState(), EArdaRHIAccelStructBuildState::Built);
		ASSERT_EQ(TraversalReadback.size(), OutputDesc.mByteSize);
		uint32_t TraversalValues[2]{};
		std::memcpy(TraversalValues, TraversalReadback.data(), sizeof(TraversalValues));
		EXPECT_EQ(TraversalValues[0], 0xC105E57u);
		EXPECT_EQ(TraversalValues[1], 0xB055u);
		EXPECT_EQ(Diagnostics.GetErrorCount(), 0u);
	}

	void VerifyExpandedDescriptorsAndResourceCollections(const char* BackendName)
	{
		using namespace arda;
		ShutdownBackend();
		FArdaExtendedDiagnosticCallback Diagnostics;
		...
		Bindless.mLayoutType = EArdaRHIBindlessLayoutType::MutableSrvUavCbv;
		Bindless.mRegisterSpaces.push_back({0, 1, EArdaRHIBindingType::RawBufferSRV});
		...
		FArdaRHIBufferDesc BufferDesc;
		BufferDesc.mByteSize = 256;
		BufferDesc.mUsage = EArdaRHIBufferUsage::Raw | EArdaRHIBufferUsage::ShaderResource;
		...
		CollectionDesc.mbDirectlyIndexed = Caps.mDescriptors.mbDirectResourceHeapIndexing;
		...
		{
			EXPECT_NE(Collection.mValue->GetFirstDescriptorIndexInHeap(), 0xffffffffu);
		}
		if (CollectionDesc.mbDirectlyIndexed)
		{
			...
			auto Shader = CreateExtendedShader(*Device,
			    BackendName,
			    "ArdaResourceCollection",
			    "ResourceCollectionCS",
			    EArdaRHIShaderStage::Compute);
			ASSERT_TRUE(Shader);
			FArdaRHIComputePipelineDesc PipelineDesc;
			PipelineDesc.mComputeShader = Shader.mValue;
	...
	void VerifyMappedTextureContents(arda::IArdaRHIDevice& Device, const arda::FArdaRHITextureRef& Texture)
	{
		using namespace arda;
		...
		for (uint32_t Z = 0; Z < Region.mDepth; ++Z)
		{
			for (uint32_t Y = 0; Y < Region.mHeight; ++Y)
			{
				auto* Row = static_cast<uint8_t*>(Write.mValue.mData) + Z * Write.mValue.mDepthPitch +
				    Y * Write.mValue.mRowPitch;
		...
		ASSERT_TRUE(Commands);
		ASSERT_TRUE(Commands.mValue->Open());
		ASSERT_TRUE(Commands.mValue->CopyTextureFromStaging(*Texture, Region, *Upload.mValue, Region));
		ASSERT_TRUE(Commands.mValue->CopyTextureToStaging(*Readback.mValue, Region, *Texture, Region));
		ASSERT_TRUE(Commands.mValue->Close());
		ASSERT_TRUE(Device.ExecuteCommandList(Commands.mValue));
		ASSERT_TRUE(Device.WaitForIdle());
		...
		for (uint32_t Z = 0; Z < Region.mDepth; ++Z)
		{
			for (uint32_t Y = 0; Y < Region.mHeight; ++Y)
			{
				const auto* Row = static_cast<const uint8_t*>(Read.mValue.mData) + Z * Read.mValue.mDepthPitch +
				    Y * Read.mValue.mRowPitch;
	...
	void VerifySparseResidencyAndStreamingBudget(const char* BackendName)
	{
		using namespace arda;
		ShutdownBackend();
		FArdaExtendedDiagnosticCallback Diagnostics;
		...
		if (!Residency.mbSparseBinding || !Residency.mbReservedBuffers || !Residency.mbReservedTexture2D)
		{
		...
		FArdaRHIHeapDesc HeapDesc;
		HeapDesc.mCapacity = eastl::max<uint64_t>(BufferMemory.mValue.mAlignment, Residency.mTileSizeInBytes * 2);
		...
		ASSERT_TRUE(Device->UpdateBufferTileMappings(Buffer.mValue, {BufferMapping}, EArdaRHIQueueType::Graphics));
		BufferMapping.mbCommit = false;
		BufferMapping.mHeap = {};
		ASSERT_TRUE(Device->UpdateBufferTileMappings(Buffer.mValue, {BufferMapping}, EArdaRHIQueueType::Graphics));
		ASSERT_TRUE(Device->CommitReservedResource(FArdaRHIResourceRef(Buffer.mValue.Get()),
		    BufferTileBytes,
		    EArdaRHIQueueType::Graphics));
		const uint32_t SparseExpected = 0x51A25EEDu;
		auto SparseCommands = Device->CreateCommandList(EArdaRHIQueueType::Graphics);
		ASSERT_TRUE(SparseCommands);
		ASSERT_TRUE(SparseCommands.mValue->Open());
		ASSERT_TRUE(SparseCommands.mValue->WriteBuffer(*Buffer.mValue, &SparseExpected, sizeof(SparseExpected), 0));
		eastl::vector<uint8_t> SparseReadback;
		ASSERT_TRUE(
		    SparseCommands.mValue->CopyBufferDeviceToHost(*Buffer.mValue, SparseReadback, 0, sizeof(SparseExpected)));
		ExpectBufferState(*SparseCommands.mValue, *Buffer.mValue, EArdaRHIResourceState::CopySource);
		...
		std::memcpy(&SparseActual, SparseReadback.data(), sizeof(SparseActual));
		EXPECT_EQ(SparseActual, SparseExpected);
		ASSERT_TRUE(
		    Device->CommitReservedResource(FArdaRHIResourceRef(Buffer.mValue.Get()), 0, EArdaRHIQueueType::Graphics));
		...
		const auto TextureMapStatus =
		    Device->UpdateTextureTileMappings(Texture.mValue, {TextureMapping}, EArdaRHIQueueType::Graphics);
		...
		ASSERT_TRUE(Device->UpdateTextureTileMappings(Texture.mValue, {TextureMapping}, EArdaRHIQueueType::Graphics));
		...
		if (Residency.mbReservedTexture3D)
		{
			...
			ASSERT_TRUE(VolumeTiling) << VolumeTiling.mStatus.mMessage.c_str();
			...
			ASSERT_TRUE(Device->UpdateTextureTileMappings(Volume.mValue, {VolumeMapping}, EArdaRHIQueueType::Graphics));
		...
		if (Residency.mbAliasedMappings)
		{
			...
			ASSERT_TRUE(Device->UpdateBufferTileMappings(Buffer.mValue, {AliasedMapping}, EArdaRHIQueueType::Graphics));
			ASSERT_TRUE(Device->UpdateBufferTileMappings(Alias.mValue, {AliasedMapping}, EArdaRHIQueueType::Graphics));
			...
			ASSERT_TRUE(
			    AliasCommands.mValue->CopyBufferDeviceToHost(*Alias.mValue, AliasReadback, 0, sizeof(SparseExpected)));
		...
		if (Residency.mbStreamingBudget)
		{
			...
			{
				ASSERT_TRUE(Device->SetStreamingBudgetReservation(Budget.mValue.mCurrentReservationBytes, true));
			}
		}
		EXPECT_EQ(Diagnostics.GetErrorCount(), 0u);
	}

	void VerifyStreamingBudgetExecution(const char* BackendName)
	{
		using namespace arda;
	...
	void VerifyQueryExecution(const char* BackendName)
	{
		using namespace arda;
		...
		EXPECT_EQ(Device->WaitEventQuery(Event.mValue).mCode, EArdaRHIResult::InvalidState);

		ASSERT_TRUE(Device->SignalEventQuery(Event.mValue, EArdaRHIQueueType::Graphics));
		EXPECT_EQ(Device->SignalEventQuery(Event.mValue, EArdaRHIQueueType::Graphics).mCode,
		...
		std::vector<uint8_t> TimedData(static_cast<size_t>(TimedBufferDesc.mByteSize), 0x5a);

		auto Commands = Device->CreateCommandList(EArdaRHIQueueType::Graphics);
		ASSERT_TRUE(Commands) << Commands.mStatus.mMessage.c_str();
		ASSERT_TRUE(Commands.mValue->Open());
		ASSERT_TRUE(Commands.mValue->BeginTimerQuery(*Timer.mValue));
		ASSERT_TRUE(Commands.mValue->WriteBuffer(*TimedBuffer.mValue, TimedData.data(), TimedData.size()));
		ASSERT_TRUE(Commands.mValue->EndTimerQuery(*Timer.mValue));
		ASSERT_TRUE(Commands.mValue->Close());
		ASSERT_TRUE(Device->ExecuteCommandList(Commands.mValue));
		ASSERT_TRUE(Device->SignalGpuFence(Fence.mValue, EArdaRHIQueueType::Graphics));
		EXPECT_EQ(Device->SignalGpuFence(Fence.mValue, EArdaRHIQueueType::Graphics).mCode,
		    EArdaRHIResult::InvalidState);
	...
	void VerifyShaderLibraryExecution(const char* BackendName, const std::filesystem::path& CacheDirectory = {})
	{
		...
		const eastl::string FileName = eastl::string("ArdaShaderStructTest") + GetShaderArtifactExtension(BackendName);
		const std::vector<uint8_t> Bytecode = LoadExtendedShaderArtifact(FileName.c_str());
		ASSERT_FALSE(Bytecode.empty());
		auto Library = Device->CreateShaderLibrary(Bytecode.data(), Bytecode.size(), "Capability shader library");
		ASSERT_TRUE(Library) << Library.mStatus.mMessage.c_str();
		auto Shader = Device->GetShaderFromLibrary(Library.mValue,
		    "ShaderStructTestCS",
		    EArdaRHIShaderStage::Compute,
	...
	void VerifyD3D12DeferredSubmissionLifetime()
	{
		...
		for (uint32_t Value = 1; Value <= SubmissionCount; ++Value)
		{
			...
			ASSERT_TRUE(Commands);
			ASSERT_TRUE(Commands.mValue->Open());
			ASSERT_TRUE(Commands.mValue->WriteBuffer(*Buffer.mValue, &Value, sizeof(Value), 0));
			ASSERT_TRUE(Commands.mValue->Close());
			const auto Submission = Device->ExecuteCommandList(Commands.mValue);
			ASSERT_TRUE(Submission) << Submission.mStatus.mMessage.c_str();
		...
		ASSERT_TRUE(ReadbackCommands.mValue->Open());
		eastl::vector<uint8_t> Readback;
		ASSERT_TRUE(ReadbackCommands.mValue->CopyBufferDeviceToHost(*Buffer.mValue, Readback, 0, sizeof(uint32_t)));
		ExpectBufferState(*ReadbackCommands.mValue, *Buffer.mValue, EArdaRHIResourceState::CopySource);
		ASSERT_TRUE(ReadbackCommands.mValue->Close());
		ASSERT_TRUE(Device->ExecuteCommandList(ReadbackCommands.mValue));
		ASSERT_EQ(Readback.size(), sizeof(uint32_t));
		uint32_t Actual = 0;
	...
	void VerifyD3D12CustomPresentExecution()
	{
		...
		const EArdaInitializeResult Result = InitializeBackendForPresentation(Surface, 16, 16, SwapChain);
		...
		auto Tracker = eastl::make_shared<FArdaCustomPresentTracker>();
		...
		Framebuffer = nullptr;
		ASSERT_TRUE(SwapChain->Resize(24, 20)) << SwapChain->GetError().c_str();
	...
	void VerifyVulkanCustomPresentExecution()
	{
		...
		eastl::unique_ptr<IArdaSwapChain> SwapChain;
		const EArdaInitializeResult Result = InitializeBackendForPresentation(Surface, 64, 64, SwapChain);
		if (Result == EArdaInitializeResult::ValidationUnavailable)
		{
			GTEST_SKIP() << GetBackendError().c_str();
		}
		...
		FArdaRHIFramebufferRef Framebuffer;
		ASSERT_TRUE(SwapChain->AcquireFrame(Framebuffer)) << SwapChain->GetError().c_str();
		ASSERT_TRUE(Framebuffer);
		SwapChain->PrepareSubmit();
		ASSERT_TRUE(SwapChain->Present()) << SwapChain->GetError().c_str();
		EXPECT_EQ(Tracker->mPresentCount, 1u);
		...
		Framebuffer = nullptr;
		ASSERT_TRUE(SwapChain->Resize(72, 68)) << SwapChain->GetError().c_str();
		EXPECT_EQ(Tracker->mResizeCount, 2u);
		EXPECT_EQ(Tracker->mWidth, SwapChain->GetWidth());
		EXPECT_EQ(Tracker->mHeight, SwapChain->GetHeight());
	...
	using FArdaCapabilityPredicate = bool (*)(const arda::FArdaRHICapabilities&);

	struct FArdaCapabilityDefinition
	{
		const char* mName = nullptr;
		FArdaCapabilityPredicate mIsAdvertised = nullptr;
		EArdaCapabilityProbe mProbe = EArdaCapabilityProbe::Contract;
		const char* mExpression = nullptr;
	};

	struct FArdaCapabilityConformanceCase
	{
		const char* mBackendName = nullptr;
		const char* mBackendLabel = nullptr;
		FArdaCapabilityDefinition mCapability;
	};

	void VerifyCapabilityInvariants(const arda::FArdaRHICapabilities& Caps)
	{
		using namespace arda;
		...
		if (Ray.mbBottomLevel || Ray.mbTopLevel || Ray.mbBuildUpdate || Ray.mbCompaction)
		{
			EXPECT_TRUE(Ray.mbAccelerationStructures);
		}
		if (Ray.mbIndirectDispatch || Ray.mbLocalShaderTableArguments || Ray.mbPersistentShaderTables)
		{
		...
		if (Descriptors.mbRuntimeDescriptorArrays || Descriptors.mbUnboundedArrays || Descriptors.mbPartiallyBound ||
		    Descriptors.mbUpdateAfterBind || Descriptors.mbUpdateUnusedWhilePending ||
		    Descriptors.mbVariableDescriptorCount || Descriptors.mbDirectResourceHeapIndexing ||
		    Descriptors.mbDirectSamplerHeapIndexing || Descriptors.mbDescriptorBuffer || Descriptors.mbDescriptorHeap)
		{
		...
		if (Residency.mbReservedBuffers || Residency.mbReservedTexture2D || Residency.mbReservedTexture3D ||
		    Residency.mbAliasedMappings)
		{
		...
		if (MachineLearning.mbSubgroupOperations)
		{
			...
			EXPECT_GE(MachineLearning.mSubgroupMaxSize, MachineLearning.mSubgroupMinSize);
		}
	}

	void RunCapabilityProbe(const FArdaCapabilityConformanceCase& TestCase)
	{
		using namespace arda;
		...
		case EArdaCapabilityProbe::GeometryShader:
			...
			return;
		case EArdaCapabilityProbe::TessellationShaders:
			VerifyRasterStageCapabilityAndExecution(TestCase.mBackendName, true);
			return;
		case EArdaCapabilityProbe::PipelineCache:
		{
			const std::filesystem::path Directory =
			    std::filesystem::path(ARDA_BACKEND_TEST_SHADER_DIR) / "CapabilityPipelineCache" / TestCase.mBackendName;
		...
		case EArdaCapabilityProbe::ExtendedCommands:
			VerifyExtendedCommands(TestCase.mBackendName);
			return;
		case EArdaCapabilityProbe::Resolve:
			VerifyResolveAndPlaneTracking(TestCase.mBackendName);
			return;
		case EArdaCapabilityProbe::HeapAliasing:
			VerifyExplicitHeapAliasing(TestCase.mBackendName);
			return;
		case EArdaCapabilityProbe::Bindless:
			VerifyBindlessDescriptorTable(TestCase.mBackendName);
			return;
		case EArdaCapabilityProbe::RuntimeDescriptors:
			VerifyRuntimeDescriptorVersions(TestCase.mBackendName);
			return;
		case EArdaCapabilityProbe::DescriptorBuffer:
			...
			return;
		case EArdaCapabilityProbe::DirectResourceHeap:
			VerifyBindlessDescriptorTable(TestCase.mBackendName, true);
			return;
		case EArdaCapabilityProbe::DirectResourceAndSamplerHeaps:
			VerifyDirectResourceAndSamplerHeapIndexing(TestCase.mBackendName);
			return;
		case EArdaCapabilityProbe::ExpandedDescriptors:
			VerifyExpandedDescriptorsAndResourceCollections(TestCase.mBackendName);
			return;
		case EArdaCapabilityProbe::Subgroup:
			VerifyComputeArithmetic(TestCase.mBackendName, "ArdaSubgroup", "SubgroupCS");
			return;
		case EArdaCapabilityProbe::Float16:
			VerifyComputeArithmetic(TestCase.mBackendName, "ArdaFloat16", "Float16CS");
			return;
		case EArdaCapabilityProbe::Int8:
			VerifyComputeArithmetic(TestCase.mBackendName, "ArdaInt8", "Int8CS");
			return;
		case EArdaCapabilityProbe::QueueBreadth:
			VerifyQueueBreadth(TestCase.mBackendName);
			return;
		case EArdaCapabilityProbe::SparseResidency:
			VerifySparseResidencyAndStreamingBudget(TestCase.mBackendName);
			return;
		case EArdaCapabilityProbe::StreamingBudget:
			VerifyStreamingBudgetExecution(TestCase.mBackendName);
			return;
		case EArdaCapabilityProbe::ShaderBundle:
			VerifyShaderBundleExecution(TestCase.mBackendName);
			return;
		case EArdaCapabilityProbe::WorkGraph:
			ASSERT_STREQ(TestCase.mBackendName, "native-d3d12") << "A backend advertised work graphs without a native "
			                                                       "conformance workload.";
			VerifyD3D12WorkGraphExecution();
			return;
		case EArdaCapabilityProbe::MeshShader:
			VerifyMeshPipelineCapabilityAndExecution(TestCase.mBackendName);
			return;
		case EArdaCapabilityProbe::RayTracingPipeline:
			VerifyRayTracingPipelineCapabilityAndExecution(TestCase.mBackendName);
			return;
		case EArdaCapabilityProbe::AccelerationStructure:
			VerifyAccelerationStructureLifecycleAndStateParity(TestCase.mBackendName);
			return;
		case EArdaCapabilityProbe::InlineRayQuery:
			VerifyRayTracingSceneHitGroupsLocalArgumentsAndIndirect(TestCase.mBackendName, true);
			return;
		case EArdaCapabilityProbe::InstanceBuffer:
			VerifyRayTracingSceneHitGroupsLocalArgumentsAndIndirect(TestCase.mBackendName, false, true);
			return;
		case EArdaCapabilityProbe::RayTracingScene:
			VerifyRayTracingSceneHitGroupsLocalArgumentsAndIndirect(TestCase.mBackendName);
			return;
		case EArdaCapabilityProbe::OpacityMicromap:
#if defined(ARDA_TEST_NATIVE_VULKAN)
...
#endif
			return;
		case EArdaCapabilityProbe::SamplerFeedback:
			...
			return;
		case EArdaCapabilityProbe::CustomPresent:
#if defined(_WIN32)
			...
			FAIL() << "Custom present was advertised without a platform "
			          "conformance implementation.";
			return;
		case EArdaCapabilityProbe::Queries:
			VerifyQueryExecution(TestCase.mBackendName);
			return;
		case EArdaCapabilityProbe::QueueTimestamps:
			VerifyPerQueueTimerExecution(TestCase.mBackendName);
			return;
		case EArdaCapabilityProbe::ShaderLibrary:
...
#define ARDA_CAPABILITY(Name, Expression, Probe)                                                                       \
	{Name,                                                                                                             \
	    +[](const arda::FArdaRHICapabilities& C)                                                                       \
	    {                                                                                                              \
		    return static_cast<bool>(Expression);                                                                      \
	    },                                                                                                             \
	    EArdaCapabilityProbe::Probe,                                                                                   \
	    #Expression}

	const std::vector<FArdaCapabilityConformanceCase>& GetCapabilityConformanceCases()
	{
		static const FArdaCapabilityDefinition Definitions[] = {
		    ARDA_CAPABILITY("RayTracingInfrastructure", C.mRayTracing.mbInfrastructure, RayTracingPipeline),
		    ARDA_CAPABILITY("HardwareRayTracing", C.mRayTracing.mbHardwareAccelerated, AccelerationStructure),
		    ARDA_CAPABILITY("RayTracingPipelineShaders", C.mRayTracing.mbPipelineShaders, RayTracingPipeline),
		    ARDA_CAPABILITY("InlineRayQueries", C.mRayTracing.mbInlineRayQueries, InlineRayQuery),
		    ARDA_CAPABILITY("AccelerationStructures", C.mRayTracing.mbAccelerationStructures, AccelerationStructure),
		    ARDA_CAPABILITY("BottomLevelAccelerationStructures", C.mRayTracing.mbBottomLevel, AccelerationStructure),
		    ARDA_CAPABILITY("TopLevelAccelerationStructures", C.mRayTracing.mbTopLevel, AccelerationStructure),
		    ARDA_CAPABILITY("AccelerationStructureBuildUpdate", C.mRayTracing.mbBuildUpdate, AccelerationStructure),
		    ARDA_CAPABILITY("AccelerationStructureCompaction", C.mRayTracing.mbCompaction, AccelerationStructure),
		    ARDA_CAPABILITY("IndirectRayDispatch", C.mRayTracing.mbIndirectDispatch, RayTracingScene),
		    ARDA_CAPABILITY("IndirectTopLevelBuild", C.mRayTracing.mbIndirectTopLevelBuild, AccelerationStructure),
		    ARDA_CAPABILITY("LocalShaderTableArguments", C.mRayTracing.mbLocalShaderTableArguments, RayTracingScene),
		    ARDA_CAPABILITY("PersistentShaderTables", C.mRayTracing.mbPersistentShaderTables, RayTracingScene),
		    ARDA_CAPABILITY("OpacityMicromaps", C.mRayTracing.mbOpacityMicromaps, OpacityMicromap),
		    ARDA_CAPABILITY("RayTracingTier",
		        C.mRayTracing.GetTier() != arda::EArdaRHIRayTracingTier::None,
		        AccelerationStructure),
		    ARDA_CAPABILITY("RayShaderIdentifierSize", C.mRayTracing.mShaderIdentifierSize > 0, RayTracingPipeline),
		    ARDA_CAPABILITY("RayShaderRecordAlignment", C.mRayTracing.mShaderRecordAlignment > 0, RayTracingPipeline),
		    ARDA_CAPABILITY("RayShaderTableAlignment", C.mRayTracing.mShaderTableAlignment > 0, RayTracingPipeline),
		    ARDA_CAPABILITY("AccelerationStructureAlignment",
		        C.mRayTracing.mAccelerationStructureAlignment > 0,
		        AccelerationStructure),
		    ARDA_CAPABILITY("MaximumRayRecursionDepth", C.mRayTracing.mMaxRecursionDepth > 0, RayTracingPipeline),
		    ARDA_CAPABILITY("MaximumRayPayloadSize", C.mRayTracing.mMaxRayPayloadSize > 0, RayTracingPipeline),
		    ARDA_CAPABILITY("MaximumRayDispatchInvocations",
		        C.mRayTracing.mMaxRayDispatchInvocations > 0,
		        RayTracingPipeline),

		    ARDA_CAPABILITY("BindlessDescriptors", C.mDescriptors.mbBindless, Bindless),
		    ARDA_CAPABILITY("RuntimeDescriptorArrays", C.mDescriptors.mbRuntimeDescriptorArrays, RuntimeDescriptors),
		    ARDA_CAPABILITY("UnboundedDescriptorArrays", C.mDescriptors.mbUnboundedArrays, RuntimeDescriptors),
		    ARDA_CAPABILITY("PartiallyBoundDescriptors", C.mDescriptors.mbPartiallyBound, RuntimeDescriptors),
		    ARDA_CAPABILITY("DescriptorUpdateAfterBind", C.mDescriptors.mbUpdateAfterBind, RuntimeDescriptors),
		    ARDA_CAPABILITY("UpdateUnusedDescriptorsWhilePending",
		        C.mDescriptors.mbUpdateUnusedWhilePending,
		        RuntimeDescriptors),
		    ARDA_CAPABILITY("VariableDescriptorCount", C.mDescriptors.mbVariableDescriptorCount, ExpandedDescriptors),
		    ...
		    ARDA_CAPABILITY("DirectSamplerHeapIndexing",
		        ...
		        DirectResourceAndSamplerHeaps),
		    ARDA_CAPABILITY("DescriptorBuffer", C.mDescriptors.mbDescriptorBuffer, DescriptorBuffer),
		    ARDA_CAPABILITY("DescriptorHeap", C.mDescriptors.mbDescriptorHeap, Bindless),
		    ARDA_CAPABILITY("MaximumResourceDescriptors", C.mDescriptors.mMaxResourceDescriptors > 0, Bindless),
		    ARDA_CAPABILITY("MaximumSamplerDescriptors", C.mDescriptors.mMaxSamplerDescriptors > 0, Bindless),

		    ARDA_CAPABILITY("GraphicsQueue", C.mQueues.mbGraphics, QueueBreadth),
		    ARDA_CAPABILITY("ComputeQueue", C.mQueues.mbCompute, QueueBreadth),
		    ARDA_CAPABILITY("CopyQueue", C.mQueues.mbCopy, QueueBreadth),
		    ARDA_CAPABILITY("DedicatedComputeQueueFamily", C.mQueues.mbDedicatedComputeFamily, QueueBreadth),
		    ARDA_CAPABILITY("DedicatedCopyQueueFamily", C.mQueues.mbDedicatedCopyFamily, QueueBreadth),
		    ARDA_CAPABILITY("GpuQueueWaits", C.mQueues.mbGpuWaits, QueueBreadth),
		    ARDA_CAPABILITY("TimelineSynchronization", C.mQueues.mbTimelineSynchronization, QueueBreadth),
		    ARDA_CAPABILITY("QueueFamilyOwnershipTransfer", C.mQueues.mbQueueFamilyOwnershipTransfer, QueueBreadth),
		    ARDA_CAPABILITY("SparseBindingQueue", C.mQueues.mbSparseBindingQueue, SparseResidency),
		    ARDA_CAPABILITY("GraphicsQueueFamilyIndex",
		        C.mQueues.mGraphicsFamily != arda::ArdaRHIInvalidQueueFamily,
		        QueueBreadth),
		    ARDA_CAPABILITY("ComputeQueueFamilyIndex",
		        C.mQueues.mComputeFamily != arda::ArdaRHIInvalidQueueFamily,
		        QueueBreadth),
		    ARDA_CAPABILITY("CopyQueueFamilyIndex",
		        C.mQueues.mCopyFamily != arda::ArdaRHIInvalidQueueFamily,
		        QueueBreadth),

		    ARDA_CAPABILITY("SparseBinding", C.mResidency.mbSparseBinding, SparseResidency),
		    ARDA_CAPABILITY("ReservedBuffers", C.mResidency.mbReservedBuffers, SparseResidency),
		    ARDA_CAPABILITY("ReservedTexture2D", C.mResidency.mbReservedTexture2D, SparseResidency),
		    ARDA_CAPABILITY("ReservedTexture3D", C.mResidency.mbReservedTexture3D, SparseResidency),
		    ARDA_CAPABILITY("AliasedSparseMappings", C.mResidency.mbAliasedMappings, SparseResidency),
		    ARDA_CAPABILITY("StreamingBudget", C.mResidency.mbStreamingBudget, StreamingBudget),
		    ARDA_CAPABILITY("StreamingBudgetReservation", C.mResidency.mbBudgetReservation, StreamingBudget),
		    ARDA_CAPABILITY("SparseTileSize", C.mResidency.mTileSizeInBytes > 0, SparseResidency),

		    ARDA_CAPABILITY("SubgroupOperations", C.mMachineLearning.mbSubgroupOperations, Subgroup),
		    ARDA_CAPABILITY("NativeFloat16", C.mMachineLearning.mbNativeFloat16, Float16),
		    ARDA_CAPABILITY("NativeInt8", C.mMachineLearning.mbNativeInt8, Int8),
		    ARDA_CAPABILITY("BufferDeviceAddress", C.mMachineLearning.mbBufferDeviceAddress, InstanceBuffer),
		    ARDA_CAPABILITY("MinimumSubgroupSize", C.mMachineLearning.mSubgroupMinSize > 0, Subgroup),
		    ARDA_CAPABILITY("MaximumSubgroupSize", C.mMachineLearning.mSubgroupMaxSize > 0, Subgroup),

		    ARDA_CAPABILITY("MeshShaders", C.mMeshShaderTier != arda::EArdaRHIMeshShaderTier::None, MeshShader),
		    ARDA_CAPABILITY("WorkGraphs", C.mWorkGraphTier != arda::EArdaRHIWorkGraphTier::None, WorkGraph),
		    ARDA_CAPABILITY("SamplerFeedback",
		        C.mSamplerFeedbackTier != arda::EArdaRHISamplerFeedbackTier::None,
		        SamplerFeedback),
		    ARDA_CAPABILITY("ShaderBundleDispatch", C.mbShaderBundleDispatch, ShaderBundle),
		    ARDA_CAPABILITY("CustomPresent", C.mbCustomPresent, CustomPresent),
		    ARDA_CAPABILITY("ResourceCollections", C.mbResourceCollections, ExpandedDescriptors),
		    ARDA_CAPABILITY("ConservativeRasterization", C.mbConservativeRasterization, Contract),
		    ...
		    ARDA_CAPABILITY("GeometryShaders", C.mbGeometryShaders, GeometryShader),
		    ARDA_CAPABILITY("TessellationShaders", C.mbTessellationShaders, TessellationShaders),
		...
		static const std::vector<FArdaCapabilityConformanceCase> Cases = []
		{
			std::vector<FArdaCapabilityConformanceCase> Result;
			const auto AddBackend = [&Result](const char* BackendName, const char* BackendLabel)
			{
				for (const FArdaCapabilityDefinition& Definition : Definitions)
				{
					Result.push_back({BackendName, BackendLabel, Definition});
				}
			};
...
#undef ARDA_CAPABILITY
	...
	class FArdaRHICapabilityConformanceTest : public testing::TestWithParam<FArdaCapabilityConformanceCase>
	{
	};
}
...
TEST(ArdaBackend, CapabilityMatrixCoversEveryPublicField)
{
	...
	for (const char* Name : {"FArdaRHIFeatureRequirements", "FArdaRHIFeatureSupportReport", "FArdaRHIFormatSupport"})
	{
		...
		Source.erase(Begin, End + 4 - Begin);
	}
...
TEST_P(FArdaRHICapabilityConformanceTest, AdvertisedCapabilityConforms)
{
	...
	const FArdaCapabilityConformanceCase& TestCase = GetParam();
	ShutdownBackend();
	FArdaExtendedDiagnosticCallback Diagnostics;
	FArdaExtendedBackendCleanup Cleanup;
	FArdaBackendConfiguration Configuration = arda::MakeArdaTestBackendConfiguration();
	...
	ASSERT_TRUE(Device);
	const arda::FArdaRHICapabilities Capabilities = Device->GetCapabilities();
	if (!TestCase.mCapability.mIsAdvertised(Capabilities))
	{
		GTEST_SKIP() << TestCase.mBackendLabel << " does not advertise " << TestCase.mCapability.mName
		             << " on this adapter.";
	}
...
INSTANTIATE_TEST_SUITE_P(NativeHardware,
    FArdaRHICapabilityConformanceTest,
    testing::ValuesIn(GetCapabilityConformanceCases()),
    [](const testing::TestParamInfo<FArdaCapabilityConformanceCase>& Info)
    {
	    return std::string(Info.param.mBackendLabel) + "_" + Info.param.mCapability.mName;
    });

#if defined(_WIN32) && defined(ARDA_TEST_NATIVE_D3D12)
TEST(ArdaBackend, D3D12ExtendedCopyTransitionAndIndirectStateParity)
{
	VerifyExtendedCommands("native-d3d12");
}

TEST(ArdaBackend, D3D12ResolveAndFormatPlaneStateParity)
{
	VerifyResolveAndPlaneTracking("native-d3d12");
}

TEST(ArdaBackend, D3D12ExplicitHeapAliasingStateParity)
{
	VerifyExplicitHeapAliasing("native-d3d12");
}

TEST(ArdaBackend, D3D12BindlessDescriptorTableExecutes)
{
	VerifyBindlessDescriptorTable("native-d3d12");
}

TEST(ArdaBackend, D3D12DirectDescriptorHeapIndexingExecutes)
{
	VerifyBindlessDescriptorTable("native-d3d12", true);
}

TEST(ArdaBackend, D3D12DirectResourceAndSamplerHeapIndexingExecutes)
{
	VerifyDirectResourceAndSamplerHeapIndexing("native-d3d12");
}

TEST(ArdaBackend, D3D12ShaderBundleExecutes)
{
	VerifyShaderBundleExecution("native-d3d12");
}
...
{
	VerifyMeshPipelineCapabilityAndExecution("native-d3d12");
}

TEST(ArdaBackend, D3D12QueueBreadthExecutes)
{
	VerifyQueueBreadth("native-d3d12");
}

TEST(ArdaBackend, D3D12RayTracingPipelineCapabilityAndExecution)
{
	VerifyRayTracingPipelineCapabilityAndExecution("native-d3d12");
}

TEST(ArdaBackend, D3D12AccelerationStructureLifecycleAndStateParity)
{
	VerifyAccelerationStructureLifecycleAndStateParity("native-d3d12");
}

TEST(ArdaBackend, D3D12RaySceneHitGroupsLocalArgumentsAndIndirectReadback)
{
	VerifyRayTracingSceneHitGroupsLocalArgumentsAndIndirect("native-d3d12");
}

TEST(ArdaBackend, D3D12ExpandedDescriptorsAndResourceCollections)
{
	VerifyExpandedDescriptorsAndResourceCollections("native-d3d12");
}

TEST(ArdaBackend, D3D12SparseResidencyAndStreamingBudget)
{
	VerifySparseResidencyAndStreamingBudget("native-d3d12");
}
...
{
	VerifyExpandedDescriptorsAndResourceCollections("native-vulkan");
}

TEST(ArdaBackend, VulkanSparseResidencyAndStreamingBudget)
{
	VerifySparseResidencyAndStreamingBudget("native-vulkan");
}
