...
{
	const float4 value = DirectHeapTexture.SampleLevel(DirectHeapSampler, float2(0.5, 0.5), 0.0);
	DirectHeapOutput[dispatchThreadId.x] = value.r > 0.2 ? 0x5A4D : 0;
}

struct FArdaWorkGraphInputRecord
{
	uint Value;
};
...
[NumThreads(1, 1, 1)]
void WorkGraphMain(DispatchNodeInputRecord<FArdaWorkGraphInputRecord> inputRecord)
{
...
{
	VulkanLayoutOutput[dispatchThreadId.xy] = float4(0.125, 0.25, 0.5, 1.0);
}

float4 PipelineStateTestVS(uint vertexId: SV_VertexID)
    : SV_Position
{
	const float2 positions[3] = { float2(-1.0, -1.0), float2(0.0, 1.0), float2(1.0, -1.0) };
	return float4(positions[vertexId], 0.0, 1.0);
}

float4 PipelineStateTestPS()
    : SV_Target
{
	return float4(1.0, 0.0, 1.0, 1.0);
}

struct FArdaMeshPipelineVertex
{
...
[numthreads(1, 1, 1)]
void MeshPipelineTestMS(uint3 groupThreadId: SV_GroupThreadID,
    out vertices FArdaMeshPipelineVertex vertices[3],
...
float4 BindingSpaceVS(uint vertexId: SV_VertexID)
    : SV_Position
{
	const float2 positions[3] = { float2(-1.0, -1.0), float2(0.0, 1.0), float2(1.0, -1.0) };
