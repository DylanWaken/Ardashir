...
namespace
{
	bool ResolveLinkedTestShaderTarget(arda::FArdaShaderTarget& OutTarget)
	{
		...
		return !Modules.empty() && ResolveShaderTarget(Modules.front().mName.c_str(), OutTarget);
	}
	...
	std::filesystem::path GetTestExecutablePath()
	{
		...
		return Length > 0 ? std::filesystem::path(std::string(Buffer.data(), Length)) : std::filesystem::path{};
#else
		return {};
#endif
	}

	struct FArdaFakeDxcWorkerBootstrap
	{
		FArdaFakeDxcWorkerBootstrap()
		{
			const std::vector<std::string> ProcessArguments = GetProcessArguments();
			const auto OutputArgument = std::find(ProcessArguments.begin(), ProcessArguments.end(), "-Fo");
			if (OutputArgument == ProcessArguments.end() || std::next(OutputArgument) == ProcessArguments.end())
			{
				return;
			}
			std::ofstream Output(std::filesystem::path(*std::next(OutputArgument)), std::ios::binary | std::ios::trunc);
	...
	const FArdaFakeDxcWorkerBootstrap GFakeDxcWorkerBootstrap;
}

namespace
{
	using FArdaStage = arda::EArdaRHIShaderStage;

	ARDA_SHADER_PERMUTATION_BOOL(FArdaUseFeature, "USE_FEATURE");
	ARDA_SHADER_PERMUTATION_INT(FArdaQualityLevel, "QUALITY_LEVEL", 3);

	struct FArdaPermutationShaderPolicy
	{
		using FArdaPermutationDomain = arda::TArdaShaderPermutationDomain<FArdaUseFeature, FArdaQualityLevel>;

		static bool ShouldCompilePermutation(const arda::FArdaShaderPermutationParameters& Parameters)
		{
			return (Parameters.mPermutationId % 2u) == 0;
		}

		static void ModifyCompilationEnvironment(const arda::FArdaShaderPermutationParameters& Parameters,
		    arda::FArdaShaderCompileEnvironment& Environment)
		{
			Environment.SetDefine("BINARY_IS_SPIRV", Parameters.mBinaryFormat == arda::EArdaShaderBinaryFormat::Spirv);
			Environment.SetDefine("CUSTOM_VALUE", uint64_t{17});
		}
	};

	class FArdaMacroStyleShader
	{
		ARDA_DECLARE_GLOBAL_SHADER(FArdaMacroStyleShader);

		using FArdaPermutationDomain = arda::TArdaShaderPermutationDomain<FArdaUseFeature>;

		static bool ShouldCompilePermutation(const arda::FArdaShaderPermutationParameters&)
		{
			return true;
		}

		static void ModifyCompilationEnvironment(const arda::FArdaShaderPermutationParameters&,
		    arda::FArdaShaderCompileEnvironment& Environment)
		{
	...
	static_assert(arda::TArdaShaderPermutationTraits<FArdaMacroStyleShader>::PermutationCount == 2);
	static_assert(arda::TArdaHasShouldCompilePermutation<FArdaMacroStyleShader>::value);
	static_assert(arda::TArdaHasModifyCompilationEnvironment<FArdaMacroStyleShader>::value);

	struct FArdaWrongHookSignatureShader
	{
		using FArdaPermutationDomain = arda::TArdaShaderPermutationDomain<FArdaUseFeature>;

		static void ShouldCompilePermutation(const arda::FArdaShaderPermutationParameters&)
		{
		}
	};

	static_assert(arda::TArdaHasNamedShouldCompilePermutation<FArdaWrongHookSignatureShader>::value);
	static_assert(!arda::TArdaHasShouldCompilePermutation<FArdaWrongHookSignatureShader>::value);

	ARDA_BEGIN_SHADER_PARAMETER_STRUCT(FArdaNestedValues)
		ARDA_SHADER_PARAMETER_VALUE(uint32_t, mMode)
	ARDA_END_SHADER_PARAMETER_STRUCT()

	ARDA_BEGIN_SHADER_PARAMETER_STRUCT(FArdaMetadataParameters)
		ARDA_SHADER_PARAMETER_STRUCT(FArdaNestedValues, mValues)
		ARDA_SHADER_TEXTURE_SRV_ARRAY(mTextures, 2, 0, 2, FArdaStage::Compute)
		ARDA_SHADER_BUFFER_UAV(mOutput, 0, 0, FArdaStage::Compute)
		ARDA_SHADER_SAMPLER(mSampler, 0, 0, FArdaStage::Compute)
	ARDA_END_SHADER_PARAMETER_STRUCT()

	ARDA_BEGIN_SHADER_PARAMETER_STRUCT(FArdaDirectParameters)
		ARDA_SHADER_BUFFER_UAV_ARRAY(mOutputs, 2, 3, 2, FArdaStage::Compute)
		ARDA_SHADER_BUFFER_UAV(mOtherOutput, 0, 0, FArdaStage::Compute)
	ARDA_END_SHADER_PARAMETER_STRUCT()

	ARDA_BEGIN_SHADER_PARAMETER_STRUCT(FArdaNestedResources)
		ARDA_SHADER_TEXTURE_SRV(mNestedTexture, 4, 2, FArdaStage::Pixel)
	ARDA_END_SHADER_PARAMETER_STRUCT()

	ARDA_BEGIN_SHADER_PARAMETER_STRUCT(FArdaNestedResourceRoot)
		ARDA_SHADER_PARAMETER_VALUE(uint32_t, mPrefix)
		ARDA_SHADER_PARAMETER_STRUCT(FArdaNestedResources, mResources)
	ARDA_END_SHADER_PARAMETER_STRUCT()

	struct FArdaPushPayload
	{
		uint32_t mFirst = 0;
		uint32_t mSecond = 0;
	};

	ARDA_BEGIN_SHADER_PARAMETER_STRUCT(FArdaPushParameters)
		ARDA_SHADER_PARAMETER_VALUE(uint32_t, mPrefix)
		ARDA_SHADER_PUSH_CONSTANTS(FArdaPushPayload, mPush, 0, 0, FArdaStage::Compute)
	ARDA_END_SHADER_PARAMETER_STRUCT()

	const arda::FArdaShaderParameterMetadata* DirectMetadata()
	{
		return &FArdaDirectParameters::GetStaticMetadata();
	}

	struct FArdaShaderCompilerConfigurationGuard
	{
		~FArdaShaderCompilerConfigurationGuard()
		{
...
{
	using namespace arda;
	using FArdaDomain = FArdaPermutationShaderPolicy::FArdaPermutationDomain;
	static_assert(FArdaDomain::PermutationCount == 6);

	for (uint32_t Id = 0; Id < FArdaDomain::PermutationCount; ++Id)
	{
		const FArdaDomain Value(Id);
		ASSERT_TRUE(Value.IsValid());
		EXPECT_EQ(Value.ToId(), Id);
		EXPECT_EQ(Value.Get<FArdaUseFeature>(), (Id % 2u) != 0);
		EXPECT_EQ(Value.Get<FArdaQualityLevel>(), Id / 2u);
	...
	FArdaDomain Mutable;
	EXPECT_TRUE(Mutable.Set<FArdaQualityLevel>(2));
	EXPECT_TRUE(Mutable.Set<FArdaUseFeature>(true));
	EXPECT_EQ(Mutable.ToId(), 5u);
	EXPECT_FALSE(Mutable.Set<FArdaQualityLevel>(3));
	EXPECT_FALSE(FArdaDomain(6).IsValid());
...
TEST(ArdaShaderStructs, DetectsHooksDeclaredAfterGlobalShaderMacro)
{
	...
	EXPECT_TRUE(arda::ShouldCompileShaderPermutation<FArdaMacroStyleShader>(Parameters));
	arda::BuildShaderCompilationEnvironment<FArdaMacroStyleShader>(Parameters, Environment);
	ASSERT_EQ(Environment.GetDefines().size(), 2u);
...
TEST(ArdaShaderStructs, RegistersOptionalPermutationPoliciesAndArtifactStems)
{
	...
	FArdaShaderTypeRegistration Registration("PermutationPolicy",
	    "PermutationSource",
	    "PermutationArtifact",
	    "Main",
	    FArdaStage::Compute,
	    nullptr,
	    arda::TArdaShaderPermutationTraits<FArdaPermutationShaderPolicy>::PermutationCount,
	    &arda::ShouldCompileShaderPermutation<FArdaPermutationShaderPolicy>,
	    &arda::BuildShaderCompilationEnvironment<FArdaPermutationShaderPolicy>);
	...
	const auto Environment = Type.BuildCompilationEnvironment(SpirvTarget, 4);
	const auto& Defines = Environment.GetDefines();
...
TEST(ArdaShaderStructs, ValidatesPermutationRegistrationAndStemCollisions)
{
	...
	FArdaShaderTypeRegistration::ResetForTests();
	{
		FArdaShaderTypeRegistration
		    Invalid("InvalidPermutation", "Source", "Artifact", "Main", FArdaStage::Compute, nullptr, 0);
		EXPECT_EQ(FArdaShaderTypeRegistration::CommitAll().mCode, EArdaShaderRegistrationError::InvalidPermutation);
	}
	{
		FArdaShaderTypeRegistration
		    Invalid("MissingCallbacks", "Source", "Artifact", "Main", FArdaStage::Compute, nullptr, 2);
		EXPECT_EQ(FArdaShaderTypeRegistration::CommitAll().mCode, EArdaShaderRegistrationError::InvalidPermutation);
	}
	{
		FArdaShaderTypeRegistration Variant("Variant",
		    "SourceA",
		    "Shared",
		    "Main",
		    FArdaStage::Compute,
		    ...
		    &arda::ShouldCompileShaderPermutation<FArdaPermutationShaderPolicy>,
		    &arda::BuildShaderCompilationEnvironment<FArdaPermutationShaderPolicy>);
		FArdaShaderTypeRegistration CollidingBase("CollidingBase",
		    "SourceB",
		    "Shared_P2",
		    "Main",
		    FArdaStage::Compute,
		    nullptr);
		EXPECT_EQ(FArdaShaderTypeRegistration::CommitAll().mCode, EArdaShaderRegistrationError::ArtifactStemCollision);
	}
}
...
TEST(ArdaShaderStructs, RejectsUnsafeAndCaseCollidingArtifactStems)
{
	using namespace arda;
	for (const char* Stem :
	    {"", ".hidden", "../escape", "folder/name", "folder\\name", "drive:name", "two..dots", "-leading-dash"})
	{
		FArdaShaderTypeRegistration::ResetForTests();
		FArdaShaderTypeRegistration Invalid("UnsafeStem", "Source", Stem, "Main", FArdaStage::Compute, nullptr);
		EXPECT_EQ(FArdaShaderTypeRegistration::CommitAll().mCode, EArdaShaderRegistrationError::InvalidType) << Stem;
	}

	FArdaShaderTypeRegistration::ResetForTests();
	FArdaShaderTypeRegistration Upper("Upper", "SourceA", "PortableArtifact", "Main", FArdaStage::Compute, nullptr);
	FArdaShaderTypeRegistration Lower("Lower", "SourceB", "portableartifact", "Main", FArdaStage::Compute, nullptr);
	EXPECT_EQ(FArdaShaderTypeRegistration::CommitAll().mCode, EArdaShaderRegistrationError::ArtifactStemCollision);
}
...
TEST(ArdaShaderStructs, CommitsInDeterministicOrderAndSupportsLookup)
{
	using namespace arda;
	FArdaShaderTypeRegistration::ResetForTests();
	FArdaShaderTypeRegistration Second("ZSecond", "Source", "Second", "Main", FArdaStage::Compute, nullptr);
	FArdaShaderTypeRegistration First("AFirst", "Source", "First", "Main", FArdaStage::Compute, nullptr);
...
TEST(ArdaShaderStructs, RejectsDuplicateNamesAndArtifactIdentities)
{
	...
	FArdaShaderTypeRegistration::ResetForTests();
	{
		FArdaShaderTypeRegistration One("Duplicate", "Source", "One", "Main", FArdaStage::Compute, nullptr);
		FArdaShaderTypeRegistration Two("Duplicate", "Source", "Two", "Main", FArdaStage::Compute, nullptr);
	...
	{
		FArdaShaderTypeRegistration One("First", "Source", "Same", "Main", FArdaStage::Compute, nullptr);
		FArdaShaderTypeRegistration Two("Second", "Source", "Same", "Main", FArdaStage::Compute, nullptr);
		const auto Status = FArdaShaderTypeRegistration::CommitAll();
...
{
	using namespace arda;
	const auto& Metadata = FArdaMetadataParameters::GetStaticMetadata();
	ASSERT_TRUE(Metadata.GetStatus());
	EXPECT_NE(Metadata.GetLayoutHash(), 0u);
	EXPECT_EQ(Metadata.GetLayoutHash(), FArdaMetadataParameters::GetStaticMetadata().GetLayoutHash());
	...
	EXPECT_EQ(Layouts[0].mVisibility, FArdaStage::Compute);
	...
	const auto& Nested = FArdaNestedResourceRoot::GetStaticMetadata();
	size_t NestedOffset = 0;
	const auto* NestedTexture = Nested.FindFlattenedMember("mResources.mNestedTexture", &NestedOffset);
	ASSERT_NE(NestedTexture, nullptr);
	EXPECT_EQ(NestedTexture->mSlot, 4u);
	EXPECT_EQ(NestedOffset,
	    offsetof(FArdaNestedResourceRoot, mResources) + offsetof(FArdaNestedResources, mNestedTexture));
...
{
	using namespace arda;
	eastl::vector<FArdaShaderParameterMember> Duplicate = {{"A",
	                                                           EArdaShaderParameterKind::TextureSRV,
	                                                           arda::EArdaRHIBindingType::TextureSRV,
	                                                           0,
	                                                           0,
	                                                           1,
	                                                           0,
	                                                           sizeof(arda::FArdaRHITextureRef),
	                                                           sizeof(arda::FArdaRHITextureRef),
	                                                           FArdaStage::Compute,
	                                                           nullptr},
	    {"B",
	        EArdaShaderParameterKind::BufferSRV,
	        arda::EArdaRHIBindingType::StructuredBufferSRV,
	        0,
	        0,
	        1,
	        sizeof(arda::FArdaRHITextureRef),
	        sizeof(arda::FArdaRHIBufferRef),
	        sizeof(arda::FArdaRHIBufferRef),
	        FArdaStage::Compute,
	        nullptr}};
	FArdaShaderParameterMetadata DuplicateMetadata("Duplicate", 64, alignof(void*), eastl::move(Duplicate));
	EXPECT_EQ(DuplicateMetadata.GetStatus().mCode, EArdaShaderStructError::DuplicateRegister);

	eastl::vector<FArdaShaderParameterMember> Disjoint = {{"VertexTexture",
	                                                          EArdaShaderParameterKind::TextureSRV,
	                                                          arda::EArdaRHIBindingType::TextureSRV,
	                                                          0,
	                                                          0,
	    ...
	    {"PixelTexture",
	        ...
	        nullptr},
	    {"Constants",
	        EArdaShaderParameterKind::ConstantBuffer,
	        arda::EArdaRHIBindingType::ConstantBuffer,
	        0,
	        0,
	        1,
	        32,
	        ...
	        nullptr},
	    {"Push",
	        EArdaShaderParameterKind::PushConstants,
	        ...
	        FArdaStage::Compute,
	        nullptr}};
	FArdaShaderParameterMetadata DisjointMetadata("Disjoint", 64, alignof(void*), eastl::move(Disjoint));
	...
	eastl::vector<FArdaShaderParameterMember> Overlapping = {{"VertexTexture",
	                                                             EArdaShaderParameterKind::TextureSRV,
	                                                             arda::EArdaRHIBindingType::TextureSRV,
	                                                             0,
	                                                             0,
	    ...
	    {"SharedTexture",
	        ...
	        sizeof(arda::FArdaRHITextureRef),
	        sizeof(arda::FArdaRHITextureRef),
	        FArdaStage::Vertex | FArdaStage::Pixel,
	        nullptr}};
	FArdaShaderParameterMetadata OverlapMetadata("Overlap", 32, alignof(void*), eastl::move(Overlapping));
	EXPECT_EQ(OverlapMetadata.GetStatus().mCode, EArdaShaderStructError::DuplicateRegister);

	eastl::vector<FArdaShaderParameterMember> Invisible = {{"Texture",
	    EArdaShaderParameterKind::TextureSRV,
	    arda::EArdaRHIBindingType::TextureSRV,
	    0,
	    0,
	    ...
	    FArdaStage::None,
	    nullptr}};
	FArdaShaderParameterMetadata InvisibleMetadata("Invisible", 16, alignof(void*), eastl::move(Invisible));
	EXPECT_EQ(InvisibleMetadata.GetStatus().mCode, EArdaShaderStructError::IncompatibleVisibility);

	eastl::vector<FArdaShaderParameterMember> Array = {{"Array",
	    EArdaShaderParameterKind::TextureSRV,
	    arda::EArdaRHIBindingType::TextureSRV,
	    0,
	    0,
	    ...
	    FArdaStage::Compute,
	    nullptr}};
	FArdaShaderParameterMetadata ArrayMetadata("Array", 16, alignof(void*), eastl::move(Array));
	EXPECT_EQ(ArrayMetadata.GetStatus().mCode, EArdaShaderStructError::MalformedArray);

	eastl::vector<FArdaShaderParameterMember> Push = {{"Push",
	    EArdaShaderParameterKind::PushConstants,
	    arda::EArdaRHIBindingType::PushConstants,
	    0,
	    0,
	    ...
	    FArdaStage::Compute,
	    nullptr}};
	FArdaShaderParameterMetadata PushMetadata("Push", 8, alignof(uint32_t), eastl::move(Push));
	EXPECT_EQ(PushMetadata.GetStatus().mCode, EArdaShaderStructError::MalformedPushConstants);

	eastl::vector<FArdaShaderParameterMember> MultiplePush = {{"First",
	                                                              EArdaShaderParameterKind::PushConstants,
	                                                              arda::EArdaRHIBindingType::PushConstants,
	                                                              0,
	                                                              0,
	    ...
	    {"Second",
	        ...
	        FArdaStage::Compute,
	        nullptr}};
	FArdaShaderParameterMetadata MultiplePushMetadata("MultiplePush", 16, alignof(uint32_t), eastl::move(MultiplePush));
	ASSERT_TRUE(MultiplePushMetadata.GetStatus());
	eastl::vector<arda::FArdaRHIBindingLayoutDesc> PushLayouts;
	EXPECT_EQ(MultiplePushMetadata.BuildBindingLayoutDescs(PushLayouts).mCode,
...
{
	using namespace arda;
	EXPECT_STREQ(GetShaderArtifactExtension("native-d3d12"), FindBackendModule("native-d3d12") ? ".dxil" : "");
	EXPECT_STREQ(GetShaderArtifactExtension("native-vulkan"), FindBackendModule("native-vulkan") ? ".spv" : "");
	const auto Missing = LoadShaderBytecode(std::filesystem::path(ARDA_BACKEND_TEST_SHADER_DIR) / "does-not-exist.spv");
...
{
	using namespace arda;
	FArdaShaderCompilerConfigurationGuard ConfigurationGuard;
	ResetShaderCompilerConfiguration();
	FArdaShaderCompilerConfiguration FakeCompilerConfiguration = GetShaderCompilerConfiguration();
	// This fixture exercises development rebuilds explicitly, including when the test binary uses Release defaults.
	FakeCompilerConfiguration.mbCompileMissingArtifacts = true;
	FakeCompilerConfiguration.mbCompileOutdatedArtifacts = true;
	...
	const std::filesystem::path Directory =
	    std::filesystem::temp_directory_path() / "ArdaRegistrationDrivenShaderCompilerTest";
	...
	FArdaShaderTypeRegistration Registration("CompilerPolicy",
	    SourceName.c_str(),
	...
	const char* PrimaryExtension = GetShaderArtifactExtension(Backends.front().c_str());
	const std::string PrimaryArtifactFilename = std::string("CompilerPolicyArtifact_P0") + PrimaryExtension;
	const size_t ExpectedJobCount = Backends.size() * 3u;
	const FArdaShaderCompileResult FirstJobs = BuildRegisteredShaderCompileJobs(Output, Backends);
	...
	EXPECT_EQ(FirstJobs.mJobs[0].mOutputPath.filename(), std::filesystem::path(PrimaryArtifactFilename));
	ASSERT_EQ(FirstJobs.mJobs[0].mEnvironment.GetDefines().size(), 4u);
	EXPECT_EQ(FirstJobs.mJobs[0].mEnvironment.GetDefines()[0].mName, "BINARY_IS_SPIRV");
	if (bHasVulkan)
	{
		const auto VulkanJob = std::find_if(FirstJobs.mJobs.begin(),
		    FirstJobs.mJobs.end(),
		...
		EXPECT_EQ(VulkanJob->mTarget.mBinaryFormat, EArdaShaderBinaryFormat::Spirv);
		EXPECT_NE(std::find(VulkanJob->mArguments.begin(), VulkanJob->mArguments.end(), eastl::string("-spirv")),
		    VulkanJob->mArguments.end());

		const FArdaShaderCompileResult NamedJobs =
		    BuildRegisteredShaderCompileJobs(Output, eastl::vector<eastl::string>{"native-vulkan"});
		ASSERT_TRUE(NamedJobs);
		ASSERT_EQ(NamedJobs.mJobs.size(), 3u);
		EXPECT_TRUE(std::all_of(NamedJobs.mJobs.begin(),
		    NamedJobs.mJobs.end(),
		    [](const FArdaShaderCompileJob& Job)
		    {
			    return Job.mTarget.mBackendName == "native-vulkan" && Job.mOutputPath.extension() == ".spv";
		    }));
		EXPECT_NE(std::find(VulkanJob->mArguments.begin(),
		              VulkanJob->mArguments.end(),
		...
		{
			if (VulkanJob->mArguments[Index] == "-fvk-u-shift" && VulkanJob->mArguments[Index + 1] == "384" &&
	...
	const FArdaShaderCompileResult SecondJobs = BuildRegisteredShaderCompileJobs(Output, Backends);
	...
	const auto AngleChangedJobs = BuildRegisteredShaderCompileJobs(Output, Backends);
	ASSERT_TRUE(AngleChangedJobs);
	EXPECT_NE(AngleChangedJobs.mJobs[0].mInputKey, FirstJobs.mJobs[0].mInputKey);
	{
		std::ofstream Stream(MacroInclude, std::ios::app);
		Stream << "// macro include invalidation\n";
	}
	const auto MacroChangedJobs = BuildRegisteredShaderCompileJobs(Output, Backends);
	ASSERT_TRUE(MacroChangedJobs);
	EXPECT_NE(MacroChangedJobs.mJobs[0].mInputKey, AngleChangedJobs.mJobs[0].mInputKey);
	const std::filesystem::path ExternalIncludes = Directory.parent_path() / "ArdaRegistrationDrivenExternalIncludes";
	...
	IncludeConfiguration.mCommonArguments.push_back(ExternalIncludes.string().c_str());
	ConfigureShaderCompiler(IncludeConfiguration);
	const auto ExternalFirstJobs = BuildRegisteredShaderCompileJobs(Output, Backends);
	ASSERT_TRUE(ExternalFirstJobs);
	{
		std::ofstream Stream(ExternalInclude, std::ios::app);
		Stream << "// external include invalidation\n";
	}
	const auto ExternalChangedJobs = BuildRegisteredShaderCompileJobs(Output, Backends);
	ASSERT_TRUE(ExternalChangedJobs);
	EXPECT_NE(ExternalChangedJobs.mJobs[0].mInputKey, ExternalFirstJobs.mJobs[0].mInputKey);
	ConfigureShaderCompiler(FakeCompilerConfiguration);
	std::filesystem::remove_all(ExternalIncludes, Error);
	...
	{
		EXPECT_EQ(MacroChangedJobs.mJobs[Index].mInputKey, OtherDirectoryJobs.mJobs[Index].mInputKey);
	}
	{
		std::ofstream Stream(Include, std::ios::app);
		Stream << "// transitive dependency invalidation\n";
	}
	const FArdaShaderCompileResult IncludeChangedJobs = BuildRegisteredShaderCompileJobs(Output, Backends);
	ASSERT_TRUE(IncludeChangedJobs);
	ASSERT_EQ(IncludeChangedJobs.mJobs.size(), FirstJobs.mJobs.size());
	EXPECT_NE(IncludeChangedJobs.mJobs[0].mInputKey, FirstJobs.mJobs[0].mInputKey);

	FArdaShaderCompileResult Cook = CompileRegisteredShaderArtifacts(Output, Backends);
	ASSERT_TRUE(Cook) << Cook.mDiagnostics.front().mMessage.c_str();
	EXPECT_EQ(Cook.mJobsCompiled, ExpectedJobCount);
	const std::filesystem::path Artifact = Output / PrimaryArtifactFilename;
	const std::filesystem::path Manifest = Output / "ArdaShaderManifest.json";
	ASSERT_TRUE(std::filesystem::is_regular_file(Artifact));
	if (bHasD3D12)
	{
		ASSERT_TRUE(std::filesystem::is_regular_file(Output / "CompilerPolicyArtifact_P0.dxil"));
	}
	if (bHasVulkan)
	{
		ASSERT_TRUE(std::filesystem::is_regular_file(Output / "CompilerPolicyArtifact_P0.spv"));
	}
	EXPECT_NE(ReadTestFile(Artifact).find("\"-T\" \"cs_6_0\""), std::string::npos);
	ASSERT_TRUE(std::filesystem::is_regular_file(Artifact.string() + ".arda-key"));
	const std::string FirstManifest = ReadTestFile(Manifest);
	ASSERT_FALSE(FirstManifest.empty());

	FArdaShaderCompileResult Cached = EnsureRegisteredShaderArtifact(Registration.GetType(), PrimaryBackend, 0, Output);
	ASSERT_TRUE(Cached);
	EXPECT_EQ(Cached.mCacheHits, 1u);
	EXPECT_EQ(Cached.mJobsCompiled, 0u);
	const FArdaShaderCompileResult BulkCached = EnsureRegisteredShaderArtifacts(Output, PrimaryBackend);
	...
	ASSERT_TRUE(std::filesystem::remove(Artifact.string() + ".arda-key", Error));
	FArdaShaderCompileResult Legacy = EnsureRegisteredShaderArtifact(Registration.GetType(), PrimaryBackend, 0, Output);
	ASSERT_TRUE(Legacy);
	...
	FArdaShaderCompileResult EmptyRebuilt =
	    EnsureRegisteredShaderArtifact(Registration.GetType(), PrimaryBackend, 0, Output);
	...
	FArdaShaderCompilerConfiguration StaleConfiguration = GetShaderCompilerConfiguration();
	...
	const FArdaShaderCompileResult Failed =
	    EnsureRegisteredShaderArtifact(Registration.GetType(), PrimaryBackend, 0, Output);
	EXPECT_FALSE(Failed);
	ASSERT_FALSE(Failed.mDiagnostics.empty());
	EXPECT_TRUE(Failed.mDiagnostics.front().mCode == EArdaShaderCompileError::ProcessLaunchFailed ||
	    Failed.mDiagnostics.front().mCode == EArdaShaderCompileError::CompilationFailed);
	EXPECT_EQ(ReadTestFile(Artifact), ArtifactBeforeFailure);
	const FArdaShaderCompileResult FailedCook = CompileRegisteredShaderArtifacts(Output, Backends);
	...
	const FArdaShaderCompileResult MissingInclude = BuildRegisteredShaderCompileJobs(Output, Backends);
	EXPECT_FALSE(MissingInclude);
	ASSERT_FALSE(MissingInclude.mDiagnostics.empty());
	EXPECT_EQ(MissingInclude.mDiagnostics.front().mCode, EArdaShaderCompileError::SourceResolutionFailed);
	EXPECT_NE(std::string(MissingInclude.mDiagnostics.front().mMessage.c_str()).find("MissingLocalInclude.hlsli"),
	    std::string::npos);
...
{
	using namespace arda;
	FArdaPushParameters Parameters;
	Parameters.mPrefix = 17;
	Parameters.mPush.mFirst = 23;
	Parameters.mPush.mSecond = 42;
	eastl::vector<arda::FArdaRHIBindingLayoutDesc> Layouts;
	const auto& Metadata = FArdaPushParameters::GetStaticMetadata();
	ASSERT_TRUE(Metadata.BuildBindingLayoutDescs(Layouts));
	ASSERT_EQ(Layouts.size(), 1u);
	ASSERT_EQ(Layouts[0].mItems.size(), 1u);
	EXPECT_EQ(Layouts[0].mItems[0].mType, arda::EArdaRHIBindingType::PushConstants);

	const void* Data = nullptr;
	size_t Size = 0;
	ASSERT_TRUE(Metadata.GetPushConstantData(&Parameters, Layouts[0], Data, Size));
	EXPECT_EQ(Data, &Parameters.mPush);
	EXPECT_EQ(Size, sizeof(FArdaPushPayload));
	EXPECT_EQ(static_cast<const FArdaPushPayload*>(Data)->mSecond, 42u);
}
...
TEST(ArdaShaderStructs, RegistrationDestructionPreservesUnrelatedTypes)
{
	using namespace arda;
	FArdaShaderTypeRegistration::ResetForTests();
	FArdaShaderTypeRegistration First("LifetimeFirst", "Source", "First", "Main", FArdaStage::Compute, nullptr);
	FArdaShaderTypeRegistration Second("LifetimeSecond", "Source", "Second", "Main", FArdaStage::Compute, nullptr);
	ASSERT_TRUE(FArdaShaderTypeRegistration::CommitAll());
	const FArdaShaderType* FirstType = FArdaShaderTypeRegistration::Find("LifetimeFirst");
	const FArdaShaderType* SecondType = FArdaShaderTypeRegistration::Find("LifetimeSecond");
	{
		FArdaShaderTypeRegistration Temporary("LifetimeTemporary",
		    "Source",
		    "Temporary",
		    "Main",
		    FArdaStage::Compute,
		    nullptr);
		ASSERT_TRUE(FArdaShaderTypeRegistration::CommitAll());
		EXPECT_NE(FArdaShaderTypeRegistration::Find("LifetimeTemporary"), nullptr);
	}
...
TEST(ArdaShaderStructs, CompileJobsOwnDescriptorsAfterRegistrationDestruction)
{
	...
	ConfigureShaderCompiler(Configuration);
	FArdaShaderTypeRegistration::ResetForTests();
	const std::filesystem::path Directory = std::filesystem::temp_directory_path() / "ArdaShaderJobLifetimeTest";
	...
	{
		const std::string SourceName = Source.string();
		FArdaShaderTypeRegistration Registration("OwnedJobType",
		    SourceName.c_str(),
		    "OwnedJobArtifact",
		    "Main",
		    FArdaStage::Compute,
		    nullptr);
		const FArdaShaderCompileResult Jobs =
		    BuildRegisteredShaderCompileJobs(Directory / "Cooked", eastl::vector<eastl::string>{Target.mBackendName});
		ASSERT_TRUE(Jobs);
...
TEST(ArdaShaderStructs, StartupModePersistsAndReusesRegisteredShaderCache)
{
	...
	ShutdownBackend();
	FArdaShaderCompilerConfigurationGuard CompilerGuard;
	FArdaShaderTypeRegistration::ResetForTests();
	const std::filesystem::path Directory = std::filesystem::temp_directory_path() / "ArdaStartupShaderCacheTest";
	...
	// Startup cache creation is the intended policy here; bytecode-only loading has its own explicit policy test below.
	...
	const std::string SourceName = Source.string();
	FArdaShaderTypeRegistration Registration("StartupCacheShader",
	    SourceName.c_str(),
	    "StartupCacheArtifact",
	    "Main",
	    FArdaStage::Compute,
	    nullptr);
	...
	const std::filesystem::path Artifact =
	    Cache / (std::string("StartupCacheArtifact") + Target.mArtifactExtension.c_str());
	ASSERT_TRUE(std::filesystem::is_regular_file(Artifact)) << GetBackendError().c_str();
	ASSERT_TRUE(std::filesystem::is_regular_file(Artifact.string() + ".arda-key"));
	...
	const FArdaShaderCompileResult Cached = EnsureRegisteredShaderArtifacts(Cache, Target.mBackendName.c_str());
	ASSERT_TRUE(Cached);
	EXPECT_EQ(Cached.mJobsCompiled, 0u);
	EXPECT_EQ(Cached.mCacheHits, 1u);
...
TEST(ArdaShaderStructs, StartupModePropagatesCompilerFailureBeforeDevice)
{
	...
	FArdaShaderCompilerConfigurationGuard CompilerGuard;
	FArdaShaderTypeRegistration::ResetForTests();
	const std::filesystem::path Directory = std::filesystem::temp_directory_path() / "ArdaStartupShaderFailureTest";
	...
	FArdaShaderCompilerConfiguration CompilerConfiguration;
	// Enable compilation so this negative case reaches the invalid executable in every build configuration.
	CompilerConfiguration.mbCompileMissingArtifacts = true;
	CompilerConfiguration.mbCompileOutdatedArtifacts = true;
	CompilerConfiguration.mCompilerExecutable = Source;
	ConfigureShaderCompiler(CompilerConfiguration);
	const std::string SourceName = Source.string();
	FArdaShaderTypeRegistration Registration("StartupFailureShader",
	    SourceName.c_str(),
	    "StartupFailureArtifact",
	    "Main",
	    FArdaStage::Compute,
	    nullptr);
	FArdaBackendConfiguration Configuration = arda::MakeArdaTestBackendConfiguration();
	...
	EXPECT_NE(GetBackendError().find("Startup shader compilation failed"), eastl::string::npos);
	std::filesystem::remove_all(Directory, Error);
}
...
TEST(ArdaShaderStructs, GlobalMapIndexesOnlyCompiledPermutations)
{
	...
	FArdaShaderTypeRegistration::ResetForTests();
	FArdaShaderTypeRegistration Registration("PermutationMapShader",
	...
	const std::filesystem::path SourceDirectory = ARDA_BACKEND_TEST_SHADER_DIR;
	const std::filesystem::path TestDirectory = std::filesystem::temp_directory_path() / "ArdaPermutationShaderMapTest";
	std::error_code Error;
	...
	const std::filesystem::path Source = SourceDirectory / (std::string("ArdaShaderStructTest") + Extension);
	for (const uint32_t Id : {0u, 2u, 4u})
	{
		const std::filesystem::path Destination =
		    TestDirectory / (std::string("PermutationMapArtifact_P") + std::to_string(Id) + Extension);
		ASSERT_TRUE(
...
TEST(ArdaShaderStructs, OnDemandMapDefersMissingArtifactAndHonorsOverride)
{
	...
	FArdaShaderCompilerConfigurationGuard CompilerGuard;
	FArdaShaderTypeRegistration::ResetForTests();
	const std::filesystem::path Directory = std::filesystem::temp_directory_path() / "ArdaOnDemandShaderMapTest";
	...
	FArdaShaderCompilerConfiguration CompilerConfiguration = GetShaderCompilerConfiguration();
	...
	FArdaShaderTypeRegistration Registration("OnDemandOverrideShader",
	    "UnusedLocalSource.hlsl",
	    "OnDemandOverrideArtifact",
	    "ShaderStructTestCS",
	    FArdaStage::Compute,
	    nullptr);
	const std::string Extension = Target.mArtifactExtension.c_str();
	ASSERT_TRUE(std::filesystem::copy_file(std::filesystem::path(ARDA_BACKEND_TEST_SHADER_DIR) /
	        (std::string("ArdaShaderStructTest") + Extension),
	    OverrideCache / (std::string("OnDemandOverrideArtifact") + Extension),
	...
	EXPECT_EQ(ConfiguredMap.GetDiagnostics().back().mCode, EArdaGlobalShaderMapError::BytecodeMissing);
	ASSERT_TRUE(std::filesystem::create_directories(ConfiguredCache, Error));
	ASSERT_TRUE(std::filesystem::copy_file(std::filesystem::path(ARDA_BACKEND_TEST_SHADER_DIR) /
	        (std::string("ArdaShaderStructTest") + Extension),
	    ConfiguredCache / (std::string("OnDemandOverrideArtifact") + Extension),
	    std::filesystem::copy_options::overwrite_existing,
	    Error));
	...
	for (size_t Index = 0; Index < ConcurrentResults.size(); ++Index)
	{
		Finders.emplace_back(
		    [&OverrideMap, &Registration, &ConcurrentResults, Index]
		    {
			    ConcurrentResults[Index] = OverrideMap.Find(Registration.GetType());
		    });
	}
...
TEST(ArdaShaderStructs, LoadsGlobalMapIdempotentlyAndBuildsDirectBindings)
{
	...
	{
		FArdaShaderTypeRegistration Registration("ShaderStructTest",
		    "ArdaShaderStructTest",
		    "ArdaShaderStructTest",
		    "ShaderStructTestCS",
		    FArdaStage::Compute,
		    &DirectMetadata);
		FArdaShaderTypeRegistration NoParameters("NoParameterShader",
		    "NoParameterSource",
		    "ArdaShaderStructTestNoParameters",
		    "ShaderStructTestCS",
		    FArdaStage::Compute,
		    nullptr);
		EXPECT_NE(Registration.GetType().GetIdentityHash(), NoParameters.GetType().GetIdentityHash());
		FArdaGlobalShaderMap Map;
		const std::filesystem::path Directory =
		...
		const std::filesystem::path CopiedSource =
		    std::filesystem::path(ARDA_BACKEND_TEST_SHADER_DIR) / (std::string("ArdaShaderStructTest") + Extension);
		for (const char* Stem : {"ArdaShaderStructTest", "ArdaShaderStructTestNoParameters"})
		{
			ASSERT_TRUE(std::filesystem::copy_file(CopiedSource,
			    Directory / (std::string(Stem) + Extension),
			    std::filesystem::copy_options::overwrite_existing,
			    ArtifactError));
		}
		{
			FArdaShaderTypeRegistration Missing("ZMissingShader",
			    "MissingSource",
			    "missing-partial-artifact",
			    "ShaderStructTestCS",
			    FArdaStage::Compute,
			    nullptr);
			EXPECT_FALSE(Map.Initialize(GetDevice(), Directory));
			ASSERT_FALSE(Map.GetDiagnostics().empty());
			EXPECT_EQ(Map.GetDiagnostics().back().mCode, EArdaGlobalShaderMapError::BytecodeMissing);
		...
		const FArdaGlobalShaderInstance* NoParameterShader = Map.Find(NoParameters.GetType());
		...
		for (const arda::FArdaRHIBindingLayoutRef& Layout : First->GetBindingLayouts())
		{
		...
		EXPECT_FALSE(Map.Initialize(GetDevice(), Directory / "different-directory"));
		ASSERT_FALSE(Map.GetDiagnostics().empty());
		EXPECT_EQ(Map.GetDiagnostics().back().mCode, EArdaGlobalShaderMapError::ResetRequired);
		...
		ASSERT_TRUE(Buffer);
		FArdaDirectParameters Parameters;
		auto IncompleteLayoutDesc = (*ArrayLayout)->GetDesc();
		IncompleteLayoutDesc.mItems[0].mArraySize = 1;
		auto IncompleteLayout = GetDevice()->CreateBindingLayout(IncompleteLayoutDesc);
		ASSERT_TRUE(IncompleteLayout);
		FArdaRHIBindingSetDesc IncompleteDesc;
		EXPECT_FALSE(FArdaDirectParameters::GetStaticMetadata().BuildBindingSetDesc(&Parameters,
		    IncompleteLayout.mValue,
		    IncompleteDesc));
		FArdaRHIBindingSetDesc NullDesc;
		EXPECT_FALSE(
		    FArdaDirectParameters::GetStaticMetadata().BuildBindingSetDesc(&Parameters, *ArrayLayout, NullDesc));
		Parameters.mOutputs[0] = Buffer.mValue;
		Parameters.mOutputs[1] = Buffer.mValue;
		FArdaRHIBindingSetDesc Desc;
		ASSERT_TRUE(FArdaDirectParameters::GetStaticMetadata().BuildBindingSetDesc(&Parameters, *ArrayLayout, Desc));
		...
		EXPECT_TRUE(FArdaDirectParameters::GetStaticMetadata().CreateBindingSet(*GetDevice(),
		    &Parameters,
		    *ArrayLayout,
		    BindingSet));
		...
		ASSERT_TRUE(FArdaPushParameters::GetStaticMetadata().BuildBindingLayoutDescs(PushLayoutDescs));
		auto PushLayout = GetDevice()->CreateBindingLayout(PushLayoutDescs[0]);
		ASSERT_TRUE(PushLayout);
		FArdaPushParameters PushParameters;
		FArdaRHIBindingSetDesc PushBindingDesc;
		ASSERT_TRUE(FArdaPushParameters::GetStaticMetadata().BuildBindingSetDesc(&PushParameters,
		    PushLayout.mValue,
		    PushBindingDesc));
		ASSERT_EQ(PushBindingDesc.mItems.size(), 1u);
		EXPECT_FALSE(PushBindingDesc.mItems[0].mResource);
		EXPECT_EQ(PushBindingDesc.mItems[0].mView.mBufferRange.mByteSize, sizeof(FArdaPushPayload));
