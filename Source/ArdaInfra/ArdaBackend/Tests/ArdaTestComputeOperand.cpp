...
{
	void BindAdd(arda::FArdaAddOperand::FArdaRegistry&);
	void BindSurface(arda::FArdaSurfaceOperand::FArdaRegistry&);
}
...
namespace arda
{
	void FArdaAddOperand::BindKernelVariants(FArdaRegistry& Registry) const
	{
#if defined(ARDA_TEST_PRECOMPILED_CUDA)
	...
	TArdaRHIResult<FArdaCudaKernelSelection> FArdaAddOperand::SelectKernel(const FArdaParameters& P,
	    ...
	    const FArdaVariants& Candidates) const
	{
		...
		const FArdaVariants::value_type* Selected = nullptr;
	...
	void FArdaSurfaceOperand::BindKernelVariants(FArdaRegistry& Registry) const
	{
#if defined(ARDA_TEST_PRECOMPILED_CUDA)
	...
	TArdaRHIResult<FArdaCudaKernelSelection> FArdaSurfaceOperand::SelectKernel(const FArdaParameters& P,
	    const FArdaCudaSelectionContext&,
	    const FArdaVariants& Candidates) const
	{
