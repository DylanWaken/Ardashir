...
namespace arda
{
	...
	public:
		...
		using FArdaVariants = eastl::vector<FArdaVariant>;
		...
		const FArdaVariants& GetVariants() const noexcept
		{
			return mVariants;
		}

		/** Filters native targets and execution requirements without launching work. */
		FArdaVariants GetCompatibleVariants(const FArdaCudaCapabilities& C) const
		{
			FArdaVariants Result;
			if (!mStatus || !C)
			{
				return Result;
			}
	...
	private:
		...
		FArdaRHIStatus AddVariant(FArdaVariant Variant)
		{
			if (mbFrozen)
			{
