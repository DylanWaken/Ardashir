...
namespace arda
{
	...
	public:
		...
		bool SetDefine(const eastl::string& Name, const eastl::string& Value)
		{
			...
			auto Position = eastl::lower_bound(mDefines.begin(),
			    mDefines.end(),
			    Name,
	...
	class TArdaShaderPermutationDomain final
	{
		...
		{
			return (mPermutationId / GetDivisor<Dimension>()) % Dimension::PermutationCount;
		}
	...
	public:
         ...
         */
		[[nodiscard]] static constexpr bool IsValidPermutationId(uint32_t PermutationId) noexcept
		{
         ...
         */
		template <typename Dimension>
		[[nodiscard]] constexpr typename Dimension::FArdaValueType Get() const noexcept
		{
         ...
         */
		template <typename Dimension>
		constexpr bool Set(typename Dimension::FArdaValueType Value) noexcept
		{
			...
			mPermutationId = mPermutationId - GetRaw<Dimension>() * Divisor + Encoded * Divisor;
			return IsValid();
		}
         ...
         */
		bool ModifyCompilationEnvironment(FArdaShaderCompileEnvironment& Environment) const
		{
			if (!IsValid())
			{
				return false;
...
#define ARDA_SHADER_PERMUTATION_BOOL(Name, DefineName)                                                                 \
	...
	{                                                                                                                  \
		...
		static constexpr FArdaValueType Decode(uint32_t Value) noexcept                                                \
		{                                                                                                              \
			return Value != 0;                                                                                         \
		}                                                                                                              \
		/** @return Mixed-radix digit encoded from Value. */                                                           \
		static constexpr uint32_t Encode(FArdaValueType Value) noexcept                                                \
		{                                                                                                              \
			return Value ? 1u : 0u;                                                                                    \
		}                                                                                                              \
		/** Adds this dimension's definition to Environment. */                                                        \
		static void AddDefine(::arda::FArdaShaderCompileEnvironment& Environment, FArdaValueType Value)                \
		{                                                                                                              \
