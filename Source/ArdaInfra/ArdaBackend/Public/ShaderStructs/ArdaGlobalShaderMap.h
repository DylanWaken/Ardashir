...
namespace arda
{
     ...
     */
	[[nodiscard]] FArdaShaderBytecodeResult LoadShaderBytecode(const std::filesystem::path& Path);
	...
	public:
		...
		[[nodiscard]] const arda::FArdaRHIShaderRef& GetShader() const noexcept
		{
			return mShader;
		}
		...
		[[nodiscard]] uint32_t GetPermutationId() const noexcept
		{
			return mPermutationId;
		}

		/** @return Binding layouts created for the shader parameters. */
		[[nodiscard]] const eastl::vector<arda::FArdaRHIBindingLayoutRef>& GetBindingLayouts() const noexcept
		{
			return mBindingLayouts;
		}
	...
	public:
         ...
         */
		[[nodiscard]] bool Initialize(arda::FArdaRHIDeviceRef Device, const std::filesystem::path& ShaderDirectory);
         ...
         */
		[[nodiscard]] const FArdaGlobalShaderInstance* Find(const FArdaShaderType& Type,
         ...
         */
		[[nodiscard]] const FArdaGlobalShaderInstance* Find(const eastl::string& Name,
		    uint32_t PermutationId = 0) const;
		...
		[[nodiscard]] eastl::vector<FArdaGlobalShaderMapDiagnostic> GetDiagnostics() const;
