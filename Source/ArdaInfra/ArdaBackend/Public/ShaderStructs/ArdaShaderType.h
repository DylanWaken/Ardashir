...
namespace arda
{
	...
	public:
		...
		using FArdaParameterMetadataFunction = const FArdaShaderParameterMetadata* (*)();

		/** Function that decides whether one permutation should be compiled. */
		using FArdaShouldCompilePermutationFunction = bool (*)(const FArdaShaderPermutationParameters&);

		/** Function that adds permutation and shader-class compilation definitions. */
		using FArdaModifyCompilationEnvironmentFunction = void (*)(const FArdaShaderPermutationParameters&,
		...
		[[nodiscard]] const char* GetSourceStem() const noexcept
		{
			return mSourceStem.c_str();
		}

		/** @return The artifact filename stem used by the bytecode loader. */
		[[nodiscard]] const char* GetOutputStem() const noexcept
		{
			return mOutputStem.c_str();
		}

		/** @return The shader entry-point name. */
		[[nodiscard]] const char* GetEntryPoint() const noexcept
		{
			return mEntryPoint.c_str();
		}

		/** @return The RHI stage implemented by the shader. */
		[[nodiscard]] arda::EArdaRHIShaderStage GetStage() const noexcept
		{
			return mStage;
		}
		...
		[[nodiscard]] uint64_t GetIdentityHash() const noexcept
		{
			return mIdentityHash;
		}

		/** @return Number of encoded permutations declared by the shader class. */
		[[nodiscard]] uint32_t GetPermutationCount() const noexcept
		{
			return mPermutationCount;
		}

		/** Evaluates compile policy for an exact backend-module target. */
		[[nodiscard]] bool ShouldCompilePermutation(const FArdaShaderTarget& Target, uint32_t PermutationId) const;

		/** Builds definitions for an exact backend-module target. */
		[[nodiscard]] FArdaShaderCompileEnvironment BuildCompilationEnvironment(const FArdaShaderTarget& Target,
         ...
         */
		[[nodiscard]] eastl::string GetPermutationArtifactStem(uint32_t PermutationId) const;
	...
	private:
		...
		FArdaParameterMetadataFunction mParameterMetadataFunction = nullptr;
		/** Number of encoded shader permutations. */
		uint32_t mPermutationCount = 1;
		/** Callback implementing optional compile filtering. */
		FArdaShouldCompilePermutationFunction mShouldCompilePermutationFunction = nullptr;
		/** Callback implementing dimension and class compilation definitions. */
		FArdaModifyCompilationEnvironmentFunction mModifyCompilationEnvironmentFunction = nullptr;
	...
	public:
         ...
         */
		FArdaShaderTypeRegistration(const char* Name,
		    ...
		    FArdaShaderType::FArdaShouldCompilePermutationFunction ShouldCompilePermutationFunction = nullptr,
		    FArdaShaderType::FArdaModifyCompilationEnvironmentFunction ModifyCompilationEnvironmentFunction = nullptr);
		...
		[[nodiscard]] const FArdaShaderType& GetType() const noexcept
		{
			return *mType;
		}
	...
	struct TArdaShaderPermutationTraits
	{
		...
		static void AddDefines(uint32_t, FArdaShaderCompileEnvironment&)
		{
		}
	};
	...
	struct TArdaShaderPermutationTraits<ShaderClass, std::void_t<typename ShaderClass::FArdaPermutationDomain>>
	{
		...
		static void AddDefines(uint32_t PermutationId, FArdaShaderCompileEnvironment& Environment)
		{
			FArdaDomain(PermutationId).ModifyCompilationEnvironment(Environment);
		}
	};
	...
	template <typename ShaderClass>
	[[nodiscard]] bool ShouldCompileShaderPermutation(const FArdaShaderPermutationParameters& Parameters)
	{
		static_assert(!TArdaHasNamedShouldCompilePermutation<ShaderClass>::value ||
		...
		if (Parameters.mPermutationId >= TArdaShaderPermutationTraits<ShaderClass>::PermutationCount)
		{
			return false;
		}
	...
	template <typename ShaderClass>
	void BuildShaderCompilationEnvironment(const FArdaShaderPermutationParameters& Parameters,
	    FArdaShaderCompileEnvironment& Environment)
	{
		static_assert(!TArdaHasNamedModifyCompilationEnvironment<ShaderClass>::value ||
		...
		TArdaShaderPermutationTraits<ShaderClass>::AddDefines(Parameters.mPermutationId, Environment);
...
#define ARDA_IMPLEMENT_GLOBAL_SHADER(ShaderClass, SourceStem, OutputStem, EntryPoint, Stage)                           \
	...
	::arda::FArdaShaderTypeRegistration ShaderClass::sArdaShaderRegistration(                                          \
	    ...
	    {                                                                                                              \
		    return &ShaderClass::FArdaParameters::GetStaticMetadata();                                                 \
	    },                                                                                                             \
 ...
 * @param Stage RHI shader stage.
 */
#define ARDA_IMPLEMENT_GLOBAL_SHADER_WITHOUT_PARAMETERS(ShaderClass, SourceStem, OutputStem, EntryPoint, Stage)        \
	/** Defines the shader class's static registration node. */                                                        \
	::arda::FArdaShaderTypeRegistration ShaderClass::sArdaShaderRegistration(#ShaderClass,                             \
	    SourceStem,                                                                                                    \
	    OutputStem,                                                                                                    \
	    EntryPoint,                                                                                                    \
	    Stage,                                                                                                         \
