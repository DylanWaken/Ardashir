...
namespace arda
{
	struct FArdaCudaBindingHelpers
	{
		...
		static FArdaRHIStatus Check(cudaError_t Error)
		{
			return Error == cudaSuccess
			    ? FArdaRHIStatus{}
			    : FArdaRHIStatus::Error(EArdaRHIResult::BackendFailure, cudaGetErrorString(Error));
		}

		template <auto Kernel>
		class TArdaEntry final : public IArdaCudaKernelEntry
		{
		public:
			explicit TArdaEntry(FArdaCudaBuildInfo Build)
			    : mBuild(eastl::move(Build))
			{
			}

			FArdaCudaKernelSignature GetSignature() const noexcept override
			{
				return TArdaSignature<decltype(Kernel)>::Get();
			}
			...
			{
				if (!GetSignature().mbSupported)
				{
					return {{},
					    FArdaRHIStatus::Error(EArdaRHIResult::InvalidArgument,
					        "CUDA kernel requires one plain parameter struct by value.")};
				}
				cudaFuncAttributes A{};
				if (auto S = Check(cudaFuncGetAttributes(&A, reinterpret_cast<const void*>(Kernel))); !S)
				{
					return {{}, S};
				}
				return {{uint32_t(A.maxThreadsPerBlock),
				            uint32_t(A.sharedSizeBytes),
				            uint32_t(A.maxDynamicSharedSizeBytes)},
				    {}};
			}

			FArdaRHIStatus Launch(void* Stream,
			    const FArdaCudaLaunchConfig& C,
			    ...
			    size_t Size) const override
			{
				...
				if (!S.mbSupported || Size != S.mSize || !Parameters || !Stream)
				{
					return FArdaRHIStatus::Error(EArdaRHIResult::InvalidArgument,
					    "Invalid CUDA compiled-kernel invocation.");
				}
				...
				return Check(cudaLaunchKernel(reinterpret_cast<const void*>(Kernel),
				    ...
				    dim3(C.mBlockSize[0], C.mBlockSize[1], C.mBlockSize[2]),
				    Arguments,
				    C.mSharedMemoryBytes,
	...
	TArdaCudaKernelVariant<Kernel, Payload> BindArdaCudaKernel(const char* Name,
	    Payload Info,
	    FArdaCudaBuildInfo Build,
	    FArdaCudaKernelRequirements Requirements = {})
	{
		return {eastl::make_shared<FArdaCudaBindingHelpers::TArdaEntry<Kernel>>(eastl::move(Build)),
		    eastl::move(Info),
		    Name,
		    Requirements};
	}
