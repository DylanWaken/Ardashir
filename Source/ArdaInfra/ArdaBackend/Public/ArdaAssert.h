...
namespace arda
{
     ...
     */
	[[noreturn]] void ReportFatalCheck(const char* expression,
	    const char* file,
	    std::uint32_t line,
     ...
     */
	[[noreturn]] void ReportFatalCheckf(const char* expression,
	    const char* file,
	    std::uint32_t line,
	    const char* function,
	    const char* format,
     ...
     * @return Always false.
     */
	[[nodiscard]] bool ReportEnsureFailure(const char* expression,
	    const char* file,
	    std::uint32_t line,
	    const char* function) noexcept;
     ...
     * @return Always false.
     */
	[[nodiscard]] bool ReportEnsureFailuref(const char* expression,
	    const char* file,
	    std::uint32_t line,
	    const char* function,
	    const char* format,
...
#define ARDA_CHECK(Condition)                                                                                          \
	...
	{                                                                                                                  \
		if (!(Condition))                                                                                              \
		{                                                                                                              \
			::arda::ReportFatalCheck(#Condition, __FILE__, static_cast<std::uint32_t>(__LINE__), __func__);            \
		}                                                                                                              \
	} while (false)
...
#define ARDA_CHECKF(Condition, Format, ...)                                                                            \
	...
	{                                                                                                                  \
		if (!(Condition))                                                                                              \
		{                                                                                                              \
			::arda::ReportFatalCheckf(#Condition,                                                                      \
			    __FILE__,                                                                                              \
...
#define ARDA_VERIFY(Condition)                                                                                         \
	(!!(Condition) ||                                                                                                  \
	    (::arda::ReportFatalCheck(#Condition, __FILE__, static_cast<std::uint32_t>(__LINE__), __func__), false))
...
#define ARDA_ENSURE(Condition)                                                                                         \
	(!!(Condition) || ::arda::ReportEnsureFailure(#Condition, __FILE__, static_cast<std::uint32_t>(__LINE__), __func__))
...
#define ARDA_ENSURE_MSGF(Condition, Format, ...)                                                                       \
	(!!(Condition) ||                                                                                                  \
	    ::arda::ReportEnsureFailuref(#Condition,                                                                       \
	        __FILE__,                                                                                                  \
	        static_cast<std::uint32_t>(__LINE__),                                                                      \
	        __func__,                                                                                                  \
	        Format,                                                                                                    \
...
/**
 * Reports an unconditional programmer error and terminates, including when checks are disabled.
 * @param Format printf-style diagnostic format.
 */
#define ARDA_CHECK_MSG(Format, ...)                                                                                    \
	::arda::ReportFatalCheckf("false", __FILE__, static_cast<std::uint32_t>(__LINE__), __func__, Format, ##__VA_ARGS__)
