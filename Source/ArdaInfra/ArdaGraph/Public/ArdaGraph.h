...
namespace arda
{
	...
	public:
		...
		struct FArdaNode
		{
			...
			explicit FArdaNode(Args&&... Arguments)
		...
		struct FArdaEdge
		{
			...
			FArdaEdge(FArdaGraphNodeHandle From, FArdaGraphNodeHandle To, EdgePayload Payload)
			    : mFrom(From),
			      mTo(To),
		...
		TArdaDirectedGraph()
		    : mIdentity(AllocateArdaGraphIdentity()),
		      mLineage(eastl::make_shared<FArdaLineage>())
		{
		}
		...
		[[nodiscard]] TArdaDirectedGraph CloneSnapshot() const
		{
			...
			for (size_t Index = 0; Index < mNodeSlots.size(); ++Index)
			{
				...
				if (Source.mValue)
				{
					Destination.mValue = eastl::make_unique<FArdaNode>(static_cast<const FArdaNode&>(*Source.mValue));
				}
			}
			for (size_t Index = 0; Index < mEdgeSlots.size(); ++Index)
			{
				...
				if (Source.mValue)
				{
					Destination.mValue = eastl::make_unique<FArdaEdge>(*Source.mValue);
				}
			}
			return Copy;
		}
		...
		[[nodiscard]] FArdaNode* TryGetNode(FArdaGraphNodeHandle Handle) noexcept
		{
			return ContainsNode(Handle) ? mNodeSlots[Handle.GetIndex()].mValue.get() : nullptr;
		}

		/** Looks up a live local node without accepting stale or foreign handles. */
		[[nodiscard]] const FArdaNode* TryGetNode(FArdaGraphNodeHandle Handle) const noexcept
		{
			return ContainsNode(Handle) ? mNodeSlots[Handle.GetIndex()].mValue.get() : nullptr;
		}

		/** Looks up a live local edge without accepting stale or foreign handles. */
		[[nodiscard]] FArdaEdge* TryGetEdge(FArdaGraphEdgeHandle Handle) noexcept
		{
			return ContainsEdge(Handle) ? mEdgeSlots[Handle.GetIndex()].mValue.get() : nullptr;
		}

		/** Looks up a live local edge without accepting stale or foreign handles. */
		[[nodiscard]] const FArdaEdge* TryGetEdge(FArdaGraphEdgeHandle Handle) const noexcept
		{
			return ContainsEdge(Handle) ? mEdgeSlots[Handle.GetIndex()].mValue.get() : nullptr;
		}
		...
		FArdaGraphNodeHandle EmplaceNode(Args&&... Arguments)
		{
			...
			auto Value = eastl::make_unique<FArdaNode>(eastl::forward<Args>(Arguments)...);
		...
		FArdaEdgeInsertion AddEdge(FArdaGraphNodeHandle From, FArdaGraphNodeHandle To, EdgePayload Payload = {})
		{
			...
			auto Value = eastl::make_unique<FArdaEdge>(From, To, eastl::move(Payload));
	...
	private:
		...
		template <typename Record>
		static void ReleaseSlot(TArdaSlot<Record>& Slot, uint32_t Index, eastl::vector<uint32_t>& Free)
		{
			Slot.mValue.reset();
		...
		uint64_t mIdentity;
		eastl::shared_ptr<FArdaLineage> mLineage;
		eastl::vector<TArdaSlot<FArdaNode>> mNodeSlots;
		eastl::vector<TArdaSlot<FArdaEdge>> mEdgeSlots;
