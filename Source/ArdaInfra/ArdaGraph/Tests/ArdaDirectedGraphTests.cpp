...
namespace
{
	...
	using FArdaGraph = TArdaDirectedGraph<int, eastl::string>;
	...
	TEST(ArdaDirectedGraph, RejectsForeignHandlesAndTransfersIdentityOnMove)
	{
		...
		FArdaGraph Moved(eastl::move(Graph));
	...
	{
		TArdaDirectedGraph<int, int, FArdaCollidingHasher> Graph;
