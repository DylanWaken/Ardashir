...
namespace
{
	...
	// Supply executable identity through the same node contract used by normal graph attachment.
	template <EArdaDependencyNodeKind Kind>
	struct TArdaAdaptiveScheduleFixtureNode
	    : TArdaDependencyNode<TArdaAdaptiveScheduleFixtureNode<Kind>, uint32_t, Kind>
	{
		...
		{
			return {Kind == EArdaDependencyNodeKind::Graphics ? "test.AdaptiveSchedule.graphics"
			        : Kind == EArdaDependencyNodeKind::Cuda   ? "test.AdaptiveSchedule.cuda"
			                                                  : "test.AdaptiveSchedule.compute",
			    1};
		}
	...
	FArdaGraphNodeHandle AddNode(FArdaDependencyGraph::FArdaImpl& G,
	    const char* Name,
	    EArdaDependencyNodeKind Kind,
	    uint32_t Cost)
	{
		using FArdaGraphicsNode = TArdaAdaptiveScheduleFixtureNode<EArdaDependencyNodeKind::Graphics>;
		using FArdaComputeNode = TArdaAdaptiveScheduleFixtureNode<EArdaDependencyNodeKind::Compute>;
		using FArdaCudaNode = TArdaAdaptiveScheduleFixtureNode<EArdaDependencyNodeKind::Cuda>;
		EXPECT_TRUE(FArdaGraphicsNode::Register());
		EXPECT_TRUE(FArdaComputeNode::Register());
		EXPECT_TRUE(FArdaCudaNode::Register());
		const auto Definition = FArdaNodeRegistry::Get().Find(Kind == EArdaDependencyNodeKind::Graphics
		        ? FArdaGraphicsNode::GetMetadata().mName
		        : Kind == EArdaDependencyNodeKind::Cuda ? FArdaCudaNode::GetMetadata().mName
		                                                : FArdaComputeNode::GetMetadata().mName);
	...
	void SetMeasuredCost(FArdaDependencyGraph::FArdaImpl& G, FArdaGraphNodeHandle Handle, double Cost)
	{
	...
	{
		FArdaDependencyGraph::FArdaImpl G;
		G.mIdentity = 44;
	...
	TEST(ArdaInductorAdaptiveSchedule, GraphicsStatesPreventAsyncPlacementDespiteEnoughSlack)
	{
		FArdaScheduleFixture Fixture;
		auto& Graph = Fixture.mGraph;
		Graph.mResources.push_back({});
		Graph.mResourceGenerations.push_back(1);
		auto& Desc = Graph.mTopology.TryGetNode(Fixture.mCompute)->mPayload.mDesc;
		Desc.mAccesses.push_back(
		    {{Graph.mIdentity, 0, 1}, EArdaDependencyAccess::Read, EArdaRHIResourceState::ShaderResource});

		// The branch normally qualifies for async placement; combined shader reads normalize to compute reads.
		auto Normal = EvaluateArdaInductorFixedSchedule(Graph, Fixture.mPlan, Graph.mCompile.mExecutionOrder, true);
		ASSERT_TRUE(Normal);
		EXPECT_EQ(Normal.mValue.mQueues.back(), EArdaRHIQueueType::Compute);

		// A declared graphics-only use overrides scheduling profitability.
		for (auto State : {EArdaRHIResourceState::ShadingRateSource,
		         EArdaRHIResourceState::PixelShaderResource,
		         EArdaRHIResourceState::ResolveSource,
		         EArdaRHIResourceState::DepthRead})
		{
			Desc.mAccesses[0].mState = State;
			auto Result = EvaluateArdaInductorFixedSchedule(Graph, Fixture.mPlan, Graph.mCompile.mExecutionOrder, true);
			ASSERT_TRUE(Result);
			EXPECT_EQ(Result.mValue.mQueues.back(), EArdaRHIQueueType::Graphics);
		}
	}
}
