...
namespace
{
	...
	class FArdaQueueGpuDiagnostics final : public IArdaDiagnosticCallback
	{
	public:
	...
	struct FArdaQueueGpuParameters
	{
		FArdaDependencyResourceHandle mSource, mDestination;
		uint32_t mValue = 0;
		uint32_t mCost = 1;
		uint32_t mTraceIndex = 0;
		eastl::shared_ptr<FArdaQueueGpuTrace> mTrace;
	};

	template <EArdaDependencyNodeKind Kind>
	struct TArdaQueueGpuNode : TArdaDependencyNode<TArdaQueueGpuNode<Kind>, FArdaQueueGpuParameters, Kind>
	{
		using FArdaParameters = FArdaQueueGpuParameters;
		...
		{
			return {Kind == EArdaDependencyNodeKind::Compute    ? "test.inductor.queue-compute"
		...
		static eastl::string GetCanonicalKey(const FArdaQueueGpuParameters& P)
		{
			return FArdaDependencyKeyBuilder{}
			    .Resource(P.mSource)
			    .Resource(P.mDestination)
			    .Value(P.mValue)
			    .Value(P.mCost)
			    .Value(P.mTraceIndex)
		...
		static FArdaDependencyNodeDesc Describe(const FArdaQueueGpuParameters& P, const FArdaState&)
		{
			FArdaDependencyNodeDesc Desc;
			Desc.mEstimatedCost = P.mCost;
			Desc.mbSideEffect = true;

			// Transfer reads and domain-specific writes must both participate in scheduling.
			if (P.mSource)
			{
				Desc.mAccesses.push_back({P.mSource, EArdaDependencyAccess::Read, EArdaRHIResourceState::CopySource});
			}
			if (P.mDestination)
			{
		...
		static FArdaRHIStatus Record(FArdaDependencyExecutionContext& Context,
		    const FArdaQueueGpuParameters& P,
		    const FArdaState&,
		    FArdaInstanceState&)
		{
			...
			// Record the actual selected queue, independently from the requested node domain.
			...
			// Resolve compiler-owned resources only when the operation has a destination.
			const auto Destination = Context.GetBuffer(P.mDestination);
			if (!Destination)
			{
				return FArdaRHIStatus::Error(EArdaRHIResult::InvalidState, "Queue test destination is unavailable.");
			}

			// Copy-domain nodes transfer existing data; other domains produce a known pattern.
			if (Kind == EArdaDependencyNodeKind::Copy)
			{
				const auto Source = Context.GetBuffer(P.mSource);
				if (!Source)
				{
					return FArdaRHIStatus::Error(EArdaRHIResult::InvalidState, "Queue test source is unavailable.");
				}
	...
	{
		if (auto S = TArdaQueueGpuNode<EArdaDependencyNodeKind::Compute>::Register(); !S)
		{
			return S;
		}
		if (auto S = TArdaQueueGpuNode<EArdaDependencyNodeKind::Graphics>::Register(); !S)
		{
			return S;
		}
		if (auto S = TArdaQueueGpuNode<EArdaDependencyNodeKind::Copy>::Register(); !S)
		{
			return S;
		}
		return {};
	...
	class FArdaInductorQueueGpu : public testing::TestWithParam<const char*>
	{
	protected:
		void SetUp() override
		{
		...
		FArdaQueueGpuDiagnostics mDiagnostics;
		FArdaRHIDeviceRef mDevice;
	};

	TEST_P(FArdaInductorQueueGpu, AutomaticComputeAndCopyQueuesPreserveResultsAcrossFrames)
	{
		ASSERT_TRUE(RegisterArdaBuiltinNodes());
		ASSERT_TRUE(RegisterQueueGpuNodes());
		const auto Trace = eastl::make_shared<FArdaQueueGpuTrace>();
		...
		for (uint32_t Index = 0; Index < 4; ++Index)
		{
			...
			Name.push_back(char('0' + Index));
			FArdaQueueGpuParameters Parameters;
		...
		FArdaQueueGpuParameters GraphicsParameters;
		...
		FArdaQueueGpuParameters JoinParameters;
		...
		FArdaQueueGpuParameters CopyParameters;
	...
	INSTANTIATE_TEST_SUITE_P(Native, FArdaInductorQueueGpu, testing::Values("native-d3d12", "native-vulkan"));
}
