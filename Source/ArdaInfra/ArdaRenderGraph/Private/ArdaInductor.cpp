...
namespace arda
{
	namespace
	{
		using FArdaHandle = FArdaGraphNodeHandle;
		...
		FArdaRHIStatus ResolveDependencies(FArdaDependencyGraph::FArdaImpl& G)
		{
			...
			for (auto H : G.mTopology.GetNodes())
			{
				...
				eastl::unordered_map<uint32_t, EArdaRHIResourceState> BufferStates;
				for (const auto& A : Node.mDesc.mAccesses)
				{
					...
					else if (!D.mbTexture)
					{
						...
						auto [State, Inserted] = BufferStates.emplace(A.mResource.mIndex, A.mState);
						if (!Inserted && State->second != A.mState)
						{
							if (IsWriteState(State->second) || IsWriteState(A.mState))
							{
								return Error(
								    "One node requests incompatible buffer states. Disjoint ranges still share native buffer state.");
							}
							State->second = State->second | A.mState;
						}
					}
					else
					{
			...
			if (auto Status = ResolveArdaDependencyResourceEdges(G, G.mTopology); !Status)
			{
				return Status;
			}
		...
		eastl::vector<FArdaHandle> LiveNodes(FArdaDependencyGraph::FArdaImpl& G, FArdaInductorCompileResult& Result)
		{
			eastl::unordered_set<uint32_t> Live;
			eastl::vector<FArdaHandle> Stack;
			for (auto H : G.mTopology.GetNodes())
			{
			...
			eastl::vector<FArdaHandle> Nodes;
			for (auto H : G.mTopology.GetNodes())
			{
		...
		FArdaRHIStatus ValidatePipelines(const FArdaDependencyGraph::FArdaImpl& Graph,
		    const eastl::vector<FArdaHandle>& Live)
		{
		...
		double NodeCost(const FArdaDependencyGraph::FArdaImpl& Graph, const FArdaDependencyNode& Node)
		{
		...
		void AssignQueues(const FArdaDependencyGraph::FArdaImpl& G, FArdaInductorCompileResult& R);

		struct FArdaScheduleSearch
		{
			FArdaDependencyGraph::FArdaImpl& mGraph;
			eastl::vector<FArdaHandle> mNodes;
			...
			eastl::vector<FArdaHandle> mBestOrder;
			FArdaMemoryPlan mBestPlan;
			...
			FArdaScheduleSearch(FArdaDependencyGraph::FArdaImpl& Graph,
			    eastl::vector<FArdaHandle> Nodes,
			...
			FArdaRHIStatus Initialize()
			{
				...
				for (uint32_t R = 0; R < mRequests.size(); ++R)
				{
					...
					Q.mIdentifier = R;
					Q.mbUsed = false;
					Q.mKind = D.mExternalAccelerationStructure ? EArdaMemoryKind::AccelerationStructure
					    : D.mbTexture                          ? EArdaMemoryKind::Texture
					                                           : EArdaMemoryKind::Buffer;
				...
				for (size_t I = 0; I < mNodes.size(); ++I)
				{
					...
					{
						FArdaMemoryRequest Scratch;
						Scratch.mIdentifier = static_cast<uint32_t>(mRequests.size());
						Scratch.mbUsed = true;
						Scratch.mKind = EArdaMemoryKind::Buffer;
				...
				if (mGraph.mDevice)
				{
					if (auto S = DescribeArdaMemory(*mGraph.mDevice, mRequests); !S)
					{
						return S;
					}
				}
				else
				{
					...
					for (auto& Q : mRequests)
					{
						if (Q.mbUsed)
						{
							...
							if (Q.mKind == EArdaMemoryKind::Texture)
							{
			...
			void Evaluate(const eastl::vector<size_t>& Order, bool Serialize)
			{
				...
				{
					if (Q[Activation.mResource].mKind == EArdaMemoryKind::Texture)
					{
		...
		void AssignQueues(const FArdaDependencyGraph::FArdaImpl& G, FArdaInductorCompileResult& R)
		{
			...
			const auto AsyncCandidate = [&](FArdaHandle Handle)
			{
				        ...
				        [&](const auto& Access)
				        {
					        return bool(G.mResources[Access.mResource.mIndex].mExternalAccelerationStructure) ||
					            (static_cast<uint32_t>(
					                 NormalizeStateForPipeline(Access.mState, EArdaRHIQueueType::Compute)) &
					                GraphicsOnlyMask) != 0;
				        });
			};
			...
			for (size_t I = 0; I < R.mExecutionOrder.size(); ++I)
			{
				...
				if (N.mDefinition->mKind == EArdaDependencyNodeKind::Copy && CopyAvailable &&
				    eastl::all_of(N.mDesc.mAccesses.begin(),
				        N.mDesc.mAccesses.end(),
				        [](const auto& Access)
				        {
					        return (static_cast<uint32_t>(Access.mState) & ~CopyMask) == 0;
				        }))
				{
				...
				eastl::vector<FArdaHandle> Chain{H};
			...
			eastl::vector<FArdaHandle> Batch;
	...
	double GetArdaInductorScheduleNodeCost(const FArdaDependencyGraph::FArdaImpl& Snapshot,
	    const FArdaDependencyNode& Node)
	{
		return NodeCost(Snapshot, Node);
	}

	TArdaRHIResult<FArdaInductorCompileResult> EvaluateArdaInductorFixedSchedule(
	    const FArdaDependencyGraph::FArdaImpl& G,
	    const FArdaMemoryPlan& FixedPlan,
	    ...
	    bool AssignAutomaticQueues)
	{
		...
		for (const auto& Activation : FixedPlan.mAliasActivations)
		{
			...
			if (FixedPlan.mRequests[Activation.mResource].mKind == EArdaMemoryKind::Texture)
			{
	...
	FArdaRHIStatus FArdaInductor::Compile(FArdaDependencyGraph& Graph)
	{
		...
		FArdaScheduleSearch Search(G, eastl::move(Nodes), Result);
