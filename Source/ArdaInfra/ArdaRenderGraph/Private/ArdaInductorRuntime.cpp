...
namespace arda
{
	    ...
	    uint32_t PassIndex)
	{
		...
		FArdaPending Pending{Promise->get_future(), eastl::move(Destination), PassIndex};
		{
	...
	{
		eastl::vector<FArdaPending> Pending;
		{
			std::lock_guard<std::mutex> Lock(mMutex);
	...
	{
		const auto* N = mGraph ? mGraph->mTopology.TryGetNode(mNode) : nullptr;
		if (!N || !GetPipeline(Slot))
		{
	...
	FArdaRHIStatus FArdaDependencyExecutionContext::SetRayTracingState(const eastl::string& Slot) const
	{
		if (!GetShaderTable(Slot))
		{
			return RuntimeError("Missing compiled ray shader table.");
		}
		FArdaRHIRayTracingState State;
		State.mShaderTable = GetShaderTable(Slot);
		State.mBindings = GetBindings(Slot);
		if (auto S = GetCommands().SetRayTracingState(State); !S)
		{
			return S;
		}
		return ApplyShaderParameters(Slot);
	}

	FArdaRHIStatus FArdaDependencyExecutionContext::DispatchWorkGraph(const void* Records,
	    uint32_t RecordCount,
	    uint32_t RecordStride,
	    const eastl::string& Slot) const
	{
		const auto* P = GetPipeline(Slot);
		if (!P || !P->mWorkGraph)
		{
			return RuntimeError("Missing compiled work-graph pipeline slot.");
		}
		if (auto S = ApplyShaderParameters(Slot); !S)
		{
			return S;
		}
		return GetCommands().DispatchWorkGraph(*P->mWorkGraph, Records, RecordCount, RecordStride, GetBindings(Slot));
	}

	FArdaRHIStatus FArdaDependencyExecutionContext::SetComputeState(const eastl::string& Slot) const
	{
		const auto* P = GetPipeline(Slot);
		if (!P || !P->mCompute)
		{
			return RuntimeError("Missing compiled compute pipeline slot.");
		}
		FArdaRHIComputeState State;
		State.mPipeline = P->mCompute;
		State.mBindings = GetBindings(Slot);
		if (auto S = GetCommands().SetComputeState(State); !S)
		{
			return S;
		}
		return ApplyShaderParameters(Slot);
	...
	FArdaRHIStatus FArdaDependencyExecutionContext::SetGraphicsState(FArdaRHIGraphicsState State,
	    const eastl::string& Slot) const
	{
		const auto* P = GetPipeline(Slot);
		if (!P || !P->mGraphics)
		{
			return RuntimeError("Missing compiled graphics pipeline slot.");
		}
		State.mPipeline = P->mGraphics;
		State.mFramebuffer = GetFramebuffer();
		State.mBindings = GetBindings(Slot);
		if (auto S = GetCommands().SetGraphicsState(State); !S)
		{
			return S;
		}
		return ApplyShaderParameters(Slot);
	...
	FArdaRHIStatus FArdaDependencyExecutionContext::SetMeshletState(FArdaRHIMeshletState State,
	    const eastl::string& Slot) const
	{
		const auto* P = GetPipeline(Slot);
		if (!P || !P->mMeshlet)
		{
			return RuntimeError("Missing compiled meshlet pipeline slot.");
		}
		State.mPipeline = P->mMeshlet;
		State.mFramebuffer = GetFramebuffer();
		State.mBindings = GetBindings(Slot);
		if (auto S = GetCommands().SetMeshletState(State); !S)
		{
			return S;
		}
		return ApplyShaderParameters(Slot);
	...
	FArdaRHIStatus FArdaInductorRuntime::Prepare(FArdaDependencyGraph::FArdaImpl& Graph,
	    ...
	    const eastl::vector<FArdaInductorReuseFrame>* Reuse)
	{
		...
		{
			auto Storage = Reuse ? TArdaRHIResult<FArdaMemoryResources>{(*Reuse)[FrameIndex].mMemory, {}}
			                     : MaterializeArdaMemory(*Graph.mDevice,
			                           Plan,
			...
			if (!Reuse)
			{
				if (auto S = PrepareArdaShaderBindings(Graph, *Frame); !S)
				{
					return S;
				}
			}
			...
			{
				if (Plan.mRequests[Activation.mResource].mKind == EArdaMemoryKind::Texture)
				{
		...
		if (Graph.mOptions.mbEnableGpuTiming)
		{
			Runtime->mAdaptivePlan = eastl::make_shared<const FArdaMemoryPlan>(Plan);
	...
	FArdaGraphExecutionResult FArdaInductorRuntime::WaitState(FArdaDependencyFrameTicket::FArdaState& State)
	{
		std::lock_guard<std::mutex> Lock(State.mMutex);
		if (State.mbComplete)
		{
	...
	FArdaRHIStatus FArdaInductorRuntime::WaitAll()
	{
		...
		eastl::vector<eastl::shared_ptr<FArdaDependencyFrameTicket::FArdaState>> Pending;
	...
	TArdaRHIResult<FArdaDependencyFrameTicket> FArdaInductorRuntime::Submit(FArdaDependencyGraph::FArdaImpl& Graph,
	    const FArdaGraphExecuteOptions& Options)
	{
		...
		struct FArdaExecutionGuard
		{
			bool& mExecuting;

			explicit FArdaExecutionGuard(bool& Executing)
		...
		auto State = eastl::make_shared<FArdaDependencyFrameTicket::FArdaState>();
	...
	FArdaGraphExecutionResult FArdaInductorRuntime::Wait(FArdaDependencyGraph::FArdaImpl& Graph,
	...
	TArdaRHIResult<bool> FArdaInductorRuntime::IsComplete(FArdaDependencyGraph::FArdaImpl& Graph,
	    const FArdaDependencyFrameTicket& Ticket)
	{
		if (Graph.mbExecuting || !Ticket.mState || Ticket.mState->mGraphIdentity != Graph.mIdentity)
		{
	...
	FArdaGraphExecutionResult FArdaInductorRuntime::Execute(FArdaDependencyGraph::FArdaImpl& Graph,
	    const FArdaGraphExecuteOptions& Options)
	{
