...
{
	uint State = Value.x * 0x9e3779b9u ^ Value.y * 0x85ebca6bu ^ Value.z * 0xc2b2ae35u ^ PathAndSeed.y;
	State ^= State >> 16;
	State *= 0x7feb352du;
	State ^= State >> 15;
	State *= 0x846ca68bu;
...
float FresnelDielectric(float CosTheta, float EtaI, float EtaT)
{
	...
	const float Rs = (EtaI * CosTheta - EtaT * CosThetaT) / (EtaI * CosTheta + EtaT * CosThetaT);
	const float Rp = (EtaT * CosTheta - EtaI * CosThetaT) / (EtaT * CosTheta + EtaI * CosThetaT);
...
void EvaluateBsdf(CornellMaterial Material, float3 Normal, float3 View, float3 Light, out float3 F, out float Pdf)
{
	...
	if (Material.Metallic > 0.0)
	{
		...
		F += Material.Metallic * D * G * Fresnel / max(4.0 * NdotV * NdotL, 1.0e-6);
    ...
    out bool Delta)
{
	...
	if (Material.Transmission > 0.5)
	{
		...
		const float Fresnel = FresnelDielectric(saturate(dot(View, Normal)), EtaI, EtaT);
	...
	if (Choose < Material.Metallic)
	{
		...
		const float CosTheta = sqrt((1.0 - U2) / max(1.0 + (A2 - 1.0) * U2, 1.0e-6));
		const float SinTheta = sqrt(max(0.0, 1.0 - CosTheta * CosTheta));
		const float3 HalfVector = ToWorld(float3(cos(Phi) * SinTheta, sin(Phi) * SinTheta, CosTheta), Normal);
	...
	else
	{
		...
		Direction = ToWorld(float3(Radius * cos(Phi), Radius * sin(Phi), sqrt(1.0 - U1)), Normal);
	}
	...
	for (uint Bounce = 0; Bounce < PathAndSeed.x; ++Bounce)
	{
		...
		if (Material.Transmission <= 0.5)
		{
			...
			const float3 LightPosition =
			    float3(lerp(-0.35, 0.35, LightSample.x), lerp(0.75, 1.35, LightSample.y), 1.99);
			...
			if (NdotL > 0.0 && CosLight > 0.0 && IsVisible(Position + Normal * kRayEpsilon, LightDirection, Distance))
			{
				float3 F;
				float BsdfPdf;
				EvaluateBsdf(Material, Normal, -Direction, LightDirection, F, BsdfPdf);
				const float LightPdf = DistanceSquared / max(CameraUpAndLightArea.w * CosLight, 1.0e-6);
				const float Mis = PowerHeuristic(LightPdf, BsdfPdf);
				Radiance += Throughput * Materials[6].Emission * F * NdotL * Mis / max(LightPdf, 1.0e-6);
			}
		}
		...
		if (Bounce >= 3)
		{
			const float ContinueProbability = clamp(max(Throughput.x, max(Throughput.y, Throughput.z)), 0.05, 0.95);
...
void CornellRayGen()
{
	...
	if (any(Pixel >= ImageAndSampling.xy) || LaunchIndex.z >= ImageAndSampling.w)
	{
		return;
	}

	const uint GlobalSample = ImageAndSampling.z + LaunchIndex.z;
	uint Rng = HashSeed(uint3(Pixel, GlobalSample + PathAndSeed.z * 65537u));
	const float2 Jitter = float2(RandomFloat(Rng), RandomFloat(Rng));
	const float2 Uv = (float2(Pixel) + Jitter) / float2(ImageAndSampling.xy);
	const float2 Ndc = float2(Uv.x * 2.0 - 1.0, 1.0 - Uv.y * 2.0);
	const float3 RayDirection = normalize(CameraForwardAndTanHalfFovY.xyz +
	    CameraRightAndExposure.xyz * (Ndc.x * CameraPositionAndTanHalfFovX.w) +
	    CameraUpAndLightArea.xyz * (Ndc.y * CameraForwardAndTanHalfFovY.w));
	const float3 Radiance = TracePath(CameraPositionAndTanHalfFovX.xyz, RayDirection, Rng);
	const uint PixelIndex = Pixel.y * ImageAndSampling.x + Pixel.x;
	const uint SampleStride = ImageAndSampling.x * ImageAndSampling.y;
	SampleRadiance[LaunchIndex.z * SampleStride + PixelIndex] = float4(Radiance, 1.0);
}
