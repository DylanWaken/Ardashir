...
void CornellAccumulateCS(uint3 DispatchThreadId: SV_DispatchThreadID)
{
	...
	{
		BatchRadiance += SampleRadiance[Sample * SampleStride + PixelIndex].rgb;
	}

	const float PreviousSampleCount = float(ImageAndSampling.z);
	const float TotalSampleCount = PreviousSampleCount + float(ImageAndSampling.w);
	const float3 Previous = ImageAndSampling.z == 0 ? 0.0 : Accumulation[Pixel].rgb;
	const float3 Average = (Previous * PreviousSampleCount + BatchRadiance) / max(TotalSampleCount, 1.0);
