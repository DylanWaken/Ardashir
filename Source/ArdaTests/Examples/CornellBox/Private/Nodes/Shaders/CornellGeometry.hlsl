...
static const uint kSphereTrianglesPerFace = kSphereFaceResolution * kSphereFaceResolution * 2;
...
void GetBoxQuad(uint Box, uint Face, out float3 P0, out float3 P1, out float3 P2, out float3 P3, out float3 Normal)
{
	const float3 Center = Box == 0 ? float3(-0.38, 1.42, 0.34) : float3(0.38, 1.23, 0.58);
	const float3 HalfSize = Box == 0 ? float3(0.29, 0.36, 0.34) : float3(0.26, 0.31, 0.58);
...
float3 CubeToSphere(float3 Cube)
{
	...
	float3 Sphere;
	Sphere.x = Cube.x * sqrt(max(0.0, 1.0 - 0.5 * Square.y - 0.5 * Square.z + Square.y * Square.z / 3.0));
	Sphere.y = Cube.y * sqrt(max(0.0, 1.0 - 0.5 * Square.z - 0.5 * Square.x + Square.z * Square.x / 3.0));
	Sphere.z = Cube.z * sqrt(max(0.0, 1.0 - 0.5 * Square.x - 0.5 * Square.y + Square.x * Square.y / 3.0));
	return normalize(Sphere);
}

float3 SphereDirection(uint Face, uint GridX, uint GridY)
{
	const float2 Grid = float2(GridX, GridY) / float(kSphereFaceResolution);
	const float2 FaceUv = Grid * 2.0 - 1.0;
	return CubeToSphere(CubeFacePosition(Face, FaceUv.x, FaceUv.y));
}

void GetSphereTriangle(uint Sphere,
    uint Triangle,
    out float3 P0,
    out float3 P1,
    out float3 P2,
    ...
    out uint MaterialId)
{
	const float3 Center = Sphere == 0 ? float3(-0.48, 0.58, 0.30) : float3(0.49, 0.56, 0.25);
...
void GenerateCornellGeometryCS(uint3 DispatchThreadId: SV_DispatchThreadID)
{
	...
	if (Triangle < 7)
	{
		if (Triangle == 0)
		{
			Materials[0] = MakeMaterial(float3(0.78, 0.78, 0.72), 0.72, 0, 0, 0, 1.5);
		}
		if (Triangle == 1)
		{
			Materials[1] = MakeMaterial(float3(0.72, 0.055, 0.035), 0.78, 0, 0, 0, 1.5);
		}
		if (Triangle == 2)
		{
			Materials[2] = MakeMaterial(float3(0.055, 0.56, 0.095), 0.76, 0, 0, 0, 1.5);
		}
		if (Triangle == 3)
		{
			Materials[3] = MakeMaterial(float3(0.68, 0.64, 0.55), 0.48, 0, 0, 0, 1.5);
		}
		if (Triangle == 4)
		{
			Materials[4] = MakeMaterial(float3(0.92, 0.64, 0.22), 0.16, 0, 0.94, 0, 1.5);
		}
		if (Triangle == 5)
		{
			Materials[5] = MakeMaterial(float3(0.96, 0.985, 1.0), 0.02, 0, 0, 1, 1.5);
		}
		if (Triangle == 6)
		{
			Materials[6] = MakeMaterial(float3(1, 1, 1), 0.0, float3(18, 16, 12), 0, 0, 1.0);
		}
	}
	...
	if (Triangle < QuadTriangleCount)
	{
		...
		{
			GetBoxQuad((Quad - 6) / 6, (Quad - 6) % 6, Q0, Q1, Q2, Q3, Normal);
			MaterialId = 3;
		}
		if (Half == 0)
		{
	...
	else
	{
		...
		GetSphereTriangle(Sphere, SphereTriangle % kSphereTriangleCount, P0, P1, P2, N0, N1, N2, MaterialId);
	}
