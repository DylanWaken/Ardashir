...
namespace arda
{
	...
	bool FArdaCornellBoxWindow::Create(const char* Title,
	    uint32_t Width,
	    uint32_t Height,
	    bool bVisible,
	    bool bFullscreen)
	{
		glfwSetErrorCallback(
		    ...
		    {
			    ARDA_LOG(LogCornellBox, Error, "%s", Description ? Description : "");
		    });
		if (!glfwInit())
		{
	...
	bool FArdaCornellBoxWindow::PumpMessages()
	{
		...
		{
			const bool bShiftDown = glfwGetKey(mWindow, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS ||
	...
	FArdaCameraInput FArdaCornellBoxWindow::ConsumeCameraInput()
	{
		...
		Input.mForward = (glfwGetKey(mWindow, GLFW_KEY_W) == GLFW_PRESS ? 1.0f : 0.0f) -
		    (glfwGetKey(mWindow, GLFW_KEY_S) == GLFW_PRESS ? 1.0f : 0.0f);
		Input.mRight = (glfwGetKey(mWindow, GLFW_KEY_D) == GLFW_PRESS ? 1.0f : 0.0f) -
	...
	arda::FArdaNativeObject FArdaCornellBoxWindow::GetD3D12WindowHandle() const noexcept
	{
#if defined(_WIN32)
	...
	eastl::vector<const char*> FArdaCornellBoxWindow::GetVulkanInstanceExtensions() const
	{
#if defined(ARDA_TEST_WITH_VULKAN)
		uint32_t Count = 0;
		const char** Extensions = glfwGetRequiredInstanceExtensions(&Count);
		return Extensions && Count ? eastl::vector<const char*>(Extensions, Extensions + Count)
	...
	arda::FArdaNativeObject FArdaCornellBoxWindow::CreateVulkanSurface(arda::FArdaNativeObject VulkanInstance,
	...
	void FArdaCornellBoxWindow::FramebufferSizeCallback(GLFWwindow* Window, int Width, int Height)
	{
		auto* Self = static_cast<FArdaCornellBoxWindow*>(glfwGetWindowUserPointer(Window));
	...
	void FArdaCornellBoxWindow::CursorPositionCallback(GLFWwindow* Window, double X, double Y)
	{
		auto* Self = static_cast<FArdaCornellBoxWindow*>(glfwGetWindowUserPointer(Window));
		if (!Self)
		{
	...
	void FArdaCornellBoxWindow::MouseButtonCallback(GLFWwindow* Window, int Button, int Action, int)
	{
		auto* Self = static_cast<FArdaCornellBoxWindow*>(glfwGetWindowUserPointer(Window));
		if (Self && Self->mbCanCaptureCursor && !Self->mbCursorCaptured && Button == GLFW_MOUSE_BUTTON_LEFT &&
		    Action == GLFW_PRESS)
		{
	...
	void FArdaCornellBoxWindow::WindowFocusCallback(GLFWwindow* Window, int Focused)
	{
		auto* Self = static_cast<FArdaCornellBoxWindow*>(glfwGetWindowUserPointer(Window));
