...
float ValueNoise(float3 value)
{
	...
	const float3 blend = fraction * fraction * (3.0 - 2.0 * fraction);
	...
	const float lower = lerp(lerp(c000, c100, blend.x), lerp(c010, c110, blend.x), blend.y);
	const float upper = lerp(lerp(c001, c101, blend.x), lerp(c011, c111, blend.x), blend.y);
...
void GenerateNoiseHeightmapCS(uint3 dispatchThreadId: SV_DispatchThreadID)
{
	...
	const float2 uv = float2(dispatchThreadId.xy) / float2(settings.mWidth - 1, settings.mHeight - 1);
	float noise = 0.0;
	float weight = 0.55;
	...
	{
		const float timeCoordinate = settings.mTime * 0.22 + float(octave) * 19.0;
	...
	GenerateHeightmap[dispatchThreadId.xy] = boundaryHeight + signedNoise * settings.mAmplitude * 0.55;
}

Texture2D<float> ErosionSource : register(t0);
...
void ErodeHeightmapCS(uint3 dispatchThreadId: SV_DispatchThreadID)
{
	...
	const float original = ErosionSource.Load(int3(dispatchThreadId.xy, 0));
	const float sediment = smoothstep(0.48, 0.9, original) * 0.065;
	const float erodedHeight = original - sediment;

	const float2 uv = float2(dispatchThreadId.xy) / float2(width - 1, height - 1);
	...
	const float boundaryHeight = 0.42;
	ErosionHeightmap[dispatchThreadId.xy] = lerp(boundaryHeight, erodedHeight, boundaryBlend);
}
...
void TriangulateTerrainCS(uint3 dispatchThreadId: SV_DispatchThreadID)
{
	...
	const uint2 coordinates[4] = { dispatchThreadId.xy,
...
float SampleHeightmap(float2 uv)
{
	...
	const float2 grid = saturate(uv) * float2(width - 1, height - 1);
	...
	const float lowerHeight =
	    lerp(RenderHeightmap.Load(int3(lower, 0)), RenderHeightmap.Load(int3(uint2(upper.x, lower.y), 0)), blend.x);
	const float upperHeight =
	    lerp(RenderHeightmap.Load(int3(uint2(lower.x, upper.y), 0)), RenderHeightmap.Load(int3(upper, 0)), blend.x);
...
float ContourMask(float height, float interval, float thickness)
{
	...
	const float distanceToLine = abs(frac(coordinate + 0.5) - 0.5);
    ...
    : SV_Target
{
	...
	const float boundaryFade = lerp(0.3, 1.0, smoothstep(0.0, 0.055, distanceFromBoundary));
	const float elevation = saturate(distanceFromBoundary / 0.28);

	const float3 aboveColor = lerp(float3(0.72, 0.72, 0.72), float3(1.0, 1.0, 1.0), elevation);
	const float3 belowColor = lerp(float3(0.72, 0.015, 0.01), float3(1.0, 0.025, 0.015), elevation);
	const float aboveBlend = smoothstep(-0.055, 0.055, relativeHeight);
	const float3 contourColor = lerp(belowColor, aboveColor, aboveBlend);

	const float3 terrainColor = contourColor * (0.012 + elevation * 0.018) * boundaryFade;
	const float contour = max(minorContour * 0.72, majorContour);
	float3 color = lerp(terrainColor, contourColor, contour * boundaryFade);
    ...
    : SV_Target
{
	...
	const float3 source = OverlaySource.Load(int3(uint2(input.mPosition.xy), 0)).rgb;
	const float3 overlay = float3(0.01, 0.018 + pulse * 0.006, 0.015);
	return float4(lerp(source, overlay, 0.08), 0.08);
}
