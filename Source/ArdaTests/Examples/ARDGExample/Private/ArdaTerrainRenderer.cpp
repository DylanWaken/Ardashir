...
namespace arda
{
	namespace
	{
		...
		{
			if (VertexBytes.size() != static_cast<size_t>(TerrainVertexCount) * sizeof(FTerrainVertex) ||
			    IndexBytes.size() != static_cast<size_t>(TerrainIndexCount) * sizeof(uint32_t))
			{
				char Message[256]{};
			...
			const auto* Vertices = reinterpret_cast<const FTerrainVertex*>(VertexBytes.data());
			...
			for (uint32_t Cell = 0; Cell < CellCount; ++Cell)
			{
				...
				for (uint32_t Corner = 0; Corner < 4; ++Corner)
				{
					...
					const float ExpectedX =
					    ((static_cast<float>(CellY + CornerY[Corner]) / static_cast<float>(HeightmapHeight - 1)) -
					        0.5f) *
					    1.45f;
					const float ExpectedY =
					    ((static_cast<float>(CellX + CornerX[Corner]) / static_cast<float>(HeightmapWidth - 1)) -
					        0.5f) *
					    1.45f;
				...
				{
					const FTerrainVertex* Right = Vertices + VertexBase + 4;
				...
				{
					const FTerrainVertex* Below = Vertices + VertexBase + (HeightmapWidth - 1) * 4;
	...
	ARDA_IMPLEMENT_GLOBAL_SHADER(FGenerateTerrainShader,
	    "/ArdaTests/ARDGExample/ArdaTerrain.hlsl",
	    "TerrainGenerateCS",
	    "GenerateNoiseHeightmapCS",
	    arda::EArdaRHIShaderStage::Compute)
	ARDA_IMPLEMENT_GLOBAL_SHADER(FErodeTerrainShader,
	    "/ArdaTests/ARDGExample/ArdaTerrain.hlsl",
	    "TerrainErodeCS",
	    "ErodeHeightmapCS",
	    arda::EArdaRHIShaderStage::Compute)
	ARDA_IMPLEMENT_GLOBAL_SHADER(FTriangulateTerrainShader,
	    "/ArdaTests/ARDGExample/ArdaTerrain.hlsl",
	    "TerrainTriangulateCS",
	    "TriangulateTerrainCS",
	    arda::EArdaRHIShaderStage::Compute)
	ARDA_IMPLEMENT_GLOBAL_SHADER(FTerrainVertexShader,
	    "/ArdaTests/ARDGExample/ArdaTerrain.hlsl",
	    "TerrainVS",
	    "TerrainVS",
	    arda::EArdaRHIShaderStage::Vertex)
	ARDA_IMPLEMENT_GLOBAL_SHADER(FTerrainPixelShader,
	    "/ArdaTests/ARDGExample/ArdaTerrain.hlsl",
	    "TerrainPS",
	    "TerrainPS",
	    arda::EArdaRHIShaderStage::Pixel)
	ARDA_IMPLEMENT_GLOBAL_SHADER_WITHOUT_PARAMETERS(FTerrainOverlayVertexShader,
	    "/ArdaTests/ARDGExample/ArdaTerrain.hlsl",
	    "TerrainOverlayVS",
	    "TerrainOverlayVS",
	    arda::EArdaRHIShaderStage::Vertex)
	ARDA_IMPLEMENT_GLOBAL_SHADER_WITHOUT_PARAMETERS(FTerrainOverlayPixelShader,
	    "/ArdaTests/ARDGExample/ArdaTerrain.hlsl",
	    "TerrainOverlayPS",
	    "TerrainOverlayPS",
	    arda::EArdaRHIShaderStage::Pixel)
	...
	bool FArdaTerrainRenderer::Initialize(arda::FArdaRHIDeviceRef device, arda::EArdaRHIFormat)
	{
		...
		if (!CreateShadersAndInitializers() || !CreateSettingsUploadBuffer() || !CreateCameraResources())
		{
			return false;
		}
		mError.clear();
	...
	bool FArdaTerrainRenderer::CreateShadersAndInitializers()
	{
		...
		if (mGenerateShader == nullptr || mErodeShader == nullptr || mTriangulateShader == nullptr ||
		    mTerrainVertexShader == nullptr || mTerrainPixelShader == nullptr || mOverlayVertexShader == nullptr ||
		...
		mGeneratePipelineInitializer =
		    arda::FArdaComputePipelineStateInitializer::FromGlobalShader(*mGenerateShader, "Generate pipeline");
		mErodePipelineInitializer =
		    arda::FArdaComputePipelineStateInitializer::FromGlobalShader(*mErodeShader, "Erode pipeline");
		mTriangulatePipelineInitializer =
		    arda::FArdaComputePipelineStateInitializer::FromGlobalShader(*mTriangulateShader, "Triangulate pipeline");
		...
		overlayFixedState.mBlendState.mTargets[0].mSourceColor = arda::EArdaRHIBlendFactor::SourceAlpha;
		overlayFixedState.mBlendState.mTargets[0].mDestinationColor = arda::EArdaRHIBlendFactor::InverseSourceAlpha;
	...
	bool FArdaTerrainRenderer::CreateCameraResources()
	{
		...
		if (mTerrainVertexShader == nullptr || mTerrainVertexShader->GetBindingLayouts().empty())
		{
		...
		const auto status = FTerrainVertexShader::FParameters::GetStaticMetadata().CreateBindingSet(*mDevice,
		    &parameters,
	...
	bool FArdaTerrainRenderer::RenderFrame(arda::IArdaSwapChain& swapChain)
	{
		...
		auto* backBuffer =
		    graph.RegisterExternalTexture(colorAttachment.mTexture, arda::EArdaRHIResourceState::Present, "BackBuffer");
		auto* settingsUpload = graph.RegisterExternalBuffer(mSettingsUploadBuffer,
		    arda::EArdaRHIResourceState::CopySource,
		    "TerrainSettingsUpload");
		auto* cameraBuffer =
		    graph.RegisterExternalBuffer(mCameraBuffer, arda::EArdaRHIResourceState::ConstantBuffer, "TerrainCamera");
		...
		heightmapDesc.mUsage = arda::EArdaRHITextureUsage::ShaderResource | arda::EArdaRHITextureUsage::UnorderedAccess;
		...
		vertexDesc.mUsage = arda::EArdaRHIBufferUsage::Structured | arda::EArdaRHIBufferUsage::UnorderedAccess |
		...
		indexDesc.mUsage = arda::EArdaRHIBufferUsage::Structured | arda::EArdaRHIBufferUsage::UnorderedAccess |
		...
		updateSettings.mDestination = {settingsUpload, arda::EArdaRHIResourceState::CopyDest, {}};
		(void)graph.AddPass("UpdateTerrainSettings",
		    &updateSettings,
		    arda::EARDGPassFlags::None,
		    [settingsData](arda::FARDGPassExecutionContext& context, const FInitializeTerrainSettingsParameters& frozen)
		    {
			    (void)context.mCommandList.WriteBuffer(*context.GetBuffer(frozen.mDestination.mBuffer),
			        &settingsData,
			        sizeof(settingsData));
		    });
		...
		(void)graph.AddPass("UploadTerrainSettings",
		    &upload,
		    arda::EARDGPassFlags::Copy,
		    [](arda::FARDGPassExecutionContext& context, const FUploadTerrainSettingsParameters& frozen)
		    {
			    (void)context.mCommandList.CopyBuffer(*context.GetBuffer(frozen.mDestination.mBuffer),
			        0,
		...
		(void)graph.AddDispatchPass(
		    ...
		    [this, executionErrors](arda::FARDGPassExecutionContext& context)
		    {
			    ...
			    const auto status = mPipelineStateCache->SetComputePipelineState(context.mCommandList,
			        mGeneratePipelineInitializer,
			        eastl::move(state));
			    if (!status)
			    {
				    executionErrors->Record("GenerateNoiseHeightmap PSO bind failed", status.mMessage);
				    return;
			    }
		    },
		...
		(void)graph.AddPass("DebugHeightmap",
		    &debugHeightmap,
		    arda::EARDGPassFlags::None,
		...
		(void)graph.AddDispatchPass(
		    ...
		    [this, executionErrors](arda::FARDGPassExecutionContext& context)
		    {
			    ...
			    if (!status)
			    {
				    executionErrors->Record("ErodeHeightmap PSO bind failed", status.mMessage);
				    return;
			    }
		    },
		    arda::EARDGPassFlags::AsyncCompute);
		...
		(void)graph.AddDispatchPass(
		    ...
		    [this, executionErrors](arda::FARDGPassExecutionContext& context)
		    {
			    ...
			    if (!status)
			    {
				    executionErrors->Record("TriangulateTerrain PSO bind failed", status.mMessage);
				    return;
			    }
		    },
		    arda::EARDGPassFlags::AsyncCompute);
		...
		{
			(void)graph.AddDeviceToHostCopyPass(terrainVertices,
			    terrainVertexReadback,
			    0,
			    arda::ArdaRHIWholeBuffer,
			    "ReadbackTerrainVertices");
			(void)graph.AddDeviceToHostCopyPass(terrainIndices,
			    terrainIndexReadback,
			    0,
			    arda::ArdaRHIWholeBuffer,
		...
		const FCameraSettings cameraSettings = {{right[0],
		                                            up[0],
		                                            forward[0],
		                                            0.0f,
		                                            right[1],
		...
		updateCamera.mDestination = {cameraBuffer, arda::EArdaRHIResourceState::CopyDest, {}};
		(void)graph.AddPass("UpdateTerrainCamera",
		    &updateCamera,
		    arda::EARDGPassFlags::None,
		    [cameraSettings](arda::FARDGPassExecutionContext& context, const FUpdateTerrainCameraParameters& frozen)
		    {
			    (void)context.mCommandList.WriteBuffer(*context.GetBuffer(frozen.mDestination.mBuffer),
			        &cameraSettings,
		...
		(void)graph.AddPass("RenderTerrain",
		    &render,
		    arda::EARDGPassFlags::Raster,
		    [this, width, height, executionErrors](arda::FARDGPassExecutionContext& context,
		        const FRenderTerrainParameters& frozen)
		    {
			    ...
			    framebufferDesc.mColorAttachments.push_back(
			        {arda::FArdaRHITextureRef(color), {frozen.mTargets.mColor[0].mSubresources}});
			    framebufferDesc.mDepthAttachment = {arda::FArdaRHITextureRef(depth),
			    ...
			    {
				    executionErrors->Record("RenderTerrain framebuffer creation failed",
				        terrainFramebuffer.mStatus.mMessage);
				    return;
			    }
			    (void)context.mCommandList.ClearTexture(*color,
			        frozen.mTargets.mColor[0].mSubresources,
			        {0.004f, 0.007f, 0.009f, 1.0f});
			    (void)context.mCommandList.ClearDepthStencilTexture(*depth,
			        frozen.mTargets.mDepthStencil.mSubresources,
			        true,
			        0.0f,
			        false,
			    ...
			    state.mBindings = {mCameraBindingSet, context.CreateBindingSet(*mTerrainPixelShader)};
			    ...
			    const auto status = mPipelineStateCache->SetGraphicsPipelineState(context.mCommandList,
			        mTerrainPipelineInitializer,
			        eastl::move(state));
			    if (!status)
			    {
				    executionErrors->Record("RenderTerrain PSO bind failed", status.mMessage);
				    return;
			    }
		...
		overlay.mTargets.mColor[0] = {backBuffer, colorAttachment.mAttachment.mSubresources};
		(void)graph.AddPass("TerrainOverlay",
		    &overlay,
		    arda::EARDGPassFlags::Raster,
		    [this, framebuffer, width, height, executionErrors](arda::FARDGPassExecutionContext& context,
		        const FTerrainOverlayParameters& frozen)
		    {
			    ...
			    if (!status)
			    {
				    executionErrors->Record("TerrainOverlay PSO bind failed", status.mMessage);
				    return;
			    }
		...
		if (!mbTerrainReadbackValidated)
		{
			mError = ValidateTerrainReadback(terrainVertexReadback, terrainIndexReadback);
			if (!mError.empty())
			{
				return false;
			}
