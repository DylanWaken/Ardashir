...
namespace arda
{
	...
	bool FArdaGlfwWindow::Create(const char* title, uint32_t width, uint32_t height, bool visible, bool fullscreen)
	{
		glfwSetErrorCallback(
		    [](int, const char* description)
		    {
			    ARDA_LOG(LogARDGExample, Error, "%s", description ? description : "");
		    });
	...
	bool FArdaGlfwWindow::PumpMessages()
	{
		...
		{
			const bool shiftDown = glfwGetKey(mWindow, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS ||
	...
	arda::FArdaNativeObject FArdaGlfwWindow::CreateVulkanSurface(arda::FArdaNativeObject vulkanInstance,
	    eastl::string& outError)
	{
		...
		if (result != VK_SUCCESS)
		{
			...
			char resultText[32];
			std::snprintf(resultText, sizeof(resultText), "%d", static_cast<int>(result));
	...
	void FArdaGlfwWindow::CursorPositionCallback(GLFWwindow* window, double x, double y)
	{
		auto* self = static_cast<FArdaGlfwWindow*>(glfwGetWindowUserPointer(window));
		if (!self)
		{
	...
	void FArdaGlfwWindow::MouseButtonCallback(GLFWwindow* window, int button, int action, int)
	{
		auto* self = static_cast<FArdaGlfwWindow*>(glfwGetWindowUserPointer(window));
