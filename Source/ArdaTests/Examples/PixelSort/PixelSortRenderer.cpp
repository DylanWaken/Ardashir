...
namespace arda
{
	...
	FPixelSortRenderer::~FPixelSortRenderer()
	{
		if (mDevice)
		{
	...
	void FPixelSortRenderer::Initialize(EArdaRHIFormat Format, const std::filesystem::path& Directory)
	{
		...
		FArdaShaderTypeRegistration Noise("PixelSortNoise",
		    Source.c_str(),
		    "PixelSortNoise",
		    "NoiseCS",
		    EArdaRHIShaderStage::Compute,
		    nullptr);
		FArdaShaderTypeRegistration VS("PixelSortVertex",
		    Source.c_str(),
		    "PixelSortVertex",
		    "PresentVS",
		    EArdaRHIShaderStage::Vertex,
		    nullptr);
		FArdaShaderTypeRegistration PS("PixelSortPixel",
		    Source.c_str(),
		    "PixelSortPixel",
		    "PresentPS",
		...
		{
			const auto Code = LoadShaderBytecode(Cache /
			    (std::string(Name) + GetShaderArtifactExtension(GetBackendConfiguration().mBackendName.c_str())));
			if (!Code)
			{
		...
		Graphics.mBindingLayouts.push_back(mPresentLayout);
		Graphics.mColorFormats.push_back(Format);
	...
	void FPixelSortRenderer::Resize(uint32_t Width, uint32_t Height)
	{
		...
		D.mDebugName = "PixelSort animated noise";
		mNoise = Take(mDevice->CreateTexture(D));

		// Separate images preserve the unsorted preview and avoid read/write aliasing.
		D.mDebugName = "PixelSort CUDA output";
		mSorted = Take(mDevice->CreateTexture(D));
		FArdaRHIBindingSetDesc B;
		B.mLayout = mNoiseLayout;
		B.mItems = {{0, 0, EArdaRHIBindingType::ConstantBuffer, mConstants, {}},
		    {0, 0, EArdaRHIBindingType::TextureUAV, mNoise, {}}};
		mNoiseBindings = Take(mDevice->CreateBindingSet(B));
		B.mLayout = mPresentLayout;
		B.mItems = {{0, 0, EArdaRHIBindingType::ConstantBuffer, mConstants, {}},
		    {0, 0, EArdaRHIBindingType::TextureSRV, mSorted, {}},
	...
	std::vector<uint32_t> FPixelSortRenderer::ReadPixels(const FArdaRHIStagingTextureRef& Staging,
	    uint32_t Width,
	    uint32_t Height)
	{
	    ...
	    const std::filesystem::path& Capture)
	{
		...
		FArdaRHIComputeState Noise;
		Noise.mPipeline = mNoisePipeline;
		Noise.mBindings.push_back(mNoiseBindings);
		Check(Commands->SetComputeState(Noise));
		Commands->Dispatch((mWidth + 7) / 8, (mHeight + 7) / 8);
		...
		Parameters.mInput.mTexture = mNoise;
		Parameters.mOutput.mTexture = mSorted;
		Parameters.mWidth = mWidth;
		Parameters.mHeight = mHeight;
		Parameters.mChannel = Channel;
		Parameters.mThreshold = Threshold;
		...
		Present.mPipeline = mPresentPipeline;
		Present.mFramebuffer = Framebuffer;
		Present.mBindings.push_back(mPresentBindings);
		...
		Check(Commands->SetGraphicsState(Present));
		Commands->Draw({3});
		...
		const auto Readback = [&](const FArdaRHITextureRef& Texture)
		{
			...
			Check(Commands->CopyTextureToStaging(*Result, {}, *Texture, {}));
			return Result;
		};
		if (Verify)
		{
			InputReadback = Readback(mNoise);
			OutputReadback = Readback(mSorted);
		}
		const auto BackBuffer = Framebuffer->GetDesc().mColorAttachments.front().mTexture;
		if (!Capture.empty())
		{
			FrameReadback = Readback(BackBuffer);
		}
		...
		Take(mDevice->ExecuteCommandList(Commands));
		...
		if (Verify || FrameReadback)
		{
			...
			if (Verify)
			{
				VerifySort(ReadPixels(InputReadback, mWidth, mHeight),
				    ReadPixels(OutputReadback, mWidth, mHeight),
				    mWidth,
				    mHeight,
				    Channel,
				    Threshold);
			...
			if (FrameReadback)
			{
				SavePng(Capture,
				    mWidth,
				    mHeight,
				    ReadPixels(FrameReadback, mWidth, mHeight),
				    BackBuffer->GetDesc().mFormat);
			}
		}
