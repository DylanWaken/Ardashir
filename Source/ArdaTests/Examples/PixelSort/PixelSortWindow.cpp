...
namespace arda
{
	...
	void FPixelSortWindow::Create(uint32_t Width, uint32_t Height, bool Hidden)
	{
		...
		Class.lpszClassName = L"ArdaPixelSort";
	...
	LRESULT CALLBACK FPixelSortWindow::Procedure(HWND Window, UINT Message, WPARAM W, LPARAM L)
	{
		...
		if (Message == WM_SIZE)
		{
			Self->mWidth = LOWORD(L);
			Self->mHeight = HIWORD(L);
			Self->mbResize = true;
			return 0;
		}
		if (Message == WM_KEYDOWN)
		{
			...
			if (W == VK_UP)
			{
				Self->mThreshold = std::min(255u, Self->mThreshold + 8);
			}
			if (W == VK_DOWN)
			{
				Self->mThreshold = Self->mThreshold > 8 ? Self->mThreshold - 8 : 0;
			}
			if (!(L & (1ll << 30)))
			{
				...
				if (W == 'C')
				{
					Self->mChannel = (Self->mChannel + 1) % 3;
				}
				if (W == VK_SPACE)
				{
					Self->mbPaused = !Self->mbPaused;
				}
				if (W == VK_TAB)
				{
					Self->mbOriginal = !Self->mbOriginal;
				}
			}
			return 0;
		}
	...
	FArdaNativeObject FPixelSortWindow::CreateVulkanSurface(FArdaNativeObject Instance, eastl::string& Error)
	{
		...
		const auto GetProc = mVulkanLoader
		    ? reinterpret_cast<PFN_vkGetInstanceProcAddr>(GetProcAddress(mVulkanLoader, "vkGetInstanceProcAddr"))
		    : nullptr;
		const auto Create = GetProc ? reinterpret_cast<PFN_vkCreateWin32SurfaceKHR>(
		                                  GetProc(Instance.As<VkInstance>(), "vkCreateWin32SurfaceKHR"))
		                            : nullptr;
		if (Create)
		{
			...
			if (Create(Instance.As<VkInstance>(), &Info, nullptr, &Surface) == VK_SUCCESS)
			{
				return FArdaNativeObject(Surface);
			}
		}
#endif
