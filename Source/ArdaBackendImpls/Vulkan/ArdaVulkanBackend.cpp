...
namespace arda
{
	namespace
	{
		...
		FVulkanSyncState ToVulkanSyncState(EArdaRHIResourceState State) noexcept
		{
		...
		struct FArdaVulkanContext
		{
			...
			~FArdaVulkanContext() noexcept
			{
				if (mDevice)
				{
					...
					const auto DestroyDescriptorHeap = [this](FDescriptorHeapState& Heap)
					{
			...
			void FreeDescriptorHeapRange(bool bSampler, uint32_t Offset, uint32_t Count) noexcept
			{
				...
				std::sort(Ranges.begin(),
				    Ranges.end(),
				    [](const FDescriptorHeapRange& Left, const FDescriptorHeapRange& Right)
				    {
		...
		public:
			...
			{
				std::lock_guard<std::mutex> Lock(mMutex);
				if (mState != EVulkanTimerQueryState::Recording || mRecordingOwner != RecordingOwner)
				{
					return FArdaRHIStatus::Error(EArdaRHIResult::InvalidState,
			...
			void CancelRecording(const void* RecordingOwner)
			{
				...
				if (mRecordingOwner != RecordingOwner ||
				    (mState != EVulkanTimerQueryState::Recording && mState != EVulkanTimerQueryState::Recorded))
				{
					return;
				}
				mRecordingOwner = nullptr;
		...
		private:
			...
			FArdaRHIStatus BindSets(const FVulkanPipeline&,
			...
			std::unordered_map<const void*, FVulkanTexture*> mTrackedTextures;
			...
			std::unordered_map<FVulkanBuffer*, FBufferTracking> mBufferStates;
			...
			std::unordered_map<FVulkanOpacityMicromap*, FAccelStructTracking> mOpacityMicromapStates;
		...
		FArdaRHIStatus FArdaVulkanProviderDevice::Initialize()
		{
			try
			{
				...
				if (mContext->mbDescriptorHeap)
				{
					const auto InitializeDescriptorHeap = [this](FArdaVulkanContext::FDescriptorHeapState& Heap,
					                                          bool bSampler,
		...
		public:
			    ...
			    const eastl::vector<FArdaProviderRayTracingGeometry>& Geometries)
			{
				...
				{
					auto* Vertex = dynamic_cast<FVulkanBuffer*>(Source.mVertexOrAABBBuffer.get());
					...
					if (Source.mDesc.mType == EArdaRHIRayTracingGeometryType::Triangles)
					{
						...
						{
							auto* Index = dynamic_cast<FVulkanBuffer*>(Source.mIndexBuffer.get());
						...
						{
							auto* Micromap = dynamic_cast<FVulkanOpacityMicromap*>(Source.mOpacityMicromap.get());
							...
							{
								auto* Index = dynamic_cast<FVulkanBuffer*>(Source.mOpacityMicromapIndexBuffer.get());
								if (!Index || !Index->mBuffer)
								{
									return FArdaRHIStatus::Error(EArdaRHIResult::WrongDevice,
		    ...
		    const FArdaProviderObjectRef& TriangleObject)
		{
			...
			auto* Triangles = dynamic_cast<FVulkanBuffer*>(TriangleObject.get());
		...
		uint64_t FArdaVulkanProviderDevice::GetOpacityMicromapDeviceAddress(
		    const FArdaProviderObjectRef& Object) const noexcept
		{
			auto* Micromap = dynamic_cast<FVulkanOpacityMicromap*>(Object.get());
		    ...
		    EArdaRHIQueueType QueueType)
		{
			...
			try
			{
				...
				{
					auto* Heap = Mapping.mHeap ? dynamic_cast<FVulkanHeap*>(Mapping.mHeap.get()) : nullptr;
					if (Mapping.mHeap &&
			...
			try
			{
				...
				if (Layout->mbBindless && Layout->mBindlessDesc.mbDirectHeapIndexing)
				{
					...
					for (const auto& Binding : Bindings)
					{
						...
						if (bSampler)
						{
							auto* Sampler = dynamic_cast<FVulkanSampler*>(Binding.mObject.get());
							if (!Sampler)
							{
								return Fail<FArdaProviderObjectRef>(
						...
						else
						{
							...
							if (auto* Buffer = dynamic_cast<FVulkanBuffer*>(Binding.mObject.get()))
							{
							...
							else if (auto* AccelStruct = dynamic_cast<FVulkanAccelStruct*>(Binding.mObject.get()))
							{
				...
				for (size_t Index = 0; Index < Bindings.size(); ++Index)
				{
					...
					else if (dynamic_cast<FVulkanAccelStruct*>(Bindings[Index].mObject.get()))
					{
		    ...
		    const eastl::vector<FArdaProviderObjectRef>& BindingSets)
		{
			try
			{
				...
				{
					auto* Logical = dynamic_cast<FVulkanBindingLayout*>(LogicalObject.get());
					if (!Logical)
					{
						return Fail<FArdaProviderObjectRef>(FArdaRHIStatus::Error(EArdaRHIResult::WrongDevice,
					...
					{
						auto* CandidateSet = dynamic_cast<FVulkanBindingSet*>(Candidate.get());
		    ...
		    FVulkanPipeline& Pipeline)
		{
			...
			for (uint32_t Space = 0; Space < Groups.size(); ++Space)
			{
				...
				{
					SetLayout = static_cast<FVulkanBindingLayout*>(Groups[Space][0].get())->mLayout;
				}
				else
				{
		...
		namespace
		{
			void ApplyDescriptorHeapShaderMappings(FVulkanPipeline& Pipeline,
			...
			void ApplyDescriptorStoragePipelineFlags(const FVulkanPipeline& Pipeline,
			    const void*& PNext,
		    ...
		    const FArdaProviderMeshletPipelineCreateInfo& Info)
		{
			...
			auto* MeshShader = dynamic_cast<FVulkanShader*>(Info.mMeshShader.get());
			if (!MeshShader)
			{
				return Fail<FArdaProviderObjectRef>(FArdaRHIStatus::Error(EArdaRHIResult::InvalidArgument,
			...
			try
			{
				...
				Pipeline->mLayout = Layout.mValue;
				eastl::vector<vk::PipelineShaderStageCreateInfo> Stages;
				if (auto* Task = dynamic_cast<FVulkanShader*>(Info.mAmplificationShader.get()))
				{
				...
				if (auto* Pixel = dynamic_cast<FVulkanShader*>(Info.mPixelShader.get()))
				{
					Stages.emplace_back(vk::PipelineShaderStageCreateFlags{},
		    ...
		    const FArdaProviderRayTracingPipelineCreateInfo& Info)
		{
			...
			try
			{
				...
				{
					auto* Shader = dynamic_cast<FVulkanShader*>(Source.mShader.get());
					if (!Shader)
					{
		...
		{
			auto* Pipeline = dynamic_cast<FVulkanRayTracingPipeline*>(PipelineObject.get());
			if (!Pipeline)
			{
				return Fail<FArdaProviderObjectRef>(FArdaRHIStatus::Error(EArdaRHIResult::WrongDevice,
		    ...
		    const FArdaProviderObjectRef& Geometry)
		{
			...
			if (Geometry && !dynamic_cast<FVulkanAccelStruct*>(Geometry.get()))
			{
				return FArdaRHIStatus::Error(EArdaRHIResult::WrongDevice,
		...
		FArdaRHIStatus FArdaVulkanProviderDevice::CommitShaderTable(const FArdaProviderObjectRef& TableObject)
		{
			...
			try
			{
				...
				const auto Write = [&](const FVulkanShaderTable::FRecord& Record, uint64_t Offset) -> FArdaRHIStatus
				{
					...
					if (auto* Geometry = dynamic_cast<FVulkanAccelStruct*>(Record.mGeometry.get()))
					{
		    ...
		    bool bRelease)
		{
			...
			for (const auto& Binding : Bindings)
			{
				...
				{
					auto* Texture = static_cast<FVulkanTexture*>(Binding.mObject.get());
		    ...
		    uint64_t Offset)
		{
			...
			try
			{
				...
				auto* Upload = dynamic_cast<FVulkanBuffer*>(UploadResult.mValue.get());
		...
		eastl::vector<vk::ImageLayout>& FArdaVulkanCommandList::GetTrackedTextureLayouts(FVulkanTexture& Texture,
		    const FArdaRHITextureDesc& Desc)
		{
		...
		FArdaVulkanCommandList::FBufferTracking& FArdaVulkanCommandList::GetBufferTracking(FVulkanBuffer& Buffer)
		{
		    ...
		    const FArdaRHITextureSubresourceRange& InputRange) const
		{
			...
			const FVulkanSyncState ExpectedSync = ToVulkanSyncState(Snapshot.mState);
		    ...
		    const FArdaRHIBufferDesc&) const
		{
			...
			const FVulkanSyncState Expected = ToVulkanSyncState(Tracking.mAbstractState);
		    ...
		    const vk::ClearColorValue& Value)
		{
			return ClearTextureWithStateRestore(Object,
			    Desc,
			    InputRange,
			    [&](FVulkanTexture& Texture, const FArdaRHITextureSubresourceRange& Range)
			    {
		...
		FArdaRHIStatus FArdaVulkanCommandList::BindSets(const FVulkanPipeline& Pipeline,
		    ...
		    vk::PipelineBindPoint BindPoint)
		{
			...
			for (const auto& Object : Objects)
			{
				...
				const bool bExpected = eastl::any_of(Pipeline.mSetGroups.begin(),
				    Pipeline.mSetGroups.end(),
				    [Set](const FVulkanPipeline::FDescriptorSetGroup& Group)
				    {
					    return eastl::any_of(Group.mLogicalLayouts.begin(),
					        Group.mLogicalLayouts.end(),
		...
		{
			auto* Table = dynamic_cast<FVulkanShaderTable*>(State.mShaderTable.get());
		...
		{
			const FVulkanPipeline* Pipeline = mBoundGraphics ? mBoundGraphics
		    ...
		    EArdaRHIAccelStructBuildFlags Flags)
		{
			...
			auto* Scratch = dynamic_cast<FVulkanBuffer*>(ScratchObject.mValue.get());
		...
		FArdaRHIStatus FArdaVulkanCommandList::BuildOpacityMicromap(const FArdaProviderObjectRef& Object)
		{
			...
			auto* Input = dynamic_cast<FVulkanBuffer*>(Micromap->mInputBuffer.get());
			auto* Triangles = dynamic_cast<FVulkanBuffer*>(Micromap->mTriangleBuffer.get());
			if (!Input || !Triangles || !Input->mBuffer || !Triangles->mBuffer)
			{
				return FArdaRHIStatus::Error(EArdaRHIResult::WrongDevice,
		...
		{
			auto* Destination = dynamic_cast<FVulkanOpacityMicromap*>(DestinationObject.get());
			auto* Source = dynamic_cast<FVulkanOpacityMicromap*>(SourceObject.get());
		    ...
		    EArdaRHIAccelStructBuildFlags Flags)
		{
			...
			for (const auto& Source : Geometries)
			{
				...
				if (Source.mOpacityMicromap)
				{
					...
					auto* Micromap = static_cast<FVulkanOpacityMicromap*>(Source.mOpacityMicromap.get());
		    ...
		    EArdaRHIAccelStructBuildFlags Flags)
		{
			...
			auto* Buffer = dynamic_cast<FVulkanBuffer*>(InstanceObject.mValue.get());
			void* Mapped = nullptr;
			if (!Instances.empty())
			{
				...
				for (size_t Index = 0; Index < Instances.size(); ++Index)
				{
					...
					auto* BottomLevel = dynamic_cast<FVulkanAccelStruct*>(Instance.mBottomLevelAccelStruct.get());
		...
		FArdaRHIStatus FArdaVulkanCommandList::CompactAccelStruct(const FArdaProviderObjectRef& DestinationObject,
		    const FArdaProviderObjectRef& SourceObject)
		{
			auto* Destination = dynamic_cast<FVulkanAccelStruct*>(DestinationObject.get());
		...
		public:
			    ...
			    const IArdaExternalDeviceProvider* ExternalProvider)
			{
				...
				Result.mResult = Runtime->Initialize(Configuration, WindowSurface, ExternalProvider);
		...
		public:
			...
			{
				return FArdaVulkanBackendRuntime::Create(Configuration, WindowSurface, ExternalProvider);
			}
			    ...
			    FArdaBackendShaderCompileInvocation& Invocation) const override
			{
				...
				if (Invocation.mStage == Stage::Amplification || Invocation.mStage == Stage::Mesh)
				{
					Invocation.mArguments.push_back("-fspv-extension=SPV_EXT_mesh_shader");
				}
				const auto AddShift = [&Invocation](const char* Flag, const char* Value, uint32_t Space)
				{
