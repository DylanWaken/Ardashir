...
namespace arda
{
	namespace
	{
		...
		{
			TD3D12PipelineSubobject<ID3D12RootSignature*, D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_ROOT_SIGNATURE> mRoot;
			TD3D12PipelineSubobject<D3D12_SHADER_BYTECODE, D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_AS> mAmplificationShader;
			TD3D12PipelineSubobject<D3D12_SHADER_BYTECODE, D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_MS> mMeshShader;
			TD3D12PipelineSubobject<D3D12_SHADER_BYTECODE, D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_PS> mPixelShader;
			TD3D12PipelineSubobject<D3D12_BLEND_DESC, D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_BLEND> mBlend;
			TD3D12PipelineSubobject<UINT, D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_SAMPLE_MASK> mSampleMask;
			TD3D12PipelineSubobject<D3D12_RASTERIZER_DESC, D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_RASTERIZER> mRasterizer;
			TD3D12PipelineSubobject<D3D12_DEPTH_STENCIL_DESC, D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_DEPTH_STENCIL>
			    mDepthStencil;
			TD3D12PipelineSubobject<D3D12_PRIMITIVE_TOPOLOGY_TYPE,
			    D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_PRIMITIVE_TOPOLOGY>
			    mTopology;
			TD3D12PipelineSubobject<D3D12_RT_FORMAT_ARRAY, D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_RENDER_TARGET_FORMATS>
			    mRenderTargets;
			TD3D12PipelineSubobject<DXGI_FORMAT, D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_DEPTH_STENCIL_FORMAT> mDepthFormat;
			TD3D12PipelineSubobject<DXGI_SAMPLE_DESC, D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_SAMPLE_DESC> mSampleDesc;
		};
		...
		public:
			...
			TArdaRHIResult<FD3D12DescriptorAllocation> Allocate(bool bSampler, uint32_t Count);
		...
		public:
			...
			{
				std::lock_guard<std::mutex> Lock(mMutex);
				if (mState != ED3D12TimerQueryState::Recording || mRecordingOwner != RecordingOwner)
				{
			...
			void CancelRecording(const void* RecordingOwner)
			{
				...
				if (mRecordingOwner == RecordingOwner &&
				    (mState == ED3D12TimerQueryState::Recording || mState == ED3D12TimerQueryState::Recorded))
				{
		...
		public:
			...
			[[nodiscard]] FD3D12SubmissionLifetime CaptureSubmissionLifetime() const
			{
		...
		private:
			...
			FArdaRHIStatus BindDescriptorSets(const FD3D12Pipeline&,
			...
			std::unordered_map<FD3D12Texture*, FTextureTracking> mTextureStates;
			std::unordered_map<FD3D12Buffer*, FBufferTracking> mBufferStates;
			std::unordered_map<FD3D12AccelStruct*, FAccelStructTracking> mAccelStructStates;
		...
		public:
			...
			TArdaRHIResult<eastl::shared_ptr<FD3D12BindingSet>> CreateClearView(ID3D12Resource*,
			    const D3D12_UNORDERED_ACCESS_VIEW_DESC&);
			TArdaRHIResult<D3D12_CPU_DESCRIPTOR_HANDLE> GetAttachmentView(FD3D12Texture&,
			    FArdaRHITextureSubresourceRange,
			    bool bDepth);

		private:
			TArdaRHIResult<FD3D12DescriptorAllocation> AllocateDescriptors(bool bSampler, uint32_t Count);
		...
		TArdaRHIResult<FD3D12DescriptorAllocation> FArdaD3D12ProviderDevice::AllocateDescriptors(bool bSampler,
		    uint32_t Count)
		{
			return mDescriptorAllocator->Allocate(bSampler, Count);
		}

		TArdaRHIResult<FD3D12DescriptorAllocation> FD3D12DescriptorAllocator::Allocate(bool bSampler, uint32_t Count)
		{
			std::lock_guard<std::mutex> Lock(mMutex);
			eastl::vector<FRange>& FreeRanges = bSampler ? mSamplerFree : mResourceFree;
			if (Count == 0)
			{
				return Fail<FD3D12DescriptorAllocation>(FArdaRHIStatus::Error(EArdaRHIResult::InvalidArgument,
			...
			return Fail<FD3D12DescriptorAllocation>(FArdaRHIStatus::Error(EArdaRHIResult::BackendFailure,
			    "The native D3D12 descriptor heap is exhausted."));
		}

		void FD3D12DescriptorAllocator::Free(const FD3D12DescriptorAllocation& Allocation) noexcept
		{
			...
			std::lock_guard<std::mutex> Lock(mMutex);
			eastl::vector<FRange>& FreeRanges = Allocation.mbSampler ? mSamplerFree : mResourceFree;
		...
		TArdaRHIResult<D3D12_CPU_DESCRIPTOR_HANDLE> FArdaD3D12ProviderDevice::GetAttachmentView(FD3D12Texture& Texture,
		    ...
		    const FArdaRHISamplerFeedbackTextureDesc& Desc)
		{
			...
			auto* Paired = dynamic_cast<FD3D12Texture*>(PairedObject.get());
			...
			auto Feedback = eastl::make_shared<FD3D12SamplerFeedbackTexture>();
		    ...
		    eastl::vector<D3D12_RAYTRACING_GEOMETRY_DESC>& NativeGeometries)
		{
			...
			for (const auto& Geometry : Geometries)
			{
				...
				auto* Vertex = dynamic_cast<FD3D12Buffer*>(Geometry.mVertexOrAABBBuffer.get());
				...
				if (Geometry.mDesc.mType == EArdaRHIRayTracingGeometryType::Triangles)
				{
					...
					{
						auto* Index = dynamic_cast<FD3D12Buffer*>(Geometry.mIndexBuffer.get());
		    ...
		    EArdaRHIQueueType QueueType)
		{
			...
			{
				auto* Heap = Mapping.mHeap ? dynamic_cast<FD3D12Heap*>(Mapping.mHeap.get()) : nullptr;
		    ...
		    const eastl::vector<FArdaProviderBinding>& Bindings)
		{
			...
			for (const auto& Item : Layout->mDesc.mItems)
			{
				...
				for (uint32_t Element = 0; Element < DescriptorCount; ++Element)
				{
					...
					auto* Feedback = dynamic_cast<FD3D12SamplerFeedbackTexture*>(Binding->mObject.get());
					auto* Buffer = dynamic_cast<FD3D12Buffer*>(Binding->mObject.get());
					auto* AccelStruct = dynamic_cast<FD3D12AccelStruct*>(Binding->mObject.get());
					switch (Item.mType)
					{
					case EArdaRHIBindingType::RayTracingAccelStruct:
					{
					...
					{
						auto* Paired =
						    Feedback ? dynamic_cast<FD3D12Texture*>(Feedback->mPairedTexture.get()) : nullptr;
		    ...
		    const FArdaProviderMeshletPipelineCreateInfo& Info)
		{
			...
			auto* Amplification =
			    Info.mAmplificationShader ? dynamic_cast<FD3D12Shader*>(Info.mAmplificationShader.get()) : nullptr;
			auto* Pixel = Info.mPixelShader ? dynamic_cast<FD3D12Shader*>(Info.mPixelShader.get()) : nullptr;
		    ...
		    const FArdaProviderRayTracingPipelineCreateInfo& Info)
		{
			...
			const auto AddLibrary = [&Libraries](const FArdaProviderRayTracingShader& ShaderInfo) -> FArdaRHIStatus
			{
				...
				auto* Shader = dynamic_cast<FD3D12Shader*>(ShaderInfo.mShader.get());
				if (!Shader)
				{
					return FArdaRHIStatus::Error(EArdaRHIResult::WrongDevice,
		    ...
		    const FArdaProviderWorkGraphPipelineCreateInfo& Info)
		{
			...
			{
				auto* Shader = dynamic_cast<FD3D12Shader*>(ShaderObject.get());
				if (!Shader)
				{
					return Fail<FArdaProviderObjectRef>(FArdaRHIStatus::Error(EArdaRHIResult::WrongDevice,
		...
		{
			auto* Pipeline = dynamic_cast<FD3D12RayTracingPipeline*>(PipelineObject.get());
			if (!Pipeline)
			{
				return Fail<FArdaProviderObjectRef>(FArdaRHIStatus::Error(EArdaRHIResult::WrongDevice,
		...
		FArdaRHIStatus FArdaD3D12ProviderDevice::RebuildShaderTable(FD3D12ShaderTable& Table)
		{
			...
			for (const auto& Record : Table.mRecords)
			{
				...
				if (auto* Bindings = dynamic_cast<FD3D12BindingSet*>(Record.mBindings.get()))
				{
			...
			const auto WriteRecord = [&](const eastl::string& Name,
			                             uint64_t Offset,
			                             const FD3D12ShaderTable::FRecord* Record = nullptr) -> FArdaRHIStatus
			{
				...
				if (Record)
				{
					...
					if (auto* Geometry = dynamic_cast<FD3D12AccelStruct*>(Record->mGeometry.get()))
					{
			...
			const auto WriteStructuredSection =
			    [&WriteRecord, RecordStride](const eastl::vector<const FD3D12ShaderTable::FRecord*>& Records,
		    ...
		    const FArdaProviderObjectRef& Geometry)
		{
			...
			if (LocalBindings && !dynamic_cast<FD3D12BindingSet*>(LocalBindings.get()))
			{
				return FArdaRHIStatus::Error(EArdaRHIResult::WrongDevice,
				    "D3D12 local shader-table bindings are invalid.");
			}
			if (Geometry && !dynamic_cast<FD3D12AccelStruct*>(Geometry.get()))
			{
		...
		FArdaD3D12CommandList::FTextureTracking& FArdaD3D12CommandList::GetTextureTracking(FD3D12Texture& Texture)
		{
		...
		FArdaD3D12CommandList::FBufferTracking& FArdaD3D12CommandList::GetBufferTracking(FD3D12Buffer& Buffer)
		{
		    ...
		    uint64_t Offset)
		{
			...
			HRESULT Result = mDevice.GetDevice().CreateCommittedResource(&Heap,
			    D3D12_HEAP_FLAG_NONE,
			    &Resource,
			    D3D12_RESOURCE_STATE_GENERIC_READ,
			    nullptr,
			...
			if (FAILED(Result))
			{
				return D3D12Failure("Failed to create a D3D12 upload buffer.", Result);
			}
			void* Mapped = nullptr;
			D3D12_RANGE ReadRange{0, 0};
		...
		TArdaRHIResult<eastl::shared_ptr<FD3D12BindingSet>> FArdaD3D12ProviderDevice::CreateClearView(
		    ID3D12Resource* Resource,
		...
		FArdaRHIStatus FArdaD3D12CommandList::BindDescriptorSets(const FD3D12Pipeline& Pipeline,
		    ...
		    bool bGraphics)
		{
			...
			for (size_t LayoutIndex = 0; LayoutIndex < Pipeline.mLayoutItemCounts.size(); ++LayoutIndex)
			{
				...
				if (LayoutIndex < Pipeline.mPushConstantRoots.size() && Pipeline.mPushConstantRoots[LayoutIndex] >= 0)
				{
					++RootIndex;
				}
			}
		...
		{
			auto* Table = dynamic_cast<FD3D12ShaderTable*>(State.mShaderTable.get());
		...
		void FArdaD3D12CommandList::SetPushConstants(const void* Data, size_t Size)
		{
			...
			FD3D12Pipeline* Pipeline = mBoundGraphicsPipeline ? mBoundGraphicsPipeline : mBoundComputePipeline;
			if (!Pipeline)
			{
				return;
			}
		    ...
		    EArdaRHIAccelStructBuildFlags Flags)
		{
			...
			auto Scratch = CreateD3D12BufferResource(&mDevice.GetDevice(),
			    ScratchSize,
			    D3D12_HEAP_TYPE_DEFAULT,
			    D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
			    D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
			...
			Desc.ScratchAccelerationStructureData = Scratch.mValue->GetGPUVirtualAddress();
			if (bUpdate)
			{
		    ...
		    EArdaRHIAccelStructBuildFlags Flags)
		{
			...
			auto Upload = CreateD3D12BufferResource(&mDevice.GetDevice(),
			    Size,
			    D3D12_HEAP_TYPE_UPLOAD,
			    D3D12_RESOURCE_STATE_GENERIC_READ);
			if (!Upload)
			...
			for (size_t Index = 0; Index < Instances.size(); ++Index)
			{
				...
				auto* BottomLevel = dynamic_cast<FD3D12AccelStruct*>(Instance.mBottomLevelAccelStruct.get());
			...
			const FArdaRHIStatus Status = RecordAccelStructBuild(Object, Inputs, Flags);
			if (Status)
			{
		...
		{
			auto* Destination = dynamic_cast<FD3D12AccelStruct*>(DestinationObject.get());
		    ...
		    uint64_t Offset)
		{
			...
			auto NativeArguments = CreateD3D12BufferResource(&mDevice.GetDevice(),
			    sizeof(Dispatch),
			    D3D12_HEAP_TYPE_DEFAULT,
			    D3D12_RESOURCE_STATE_COPY_DEST);
			if (!NativeArguments)
			...
			mCommandList->CopyBufferRegion(NativeArguments.mValue.Get(), 0, Upload.mValue.Get(), 0, sizeof(Dispatch));
			if (auto Status = TransitionBuffer(Arguments, EArdaRHIResourceState::CopySource); !Status)
			{
				return Status;
			}
			mCommandList->CopyBufferRegion(NativeArguments.mValue.Get(),
			...
			ToIndirect.Transition.StateBefore = D3D12_RESOURCE_STATE_COPY_DEST;
			...
			mCommandList->ExecuteIndirect(Signature.Get(), 1, NativeArguments.mValue.Get(), 0, nullptr, 0);
			mUploadResources.push_back(eastl::move(Upload.mValue));
		...
		FArdaRHIStatus FArdaD3D12CommandList::DispatchWorkGraph(const FArdaProviderObjectRef& PipelineObject,
		    ...
		    const eastl::vector<FArdaProviderObjectRef>& Bindings)
		{
			auto* Pipeline = dynamic_cast<FD3D12WorkGraph*>(PipelineObject.get());
			...
			Program.WorkGraph.Flags = Pipeline->mbInitialized.load(std::memory_order_acquire)
		...
		{
			auto* Feedback = dynamic_cast<FD3D12SamplerFeedbackTexture*>(Object.get());
		...
		{
			auto* Destination = dynamic_cast<FD3D12Texture*>(DestinationObject.get());
			auto* Feedback = dynamic_cast<FD3D12SamplerFeedbackTexture*>(FeedbackObject.get());
		    ...
		    EArdaRHIQueueType QueueType)
		{
			...
			if (!Native->mCudaSegments.empty())
			{
				...
				for (const auto& Segment : Native->mCudaSegments)
				{
					...
					{
						return Fail<uint64_t>(D3D12Failure("Signal graphics to CUDA", HandoffResult));
					}
					...
					if (FAILED(HandoffResult))
					{
						return Fail<uint64_t>(D3D12Failure("Wait for CUDA completion", HandoffResult));
					}
				}
			}
			...
			const uint64_t QueueValue = mQueueFenceValues[QueueIndex].fetch_add(1, std::memory_order_relaxed) + 1;
			HRESULT Result = Queue->Signal(mQueueFences[QueueIndex].Get(), QueueValue);
			if (FAILED(Result))
			{
				return Fail<uint64_t>(D3D12Failure("Failed to signal the D3D12 submission fence.", Result));
			}
			...
			Native->CommitTrackedStates();
			{
				...
				mPendingSubmissions.push_back({QueueType, QueueValue, Native->CaptureSubmissionLifetime()});
			}
		...
		{
			std::lock_guard<std::mutex> Lock(mSubmissionMutex);
			mPendingSubmissions.erase(eastl::remove_if(mPendingSubmissions.begin(),
		...
		private:
			...
			EArdaInitializeResult InitializeOwned(const FArdaBackendConfiguration& Configuration)
			{
				...
				for (UINT Index = 0; mFactory->EnumAdapterByGpuPreference(Index,
		...
		public:
			...
			{
				return FArdaD3D12BackendRuntime::Create(Configuration, WindowSurface, ExternalProvider);
			}
