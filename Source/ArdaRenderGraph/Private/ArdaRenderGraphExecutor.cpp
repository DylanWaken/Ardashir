...
namespace arda
{
	namespace
	{
		...
		[[nodiscard]] arda::EArdaRHIQueueType GetCommandQueue(EARDGPipeline Pipeline) noexcept
		{
			switch (Pipeline)
			{
			case EARDGPipeline::Graphics:
				return arda::EArdaRHIQueueType::Graphics;
			case EARDGPipeline::AsyncCompute:
				return arda::EArdaRHIQueueType::Compute;
			case EARDGPipeline::Copy:
				return arda::EArdaRHIQueueType::Copy;
			}
			return arda::EArdaRHIQueueType::Graphics;
		}
         ...
         */
		[[nodiscard]] bool TextureDescriptorsEqual(const arda::FArdaRHITextureDesc& Left,
		    const arda::FArdaRHITextureDesc& Right) noexcept
		{
			return Left.mWidth == Right.mWidth && Left.mHeight == Right.mHeight && Left.mDepth == Right.mDepth &&
			    Left.mArraySize == Right.mArraySize && Left.mMipLevels == Right.mMipLevels &&
			    Left.mSampleCount == Right.mSampleCount && Left.mFormat == Right.mFormat &&
			    Left.mDimension == Right.mDimension && Left.mUsage == Right.mUsage &&
			    Left.mbCudaInterop == Right.mbCudaInterop && Left.mbUseClearValue == Right.mbUseClearValue &&
		...
		[[nodiscard]] bool BufferDescriptorsEqual(const arda::FArdaRHIBufferDesc& Left,
		    const arda::FArdaRHIBufferDesc& Right) noexcept
		{
			return Left.mByteSize == Right.mByteSize && Left.mStructureStride == Right.mStructureStride &&
			    Left.mMaxVersions == Right.mMaxVersions && Left.mbCudaInterop == Right.mbCudaInterop &&
			    Left.mFormat == Right.mFormat && Left.mUsage == Right.mUsage && Left.mCpuAccess == Right.mCpuAccess;
		}
         ...
         */
		[[nodiscard]] size_t HashTextureDescriptor(const arda::FArdaRHITextureDesc& Desc)
		{
			arda::FArdaRHITextureDesc PoolDesc = Desc;
			PoolDesc.mDebugName.clear();
			return arda::HashValue(PoolDesc);
		}
         ...
         */
		[[nodiscard]] size_t HashBufferDescriptor(const arda::FArdaRHIBufferDesc& Desc)
		{
			arda::FArdaRHIBufferDesc PoolDesc = Desc;
			PoolDesc.mDebugName.clear();
			return arda::HashValue(PoolDesc);
		}
		...
		public:
			...
			FARDGTexturePool(arda::IArdaRHIDevice& Device, FARDGExecutionResult& Result)
			    : mDevice(Device),
			      mResult(Result)
			{
			}
             ...
             */
			[[nodiscard]] arda::FArdaRHITextureRef Acquire(arda::FArdaRHITextureDesc Desc,
			    uint32_t FirstUse,
			    uint32_t LastUse,
			    int32_t ReuseDomain)
			{
				...
				if (ReuseDomain >= 0)
				{
					...
					{
						if (Entry.mAvailableAfter < FirstUse && Entry.mReuseDomain == ReuseDomain &&
				...
				if (!TextureResult)
				{
					ARDA_CHECK_MSG("The RHI failed to create a render-graph texture.");
				}
				Bucket.push_back({eastl::move(Desc), TextureResult.mValue, LastUse, ReuseDomain});
		...
		private:
			struct FEntry
			{
				...
				arda::FArdaRHITextureRef mTexture;
		...
		public:
			...
			FARDGBufferPool(arda::IArdaRHIDevice& Device, FARDGExecutionResult& Result)
			    : mDevice(Device),
			      mResult(Result)
			{
			}
             ...
             */
			[[nodiscard]] arda::FArdaRHIBufferRef Acquire(arda::FArdaRHIBufferDesc Desc,
			    uint32_t FirstUse,
			    uint32_t LastUse,
			    int32_t ReuseDomain)
			{
				...
				if (!BufferResult)
				{
					ARDA_CHECK_MSG("The RHI failed to create a render-graph buffer.");
				}
				Bucket.push_back({eastl::move(Desc), BufferResult.mValue, LastUse, ReuseDomain});
		...
		private:
			...
			{
				/** Normalized descriptor used for definitive compatibility checks. */
				arda::FArdaRHIBufferDesc mDesc;
				/** Pool-owned reference to the reusable physical buffer. */
				arda::FArdaRHIBufferRef mBuffer;
				/** Inclusive execution-order index of the latest logical user's last use. */
				uint32_t mAvailableAfter = 0;
				/** Queue reuse domain index, or -1 when this entry cannot be recycled. */
				int32_t mReuseDomain = -1;
		...
		void MaterializeTransientHeaps(FARDGBuilder::FImpl& Graph,
		    ...
		    eastl::vector<FARDGAliasingTransition>& OutAliases)
		{
			struct FARDGPlacedResource
			{
				...
				arda::FArdaRHIMemoryRequirements mRequirements;
				arda::FArdaRHITextureRef mTexture;
				arda::FArdaRHIBufferRef mBuffer;
				eastl::array<FARDGPassHandle, arda::ArdaRHIQueueTypeCount> mLastUses;
			};
			...
			for (const FARDGResourceLifetime& Lifetime : Graph.mCompileResult.mResourceLifetimes)
			{
				...
				if (Capabilities.mbVirtualResources && Capabilities.mbHeaps && Capabilities.mbAliasingBarriers)
				{
					if (Lifetime.mType == EARDGResourceType::Texture)
					{
						auto Desc = Graph.mTextures.Get(FARDGTextureHandle(Lifetime.mResourceIndex)).GetDesc();
						Desc.mInitialState = NormalizeInitialState(Desc.mInitialState);
						Desc.mbKeepInitialState = false;
						Desc.mbVirtual = true;
					...
					{
						auto Desc = Graph.mBuffers.Get(FARDGBufferHandle(Lifetime.mResourceIndex)).GetDesc();
						if (Desc.mCpuAccess == arda::EArdaRHICpuAccess::None)
						{
							Desc.mInitialState = NormalizeInitialState(Desc.mInitialState);
							Desc.mbKeepInitialState = false;
							Desc.mbVirtual = true;
				...
				if ((!Resource.mTexture && !Resource.mBuffer) || !Resource.mRequirements.mSize ||
				    !Resource.mRequirements.mAlignment || !Resource.mRequirements.mMemoryTypeBits)
				{
				...
				for (uint32_t Index = Lifetime.mFirstUse; Index <= Lifetime.mLastUse; ++Index)
				{
					...
					const bool bUsesResource = Resource.mTexture
					    ? eastl::any_of(State.mTextureStates.begin(),
					          State.mTextureStates.end(),
					          [&](const auto& Access)
					          {
						          return Access.mTexture.GetIndex() == Lifetime.mResourceIndex;
					          })
					    : eastl::any_of(State.mBufferStates.begin(),
					          State.mBufferStates.end(),
					          [&](const auto& Access)
					          {
					...
					if (bUsesResource)
					{
						Resource.mLastUses[arda::GetArdaRHIQueueIndex(GetCommandQueue(State.mPipeline))] = Handle;
					}
				}
				...
				auto Group = eastl::find_if(Groups.begin(),
				    Groups.end(),
				    [&](const auto& Entry)
				    {
			...
			for (auto& Group : Groups)
			{
				...
				arda::FArdaRHIHeapDesc HeapDesc;
				HeapDesc.mCapacity = Layout.mCapacity;
				HeapDesc.mType = arda::EArdaRHIHeapType::DeviceLocal;
				...
				for (const auto& Allocation : Layout.mAllocations)
				{
					...
					for (const auto& Previous : Layout.mAllocations)
					{
						...
						{
							if (Use.IsValid() &&
							    eastl::find(Alias.mProducers.begin(), Alias.mProducers.end(), Use) ==
							        Alias.mProducers.end())
							{
		...
		void CaptureTextureState(FARDGRecordedPass& Recorded,
		...
		void CaptureBufferState(FARDGRecordedPass& Recorded,
		    const FARDGPass& Pass,
		    FARDGPassHandle PassHandle,
		    FARDGBuffer& Buffer,
         ...
         */
		[[nodiscard]] int32_t GetTextureReuseDomain(const FARDGBuilder::FImpl& Graph, FARDGTextureHandle Texture)
		{
			int32_t Domain = -1;
			for (FARDGPassHandle Handle : Graph.mCompileResult.mExecutionOrder)
			{
				const FARDGPass& Pass = Graph.mPasses.Get(Handle);
				const bool bUsesTexture = eastl::any_of(Pass.GetState().mTextureStates.begin(),
				...
				const int32_t PassDomain =
				    static_cast<int32_t>(arda::GetArdaRHIQueueIndex(GetCommandQueue(Pass.GetState().mPipeline)));
		...
		[[nodiscard]] int32_t GetBufferReuseDomain(const FARDGBuilder::FImpl& Graph, FARDGBufferHandle Buffer)
		{
			int32_t Domain = -1;
			for (FARDGPassHandle Handle : Graph.mCompileResult.mExecutionOrder)
			{
				const FARDGPass& Pass = Graph.mPasses.Get(Handle);
				const bool bUsesBuffer = eastl::any_of(Pass.GetState().mBufferStates.begin(),
		...
		void AddExecutionDependency(FARDGBuilder::FImpl& Graph, FARDGPassHandle Producer, FARDGPassHandle Consumer)
		{
         ...
         */
		void MaterializeResources(FARDGBuilder::FImpl& Graph,
		    arda::IArdaRHIDevice& Device,
		    FARDGExecutionResult& Result,
		    eastl::vector<FARDGAliasingTransition>& OutAliases)
		{
			MaterializeTransientHeaps(Graph, Device, Result, OutAliases);

			FARDGTexturePool TexturePool(Device, Result);
			FARDGBufferPool BufferPool(Device, Result);
			eastl::vector<FARDGResourceLifetime> Lifetimes = Graph.mCompileResult.mResourceLifetimes;
			eastl::sort(Lifetimes.begin(),
			    Lifetimes.end(),
			...
			for (const FARDGResourceLifetime& Lifetime : Lifetimes)
			{
				if (Lifetime.mType == EARDGResourceType::Texture)
				{
					FARDGTexture& Texture = Graph.mTextures.Get(FARDGTextureHandle(Lifetime.mResourceIndex));
					if (Texture.IsExternal())
					{
						if (!Texture.GetTexture())
						{
							ARDA_CHECK_MSG("A render-graph external texture lost its handle.");
						}
						continue;
					}
					if (Texture.GetTexture())
					...
					Texture.BindTexture(TexturePool.Acquire(Texture.GetDesc(),
					    Lifetime.mFirstUse,
					    Lifetime.mLastUse,
					    Lifetime.mbTransient ? GetTextureReuseDomain(Graph, Texture.GetHandle()) : -1));
				}
				else if (Lifetime.mType == EARDGResourceType::Buffer)
				{
					FARDGBuffer& Buffer = Graph.mBuffers.Get(FARDGBufferHandle(Lifetime.mResourceIndex));
					if (Buffer.IsExternal())
					{
						if (!Buffer.GetBuffer())
						{
							ARDA_CHECK_MSG("A render-graph external buffer lost its handle.");
						}
						continue;
					}
					if (Buffer.GetBuffer())
					...
					Buffer.BindBuffer(BufferPool.Acquire(Buffer.GetDesc(),
					    Lifetime.mFirstUse,
					    Lifetime.mLastUse,
					    Lifetime.mbTransient ? GetBufferReuseDomain(Graph, Buffer.GetHandle()) : -1));
				}
				else if (Lifetime.mType == EARDGResourceType::AccelStruct)
				{
					FARDGAccelStruct& AccelStruct =
					    Graph.mAccelStructs.Get(FARDGAccelStructHandle(Lifetime.mResourceIndex));
					if (AccelStruct.IsExternal())
					{
						if (!AccelStruct.GetAccelStruct())
						{
							ARDA_CHECK_MSG("A render-graph external acceleration structure lost its handle.");
						}
						continue;
					}
					...
					if (!Created)
					{
						ARDA_CHECK_MSG("The RHI failed to create a graph acceleration structure.");
					}
					AccelStruct.BindAccelStruct(eastl::move(Created.mValue));
				}
			}

			for (FARDGUniformBuffer* UniformBuffer : Graph.mUniformBuffers.GetEntries())
			{
				auto Buffer = Device.CreateBuffer(UniformBuffer->GetDesc());
				if (!Buffer)
				{
					ARDA_CHECK_MSG("The RHI failed to create a graph uniform buffer.");
				}
         ...
         */
		[[nodiscard]] eastl::vector<FARDGRuntimePassTransitions> BuildPhysicalTransitions(FARDGBuilder::FImpl& Graph)
		{
			...
			eastl::vector<FARDGRuntimePassTransitions> Runtime(Graph.mPasses.GetCount());
			for (FARDGPassHandle Handle : Graph.mCompileResult.mExecutionOrder)
			{
				const FARDGPass& Pass = Graph.mPasses.Get(Handle);
				auto& Out = Runtime[Handle.GetIndex()];
				const arda::EArdaRHIQueueType Queue = GetCommandQueue(Pass.GetState().mPipeline);
				for (const FARDGTextureTransition& Compiled : Pass.GetState().mTextureTransitions)
				{
					const FARDGTexture& Texture = Graph.mTextures.Get(Compiled.mTexture);
					...
					if (States.empty())
					{
						...
						States.resize(static_cast<size_t>(Desc.mMipLevels) * Desc.mArraySize,
						    NormalizeInitialState(Texture.GetInitialState()));
						auto& History = TextureQueues[Physical];
						History.mPasses.assign(States.size(), Graph.mCompileResult.mPrologue);
						History.mQueues.assign(States.size(),
						    Texture.IsExternal() ? arda::EArdaRHIQueueType::Graphics : Queue);
						History.mTextures.assign(States.size(), Compiled.mTexture);
					}
					auto& History = TextureQueues[Physical];
					const arda::FArdaRHITextureSubresourceRange Subresources = Compiled.mSubresources.Resolve(Desc);
					for (uint32_t ArraySlice = Subresources.mBaseArraySlice;
					    ArraySlice < Subresources.mBaseArraySlice + Subresources.mArraySliceCount;
					    ++ArraySlice)
					{
						for (uint32_t MipLevel = Subresources.mBaseMipLevel;
						    MipLevel < Subresources.mBaseMipLevel + Subresources.mMipLevelCount;
						    ++MipLevel)
						{
							const size_t Index = static_cast<size_t>(ArraySlice) * Desc.mMipLevels + MipLevel;
							const arda::FArdaRHITextureSubresourceRange Cell{MipLevel, 1, ArraySlice, 1};
							if (!Texture.IsExternal() && Texture.GetTexture()->GetDesc().mbVirtual &&
							    History.mPasses[Index] == Graph.mCompileResult.mPrologue &&
							    (Compiled.mStateAfter == arda::EArdaRHIResourceState::RenderTarget ||
							        Compiled.mStateAfter == arda::EArdaRHIResourceState::DepthWrite))
							{
							...
							if (History.mQueues[Index] != Queue)
							{
								...
								const auto Transfer =
								    FARDGRuntimePassTransitions::FTextureQueueTransfer{History.mTextures[Index],
								        Cell,
							...
							Out.mTextures.push_back({Compiled.mTexture,
							    ...
							    Compiled.mbForceBarrier && States[Index] == Compiled.mStateAfter});
				...
				for (const FARDGBufferTransition& Compiled : Pass.GetState().mBufferTransitions)
				{
					const FARDGBuffer& Buffer = Graph.mBuffers.Get(Compiled.mBuffer);
					...
					if (Existing == BufferStates.end())
					{
						...
						BufferQueues.emplace(Physical,
						    FBufferQueueHistory{Graph.mCompileResult.mPrologue,
						        Buffer.IsExternal() ? arda::EArdaRHIQueueType::Graphics : Queue,
						        Compiled.mBuffer});
					}
					...
					Out.mBuffers.push_back({Compiled.mBuffer,
					    ...
					    Compiled.mbForceBarrier && Existing->second == Compiled.mStateAfter});
					Existing->second = Compiled.mStateAfter;
					History = {Handle, Queue, Compiled.mBuffer};
				}
				for (const FARDGAccelStructTransition& Compiled : Pass.GetState().mAccelStructTransitions)
				{
			...
			if (Graph.mContext.mDebugOptions.mbClobberFirstWrites)
			{
				...
				{
					ProducedBuffers[Buffer->GetHandle().GetIndex()] = Buffer->IsExternal();
				}
				...
				for (FARDGPassHandle Handle : Graph.mCompileResult.mExecutionOrder)
				{
					...
					const bool bCanIssueClobber = Pass.GetState().mPipeline != EARDGPipeline::Copy;
					for (const FARDGPassTextureState& Access : Pass.GetState().mTextureStates)
					{
						    ...
						    ++Slice)
						{
							for (uint32_t Mip = Range.mBaseMipLevel; Mip < Range.mBaseMipLevel + Range.mMipLevelCount;
							    ++Mip)
							{
								bAllUnproduced &= !ProducedTextures[Access.mTexture.GetIndex()]
								                                   [static_cast<size_t>(Slice) * Desc.mMipLevels + Mip];
							}
						}
						const arda::FArdaRHIFormatInfo& Format = arda::GetArdaRHIFormatInfo(Desc.mFormat);
						const bool bColorClear = !Format.mbDepth && !Format.mbStencil &&
						    (arda::HasAnyFlags(Desc.mUsage, arda::EArdaRHITextureUsage::RenderTarget) ||
						        arda::HasAnyFlags(Desc.mUsage, arda::EArdaRHITextureUsage::UnorderedAccess));
						const bool bDepthClear = (Format.mbDepth || Format.mbStencil) &&
						    arda::HasAnyFlags(Desc.mUsage, arda::EArdaRHITextureUsage::DepthStencil) &&
						    Pass.GetState().mPipeline == EARDGPipeline::Graphics;
						if (bCanIssueClobber && bAllUnproduced && (bColorClear || bDepthClear))
						{
						    ...
						    ++Slice)
						{
							for (uint32_t Mip = Range.mBaseMipLevel; Mip < Range.mBaseMipLevel + Range.mMipLevelCount;
							    ++Mip)
							{
								ProducedTextures[Access.mTexture.GetIndex()]
								                [static_cast<size_t>(Slice) * Desc.mMipLevels + Mip] = true;
							}
						}
					}
					for (const FARDGPassBufferState& Access : Pass.GetState().mBufferStates)
					{
						...
						const FARDGBuffer& Buffer = Graph.mBuffers.Get(Access.mBuffer);
						const uint32_t Index = Access.mBuffer.GetIndex();
						const bool bCanClobber = bCanIssueClobber && !ProducedBuffers[Index] && !Buffer.IsExternal() &&
						    arda::HasAnyFlags(Buffer.GetDesc().mUsage, arda::EArdaRHIBufferUsage::UnorderedAccess) &&
						    Access.mRange.IsWholeBuffer(Buffer.GetDesc()) &&
						    (Access.mState & arda::EArdaRHIResourceState::UnorderedAccess) !=
         ...
         */
		[[nodiscard]] FARDGRecordedPass RecordPass(FARDGBuilder& Builder,
		    ...
		    bool bClose = true)
		{
			...
			if (!Recorded.mCommandList)
			{
				...
				auto CommandListResult = Graph.mContext.mDevice->CreateCommandList(Recorded.mQueue,
				...
				for (const auto& Alias : Transitions.mAliasingResources)
				{
					...
					{
						ResourceAfter =
						    Graph.mTextures.Get(FARDGTextureHandle(Alias.mResourceIndex)).GetTexture().Get();
					}
					else if (Alias.mType == EARDGResourceType::Buffer)
					{
						ResourceAfter = Graph.mBuffers.Get(FARDGBufferHandle(Alias.mResourceIndex)).GetBuffer().Get();
					}
					if (!ResourceAfter || !Recorded.mCommandList->AliasingBarrier(nullptr, ResourceAfter))
					{
						ARDA_CHECK_MSG("The RHI failed to encode an RDG aliasing barrier.");
					}
				}
				...
				{
					FARDGTexture& Texture = Graph.mTextures.Get(Transfer.mTexture);
					Recorded.mCommandList->BeginTrackingTextureState(*Texture.GetTexture(),
					...
					if (!Recorded.mCommandList->TransitionTexture(*Texture.GetTexture(), Acquire))
					{
						ARDA_CHECK_MSG("The RHI failed to encode an RDG texture queue acquire.");
					}
					if (bValidateResourceStates)
					{
						CaptureTextureState(Recorded,
						    ...
						    Graph.mContext.mDevice->GetCapabilities().mQueues.GetFamily(Transfer.mDestinationQueue));
					}
				}
				for (const auto& Transfer : Transitions.mBufferAcquires)
				{
					FARDGBuffer& Buffer = Graph.mBuffers.Get(Transfer.mBuffer);
					Recorded.mCommandList->BeginTrackingBufferState(*Buffer.GetBuffer(),
					    arda::EArdaRHIResourceState::Common);
					...
					Acquire.mDestinationQueue = Transfer.mDestinationQueue;
					Acquire.mbQueueOwnershipTransfer = true;
					if (!Recorded.mCommandList->TransitionBuffer(*Buffer.GetBuffer(), Acquire))
					{
						ARDA_CHECK_MSG("The RHI failed to encode an RDG buffer queue acquire.");
					}
					if (bValidateResourceStates)
					{
				...
				for (const FARDGTextureTransition& Transition : Transitions.mTextures)
				{
					FARDGTexture& Texture = Graph.mTextures.Get(Transition.mTexture);
					Recorded.mCommandList->BeginTrackingTextureState(*Texture.GetTexture(),
					...
					{
						Recorded.mCommandList->SetTextureState(*Texture.GetTexture(),
						    Transition.mSubresources,
						    arda::EArdaRHIResourceState::Common);
						Recorded.mCommandList->CommitBarriers();
						if (bValidateResourceStates)
					...
					{
						Recorded.mCommandList->SetUAVBarriersForTexture(*Texture.GetTexture(), true);
					}
					Recorded.mCommandList->SetTextureState(*Texture.GetTexture(),
					    Transition.mSubresources,
				...
				for (const FARDGBufferTransition& Transition : Transitions.mBuffers)
				{
					FARDGBuffer& Buffer = Graph.mBuffers.Get(Transition.mBuffer);
					Recorded.mCommandList->BeginTrackingBufferState(*Buffer.GetBuffer(), Transition.mStateBefore);
					if (bValidateResourceStates)
					{
						CaptureBufferState(Recorded,
						    Pass,
					...
					if (Transition.mbForceBarrier)
					{
						Recorded.mCommandList->SetBufferState(*Buffer.GetBuffer(), arda::EArdaRHIResourceState::Common);
						Recorded.mCommandList->CommitBarriers();
						if (bValidateResourceStates)
						{
							CaptureBufferState(Recorded,
					...
					if (IsUAVState(Transition.mStateAfter))
					{
						Recorded.mCommandList->SetUAVBarriersForBuffer(*Buffer.GetBuffer(), true);
					}
					Recorded.mCommandList->SetBufferState(*Buffer.GetBuffer(), Transition.mStateAfter);
					if (bValidateResourceStates)
					{
						CaptureBufferState(Recorded,
						    Pass,
				...
				for (const FARDGAccelStructTransition& Transition : Transitions.mAccelStructs)
				{
					FARDGAccelStruct& AccelStruct = Graph.mAccelStructs.Get(Transition.mAccelStruct);
					if (Transition.mbForceBarrier)
					{
						Recorded.mCommandList->SetAccelStructState(*AccelStruct.GetAccelStruct(),
						    arda::EArdaRHIResourceState::Common);
						Recorded.mCommandList->CommitBarriers();
					}
					Recorded.mCommandList->SetAccelStructState(*AccelStruct.GetAccelStruct(), Transition.mStateAfter);
				}
				Recorded.mCommandList->CommitBarriers();
				...
				for (const FARDGPassTextureState& Clobber : Transitions.mTextureClobbers)
				{
					FARDGTexture& Texture = Graph.mTextures.Get(Clobber.mTexture);
					const arda::FArdaRHIFormatInfo& Format = arda::GetArdaRHIFormatInfo(Texture.GetDesc().mFormat);
					...
					{
						Recorded.mCommandList->ClearTextureUInt(*Texture.GetTexture(),
						    Clobber.mSubresources,
						    0xCDCDCDCDu);
					}
					else
					{
						Recorded.mCommandList->ClearTexture(*Texture.GetTexture(),
						    Clobber.mSubresources,
						    arda::FArdaRHIColor{1.0f, 0.0f, 1.0f, 1.0f});
					}
					++Recorded.mClobberedResourceCount;
				}
				for (const FARDGPassBufferState& Clobber : Transitions.mBufferClobbers)
				{
					FARDGBuffer& Buffer = Graph.mBuffers.Get(Clobber.mBuffer);
					Recorded.mCommandList->ClearBufferUInt(*Buffer.GetBuffer(), 0xCDCDCDCDu);
					++Recorded.mClobberedResourceCount;
				}
			}
			...
			if (!Pass.GetState().mbSentinel)
			{
				...
				FARDGPassExecutionContext Context(Builder, Handle, *Recorded.mCommandList, Pass.GetState().mPipeline);
			...
			if (bValidateResourceStates)
			{
				for (const FARDGTextureTransition& State : Transitions.mTextures)
				{
					FARDGTexture& Texture = Graph.mTextures.Get(State.mTexture);
					CaptureTextureState(Recorded,
					    Pass,
					    Handle,
					    Texture,
				...
				for (const FARDGBufferTransition& State : Transitions.mBuffers)
				{
					FARDGBuffer& Buffer = Graph.mBuffers.Get(State.mBuffer);
					CaptureBufferState(Recorded,
					    Pass,
					    Handle,
					    Buffer,
			...
			{
				FARDGTexture& Texture = Graph.mTextures.Get(Transfer.mTexture);
				if (!Recorded.mCommandList->SetTextureState(*Texture.GetTexture(),
				        Transfer.mSubresources,
				        arda::EArdaRHIResourceState::Common))
				{
					ARDA_CHECK_MSG("The RHI failed to transition an RDG texture to its queue-release state.");
				}
				Recorded.mCommandList->CommitBarriers();
				...
				if (!Recorded.mCommandList->TransitionTexture(*Texture.GetTexture(), Release))
				{
					ARDA_CHECK_MSG("The RHI failed to encode an RDG texture queue release.");
				}
				if (bValidateResourceStates)
				{
					CaptureTextureState(Recorded,
			...
			{
				FARDGBuffer& Buffer = Graph.mBuffers.Get(Transfer.mBuffer);
				if (!Recorded.mCommandList->SetBufferState(*Buffer.GetBuffer(), arda::EArdaRHIResourceState::Common))
				{
					ARDA_CHECK_MSG("The RHI failed to transition an RDG buffer to its queue-release state.");
				}
				Recorded.mCommandList->CommitBarriers();
				...
				Release.mDestinationQueue = Transfer.mDestinationQueue;
				Release.mbQueueOwnershipTransfer = true;
				if (!Recorded.mCommandList->TransitionBuffer(*Buffer.GetBuffer(), Release))
				{
					ARDA_CHECK_MSG("The RHI failed to encode an RDG buffer queue release.");
				}
				if (bValidateResourceStates)
				{
					CaptureBufferState(Recorded,
         ...
         */
		[[nodiscard]] arda::TArdaRHIResult<uint64_t> UploadUniformBuffers(FARDGBuilder::FImpl& Graph)
		{
			...
			auto CommandListResult = Graph.mContext.mDevice->CreateCommandList(arda::EArdaRHIQueueType::Graphics);
			if (!CommandListResult)
			{
				return {0, CommandListResult.mStatus};
			}
			arda::FArdaRHICommandListRef CommandList = eastl::move(CommandListResult.mValue);
			if (auto Status = CommandList->Open(); !Status)
			{
				return {0, Status};
			}
			for (const FARDGUniformBuffer* UniformBuffer : Graph.mUniformBuffers.GetEntries())
			{
				CommandList->BeginTrackingBufferState(*UniformBuffer->GetBuffer(),
				    NormalizeInitialState(UniformBuffer->GetDesc().mInitialState));
				if (auto Status = CommandList->WriteBuffer(*UniformBuffer->GetBuffer(),
				...
				CommandList->SetBufferState(*UniformBuffer->GetBuffer(), arda::EArdaRHIResourceState::ConstantBuffer);
			}
		...
		void CompleteExtractions(FARDGBuilder::FImpl& Graph)
		{
			...
			{
				*Extraction.mOutput = Extraction.mAccelStruct->GetAccelStruct();
			}
		}
	}
     ...
     */
	const FARDGExecutionResult& FARDGExecutor::Execute(FARDGBuilder& Builder, const FARDGExecuteOptions& Options)
	{
		...
		{
			ARDA_CHECK_MSG("Render-graph execution requires an RHI device.");
		}
		...
		Graph.mExecutionResult.mbUsedImmediateMode = Graph.mContext.mDebugOptions.mbImmediateMode;
		...
		MaterializeResources(Graph, *Graph.mContext.mDevice, Graph.mExecutionResult, AliasingTransitions);
		eastl::vector<FARDGRuntimePassTransitions> RuntimeTransitions = BuildPhysicalTransitions(Graph);
		for (const FARDGAliasingTransition& Alias : AliasingTransitions)
		{
			if (Alias.mExecutionIndex >= Graph.mCompileResult.mExecutionOrder.size())
			{
				ARDA_CHECK_MSG("An RDG aliasing transition has an invalid execution index.");
			}
			const FARDGPassHandle Pass = Graph.mCompileResult.mExecutionOrder[Alias.mExecutionIndex];
		...
		for (auto Handle : Graph.mCompileResult.mExecutionOrder)
		{
			...
			bool bMerge = Options.mbMergeRasterPasses && !Graph.mContext.mDebugOptions.mbImmediateMode &&
			    Previous.IsValid() && Pass.GetState().mRasterGroup != UINT32_MAX &&
			...
			if (bMerge)
			{
				...
				bMerge = Next.mBuffers.empty() && Next.mAccelStructs.empty() && Next.mTextureAcquires.empty() &&
				    Next.mBufferAcquires.empty() && Next.mAliasingResources.empty() && Next.mTextureClobbers.empty() &&
				    Next.mTextureInitializations.empty() && Next.mBufferClobbers.empty() &&
				    Prior.mTextureReleases.empty() && Prior.mBufferReleases.empty() &&
				    eastl::none_of(Graph.mExecutionResult.mQueueDependencies.begin(),
				        Graph.mExecutionResult.mQueueDependencies.end(),
				        [Handle](const auto& Edge)
				        {
					        return Edge.mConsumer == Handle;
				        }) &&
				    eastl::all_of(Next.mTextures.begin(),
				        Next.mTextures.end(),
				        [&Prior](const auto& Transition)
				        {
					        return Transition.mStateBefore == Transition.mStateAfter && !Transition.mbUAVBarrier &&
					            !Transition.mbForceBarrier &&
					            eastl::any_of(Prior.mTextures.begin(),
					                Prior.mTextures.end(),
		...
		for (FARDGPassHandle Handle : Graph.mCompileResult.mExecutionOrder)
		{
			...
			auto AccumulateLevel = [&Graph, &Levels, &Level](FARDGPassHandle Producer)
			{
				const FARDGPass* ProducerPass = Graph.mPasses.TryGet(Producer);
				if (ProducerPass != nullptr && !ProducerPass->GetState().mbCulled)
				{
			...
			for (FARDGPassHandle Producer : Pass.GetState().mSynchronizationProducers)
			{
				AccumulateLevel(Producer);
			}
		...
		const auto RecordBatch = [&](FARDGPassHandle Head)
		{
			...
			{
				Output = RecordPass(Builder,
				    Graph,
				    Member,
				    RuntimeTransitions[Member.GetIndex()],
		...
		for (uint32_t Level = 0; Level <= MaxLevel; ++Level)
		{
			...
			for (FARDGPassHandle Handle : Graph.mCompileResult.mExecutionOrder)
			{
				...
				if (!Graph.mContext.mDebugOptions.mbImmediateMode && Options.mbParallelRecording &&
				    !HasAllFlags(Pass.GetFlags(), EARDGPassFlags::NeverParallel))
				{
		...
		eastl::array<bool, arda::ArdaRHIQueueTypeCount> bUploadWaited{};
		...
		for (FARDGPassHandle Handle : Graph.mCompileResult.mExecutionOrder)
		{
			...
			for (FARDGStateConformanceRecord& Record : Pass.mStateConformanceRecords)
			{
			...
			if (UploadInstance != 0 && Pass.mQueue != arda::EArdaRHIQueueType::Graphics &&
			    !bUploadWaited[ConsumerQueueIndex])
			{
				RequiredInstances[arda::GetArdaRHIQueueIndex(arda::EArdaRHIQueueType::Graphics)] = UploadInstance;
				bUploadWaited[ConsumerQueueIndex] = true;
			}

			for (const FARDGQueueDependency& Dependency : Graph.mExecutionResult.mQueueDependencies)
			{
				if (Dependency.mConsumer != Handle)
				{
					continue;
				}
				const uint64_t ProducerInstance = PassInstances[Dependency.mProducer.GetIndex()];
				if (ProducerInstance == 0)
				{
					continue;
				}
				auto& Required =
				    RequiredInstances[arda::GetArdaRHIQueueIndex(GetCommandQueue(Dependency.mProducerPipeline))];
			...
			const auto SubmitResult = Graph.mContext.mDevice->ExecuteCommandList(Pass.mCommandList);
			if (!SubmitResult)
			{
			...
			Graph.mExecutionResult.mClobberedResourceCount += Pass.mClobberedResourceCount;
		}
		...
		ARDA_TRACE_COUNTER("ARDG Submitted Command Lists", Graph.mExecutionResult.mSubmittedCommandListCount);
		ARDA_TRACE_COUNTER("ARDG Queue Waits", Graph.mExecutionResult.mQueueWaitCount);
		ARDA_TRACE_COUNTER("ARDG Clobbered Resources", Graph.mExecutionResult.mClobberedResourceCount);
		return Graph.mExecutionResult;
	}
}
