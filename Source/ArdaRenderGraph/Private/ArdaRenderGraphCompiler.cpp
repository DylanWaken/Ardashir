...
namespace arda
{
	namespace
	{
		    ...
		    arda::EArdaRHIResourceState Required)
		{
			...
			ARDA_CHECK_MSG("A render-graph pass declares conflicting states for one resource.");
		}
		...
		[[nodiscard]] bool IsCopyCompatible(const FARDGPass& Pass) noexcept
		{
			...
			for (const FARDGPassTextureState& State : Pass.GetState().mTextureStates)
			{
			...
			for (const FARDGPassBufferState& State : Pass.GetState().mBufferStates)
			{
				const uint32_t Value = static_cast<uint32_t>(State.mState);
				if (Value == 0 || (Value & ~CopyMask) != 0)
				{
         ...
         */
		[[nodiscard]] bool IsAsyncComputeCompatible(const FARDGPass& Pass) noexcept
		{

			// Use the same queue-compatibility rule for textures and buffers.
			const auto IsCompatibleState = [](arda::EArdaRHIResourceState State)
			{
				const bool bPixelShaderResource =
				    (State & arda::EArdaRHIResourceState::PixelShaderResource) != arda::EArdaRHIResourceState::Unknown;
				const bool bNonPixelShaderResource = (State & arda::EArdaRHIResourceState::NonPixelShaderResource) !=
			...
			for (const FARDGPassAccelStructState& State : Pass.GetState().mAccelStructStates)
			{
				if (!IsCompatibleState(State.mState))
				{
					return false;
		...
		void AssignPipelines(FARDGBuilder::FImpl& Graph)
		{
			for (FARDGPass* Pass : Graph.mPasses.GetEntries())
			{
				...
				const bool bHostBuffer = eastl::any_of(Pass->GetState().mBufferStates.begin(),
				    Pass->GetState().mBufferStates.end(),
				    [&Graph](const auto& Access)
				    {
					    return Graph.mBuffers.Get(Access.mBuffer).GetDesc().mCpuAccess != arda::EArdaRHICpuAccess::None;
				    });
				...
				const bool bDepthTransfer = eastl::any_of(Pass->GetState().mTextureStates.begin(),
				    Pass->GetState().mTextureStates.end(),
				    [&Graph](const auto& Access)
				    {
					    const auto& Format =
					        arda::GetArdaRHIFormatInfo(Graph.mTextures.Get(Access.mTexture).GetDesc().mFormat);
					    return (Format.mbDepth || Format.mbStencil) &&
					        (static_cast<uint32_t>(Access.mState) & CopyMask) != 0;
				    });
				if (!Graph.mContext.mDebugOptions.mbImmediateMode && !bHostBuffer && !bDepthTransfer &&
				    !Pass->GetState().mbSentinel && HasAllFlags(Pass->GetFlags(), EARDGPassFlags::Copy) &&
				    Graph.mContext.mQueuePolicy.mbCopy && IsCopyCompatible(*Pass))
				{
					Pipeline = EARDGPipeline::Copy;
				}
				else if (!Graph.mContext.mDebugOptions.mbImmediateMode && !bHostBuffer && !bDepthTransfer &&
				    !Pass->GetState().mbSentinel && HasAllFlags(Pass->GetFlags(), EARDGPassFlags::AsyncCompute) &&
				    Graph.mContext.mQueuePolicy.mbCompute && IsAsyncComputeCompatible(*Pass))
				{
		...
		void BuildConsumerEdges(FARDGBuilder::FImpl& Graph)
		{
			...
			{
				eastl::sort(Consumer->GetState().mProducers.begin(), Consumer->GetState().mProducers.end());
				for (FARDGPassHandle ProducerHandle : Consumer->GetState().mProducers)
				{
					FARDGPass* Producer = Graph.mPasses.TryGet(ProducerHandle);
					if (Producer == nullptr || Consumer->GetHandle() < ProducerHandle ||
					    Consumer->GetHandle() == ProducerHandle)
					{
						ARDA_CHECK_MSG("The render graph contains an invalid producer edge.");
					}
					Producer->AddConsumer(Consumer->GetHandle());
				}

				eastl::sort(Consumer->GetState().mSynchronizationProducers.begin(),
				    Consumer->GetState().mSynchronizationProducers.end());
				for (FARDGPassHandle ProducerHandle : Consumer->GetState().mSynchronizationProducers)
				{
					FARDGPass* Producer = Graph.mPasses.TryGet(ProducerHandle);
					if (Producer == nullptr || Consumer->GetHandle() < ProducerHandle ||
					    Consumer->GetHandle() == ProducerHandle)
					{
						ARDA_CHECK_MSG("The render graph contains an invalid synchronization edge.");
					}
		...
		{
			eastl::vector<uint32_t> ExecutionIndices(Graph.mPasses.GetCount(), InvalidGroup);
			for (uint32_t Index = 0; Index < Graph.mCompileResult.mExecutionOrder.size(); ++Index)
			{
				ExecutionIndices[Graph.mCompileResult.mExecutionOrder[Index].GetIndex()] = Index;
			}
			...
			for (const FARDGTexture* Texture : Graph.mTextures.GetEntries())
			{
				...
				Graph.mCompileResult.mResourceLifetimes.push_back({EARDGResourceType::Texture,
				    Texture->GetHandle().GetIndex(),
				    ExecutionIndices[Texture->GetFirstUse().GetIndex()],
				    ExecutionIndices[Texture->GetLastUse().GetIndex()],
				    HasAllFlags(Texture->GetFlags(), EARDGResourceFlags::Transient) && !Texture->IsExternal() &&
				        !Texture->IsExtracted()});
			}
			for (const FARDGBuffer* Buffer : Graph.mBuffers.GetEntries())
			{
				...
				Graph.mCompileResult.mResourceLifetimes.push_back({EARDGResourceType::Buffer,
				    Buffer->GetHandle().GetIndex(),
				    ExecutionIndices[Buffer->GetFirstUse().GetIndex()],
				    ExecutionIndices[Buffer->GetLastUse().GetIndex()],
				    HasAllFlags(Buffer->GetFlags(), EARDGResourceFlags::Transient) && !Buffer->IsExternal() &&
				        !Buffer->IsExtracted()});
			}
			for (const FARDGAccelStruct* AccelStruct : Graph.mAccelStructs.GetEntries())
			{
				...
				Graph.mCompileResult.mResourceLifetimes.push_back({EARDGResourceType::AccelStruct,
			...
			{
				const uint32_t Last = static_cast<uint32_t>(Graph.mCompileResult.mExecutionOrder.size() - 1u);
				for (FARDGResourceLifetime& Lifetime : Graph.mCompileResult.mResourceLifetimes)
				{
		...
		void CompileBarriers(FARDGBuilder::FImpl& Graph)
		{
			...
			for (FARDGPassHandle Handle : Graph.mCompileResult.mExecutionOrder)
			{
				...
				for (const FARDGPassTextureState& Access : Pass.GetState().mTextureStates)
				{
					...
					if (Required.empty())
					{
						Required.resize(static_cast<size_t>(Desc.mMipLevels) * Desc.mArraySize,
						    arda::EArdaRHIResourceState::Unknown);
					}
					const arda::FArdaRHITextureSubresourceRange Subresources = Access.mSubresources.Resolve(Desc);
				...
				for (uint32_t TextureIndex = 0; TextureIndex < RequiredTextures.size(); ++TextureIndex)
				{
					...
					const FARDGTexture& Texture = Graph.mTextures.Get(FARDGTextureHandle(TextureIndex));
					const arda::FArdaRHITextureDesc& Desc = Texture.GetDesc();
					auto& Current = TextureStates[TextureIndex];
					for (uint32_t ArraySlice = 0; ArraySlice < Desc.mArraySize; ++ArraySlice)
					{
						for (uint32_t MipLevel = 0; MipLevel < Desc.mMipLevels; ++MipLevel)
						{
							const size_t Index = static_cast<size_t>(ArraySlice) * Desc.mMipLevels + MipLevel;
							if (Required[Index] == arda::EArdaRHIResourceState::Unknown)
							{
								continue;
							}
							const bool bUAVBarrier = Current[Index] == Required[Index] && IsUAVState(Required[Index]);
							const bool bForceBarrier = Graph.mContext.mDebugOptions.mbConservativeBarriers &&
							    Current[Index] == Required[Index] && !bUAVBarrier &&
							    Required[Index] != arda::EArdaRHIResourceState::Common;
							Pass.GetState().mTextureTransitions.push_back({FARDGTextureHandle(TextureIndex),
							    arda::FArdaRHITextureSubresourceRange{MipLevel, 1, ArraySlice, 1},
							    Current[Index],
							    Required[Index],
							    bUAVBarrier,
							    bForceBarrier});
				...
				for (uint32_t BufferIndex = 0; BufferIndex < RequiredBuffers.size(); ++BufferIndex)
				{
					...
					const bool bForceBarrier = Graph.mContext.mDebugOptions.mbConservativeBarriers &&
					    BufferStates[BufferIndex] == RequiredBuffers[BufferIndex] && !bUAVBarrier &&
					    RequiredBuffers[BufferIndex] != arda::EArdaRHIResourceState::Common;
					Pass.GetState().mBufferTransitions.push_back({FARDGBufferHandle(BufferIndex),
				...
				for (uint32_t Index = 0; Index < RequiredAccelStructs.size(); ++Index)
				{
					...
					Pass.GetState().mAccelStructTransitions.push_back({FARDGAccelStructHandle(Index),
					    AccelStructStates[Index],
					    Required,
					    Graph.mContext.mDebugOptions.mbConservativeBarriers && AccelStructStates[Index] == Required});
					AccelStructStates[Index] = Required;
				}
			}

			// The synthetic epilogue owns graph-exit transitions and UAV ordering.
			FARDGPass& Epilogue = Graph.mPasses.Get(Graph.mCompileResult.mEpilogue);
			for (const FARDGTexture* Texture : Graph.mTextures.GetEntries())
			{
				...
				for (uint32_t ArraySlice = 0; ArraySlice < Desc.mArraySize; ++ArraySlice)
				{
					for (uint32_t MipLevel = 0; MipLevel < Desc.mMipLevels; ++MipLevel)
					{
						...
						if (Current[Index] != Texture->GetFinalState() || bUAVBarrier)
						{
							Epilogue.GetState().mTextureTransitions.push_back({Texture->GetHandle(),
							    arda::FArdaRHITextureSubresourceRange{MipLevel, 1, ArraySlice, 1},
							    Current[Index],
			...
			{
				if ((!AccelStruct->IsExternal() && !AccelStruct->IsExtracted()) ||
				    !AccelStruct->GetFirstUse().IsValid())
				{
					continue;
				}
				...
				if (AccelStructStates[Index] != AccelStruct->GetFinalState())
				{
					Epilogue.GetState().mAccelStructTransitions.push_back(
					    {AccelStruct->GetHandle(), AccelStructStates[Index], AccelStruct->GetFinalState(), false});
				}
			}
		}
		...
		void CompileQueueDependencies(FARDGBuilder::FImpl& Graph)
		{
			...
			for (FARDGPassHandle ConsumerHandle : Graph.mCompileResult.mExecutionOrder)
			{
				const FARDGPass& Consumer = Graph.mPasses.Get(ConsumerHandle);
				eastl::vector<FARDGPassHandle> Producers = Consumer.GetState().mProducers;
				Producers.insert(Producers.end(),
				    Consumer.GetState().mSynchronizationProducers.begin(),
				    Consumer.GetState().mSynchronizationProducers.end());
				eastl::sort(Producers.begin(), Producers.end());
				Producers.erase(eastl::unique(Producers.begin(), Producers.end()), Producers.end());
				for (FARDGPassHandle ProducerHandle : Producers)
				{
					const FARDGPass& Producer = Graph.mPasses.Get(ProducerHandle);
					if (Producer.GetState().mbCulled || Producer.GetState().mPipeline == Consumer.GetState().mPipeline)
					{
						continue;
					}
					Graph.mCompileResult.mQueueDependencies.push_back(
					    {ProducerHandle, ConsumerHandle, Producer.GetState().mPipeline, Consumer.GetState().mPipeline});
				}
			}
		}
		...
		void CompileAsyncMetadata(FARDGBuilder::FImpl& Graph)
		{
			for (FARDGPassHandle Handle : Graph.mCompileResult.mExecutionOrder)
			{
				...
				eastl::vector<FARDGPassHandle> ProducerWorklist = Pass.GetState().mProducers;
				ProducerWorklist.insert(ProducerWorklist.end(),
				...
				{
					const FARDGPassHandle ProducerHandle = ProducerWorklist.back();
				...
				eastl::vector<FARDGPassHandle> ConsumerWorklist = Pass.GetState().mConsumers;
				ConsumerWorklist.insert(ConsumerWorklist.end(),
				...
				{
					const FARDGPassHandle ConsumerHandle = ConsumerWorklist.back();
		...
		void CompileRasterGroups(FARDGBuilder::FImpl& Graph)
		{
			...
			for (FARDGPassHandle Handle : Graph.mCompileResult.mExecutionOrder)
			{
				...
				const bool bRaster = HasAllFlags(Pass.GetFlags(), EARDGPassFlags::Raster) &&
				...
				if (PreviousRaster == nullptr ||
				    PreviousRaster->GetState().mRasterBindings != Pass.GetState().mRasterBindings)
				{
					Pass.GetState().mRasterGroup = GroupCount++;
				}
				else
				{
					Pass.GetState().mRasterGroup = PreviousRaster->GetState().mRasterGroup;
				}
				PreviousRaster = &Pass;
			}
	...
	const FARDGCompileResult& FARDGCompiler::Compile(FARDGBuilder& Builder)
	{
		...
		for (const FARDGTexture* Texture : Graph.mTextures.GetEntries())
		{
			if (Texture->IsExtracted() && !Texture->IsExternal() && !Texture->GetLastProducer().IsValid())
			{
				ARDA_CHECK_MSG("A render-graph texture is extracted before it is produced.");
			}
		}
		for (const FARDGBuffer* Buffer : Graph.mBuffers.GetEntries())
		{
			if (Buffer->IsExtracted() && !Buffer->IsExternal() && !Buffer->GetLastProducer().IsValid())
			{
				ARDA_CHECK_MSG("A render-graph buffer is extracted before it is produced.");
			}
		}
		for (const FARDGAccelStruct* AccelStruct : Graph.mAccelStructs.GetEntries())
		{
			if (AccelStruct->IsExtracted() && !AccelStruct->IsExternal() && !AccelStruct->GetLastProducer().IsValid())
			{
				ARDA_CHECK_MSG("A render-graph acceleration structure is extracted before it is produced.");
			}
		}
		...
		Graph.mCompileResult.mEpilogue = Graph.mPasses.Emplace<FARDGSentinelPass>("GraphEpilogue");
		FARDGPass& Epilogue = Graph.mPasses.Get(Graph.mCompileResult.mEpilogue);

		for (const FARDGTexture* Texture : Graph.mTextures.GetEntries())
		{
			if ((Texture->IsExternal() || Texture->IsExtracted()) && Texture->GetLastProducer().IsValid())
			{
		...
		for (const FARDGAccelStruct* AccelStruct : Graph.mAccelStructs.GetEntries())
		{
			if ((AccelStruct->IsExternal() || AccelStruct->IsExtracted()) && AccelStruct->GetLastProducer().IsValid())
			{
		...
		for (const FARDGBuffer* Buffer : Graph.mBuffers.GetEntries())
		{
			if ((Buffer->IsExternal() || Buffer->IsExtracted()) && Buffer->GetLastProducer().IsValid())
			{
