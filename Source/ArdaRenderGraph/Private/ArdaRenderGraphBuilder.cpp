...
namespace arda
{
	namespace
	{
		...
		[[nodiscard]] EARDGBindingClass GetBindingClass(EARDGParameterType Type) noexcept
		{
			switch (Type)
			{
		...
		[[nodiscard]] bool IsCompatibleBindingType(EARDGParameterType ParameterType,
		    arda::EArdaRHIBindingType LayoutType) noexcept
		{
			...
			case EARDGParameterType::AccelStructAccess:
				return LayoutType == arda::EArdaRHIBindingType::RayTracingAccelStruct;
         ...
         */
		void AppendStateName(std::ostream& Stream, arda::EArdaRHIResourceState State)
		{
			struct FStateName
			{
				/**
			...
			static constexpr FStateName Names[] = {{arda::EArdaRHIResourceState::Common, "Common"},
         ...
         */
		[[nodiscard]] bool IsBuilding(const FARDGBuilder::FImpl& Graph) noexcept
		{
			return !Graph.mbCompiled && !Graph.mbCompiling && !Graph.mbExecutionStarted && !Graph.mbFailed;
		}
		...
		struct FARDGSetupContext
		{
			...
			{
				if (eastl::find(mPass.GetState().mViews.begin(), mPass.GetState().mViews.end(), View) ==
			    ...
			    bool bReadOnlyUav = false)
			{
				...
				{
					ARDA_CHECK_MSG("A pass references a texture owned by another render graph.");
				}
				if (State == arda::EArdaRHIResourceState::Unknown)
				{
					ARDA_CHECK_MSG("A pass declares a texture access with an unknown state.");
				}

				const bool bWrite = IsWriteState(State) && !bReadOnlyUav;
				mPass.AddTextureState({Texture->GetHandle(), Subresources, State, bWrite});
             ...
             */
			void AddBuffer(FARDGBufferRef Buffer,
			    arda::EArdaRHIResourceState State,
			    ...
			    bool bReadOnlyUav = false)
			{
				...
				{
					ARDA_CHECK_MSG("A pass references a buffer owned by another render graph.");
				}
				if (State == arda::EArdaRHIResourceState::Unknown)
				{
					ARDA_CHECK_MSG("A pass declares a buffer access with an unknown state.");
				}
			...
			void AddAccelStruct(FARDGAccelStructRef AccelStruct, arda::EArdaRHIResourceState State)
			{
				...
				{
					ARDA_CHECK_MSG("A pass references an acceleration structure owned by another render graph.");
				}
				if (State == arda::EArdaRHIResourceState::Unknown)
				{
					ARDA_CHECK_MSG("A pass declares acceleration-structure access with an unknown state.");
				}
				const bool bWrite = IsWriteState(State);
				mPass.AddAccelStructState({AccelStruct->GetHandle(), State, bWrite});
			...
			void AddUniformBuffer(FARDGUniformBufferRef UniformBuffer)
			{
				...
				{
					ARDA_CHECK_MSG("A pass references a uniform buffer owned by another render graph.");
				}
				if (!mVisitedUniformBuffers.insert(UniformBuffer->GetHandle().GetIndex()).second)
				{
					return;
				}
				mPass.GetState().mUniformBuffers.push_back(UniformBuffer->GetHandle());
				if (UniformBuffer->GetMetadata() != nullptr && UniformBuffer->GetContents() != nullptr)
				{
					Visit(*UniformBuffer->GetMetadata(), UniformBuffer->GetContents());
				}
			}
			...
			void Visit(const FARDGParameterMetadata& Metadata, const void* Parameters)
			{
				Metadata.Enumerate(Parameters,
				    ...
				    [this](const FARDGParameter& Parameter)
				    {
					    ...
					    case EARDGParameterType::Texture:
						    AddTexture(Parameter.GetValue<FARDGTextureRef>(), Member.mDefaultState);
						    break;

					    case EARDGParameterType::Buffer:
						    AddBuffer(Parameter.GetValue<FARDGBufferRef>(), Member.mDefaultState);
						    break;

					    case EARDGParameterType::TextureShaderResourceView:
					    {
						    const FARDGTextureSRVRef View = Parameter.GetValue<FARDGTextureSRVRef>();
						    if (View == nullptr)
						    {
							    break;
						    }
						    ...
						    {
							    ARDA_CHECK_MSG("A pass references a texture SRV owned by another graph.");
						    }
						    AddView(View->GetHandle());
						    AddTexture(mTextures.TryGet(View->GetDesc().mTexture),
					    ...
					    case EARDGParameterType::TextureUnorderedAccessView:
					    {
						    const FARDGTextureUAVRef View = Parameter.GetValue<FARDGTextureUAVRef>();
						    if (View == nullptr)
						    {
							    break;
						    }
						    if (mViews.TryGet(View->GetHandle()) != View)
						    {
							    ARDA_CHECK_MSG("A pass references a texture UAV owned by another graph.");
						    }
						    AddView(View->GetHandle());
						    AddTexture(mTextures.TryGet(View->GetDesc().mTexture),
						        Member.mDefaultState,
					    ...
					    case EARDGParameterType::BufferShaderResourceView:
					    {
						    const FARDGBufferSRVRef View = Parameter.GetValue<FARDGBufferSRVRef>();
						    if (View == nullptr)
						    {
							    break;
						    }
						    if (mViews.TryGet(View->GetHandle()) != View)
						    {
							    ARDA_CHECK_MSG("A pass references a buffer SRV owned by another graph.");
						    }
						    AddView(View->GetHandle());
						    AddBuffer(mBuffers.TryGet(View->GetDesc().mBuffer),
						        Member.mDefaultState,
						        View->GetDesc().mRange);
						    break;
					    }
					    ...
					    case EARDGParameterType::BufferUnorderedAccessView:
					    {
						    const FARDGBufferUAVRef View = Parameter.GetValue<FARDGBufferUAVRef>();
						    if (View == nullptr)
						    {
							    break;
						    }
						    if (mViews.TryGet(View->GetHandle()) != View)
						    {
							    ARDA_CHECK_MSG("A pass references a buffer UAV owned by another graph.");
						    }
						    AddView(View->GetHandle());
						    AddBuffer(mBuffers.TryGet(View->GetDesc().mBuffer),
						        Member.mDefaultState,
					    ...
					    case EARDGParameterType::TextureAccess:
					    {
						    const FARDGTextureAccess& Access = Parameter.GetValue<FARDGTextureAccess>();
						    AddTexture(Access.mTexture, Access.mState, Access.mSubresources, Member.mbReadOnlyUav);
						    break;
					    }

					    case EARDGParameterType::BufferAccess:
					    {
						    const FARDGBufferAccess& Access = Parameter.GetValue<FARDGBufferAccess>();
					    ...
					    case EARDGParameterType::AccelStructAccess:
					    {
						    const FARDGAccelStructAccess& Access = Parameter.GetValue<FARDGAccelStructAccess>();
						    AddAccelStruct(Access.mAccelStruct, Access.mState);
						    break;
					    }

					    case EARDGParameterType::UniformBuffer:
						    AddUniformBuffer(Parameter.GetValue<FARDGUniformBufferRef>());
						    break;
					    ...
					    case EARDGParameterType::RenderTargetBindingSlots:
					    {
						    ...
						    {
							    const FARDGRenderTargetBinding& Binding = Bindings.mColor[Index];
							    if (Binding.mTexture != nullptr)
							    {
								    mPass.GetState().mRasterBindings.mColor[Index] = Binding.mTexture->GetHandle();
								    mPass.GetState().mRasterBindings.mColorSubresources[Index] = Binding.mSubresources;
								    AddTexture(Binding.mTexture,
						    ...
						    if (Bindings.mDepthStencil.mTexture != nullptr)
						    {
							    ...
							    mPass.GetState().mRasterBindings.mDepthStencilSubresources =
							        Bindings.mDepthStencil.mSubresources;
							    AddTexture(Bindings.mDepthStencil.mTexture,
		...
		{
			constexpr uint16_t KnownFlags = static_cast<uint16_t>(EARDGPassFlags::Raster) |
			...
			if ((static_cast<uint16_t>(Flags) & ~KnownFlags) != 0 || (bRaster && (bCompute || bAsync || bCopy)) ||
			    (bCopy && (bCompute || bAsync)) || (bAsync && !bCompute) ||
	    ...
	    : mImpl(eastl::make_unique<FImpl>(eastl::move(Context)))
	{
		...
		{
			ARDA_CHECK_MSG("A render graph requires graphics queue capability.");
		}
	}
     ...
     */
	FARDGPassExecutionContext::FARDGPassExecutionContext(FARDGBuilder& Graph,
	...
	arda::IArdaRHITexture* FARDGPassExecutionContext::GetTexture(FARDGTexture* Texture) const
	{
		return mGraph.ResolveTextureForPass(mPass, Texture);
	}

	/** Resolves the parent physical texture of a declared logical SRV. */
	arda::IArdaRHITexture* FARDGPassExecutionContext::GetTexture(FARDGTextureSRV* View) const
	{
		return mGraph.ResolveTextureViewForPass(mPass, View);
	}

	/** Resolves the parent physical texture of a declared logical UAV. */
	arda::IArdaRHITexture* FARDGPassExecutionContext::GetTexture(FARDGTextureUAV* View) const
	{
		return mGraph.ResolveTextureViewForPass(mPass, View);
	}

	/** Resolves a directly declared logical buffer during pass recording. */
	arda::IArdaRHIBuffer* FARDGPassExecutionContext::GetBuffer(FARDGBuffer* Buffer) const
	{
		return mGraph.ResolveBufferForPass(mPass, Buffer);
	}

	/** Resolves the parent physical buffer of a declared logical SRV. */
	arda::IArdaRHIBuffer* FARDGPassExecutionContext::GetBuffer(FARDGBufferSRV* View) const
	{
		return mGraph.ResolveBufferViewForPass(mPass, View);
	}

	/** Resolves the parent physical buffer of a declared logical UAV. */
	arda::IArdaRHIBuffer* FARDGPassExecutionContext::GetBuffer(FARDGBufferUAV* View) const
	{
		return mGraph.ResolveBufferViewForPass(mPass, View);
	}

	/** Resolves a declared logical uniform buffer's physical constant buffer. */
	arda::IArdaRHIBuffer* FARDGPassExecutionContext::GetUniformBuffer(FARDGUniformBuffer* UniformBuffer) const
	{
		return mGraph.ResolveUniformBufferForPass(mPass, UniformBuffer);
	}

	arda::IArdaRHIAccelStruct* FARDGPassExecutionContext::GetAccelStruct(FARDGAccelStruct* AccelStruct) const
	{
	    ...
	    arda::IArdaRHIBindingLayout* BindingLayout) const
	{
		...
		const arda::FArdaShaderStructStatus Status = ShaderParameters.BuildBindingLayoutDescs(Generated);
		if (!Status)
		{
			ARDA_CHECK_MSG("Registered shader parameter metadata is invalid.");
		}
		const bool bMatches = eastl::any_of(Generated.begin(),
		    Generated.end(),
		...
		if (!bMatches)
		{
			ARDA_CHECK_MSG("The supplied binding layout was not generated from the registered shader metadata.");
		}
		return mGraph.CreateBindingSetForPass(mPass, ShaderParameters, BindingLayout);
	}
	...
	{
		const arda::FArdaShaderParameterMetadata* Metadata = Shader.GetParameterMetadata();
		if (Metadata == nullptr || LayoutIndex >= Shader.GetBindingLayouts().size())
		{
			ARDA_CHECK_MSG("The registered shader has no requested parameter binding layout.");
		}
		return CreateBindingSet(*Metadata, Shader.GetBindingLayouts()[LayoutIndex].Get());
	}

	eastl::vector<arda::FArdaRHIBindingSetRef> FARDGPassExecutionContext::CreateBindingSets(
	    const arda::FArdaGlobalShaderInstance& Shader) const
	{
		...
		for (size_t LayoutIndex = 0; LayoutIndex < Shader.GetBindingLayouts().size(); ++LayoutIndex)
		{
	...
	void FARDGBuilder::BeginPassAccess(FARDGPassHandle Pass)
	{
		...
		if (!mImpl->mbExecutionStarted || mImpl->mbExecuted || mImpl->mbFailed || PassRecord == nullptr ||
		    PassRecord->GetState().mbCulled || PassRecord->GetState().mbSentinel ||
		    !mImpl->mActivePassAccess.insert(Pass.GetIndex()).second)
		{
			ARDA_CHECK_MSG("A render-graph pass physical-access gate cannot be opened.");
		}
	}
     ...
     */
	arda::IArdaRHITexture* FARDGBuilder::ResolveTextureForPass(FARDGPassHandle Pass, FARDGTexture* Texture) const
	{
		std::lock_guard<std::mutex> Lock(mImpl->mPassAccessMutex);
		const FARDGPass* PassRecord = mImpl->mPasses.TryGet(Pass);
		...
		{
			ARDA_CHECK_MSG("A pass requested an unavailable render-graph texture.");
		}
		...
		if (!bDeclared)
		{
			ARDA_CHECK_MSG("A pass requested a texture absent from its parameter declarations.");
		}
     ...
     */
	arda::IArdaRHITexture* FARDGBuilder::ResolveTextureViewForPass(FARDGPassHandle Pass, FARDGView* View) const
	{
		if (View == nullptr || mImpl->mViews.TryGet(View->GetHandle()) != View)
		{
			ARDA_CHECK_MSG("A pass requested a texture view owned by another graph.");
		}
		{
			std::lock_guard<std::mutex> Lock(mImpl->mPassAccessMutex);
			const FARDGPass* PassRecord = mImpl->mPasses.TryGet(Pass);
			if (PassRecord == nullptr ||
			    mImpl->mActivePassAccess.find(Pass.GetIndex()) == mImpl->mActivePassAccess.end() ||
			    eastl::find(PassRecord->GetState().mViews.begin(),
			        PassRecord->GetState().mViews.end(),
			        View->GetHandle()) == PassRecord->GetState().mViews.end())
			{
				ARDA_CHECK_MSG("A pass requested a texture view absent from its parameters.");
			}
		}
		FARDGTextureHandle Texture;
		if (View->GetType() == EARDGResourceType::TextureShaderResourceView)
		{
			Texture = static_cast<FARDGTextureSRV*>(View)->GetDesc().mTexture;
		}
		else if (View->GetType() == EARDGResourceType::TextureUnorderedAccessView)
		{
			Texture = static_cast<FARDGTextureUAV*>(View)->GetDesc().mTexture;
		}
		else
		{
			ARDA_CHECK_MSG("A pass requested a non-texture view as a texture.");
		}
     ...
     */
	arda::IArdaRHIBuffer* FARDGBuilder::ResolveBufferForPass(FARDGPassHandle Pass, FARDGBuffer* Buffer) const
	{
		std::lock_guard<std::mutex> Lock(mImpl->mPassAccessMutex);
		const FARDGPass* PassRecord = mImpl->mPasses.TryGet(Pass);
		...
		{
			ARDA_CHECK_MSG("A pass requested an unavailable render-graph buffer.");
		}
		...
		if (!bDeclared)
		{
			ARDA_CHECK_MSG("A pass requested a buffer absent from its parameter declarations.");
		}
		return Buffer->GetBuffer().Get();
	}

	arda::IArdaRHIAccelStruct* FARDGBuilder::ResolveAccelStructForPass(FARDGPassHandle Pass,
     ...
     */
	arda::IArdaRHIBuffer* FARDGBuilder::ResolveBufferViewForPass(FARDGPassHandle Pass, FARDGView* View) const
	{
		if (View == nullptr || mImpl->mViews.TryGet(View->GetHandle()) != View)
		{
			ARDA_CHECK_MSG("A pass requested a buffer view owned by another graph.");
		}
		{
			std::lock_guard<std::mutex> Lock(mImpl->mPassAccessMutex);
			const FARDGPass* PassRecord = mImpl->mPasses.TryGet(Pass);
			if (PassRecord == nullptr ||
			    ...
			    eastl::find(PassRecord->GetState().mViews.begin(),
			        ...
			        View->GetHandle()) == PassRecord->GetState().mViews.end())
			{
				ARDA_CHECK_MSG("A pass requested a buffer view absent from its parameters.");
			}
		}
		FARDGBufferHandle Buffer;
		if (View->GetType() == EARDGResourceType::BufferShaderResourceView)
		{
			Buffer = static_cast<FARDGBufferSRV*>(View)->GetDesc().mBuffer;
		}
		else if (View->GetType() == EARDGResourceType::BufferUnorderedAccessView)
		{
			Buffer = static_cast<FARDGBufferUAV*>(View)->GetDesc().mBuffer;
		}
		else
		{
			ARDA_CHECK_MSG("A pass requested a non-buffer view as a buffer.");
		}
     ...
     */
	arda::IArdaRHIBuffer* FARDGBuilder::ResolveUniformBufferForPass(FARDGPassHandle Pass,
	    FARDGUniformBuffer* UniformBuffer) const
	{
		std::lock_guard<std::mutex> Lock(mImpl->mPassAccessMutex);
		const FARDGPass* PassRecord = mImpl->mPasses.TryGet(Pass);
		if (UniformBuffer == nullptr || PassRecord == nullptr ||
		    mImpl->mActivePassAccess.find(Pass.GetIndex()) == mImpl->mActivePassAccess.end() ||
		    mImpl->mUniformBuffers.TryGet(UniformBuffer->GetHandle()) != UniformBuffer || !UniformBuffer->GetBuffer())
		{
			ARDA_CHECK_MSG("A pass requested an unavailable graph uniform buffer.");
		}
		const bool bDeclared = eastl::find(PassRecord->GetState().mUniformBuffers.begin(),
		                           PassRecord->GetState().mUniformBuffers.end(),
		                           UniformBuffer->GetHandle()) != PassRecord->GetState().mUniformBuffers.end();
		if (!bDeclared)
		{
			ARDA_CHECK_MSG("A pass requested a uniform buffer absent from its parameters.");
		}
     ...
     */
	arda::FArdaRHIBindingSetRef FARDGBuilder::CreateBindingSetForPass(FARDGPassHandle Pass,
	    arda::IArdaRHIBindingLayout* BindingLayout) const
	{
	...
	arda::FArdaRHIBindingSetRef FARDGBuilder::CreateBindingSetForPass(FARDGPassHandle Pass,
	    ...
	    arda::IArdaRHIBindingLayout* BindingLayout) const
	{
		return CreateBindingSetForPassInternal(Pass, &ShaderParameters, BindingLayout);
	}

	arda::FArdaRHIBindingSetRef FARDGBuilder::CreateBindingSetForPassInternal(FARDGPassHandle Pass,
	    const arda::FArdaShaderParameterMetadata* ShaderParameters,
	    arda::IArdaRHIBindingLayout* BindingLayout) const
	{
		if (BindingLayout == nullptr)
		{
			ARDA_CHECK_MSG("Cannot create pass bindings from a null binding layout.");
		}
		...
		arda::IArdaRHIDevice* Device = nullptr;
		{
			...
			if (PassRecord == nullptr ||
			    mImpl->mActivePassAccess.find(Pass.GetIndex()) == mImpl->mActivePassAccess.end() ||
			    PassRecord->GetParameters() == nullptr || PassRecord->GetParameterMetadata() == nullptr ||
			    Device == nullptr)
			{
				ARDA_CHECK_MSG("Pass bindings require active parameters and an RHI device.");
			}
		}

		const arda::FArdaRHIBindingLayoutDesc& LayoutDesc = BindingLayout->GetDesc();

		auto MakeBufferItem = [](arda::EArdaRHIBindingType Type,
		                          uint32_t Slot,
		                          ...
		                          arda::FArdaRHIBufferRange Range)
		{
			...
			case arda::EArdaRHIBindingType::RawBufferUAV:
				return Item;
			default:
				ARDA_CHECK_MSG("A buffer parameter matched a non-buffer binding layout item.");
				return Item;
			}
		};
		...
		eastl::vector<uint32_t> MatchedElements(LayoutDesc.mItems.size(), 0u);
		...
		eastl::vector<arda::FArdaFlattenedShaderParameterMember> FlattenedShaderMembers;
		if (ShaderParameters != nullptr)
		{
			...
			PassRecord->GetParameterMetadata()->Enumerate(PassRecord->GetParameters(),
			    [&BindableParameters](const FARDGParameter& Parameter)
			    {
				    if (GetBindingClass(Parameter.mMember->mType) != EARDGBindingClass::None)
				    {
			...
			for (const arda::FArdaFlattenedShaderParameterMember& Resolved : FlattenedShaderMembers)
			{
				const arda::FArdaShaderParameterMember& Member = *Resolved.mMember;
				...
				for (uint32_t Element = 0; Element < Member.mArrayCount; ++Element)
				{
					while (NextParameter < BindableParameters.size() &&
					    !IsCompatibleBindingType(BindableParameters[NextParameter].mMember->mType, Member.mBindingType))
					{
						++NextParameter;
					}
					if (NextParameter == BindableParameters.size())
					{
						ARDA_CHECK_MSG("ARDG parameters cannot satisfy registered shader metadata.");
					}
					const FARDGParameter& Parameter = BindableParameters[NextParameter++];
					ShaderAssignments.push_back({Parameter.mPath, Parameter.mArrayIndex, &Member, Element});
				}
			}
		}
		...
		PassRecord->GetParameterMetadata()->Enumerate(PassRecord->GetParameters(),
		    ...
		    {
			    const EARDGParameterType ParameterType = Parameter.mMember->mType;
			    const EARDGBindingClass BindingClass = GetBindingClass(ParameterType);
			    ...
			    uint32_t Slot = 0;
			    uint32_t ArrayElement = static_cast<uint32_t>(Parameter.mArrayIndex);
			    const arda::FArdaShaderParameterMember* ShaderMember = nullptr;
			    if (ShaderParameters != nullptr)
			    {
				    const auto Assignment = eastl::find_if(ShaderAssignments.begin(),
				        ShaderAssignments.end(),
				        [&Parameter](const FShaderAssignment& Candidate)
				        {
					        return Candidate.mParameterPath == Parameter.mPath &&
					            Candidate.mParameterArrayIndex == Parameter.mArrayIndex;
				        });
			    ...
			    else
			    {
				    ...
				    Slot = *NextSlot;
				    if (Parameter.mArrayIndex + 1 == Parameter.mMember->mElementCount)
				    {
					    ++*NextSlot;
				    }
			    }
			    ...
			    {
				    const arda::FArdaRHIBindingLayoutItem& Candidate = LayoutDesc.mItems[Index];
				    if (Candidate.mSlot == Slot && IsCompatibleBindingType(ParameterType, Candidate.mType) &&
				        (ShaderMember == nullptr || Candidate.mType == ShaderMember->mBindingType))
				    {
					    LayoutIndex = Index;
					    break;
				    }
			    ...
			    const arda::FArdaRHIBindingLayoutItem& LayoutBinding = LayoutDesc.mItems[LayoutIndex];
			    if (ArrayElement >= LayoutBinding.mArraySize)
			    {
				    ARDA_CHECK_MSG("A pass parameter array exceeds its binding layout.");
			    }
			    ...
			    {
				    const FARDGTextureSRVRef View = Parameter.GetValue<FARDGTextureSRVRef>();
				    Item.mResource = arda::TArdaRHIRef<arda::IArdaRHIResource>(ResolveTextureViewForPass(Pass, View));
			    ...
			    case EARDGParameterType::Buffer:
			    {
				    const FARDGBufferRef Buffer = Parameter.GetValue<FARDGBufferRef>();
				    arda::IArdaRHIBuffer* Physical = ResolveBufferForPass(Pass, Buffer);
				    Item = MakeBufferItem(LayoutBinding.mType, Slot, Physical, Physical->GetDesc().mFormat, {});
				    break;
			    }
			    ...
			    {
				    const FARDGBufferSRVRef View = Parameter.GetValue<FARDGBufferSRVRef>();
				    Item = MakeBufferItem(LayoutBinding.mType,
				        Slot,
			    ...
			    case EARDGParameterType::UniformBuffer:
				    Item.mResource = arda::TArdaRHIRef<arda::IArdaRHIResource>(
				        ResolveUniformBufferForPass(Pass, Parameter.GetValue<FARDGUniformBufferRef>()));
				    break;
			    ...
			    case EARDGParameterType::AccelStructAccess:
			    {
				    const FARDGAccelStructAccess& Access = Parameter.GetValue<FARDGAccelStructAccess>();
				    Item.mResource =
				        arda::TArdaRHIRef<arda::IArdaRHIResource>(ResolveAccelStructForPass(Pass, Access.mAccelStruct));
				    break;
			    }
			    ...
			    {
				    ARDA_CHECK_MSG("A pass parameter does not match its binding layout type.");
			    }
			    Item.mArrayElement = ArrayElement;
		...
		if (ShaderParameters != nullptr)
		{
			...
			{
				const arda::FArdaShaderParameterMember& Member = *Resolved.mMember;
				if (Member.mKind != arda::EArdaShaderParameterKind::PushConstants ||
				    Member.mRegisterSpace != LayoutDesc.mRegisterSpace || Member.mVisibility != LayoutDesc.mVisibility)
				{
					continue;
				}
				for (size_t Index = 0; Index < LayoutDesc.mItems.size(); ++Index)
				{
					const arda::FArdaRHIBindingLayoutItem& LayoutItem = LayoutDesc.mItems[Index];
					if (LayoutItem.mSlot == Member.mSlot &&
					    LayoutItem.mType == arda::EArdaRHIBindingType::PushConstants)
					{
						arda::FArdaRHIBindingItem Item;
						Item.mSlot = Member.mSlot;
						Item.mType = arda::EArdaRHIBindingType::PushConstants;
		...
		for (size_t Index = 0; Index < LayoutDesc.mItems.size(); ++Index)
		{
			if (MatchedElements[Index] != LayoutDesc.mItems[Index].mArraySize)
			{
				ARDA_CHECK_MSG("A binding layout item has no matching pass parameter descriptor.");
			}
		}
		...
		if (!BindingSet)
		{
			ARDA_CHECK_MSG("The RHI failed to create a pass parameter binding set.");
		}
     ...
     */
	void FARDGBuilder::RegisterParameterDestructor(void* Object, void (*Destroy)(void*))
	{
	...
	{
		return mImpl->mParameterStorage.find(Parameters) != mImpl->mParameterStorage.end();
	}
     ...
     */
	FARDGTextureRef FARDGBuilder::CreateTexture(arda::FArdaRHITextureDesc Desc, EARDGResourceFlags Flags)
	{
		if (!IsBuilding(*mImpl))
		{
		...
		const uint8_t AllowedFlags = static_cast<uint8_t>(EARDGResourceFlags::Transient);
		if (Desc.mDebugName.empty() || Desc.mWidth == 0 || Desc.mHeight == 0 || Desc.mDepth == 0 ||
		    Desc.mArraySize == 0 || Desc.mMipLevels == 0 || (FlagValue & ~AllowedFlags) != 0)
		{
     ...
     */
	FARDGBufferRef FARDGBuilder::CreateBuffer(arda::FArdaRHIBufferDesc Desc, EARDGResourceFlags Flags)
	{
		if (!IsBuilding(*mImpl))
		{
		...
		const uint8_t FlagValue = static_cast<uint8_t>(Flags);
		const uint8_t AllowedFlags = static_cast<uint8_t>(EARDGResourceFlags::Transient);
		if (Desc.mDebugName.empty() || Desc.mByteSize == 0 || (FlagValue & ~AllowedFlags) != 0)
		{
	...
	FARDGAccelStructRef FARDGBuilder::CreateAccelStruct(arda::FArdaRHIAccelStructDesc Desc, EARDGResourceFlags Flags)
	{
		if (!IsBuilding(*mImpl) || Desc.mDebugName.empty() ||
		    (Desc.mbTopLevel ? Desc.mTopLevelMaxInstances == 0 : Desc.mBottomLevelGeometries.empty()) ||
		    HasAllFlags(Flags, EARDGResourceFlags::External))
		{
			ARDA_CHECK_MSG("Invalid logical acceleration-structure declaration.");
		}
		return &mImpl->mAccelStructs.Get(mImpl->mAccelStructs.Emplace(eastl::move(Desc), Flags));
	}
     ...
     */
	FARDGTextureSRVRef FARDGBuilder::CreateTextureSRV(eastl::string Name, FARDGTextureViewDesc Desc)
	{
		if (!IsBuilding(*mImpl))
		{
		...
		const FARDGViewHandle Handle = mImpl->mViews.Emplace<FARDGTextureSRV>(eastl::move(Name), eastl::move(Desc));
     ...
     */
	FARDGTextureUAVRef FARDGBuilder::CreateTextureUAV(eastl::string Name, FARDGTextureViewDesc Desc)
	{
		if (!IsBuilding(*mImpl))
		{
			ARDA_CHECK_MSG("Cannot create a view after graph compilation.");
		...
		const FARDGViewHandle Handle = mImpl->mViews.Emplace<FARDGTextureUAV>(eastl::move(Name), eastl::move(Desc));
     ...
     */
	FARDGBufferSRVRef FARDGBuilder::CreateBufferSRV(eastl::string Name, FARDGBufferViewDesc Desc)
	{
		if (!IsBuilding(*mImpl))
		{
			ARDA_CHECK_MSG("Cannot create a view after graph compilation.");
		...
		const FARDGViewHandle Handle = mImpl->mViews.Emplace<FARDGBufferSRV>(eastl::move(Name), eastl::move(Desc));
     ...
     */
	FARDGBufferUAVRef FARDGBuilder::CreateBufferUAV(eastl::string Name, FARDGBufferViewDesc Desc)
	{
		if (!IsBuilding(*mImpl))
		{
			ARDA_CHECK_MSG("Cannot create a view after graph compilation.");
		...
		const FARDGViewHandle Handle = mImpl->mViews.Emplace<FARDGBufferUAV>(eastl::move(Name), eastl::move(Desc));
     ...
     */
	FARDGTextureRef FARDGBuilder::RegisterExternalTexture(arda::FArdaRHITextureRef Texture,
	    ...
	    eastl::string Name)
	{
		...
		{
			ARDA_CHECK_MSG("An external RHI texture requires a reference and known initial state.");
		}
		...
		if (Existing != mImpl->mImportedTextures.end())
		{
			...
			{
				ARDA_CHECK_MSG("An imported texture was registered with conflicting initial states.");
			}
		...
		const FARDGTextureHandle Handle =
		    mImpl->mTextures.Emplace(eastl::move(Desc), EARDGResourceFlags::External, Texture);
     ...
     */
	FARDGBufferRef FARDGBuilder::RegisterExternalBuffer(arda::FArdaRHIBufferRef Buffer,
	    arda::EArdaRHIResourceState InitialState,
	    eastl::string Name)
	{
		if (!IsBuilding(*mImpl))
		...
		{
			ARDA_CHECK_MSG("An external RHI buffer requires a reference and known initial state.");
		}
		...
		if (Existing != mImpl->mImportedBuffers.end())
		{
			if (Existing->second->GetInitialState() != InitialState)
			{
				ARDA_CHECK_MSG("An imported buffer was registered with conflicting initial states.");
			}
			return Existing->second;
		}
		...
		Desc.mInitialState = InitialState;
		const FARDGBufferHandle Handle =
		    mImpl->mBuffers.Emplace(eastl::move(Desc), EARDGResourceFlags::External, Buffer);
	...
	FARDGAccelStructRef FARDGBuilder::RegisterExternalAccelStruct(arda::FArdaRHIAccelStructRef AccelStruct,
	    arda::EArdaRHIResourceState InitialState,
	    eastl::string Name)
	{
		if (!IsBuilding(*mImpl) || !AccelStruct || InitialState == arda::EArdaRHIResourceState::Unknown)
		{
			ARDA_CHECK_MSG("An external acceleration structure requires a reference and known initial state.");
		}
		const void* Identity = AccelStruct->GetPhysicalIdentity();
		if (!Identity)
		{
			ARDA_CHECK_MSG("An imported acceleration structure requires physical identity.");
		}
		...
		if (Existing != mImpl->mImportedAccelStructs.end())
		{
			if (Existing->second->GetInitialState() != InitialState)
			{
				ARDA_CHECK_MSG("An imported acceleration structure was registered with conflicting initial states.");
			}
			return Existing->second;
		}
		...
		if (Desc.mDebugName.empty())
		{
			Desc.mDebugName = "ExternalAccelStruct";
		}
		const FARDGAccelStructHandle Handle =
		    mImpl->mAccelStructs.Emplace(eastl::move(Desc), EARDGResourceFlags::External, AccelStruct, InitialState);
     ...
     */
	FARDGUniformBufferRef FARDGBuilder::CreateUniformBufferInternal(eastl::string Name,
	    size_t ByteSize,
	    const FARDGParameterMetadata* Metadata,
	    const void* Contents)
	{
		if (!IsBuilding(*mImpl) || Name.empty() || Metadata == nullptr || Contents == nullptr)
		{
		...
		const FARDGUniformBufferHandle Handle =
		    mImpl->mUniformBuffers.Emplace(Name, eastl::move(Desc), Metadata, Contents);
     ...
     */
	void FARDGBuilder::QueueTextureExtraction(FARDGTextureRef Texture,
	    arda::FArdaRHITextureRef* Output,
	    arda::EArdaRHIResourceState FinalState)
	{
		if (!IsBuilding(*mImpl) || Texture == nullptr || Output == nullptr ||
		...
		const auto Existing = eastl::find_if(mImpl->mTextureExtractions.begin(),
		    mImpl->mTextureExtractions.end(),
		    [Texture, Output](const FARDGTextureExtraction& Extraction)
		    {
			    return Extraction.mTexture == Texture || Extraction.mOutput == Output;
		    });
		if (Existing != mImpl->mTextureExtractions.end())
		{
			ARDA_CHECK_MSG("A logical texture extraction cannot be queued twice.");
		}
     ...
     */
	void FARDGBuilder::QueueBufferExtraction(FARDGBufferRef Buffer,
	    arda::FArdaRHIBufferRef* Output,
	    arda::EArdaRHIResourceState FinalState)
	{
		if (!IsBuilding(*mImpl) || Buffer == nullptr || Output == nullptr ||
		    mImpl->mBuffers.TryGet(Buffer->GetHandle()) != Buffer || FinalState == arda::EArdaRHIResourceState::Unknown)
		{
		...
		const auto Existing = eastl::find_if(mImpl->mBufferExtractions.begin(),
		    mImpl->mBufferExtractions.end(),
		    [Buffer, Output](const FARDGBufferExtraction& Extraction)
		    {
			    return Extraction.mBuffer == Buffer || Extraction.mOutput == Output;
		    });
		if (Existing != mImpl->mBufferExtractions.end())
		{
			ARDA_CHECK_MSG("A logical buffer extraction cannot be queued twice.");
		}
	...
	void FARDGBuilder::QueueAccelStructExtraction(FARDGAccelStructRef AccelStruct,
	    arda::FArdaRHIAccelStructRef* Output,
	    arda::EArdaRHIResourceState FinalState)
	{
		if (!IsBuilding(*mImpl) || AccelStruct == nullptr || Output == nullptr ||
		...
		const auto Existing = eastl::find_if(mImpl->mAccelStructExtractions.begin(),
		    mImpl->mAccelStructExtractions.end(),
		    [AccelStruct, Output](const FARDGAccelStructExtraction& Extraction)
		    {
			    return Extraction.mAccelStruct == AccelStruct || Extraction.mOutput == Output;
		    });
		if (Existing != mImpl->mAccelStructExtractions.end())
		{
			ARDA_CHECK_MSG("A logical acceleration-structure extraction cannot be queued twice.");
		}
		AccelStruct->AddFlags(EARDGResourceFlags::Extracted);
		AccelStruct->SetFinalState(FinalState);
		mImpl->mAccelStructExtractions.push_back({AccelStruct, Output, FinalState});
	}

	FARDGPassHandle FARDGBuilder::AddHostToDeviceCopyPass(FARDGBufferRef Destination,
	    const void* SourceData,
	    size_t Size,
	    uint64_t DestinationOffset,
	    eastl::string Name)
	{
		if (!Destination || !SourceData || Size == 0 || DestinationOffset > Destination->GetDesc().mByteSize ||
		...
		Parameters.mDestination = {Destination, arda::EArdaRHIResourceState::CopyDest, {DestinationOffset, Size}};
		return AddPass(eastl::move(Name),
		    &Parameters,
		    EARDGPassFlags::Copy | EARDGPassFlags::NeverCull | EARDGPassFlags::NeverParallel,
		    [Bytes = eastl::move(OwnedBytes), DestinationOffset](FARDGPassExecutionContext& Context,
		        const FARDGHostToDeviceCopyParameters& Frozen)
		    {
			    (void)Context.mCommandList.CopyBufferHostToDevice(*Context.GetBuffer(Frozen.mDestination.mBuffer),
			        Bytes.data(),
			        Bytes.size(),
			        DestinationOffset);
		    });
	...
	FARDGPassHandle FARDGBuilder::AddHostToDeviceCopyPassAsync(FARDGBufferRef Destination,
	    const void* SourceData,
	    size_t Size,
	    ...
	    eastl::string Name)
	{
		...
		return AddPass(eastl::move(Name),
		    &Parameters,
		    EARDGPassFlags::Copy | EARDGPassFlags::NeverCull | EARDGPassFlags::NeverParallel,
		    [Bytes = eastl::move(OwnedBytes), Callback = eastl::move(Completion), DestinationOffset](
		        FARDGPassExecutionContext& Context,
		        const FARDGHostToDeviceCopyParameters& Frozen) mutable
		    {
			    const auto Status =
			        Context.mCommandList.CopyBufferHostToDeviceAsync(*Context.GetBuffer(Frozen.mDestination.mBuffer),
			            Bytes.data(),
			            Bytes.size(),
			            Callback,
			            DestinationOffset);
	...
	FARDGPassHandle FARDGBuilder::AddDeviceToHostCopyPass(FARDGBufferRef Source,
	    ...
	    eastl::string Name)
	{
		...
		const uint64_t ResolvedSize =
		    Size == arda::ArdaRHIWholeBuffer ? Source->GetDesc().mByteSize - SourceOffset : Size;
		if (ResolvedSize == 0 || ResolvedSize > Source->GetDesc().mByteSize - SourceOffset)
		{
			ARDA_CHECK_MSG("Invalid device-to-host render-graph copy range.");
		}

		FARDGDeviceToHostCopyParameters Parameters;
		Parameters.mSource = {Source, arda::EArdaRHIResourceState::CopySource, {SourceOffset, ResolvedSize}};
		return AddPass(eastl::move(Name),
		    &Parameters,
		    EARDGPassFlags::Copy | EARDGPassFlags::NeverCull | EARDGPassFlags::NeverParallel,
		    [&Output, SourceOffset, ResolvedSize](FARDGPassExecutionContext& Context,
		        const FARDGDeviceToHostCopyParameters& Frozen)
		    {
			    const auto Status =
			        Context.mCommandList.CopyBufferDeviceToHost(*Context.GetBuffer(Frozen.mSource.mBuffer),
			            Output,
			            SourceOffset,
			            ResolvedSize);
			    if (!Status)
	...
	FARDGPassHandle FARDGBuilder::AddDeviceToHostCopyPassAsync(FARDGBufferRef Source,
	    arda::FArdaRHIDeviceToHostCopyCallback Completion,
	    uint64_t SourceOffset,
	    uint64_t Size,
	    eastl::string Name)
	{
		if (!Source || !Completion || SourceOffset > Source->GetDesc().mByteSize)
		{
		...
		return AddPass(eastl::move(Name),
		    &Parameters,
		    EARDGPassFlags::Copy | EARDGPassFlags::NeverCull | EARDGPassFlags::NeverParallel,
		    [Callback = eastl::move(Completion), SourceOffset, ResolvedSize](FARDGPassExecutionContext& Context,
		        const FARDGDeviceToHostCopyParameters& Frozen) mutable
		    {
			    const auto Status =
			        Context.mCommandList.CopyBufferDeviceToHostAsync(*Context.GetBuffer(Frozen.mSource.mBuffer),
			            Callback,
			            SourceOffset,
			            ResolvedSize);
			    if (!Status)
     ...
     */
	FARDGPassHandle FARDGBuilder::AddPassInternal(eastl::string Name,
	    ...
	    FARDGPassExecuteFunction Execute)
	{
		...
		const FARDGPassHandle Handle = mImpl->mPasses.Emplace<FARDGLambdaPass>(eastl::move(Name),
		    Flags,
		    Parameters,
		    Metadata,
		    eastl::move(Execute));
		...
		if (Metadata != nullptr)
		{
			FARDGSetupContext Setup{mImpl->mPasses,
			    mImpl->mTextures,
			    mImpl->mBuffers,
			    mImpl->mAccelStructs,
			    mImpl->mViews,
     ...
     */
	void FARDGBuilder::AddDependency(FARDGPassHandle Producer, FARDGPassHandle Consumer)
	{
		if (!IsBuilding(*mImpl))
		{
			ARDA_CHECK_MSG("Cannot add a dependency outside graph building.");
		}
		if (!Producer.IsValid() || !Consumer.IsValid() || Producer == Consumer || Consumer < Producer ||
		    mImpl->mPasses.TryGet(Producer) == nullptr || mImpl->mPasses.TryGet(Consumer) == nullptr)
		{
			ARDA_CHECK_MSG("Manual dependencies must name distinct registered passes in execution order.");
		}
	...
	const FARDGCompileResult& FARDGBuilder::Compile()
	{
		...
		{
			ARDA_CHECK_MSG("A render graph cannot compile in its current lifecycle state.");
		}
     ...
     */
	const FARDGExecutionResult& FARDGBuilder::Execute(const FARDGExecuteOptions& Options)
	{
     ...
     */
	const FARDGExecutionResult* FARDGBuilder::GetLastExecutionResult() const noexcept
	{
	...
	FARDGBlackboard& FARDGBuilder::GetBlackboard()
	{
		if (!IsBuilding(*mImpl))
		{
			ARDA_CHECK_MSG("Cannot mutate the render-graph blackboard after building.");
		}
	...
	const FARDGTexture* FARDGBuilder::TryGetTexture(FARDGTextureHandle Handle) const noexcept
	{
		return mImpl->mTextures.TryGet(Handle);
	}
	...
	const FARDGBuffer* FARDGBuilder::TryGetBuffer(FARDGBufferHandle Handle) const noexcept
	{
		return mImpl->mBuffers.TryGet(Handle);
	}

	FARDGAccelStruct* FARDGBuilder::TryGetAccelStruct(FARDGAccelStructHandle Handle) noexcept
	{
		return mImpl->mAccelStructs.TryGet(Handle);
	}

	const FARDGAccelStruct* FARDGBuilder::TryGetAccelStruct(FARDGAccelStructHandle Handle) const noexcept
	{
		return mImpl->mAccelStructs.TryGet(Handle);
	}
	...
	const FARDGView* FARDGBuilder::TryGetView(FARDGViewHandle Handle) const noexcept
	{
		return mImpl->mViews.TryGet(Handle);
	}

	/** Resolves a uniform-buffer handle to mutable logical storage, or null. */
	FARDGUniformBuffer* FARDGBuilder::TryGetUniformBuffer(FARDGUniformBufferHandle Handle) noexcept
	{
		return mImpl->mUniformBuffers.TryGet(Handle);
	}

	/** Resolves a uniform-buffer handle to immutable logical storage, or null. */
	const FARDGUniformBuffer* FARDGBuilder::TryGetUniformBuffer(FARDGUniformBufferHandle Handle) const noexcept
	{
		return mImpl->mUniformBuffers.TryGet(Handle);
	}
     ...
     */
	const eastl::vector<FARDGTextureExtraction>& FARDGBuilder::GetTextureExtractions() const noexcept
	{
     ...
     */
	const eastl::vector<FARDGBufferExtraction>& FARDGBuilder::GetBufferExtractions() const noexcept
	{
		return mImpl->mBufferExtractions;
	}

	const eastl::vector<FARDGAccelStructExtraction>& FARDGBuilder::GetAccelStructExtractions() const noexcept
	{
	...
	eastl::string FARDGBuilder::DumpGraph() const
	{
		...
		Stream << "Debug immediate=" << mImpl->mContext.mDebugOptions.mbImmediateMode
		       << " conservativeBarriers=" << mImpl->mContext.mDebugOptions.mbConservativeBarriers
		       << " extendedLifetimes=" << mImpl->mContext.mDebugOptions.mbExtendResourceLifetimes
		       << " clobberFirstWrites=" << mImpl->mContext.mDebugOptions.mbClobberFirstWrites << "\n";
		Stream << "ExecutionOrder [";
		for (size_t Index = 0; Index < mImpl->mCompileResult.mExecutionOrder.size(); ++Index)
		{
			if (Index != 0)
			{
				Stream << ",";
			}
			Stream << "P" << mImpl->mCompileResult.mExecutionOrder[Index].GetIndex();
		}
		Stream << "]\n";
		...
		{
			Stream << " T" << Texture->GetHandle().GetIndex() << " \"" << Texture->GetName().c_str() << "\""
			       << " external=" << Texture->IsExternal() << " extracted=" << Texture->IsExtracted()
		...
		{
			Stream << " B" << Buffer->GetHandle().GetIndex() << " \"" << Buffer->GetName().c_str() << "\""
			       << " external=" << Buffer->IsExternal() << " extracted=" << Buffer->IsExtracted()
		...
		for (const FARDGPass* Pass : mImpl->mPasses.GetEntries())
		{
			...
			Stream << " P" << Pass->GetHandle().GetIndex() << " \"" << Pass->GetName().c_str() << "\""
			       << " flags=" << static_cast<uint16_t>(Pass->GetFlags())
			       << " pipeline=" << GetPipelineName(State.mPipeline) << " culled=" << State.mbCulled
			       << " sentinel=" << State.mbSentinel << " rasterGroup=" << State.mRasterGroup
			       << " fork=" << State.mAsyncFork.GetIndex() << " join=" << State.mAsyncJoin.GetIndex()
			       << " producers=[";
			...
			Stream << "] sync=[";
			for (size_t Index = 0; Index < State.mSynchronizationProducers.size(); ++Index)
			{
				if (Index != 0)
				{
					Stream << ",";
			...
			Stream << "]\n";
			Stream << "  transitions textures=" << State.mTextureTransitions.size()
			       << " buffers=" << State.mBufferTransitions.size() << "\n";
			for (const FARDGTextureTransition& Transition : State.mTextureTransitions)
			{
				Stream << "   T" << Transition.mTexture.GetIndex() << " mip=" << Transition.mSubresources.mBaseMipLevel
				...
				Stream << " uav=" << Transition.mbUAVBarrier << " forced=" << Transition.mbForceBarrier << "\n";
			}
			for (const FARDGBufferTransition& Transition : State.mBufferTransitions)
			{
		...
		Stream << "Lifetimes " << mImpl->mCompileResult.mResourceLifetimes.size() << "\n";
		for (const FARDGResourceLifetime& Lifetime : mImpl->mCompileResult.mResourceLifetimes)
		{
			Stream << " " << (Lifetime.mType == EARDGResourceType::Texture ? "T" : "B") << Lifetime.mResourceIndex
			       << " first=" << Lifetime.mFirstUse << " last=" << Lifetime.mLastUse
			       << " transient=" << Lifetime.mbTransient << "\n";
		}
		Stream << "QueueDependencies " << mImpl->mCompileResult.mQueueDependencies.size() << "\n";
		for (const FARDGQueueDependency& Dependency : mImpl->mCompileResult.mQueueDependencies)
		{
			Stream << " P" << Dependency.mProducer.GetIndex() << "->P" << Dependency.mConsumer.GetIndex() << " "
