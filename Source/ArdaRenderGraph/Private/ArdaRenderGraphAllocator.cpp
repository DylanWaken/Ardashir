...
namespace arda
{
	    ...
	    bool bAllowAliasing)
	{
		...
		{
			if (Request.mSize == 0 || !IsPowerOfTwo(static_cast<size_t>(Request.mAlignment)) ||
			    Request.mFirstUse > Request.mLastUse)
			{
				ARDA_CHECK_MSG("Invalid render-graph transient allocation request.");
			}
			...
			if (bAllowAliasing)
			{
				...
				for (auto Iterator = Active.begin(); Iterator != Active.end();)
				{
					...
					{
						FreeRanges.push_back({Iterator->mOffset, Iterator->mSize});
				...
				{
					if (!Merged.empty() && Merged.back().mOffset + Merged.back().mSize == Range.mOffset)
					{
			...
			if (bAllowAliasing)
			{
				...
				for (size_t Index = 0; Index < FreeRanges.size(); ++Index)
				{
					...
					const uint64_t Candidate = AlignUp(Range.mOffset, Request.mAlignment);
					if (Candidate >= Range.mOffset && Candidate - Range.mOffset <= Range.mSize &&
					    Request.mSize <= Range.mSize - (Candidate - Range.mOffset))
					{
						Offset = Candidate;
						bReused = true;
						...
						{
							FreeRanges.push_back({Range.mOffset, Candidate - Range.mOffset});
						}
			...
			Layout.mCapacity = eastl::max(Layout.mCapacity, Offset + Request.mSize);
			Layout.mbContainsAliases |= bReused;
			Layout.mAllocations.push_back({Request.mIdentifier, Offset, Request.mSize, bReused});
			Active.push_back({Request.mLastUse, Offset, Request.mSize});
		}

		eastl::sort(Layout.mAllocations.begin(),
	...
	{
		for (auto DestructorIterator = mDestructors.rbegin(); DestructorIterator != mDestructors.rend();
		    ++DestructorIterator)
		{
		...
		for (auto BlockIterator = mBlocks.rbegin(); BlockIterator != mBlocks.rend(); ++BlockIterator)
		{
			::operator delete(BlockIterator->mMemory, std::align_val_t(BlockIterator->mAlignment));
		}
		mBlocks.clear();
		mObjectCount = 0;
	}
	...
	FARDGArena::FARDGBlock& FARDGArena::AddBlock(size_t MinimumSize, size_t Alignment)
	{
		...
		Block.mMemory = static_cast<std::byte*>(::operator new(Block.mCapacity, std::align_val_t(Block.mAlignment)));
