...
namespace arda
{
	...
	struct FARDGStateConformanceRecord
	{
		...
		EARDGStateCheckpoint mCheckpoint = EARDGStateCheckpoint::BeforeTransition;
	...
	public:
		...
		{
			static_assert(eastl::is_standard_layout_v<ParameterType>,
			    "Render-graph parameters must use standard layout.");

			void* Storage = AllocateParameterStorage(sizeof(ParameterType), alignof(ParameterType));
			ParameterType* Parameters = new (Storage) ParameterType(eastl::forward<ArgumentTypes>(Arguments)...);
			if constexpr (!eastl::is_trivially_destructible_v<ParameterType>)
			{
				RegisterParameterDestructor(Parameters,
				    [](void* Address)
				    {
		...
		[[nodiscard]] FARDGTextureRef CreateTexture(arda::FArdaRHITextureDesc Desc,
		    EARDGResourceFlags Flags = EARDGResourceFlags::Transient);

		/** Creates a deferred logical buffer. */
		[[nodiscard]] FARDGBufferRef CreateBuffer(arda::FArdaRHIBufferDesc Desc,
		    EARDGResourceFlags Flags = EARDGResourceFlags::Transient);

		/** Creates a deferred logical acceleration structure. */
		[[nodiscard]] FARDGAccelStructRef CreateAccelStruct(arda::FArdaRHIAccelStructDesc Desc,
		    EARDGResourceFlags Flags = EARDGResourceFlags::None);

		/** Creates a logical texture shader-resource view. */
		[[nodiscard]] FARDGTextureSRVRef CreateTextureSRV(eastl::string Name, FARDGTextureViewDesc Desc);

		/** Creates a logical texture unordered-access view. */
		[[nodiscard]] FARDGTextureUAVRef CreateTextureUAV(eastl::string Name, FARDGTextureViewDesc Desc);

		/** Creates a logical buffer shader-resource view. */
		[[nodiscard]] FARDGBufferSRVRef CreateBufferSRV(eastl::string Name, FARDGBufferViewDesc Desc);

		/** Creates a logical buffer unordered-access view. */
		[[nodiscard]] FARDGBufferUAVRef CreateBufferUAV(eastl::string Name, FARDGBufferViewDesc Desc);

		/** UE-style alias for creating a texture SRV. */
		[[nodiscard]] FARDGTextureSRVRef CreateSRV(eastl::string Name, FARDGTextureViewDesc Desc)
		{
			return CreateTextureSRV(eastl::move(Name), eastl::move(Desc));
		}

		/** UE-style alias for creating a buffer SRV. */
		[[nodiscard]] FARDGBufferSRVRef CreateSRV(eastl::string Name, FARDGBufferViewDesc Desc)
		{
			return CreateBufferSRV(eastl::move(Name), eastl::move(Desc));
		}

		/** UE-style alias for creating a texture UAV. */
		[[nodiscard]] FARDGTextureUAVRef CreateUAV(eastl::string Name, FARDGTextureViewDesc Desc)
		{
			return CreateTextureUAV(eastl::move(Name), eastl::move(Desc));
		}

		/** UE-style alias for creating a buffer UAV. */
		[[nodiscard]] FARDGBufferUAVRef CreateUAV(eastl::string Name, FARDGBufferViewDesc Desc)
		{
			return CreateBufferUAV(eastl::move(Name), eastl::move(Desc));
		}

		/** Imports an externally owned texture into the logical graph. */
		[[nodiscard]] FARDGTextureRef RegisterExternalTexture(arda::FArdaRHITextureRef Texture,
		    arda::EArdaRHIResourceState InitialState,
		    eastl::string Name = {});

		/** Imports an externally owned buffer into the logical graph. */
		[[nodiscard]] FARDGBufferRef RegisterExternalBuffer(arda::FArdaRHIBufferRef Buffer,
		    arda::EArdaRHIResourceState InitialState,
		    eastl::string Name = {});

		/** Imports an externally owned acceleration structure. */
		[[nodiscard]] FARDGAccelStructRef RegisterExternalAccelStruct(arda::FArdaRHIAccelStructRef AccelStruct,
		    arda::EArdaRHIResourceState InitialState,
		    eastl::string Name = {});

		/** Imports a texture using the initial state stored in its descriptor. */
		[[nodiscard]] FARDGTextureRef RegisterExternalTexture(arda::FArdaRHITextureRef Texture, eastl::string Name = {})
		{
			if (!Texture)
			{
				ARDA_CHECK_MSG("Cannot import a null RHI texture.");
			}
			return RegisterExternalTexture(Texture, Texture->GetDesc().mInitialState, eastl::move(Name));
		}

		/** Imports a buffer using the initial state stored in its descriptor. */
		[[nodiscard]] FARDGBufferRef RegisterExternalBuffer(arda::FArdaRHIBufferRef Buffer, eastl::string Name = {})
		{
			if (!Buffer)
			{
				ARDA_CHECK_MSG("Cannot import a null RHI buffer.");
			}
			return RegisterExternalBuffer(Buffer, Buffer->GetDesc().mInitialState, eastl::move(Name));
		}

		/** Creates a logical uniform buffer from graph-scoped parameter contents. */
		template <typename ParameterType>
		[[nodiscard]] FARDGUniformBufferRef CreateUniformBuffer(eastl::string Name, const ParameterType* Parameters)
		{
			if (Parameters == nullptr)
			{
				ARDA_CHECK_MSG("Cannot create a render-graph uniform buffer from null parameters.");
			}
			const ParameterType* FrozenParameters = FreezeParameters(Parameters);
			return CreateUniformBufferInternal(eastl::move(Name),
		...
		void QueueTextureExtraction(FARDGTextureRef Texture,
		...
		void QueueBufferExtraction(FARDGBufferRef Buffer,
		...
		void QueueAccelStructExtraction(FARDGAccelStructRef AccelStruct,
		...
		void QueueAccelStructExtraction(FARDGAccelStructRef AccelStruct,
		    ...
		    arda::EArdaRHIResourceState FinalState)
		{
			QueueAccelStructExtraction(AccelStruct, eastl::addressof(Output), FinalState);
		}
         ...
         */
		[[nodiscard]] FARDGPassHandle AddHostToDeviceCopyPass(FARDGBufferRef Destination,
		...
		[[nodiscard]] FARDGPassHandle AddHostToDeviceCopyPassAsync(FARDGBufferRef Destination,
		    const void* SourceData,
		    size_t Size,
		...
		[[nodiscard]] FARDGPassHandle AddDeviceToHostCopyPass(FARDGBufferRef Source,
		...
		[[nodiscard]] FARDGPassHandle AddDeviceToHostCopyPassAsync(FARDGBufferRef Source,
		...
		[[nodiscard]] FARDGPassHandle QueueBufferUpload(FARDGBufferRef Destination,
		    const void* SourceData,
		    size_t Size,
		    uint64_t DestinationOffset = 0,
		    eastl::string Name = "QueueBufferUpload")
		{
			return AddHostToDeviceCopyPass(Destination, SourceData, Size, DestinationOffset, eastl::move(Name));
		}

		/** Unreal-style alias for an asynchronous GPU buffer readback pass. */
		[[nodiscard]] FARDGPassHandle AddEnqueueCopyPass(FARDGBufferRef Source,
		    arda::FArdaRHIDeviceToHostCopyCallback Completion,
		    uint64_t SourceOffset = 0,
		    uint64_t Size = arda::ArdaRHIWholeBuffer,
		    eastl::string Name = "EnqueueBufferReadback")
		{
			return AddDeviceToHostCopyPassAsync(Source, eastl::move(Completion), SourceOffset, Size, eastl::move(Name));
		}
		...
		[[nodiscard]] FARDGPassHandle AddPass(eastl::string Name,
		    ...
		    ExecuteType&& Execute)
		{
			if (Parameters == nullptr)
			{
				ARDA_CHECK_MSG("Cannot register a render-graph pass with null parameters.");
			}
			const ParameterType* FrozenParameters = FreezeParameters(Parameters);
			using StoredExecuteType = eastl::decay_t<ExecuteType>;
			FARDGPassExecuteFunction ExecuteFunction =
			    [Function = StoredExecuteType(eastl::forward<ExecuteType>(Execute)), FrozenParameters](
			        FARDGPassExecutionContext& Context) mutable
			{
				InvokePassLambda(Function, Context, *FrozenParameters);
			};

			return AddPassInternal(eastl::move(Name),
			    Flags,
			    FrozenParameters,
			    &ParameterType::GetStaticMetadata(),
		...
		[[nodiscard]] FARDGPassHandle AddPass(eastl::string Name, EARDGPassFlags Flags, ExecuteType&& Execute)
		{
			using StoredExecuteType = eastl::decay_t<ExecuteType>;
			FARDGPassExecuteFunction ExecuteFunction = [Function = StoredExecuteType(eastl::forward<ExecuteType>(
			                                                Execute))](FARDGPassExecutionContext& Context) mutable
			{
				InvokeParameterlessPassLambda(Function, Context);
			};
			return AddPassInternal(eastl::move(Name), Flags, nullptr, nullptr, eastl::move(ExecuteFunction));
		}
         ...
         */
		template <typename ParameterType, typename ExecuteType>
		[[nodiscard]] FARDGPassHandle AddDispatchPass(eastl::string Name,
		    const ParameterType* Parameters,
		    ...
		    EARDGPassFlags Flags = EARDGPassFlags::Compute)
		{
			...
			return AddPass(eastl::move(Name),
			    Parameters,
			    Flags,
			    [Function = eastl::decay_t<ExecuteType>(eastl::forward<ExecuteType>(Setup)),
			        Dispatch](FARDGPassExecutionContext& Context, const ParameterType& FrozenParameters) mutable
			    {
				    InvokePassLambda(Function, Context, FrozenParameters);
				    if (Context.GetStatus())
				    {
					    Context.mUnsafeRawCommandList.Dispatch(Dispatch.mGroupCountX,
		...
		template <typename ParameterType, typename ExecuteType>
		[[nodiscard]] FARDGPassHandle AddRayDispatchPass(eastl::string Name,
		    const ParameterType* Parameters,
		    ...
		    EARDGPassFlags Flags = EARDGPassFlags::Compute)
		{
			...
			return AddPass(eastl::move(Name),
			        ...
			        Dispatch](FARDGPassExecutionContext& Context, const ParameterType& FrozenParameters) mutable
			    {
				    ...
				    if (Context.GetStatus())
				    {
					    Context.ReportStatus(Context.mUnsafeRawCommandList.DispatchRays(Dispatch.mWidth,
					        Dispatch.mHeight,
					        Dispatch.mDepth));
				    }
			    });
         ...
         */
		[[nodiscard]] const FARDGExecutionResult& Execute(const FARDGExecuteOptions& Options = {});

		/** Returns the builder-owned report, including failure, or null before Execute. */
		[[nodiscard]] const FARDGExecutionResult* GetLastExecutionResult() const noexcept;
		...
		[[nodiscard]] const FARDGTexture* TryGetTexture(FARDGTextureHandle Handle) const noexcept;
		...
		[[nodiscard]] const FARDGBuffer* TryGetBuffer(FARDGBufferHandle Handle) const noexcept;

		[[nodiscard]] FARDGAccelStruct* TryGetAccelStruct(FARDGAccelStructHandle Handle) noexcept;
		[[nodiscard]] const FARDGAccelStruct* TryGetAccelStruct(FARDGAccelStructHandle Handle) const noexcept;
		...
		[[nodiscard]] const FARDGView* TryGetView(FARDGViewHandle Handle) const noexcept;

		/** Returns a logical uniform buffer, or null for an invalid handle. */
		[[nodiscard]] FARDGUniformBuffer* TryGetUniformBuffer(FARDGUniformBufferHandle Handle) noexcept;

		/** Returns an immutable uniform buffer, or null for an invalid handle. */
		[[nodiscard]] const FARDGUniformBuffer* TryGetUniformBuffer(FARDGUniformBufferHandle Handle) const noexcept;

		/** Returns texture extraction declarations in registration order. */
		[[nodiscard]] const eastl::vector<FARDGTextureExtraction>& GetTextureExtractions() const noexcept;

		/** Returns buffer extraction declarations in registration order. */
		[[nodiscard]] const eastl::vector<FARDGBufferExtraction>& GetBufferExtractions() const noexcept;

		/** Returns acceleration-structure extraction declarations in registration order. */
		[[nodiscard]] const eastl::vector<FARDGAccelStructExtraction>& GetAccelStructExtractions() const noexcept;
	...
	private:
		...
		[[nodiscard]] FARDGUniformBufferRef CreateUniformBufferInternal(eastl::string Name,
		    size_t ByteSize,
		    const FARDGParameterMetadata* Metadata,
		    const void* Contents);

		[[nodiscard]] FARDGPassHandle AddPassInternal(eastl::string Name,
		    EARDGPassFlags Flags,
		...
		[[nodiscard]] arda::IArdaRHITexture* ResolveTextureForPass(FARDGPassHandle Pass, FARDGTexture* Texture) const;
		[[nodiscard]] arda::IArdaRHITexture* ResolveTextureViewForPass(FARDGPassHandle Pass, FARDGView* View) const;
		[[nodiscard]] arda::IArdaRHIBuffer* ResolveBufferForPass(FARDGPassHandle Pass, FARDGBuffer* Buffer) const;
		[[nodiscard]] arda::IArdaRHIBuffer* ResolveBufferViewForPass(FARDGPassHandle Pass, FARDGView* View) const;
		[[nodiscard]] arda::IArdaRHIAccelStruct* ResolveAccelStructForPass(FARDGPassHandle Pass,
		    FARDGAccelStruct* AccelStruct) const;
		[[nodiscard]] arda::IArdaRHIBuffer* ResolveUniformBufferForPass(FARDGPassHandle Pass,
		    FARDGUniformBuffer* UniformBuffer) const;
		[[nodiscard]] arda::FArdaRHIBindingSetRef CreateBindingSetForPass(FARDGPassHandle Pass,
		    arda::IArdaRHIBindingLayout* BindingLayout) const;
		[[nodiscard]] arda::FArdaRHIBindingSetRef CreateBindingSetForPass(FARDGPassHandle Pass,
		    const arda::FArdaShaderParameterMetadata& ShaderParameters,
		    arda::IArdaRHIBindingLayout* BindingLayout) const;
		[[nodiscard]] arda::FArdaRHIBindingSetRef CreateBindingSetForPassInternal(FARDGPassHandle Pass,
		    const arda::FArdaShaderParameterMetadata* ShaderParameters,
		    arda::IArdaRHIBindingLayout* BindingLayout) const;
		...
		static void InvokeCallback(FARDGPassExecutionContext& Context,
		    ExecuteType& Execute,
		...
		static void InvokePassLambda(ExecuteType& Execute,
		    FARDGPassExecutionContext& Context,
		    const ParameterType& Parameters)
		{
			if constexpr (eastl::is_invocable_v<ExecuteType&, FARDGPassExecutionContext&, const ParameterType&>)
			{
				InvokeCallback(Context, Execute, Context, Parameters);
			}
			else if constexpr (eastl::is_invocable_v<ExecuteType&, const ParameterType&, FARDGPassExecutionContext&>)
			{
				InvokeCallback(Context, Execute, Parameters, Context);
			}
			else if constexpr (eastl::is_invocable_v<ExecuteType&, arda::IArdaRHICommandList&, const ParameterType&>)
			{
				InvokeCallback(Context, Execute, Context.mUnsafeRawCommandList, Parameters);
			}
			else if constexpr (eastl::is_invocable_v<ExecuteType&, const ParameterType&, arda::IArdaRHICommandList&>)
			{
				InvokeCallback(Context, Execute, Parameters, Context.mUnsafeRawCommandList);
			}
			else if constexpr (eastl::is_invocable_v<ExecuteType&, FARDGPassExecutionContext&>)
			{
			...
			{
				static_assert(eastl::is_invocable_v<ExecuteType&>, "Unsupported render-graph pass lambda signature.");
			}
		}

		template <typename ExecuteType>
		static void InvokeParameterlessPassLambda(ExecuteType& Execute, FARDGPassExecutionContext& Context)
		{
