...
namespace arda
{
	...
	struct FARDGRasterBindingSignature
	{
		...
		eastl::array<arda::FArdaRHITextureSubresourceRange, arda::ArdaRHIMaxRenderTargets> mColorSubresources;
		...
		friend bool operator==(const FARDGRasterBindingSignature& Left,
		    const FARDGRasterBindingSignature& Right) noexcept
		{
			return Left.mColor == Right.mColor && Left.mColorSubresources == Right.mColorSubresources &&
	...
	public:
         ...
         */
		FARDGPassExecutionContext(FARDGBuilder& Graph,
		...
		[[nodiscard]] arda::IArdaRHIBuffer* GetUniformBuffer(FARDGUniformBuffer* UniformBuffer) const;

		/** Returns a declared acceleration structure's physical handle. */
		[[nodiscard]] arda::IArdaRHIAccelStruct* GetAccelStruct(FARDGAccelStruct* AccelStruct) const;
         ...
         */
		[[nodiscard]] arda::FArdaRHIBindingSetRef CreateBindingSet(arda::IArdaRHIBindingLayout* BindingLayout) const;
		...
		[[nodiscard]] arda::FArdaRHIBindingSetRef CreateBindingSet(const arda::FArdaGlobalShaderInstance& Shader,
	...
	public:
         ...
         */
		FARDGPass(FARDGPassHandle Handle,
		    eastl::string Name,
		...
		void AddProducer(FARDGPassHandle Producer)
		{
			...
			const auto Existing = eastl::find(mState.mProducers.begin(), mState.mProducers.end(), Producer);
		...
		void AddConsumer(FARDGPassHandle Consumer)
		{
			...
			const auto Existing = eastl::find(mState.mConsumers.begin(), mState.mConsumers.end(), Consumer);
		...
		void AddSynchronizationProducer(FARDGPassHandle Producer)
		{
			...
			const auto Existing =
			    eastl::find(mState.mSynchronizationProducers.begin(), mState.mSynchronizationProducers.end(), Producer);
		...
		void AddSynchronizationConsumer(FARDGPassHandle Consumer)
		{
			...
			const auto Existing =
			    eastl::find(mState.mSynchronizationConsumers.begin(), mState.mSynchronizationConsumers.end(), Consumer);
	...
	public:
         ...
         */
		FARDGLambdaPass(FARDGPassHandle Handle,
		    eastl::string Name,
		    EARDGPassFlags Flags,
		    const void* Parameters,
		    const FARDGParameterMetadata* ParameterMetadata,
		    FARDGPassExecuteFunction ExecuteFunction)
		    : FARDGPass(Handle, eastl::move(Name), Flags, Parameters, ParameterMetadata),
	...
	public:
		...
		FARDGSentinelPass(FARDGPassHandle Handle, eastl::string Name)
		    : FARDGPass(Handle, eastl::move(Name), EARDGPassFlags::NeverCull)
		{
