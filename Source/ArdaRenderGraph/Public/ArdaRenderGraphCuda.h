...
namespace arda
{
	...
	struct FARDGCudaParameterDomain
	{
		...
		using Buffer = FARDGBufferAccess;
		template <class T, EArdaComputeAccess Access>
		using Surface = FARDGTextureAccess;
	};
	...
	struct FArdaCudaGraphHelpers
	{
		struct MetadataVisitor
		{
			...
			template <class T>
			void Value(const char* Name, size_t Offset)
			{
			...
			void Buffer(const char* Name, size_t Offset)
			{
				mMembers.push_back({Name,
				    EARDGParameterType::BufferAccess,
				    Offset,
				    sizeof(FARDGBufferAccess),
				    alignof(FARDGBufferAccess),
				    1,
				    sizeof(FARDGBufferAccess),
				    EArdaRHIResourceState::UnorderedAccess,
				    nullptr,
				    A == EArdaComputeAccess::Read});
			}

			template <class T, EArdaComputeAccess A>
			void Surface(const char* Name, size_t Offset, EArdaRHIFormat)
			{
				mMembers.push_back({Name,
				    EARDGParameterType::TextureAccess,
				    Offset,
				    sizeof(FARDGTextureAccess),
				    alignof(FARDGTextureAccess),
				    1,
				    sizeof(FARDGTextureAccess),
				    EArdaRHIResourceState::UnorderedAccess,
				    nullptr,
		...
		struct NormalizeVisitor
		{
			...
			void Buffer(const char*, size_t Offset)
			{
				...
				{
					mStatus = FArdaRHIStatus::Error(EArdaRHIResult::InvalidArgument,
					    "RDG CUDA buffer must be declared with CUDA sharing.");
				}
				else
				{
					mStatus = ValidateArdaCudaBuffer(P.mBuffer->GetDesc());
				}
			}

			template <class T, EArdaComputeAccess A>
			void Surface(const char*, size_t Offset, EArdaRHIFormat Format)
			{
				if (!mStatus)
				{
					return;
				...
				{
					mStatus = FArdaRHIStatus::Error(EArdaRHIResult::InvalidArgument,
					    "RDG CUDA surface must use its declared format and CUDA sharing.");
				}
				else
				{
					mStatus = ValidateArdaCudaTexture(P.mTexture->GetDesc());
				}
			}
		};
		...
		struct ResolveVisitor
		{
			...
			void Buffer(const char*, size_t Offset)
			{
				...
				auto& Destination =
				    *reinterpret_cast<FArdaComputeBufferParameter*>(mHost + mMembers[mIndex++].mHostOffset);
			...
			void Surface(const char*, size_t Offset, EArdaRHIFormat)
			{
				...
				auto& Destination =
				    *reinterpret_cast<FArdaComputeTextureParameter*>(mHost + mMembers[mIndex++].mHostOffset);
	...
	struct TARDGCudaParameters : Parameters::template Rebind<FARDGCudaParameterDomain>
	{
		...
		static const FARDGParameterMetadata& GetStaticMetadata()
		{
			static const FARDGParameterMetadata Metadata = []
			{
				...
				return FARDGParameterMetadata(Parameters::GetStaticMetadata().GetName(),
				    sizeof(FStorage),
				    alignof(FStorage),
				    eastl::move(Visitor.mMembers));
			}();
			return Metadata;
		}
	};
	...
	TArdaRHIResult<FARDGPassHandle> AddArdaCudaPass(FARDGBuilder& Graph,
	    eastl::string Name,
	    eastl::shared_ptr<OperandType> Operand,
	    TARDGCudaParameters<typename OperandType::FParameters> Parameters,
	    EARDGPassFlags Flags = EARDGPassFlags::Compute)
	{
		...
		if (!Operand)
		{
			return {{}, FArdaRHIStatus::Error(EArdaRHIResult::InvalidArgument, "RDG CUDA pass requires an operand.")};
		}
		if (auto S = Operand->GetOperandSupport(); !S)
		{
			return {{}, S};
		}
		...
		{
			return {{},
			    FArdaRHIStatus::Error(EArdaRHIResult::WrongDevice, "CUDA operand belongs to another graph device.")};
		}
		...
		{
			return {{},
			    FArdaRHIStatus::Error(EArdaRHIResult::InvalidArgument, "CUDA pass requires a compute pipeline.")};
		}
			...
			if (HasAllFlags(Flags, EARDGPassFlags::AsyncCompute))
			{
				return {{},
				    FArdaRHIStatus::Error(EArdaRHIResult::Unsupported,
				        "D3D12 CiG cannot use the async compute queue.")};
			}
			Flags |= EARDGPassFlags::NeverParallel | EARDGPassFlags::RecordAtSubmit;
		}
		FArdaCudaGraphHelpers::NormalizeVisitor Normalize{
		    reinterpret_cast<uint8_t*>(static_cast<typename G::FStorage*>(&Parameters))};
		G::FStorage::VisitMembers(Normalize);
		if (!Normalize.mStatus)
		{
			return {{}, Normalize.mStatus};
		}
		auto Pass = Graph.AddPass(eastl::move(Name),
		    &Parameters,
		    Flags,
		    [Operand = eastl::move(Operand)](FARDGPassExecutionContext& Context, const G& Values) -> FArdaRHIStatus
		    {
			    P Host;
			    FArdaCudaGraphHelpers::ResolveVisitor Resolve{
			        reinterpret_cast<const uint8_t*>(static_cast<const typename G::FStorage*>(&Values)),
