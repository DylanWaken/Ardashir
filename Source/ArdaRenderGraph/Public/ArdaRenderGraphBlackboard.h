...
namespace arda
{
	...
	public:
		...
		{
			static_assert(eastl::is_copy_constructible_v<ValueType>,
		...
		[[nodiscard]] ValueType* TryGet() noexcept
		{
			...
			return Iterator == mValues.end() ? nullptr : eastl::any_cast<ValueType>(&Iterator->second);
		}
	...
	private:
		eastl::unordered_map<std::type_index, eastl::any, FARDGTypeIndexHash> mValues;
	};
}
