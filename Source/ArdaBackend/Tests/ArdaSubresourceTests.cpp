...
TEST(ArdaSubresources, VisitsResolvedPartialRangeInNativeOrder)
{
	...
	const std::vector<std::array<uint32_t, 3>>
	    Expected{{3, 2, 0}, {4, 2, 0}, {3, 3, 0}, {4, 3, 0}, {3, 2, 1}, {4, 2, 1}, {3, 3, 1}, {4, 3, 1}};
	EXPECT_EQ(Actual, Expected);
}

TEST(ArdaSubresources, EmptyAxisNeverProducesAnElement)
{
	for (const arda::FArdaRHITextureSubresourceRange Range : {arda::FArdaRHITextureSubresourceRange{2, 0, 1, 2, 0, 2},
