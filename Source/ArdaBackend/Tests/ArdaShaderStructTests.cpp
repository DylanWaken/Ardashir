...
namespace
{
	...
	{
		using FPermutationDomain = arda::TArdaShaderPermutationDomain<FUseFeature, FQualityLevel>;
	...
	class FMacroStyleShader
	{
		...
		using FPermutationDomain = arda::TArdaShaderPermutationDomain<FUseFeature>;
	...
	static_assert(arda::TArdaShaderPermutationTraits<FMacroStyleShader>::PermutationCount == 2);
	static_assert(arda::TArdaHasShouldCompilePermutation<FMacroStyleShader>::value);
	static_assert(arda::TArdaHasModifyCompilationEnvironment<FMacroStyleShader>::value);
	...
	static_assert(arda::TArdaHasNamedShouldCompilePermutation<FWrongHookSignatureShader>::value);
	static_assert(!arda::TArdaHasShouldCompilePermutation<FWrongHookSignatureShader>::value);
	...
	ARDA_BEGIN_SHADER_PARAMETER_STRUCT(FPushParameters)
		ARDA_SHADER_PARAMETER_VALUE(uint32_t, mPrefix)
		ARDA_SHADER_PUSH_CONSTANTS(FPushPayload, mPush, 0, 0, Stage::Compute)
	ARDA_END_SHADER_PARAMETER_STRUCT()
...
TEST(ArdaShaderStructs, DetectsHooksDeclaredAfterGlobalShaderMacro)
{
	...
	EXPECT_TRUE(arda::ShouldCompileShaderPermutation<FMacroStyleShader>(Parameters));
	arda::BuildShaderCompilationEnvironment<FMacroStyleShader>(Parameters, Environment);
	ASSERT_EQ(Environment.GetDefines().size(), 2u);
...
TEST(ArdaShaderStructs, RegistersOptionalPermutationPoliciesAndArtifactStems)
{
	...
	FArdaShaderTypeRegistration Registration("PermutationPolicy",
	    ...
	    Stage::Compute,
	    nullptr,
	    arda::TArdaShaderPermutationTraits<FPermutationShaderPolicy>::PermutationCount,
...
TEST(ArdaShaderStructs, ValidatesPermutationRegistrationAndStemCollisions)
{
	...
	FArdaShaderTypeRegistration::ResetForTests();
	{
		FArdaShaderTypeRegistration
		    Invalid("InvalidPermutation", "Source", "Artifact", "Main", Stage::Compute, nullptr, 0);
		EXPECT_EQ(FArdaShaderTypeRegistration::CommitAll().mCode, EArdaShaderRegistrationError::InvalidPermutation);
	}
	{
		FArdaShaderTypeRegistration
		    Invalid("MissingCallbacks", "Source", "Artifact", "Main", Stage::Compute, nullptr, 2);
		EXPECT_EQ(FArdaShaderTypeRegistration::CommitAll().mCode, EArdaShaderRegistrationError::InvalidPermutation);
	}
	{
...
TEST(ArdaShaderStructs, RejectsUnsafeAndCaseCollidingArtifactStems)
{
	...
	{
		FArdaShaderTypeRegistration::ResetForTests();
		FArdaShaderTypeRegistration Invalid("UnsafeStem", "Source", Stem, "Main", Stage::Compute, nullptr);
		EXPECT_EQ(FArdaShaderTypeRegistration::CommitAll().mCode, EArdaShaderRegistrationError::InvalidType) << Stem;
	}

	FArdaShaderTypeRegistration::ResetForTests();
	FArdaShaderTypeRegistration Upper("Upper", "SourceA", "PortableArtifact", "Main", Stage::Compute, nullptr);
	FArdaShaderTypeRegistration Lower("Lower", "SourceB", "portableartifact", "Main", Stage::Compute, nullptr);
	EXPECT_EQ(FArdaShaderTypeRegistration::CommitAll().mCode, EArdaShaderRegistrationError::ArtifactStemCollision);
}
...
TEST(ArdaShaderStructs, CommitsInDeterministicOrderAndSupportsLookup)
{
	using namespace arda;
	FArdaShaderTypeRegistration::ResetForTests();
	FArdaShaderTypeRegistration Second("ZSecond", "Source", "Second", "Main", Stage::Compute, nullptr);
	FArdaShaderTypeRegistration First("AFirst", "Source", "First", "Main", Stage::Compute, nullptr);
...
TEST(ArdaShaderStructs, RejectsDuplicateNamesAndArtifactIdentities)
{
	...
	FArdaShaderTypeRegistration::ResetForTests();
	{
		FArdaShaderTypeRegistration One("Duplicate", "Source", "One", "Main", Stage::Compute, nullptr);
		FArdaShaderTypeRegistration Two("Duplicate", "Source", "Two", "Main", Stage::Compute, nullptr);
	...
	{
		FArdaShaderTypeRegistration One("First", "Source", "Same", "Main", Stage::Compute, nullptr);
		FArdaShaderTypeRegistration Two("Second", "Source", "Same", "Main", Stage::Compute, nullptr);
		const auto Status = FArdaShaderTypeRegistration::CommitAll();
...
TEST(ArdaShaderStructs, EnumeratesMetadataAndGeneratesStableLayout)
{
	...
	EXPECT_EQ(Metadata.GetLayoutHash(), FMetadataParameters::GetStaticMetadata().GetLayoutHash());
	...
	EXPECT_EQ(NestedOffset, offsetof(FNestedResourceRoot, mResources) + offsetof(FNestedResources, mNestedTexture));
...
TEST(ArdaShaderStructs, RegistrationDestructionPreservesUnrelatedTypes)
{
	using namespace arda;
	FArdaShaderTypeRegistration::ResetForTests();
	FArdaShaderTypeRegistration First("LifetimeFirst", "Source", "First", "Main", Stage::Compute, nullptr);
	FArdaShaderTypeRegistration Second("LifetimeSecond", "Source", "Second", "Main", Stage::Compute, nullptr);
	ASSERT_TRUE(FArdaShaderTypeRegistration::CommitAll());
...
TEST(ArdaShaderStructs, LoadsGlobalMapIdempotentlyAndBuildsDirectBindings)
{
...

	{
		...
		EXPECT_FALSE(FDirectParameters::GetStaticMetadata().BuildBindingSetDesc(&Parameters,
		    IncompleteLayout.mValue,
		    IncompleteDesc));
		FArdaRHIBindingSetDesc NullDesc;
		EXPECT_FALSE(FDirectParameters::GetStaticMetadata().BuildBindingSetDesc(&Parameters, *ArrayLayout, NullDesc));
		Parameters.mOutputs[0] = Buffer.mValue;
		Parameters.mOutputs[1] = Buffer.mValue;
		FArdaRHIBindingSetDesc Desc;
		ASSERT_TRUE(FDirectParameters::GetStaticMetadata().BuildBindingSetDesc(&Parameters, *ArrayLayout, Desc));
		...
		EXPECT_TRUE(FDirectParameters::GetStaticMetadata().CreateBindingSet(*GetDevice(),
		    &Parameters,
		    *ArrayLayout,
		    BindingSet));
		...
		ASSERT_TRUE(FPushParameters::GetStaticMetadata().BuildBindingLayoutDescs(PushLayoutDescs));
		...
		ASSERT_TRUE(FPushParameters::GetStaticMetadata().BuildBindingSetDesc(&PushParameters,
		    PushLayout.mValue,
		    PushBindingDesc));
		ASSERT_EQ(PushBindingDesc.mItems.size(), 1u);
		EXPECT_FALSE(PushBindingDesc.mItems[0].mResource);
		EXPECT_EQ(PushBindingDesc.mItems[0].mView.mBufferRange.mByteSize, sizeof(FPushPayload));
