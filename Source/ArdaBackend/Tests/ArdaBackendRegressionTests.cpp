...
namespace
{
	...
	TEST_P(FArdaBackendRegressionTest, IntegerBufferClearPreservesStateAndWritesEveryWord)
	{
		...
		ASSERT_TRUE(Commands.mValue->CopyBufferDeviceToHost(*Buffer.mValue, Readback, 0, Desc.mByteSize));
	...
	TEST_P(FArdaBackendRegressionTest, IntegerTextureClearHonorsMipAndArrayRange)
	{
		...
		for (uint32_t Slice = 0; Slice < Desc.mArraySize; ++Slice)
		{
			for (uint32_t Mip = 0; Mip < Desc.mMipLevels; ++Mip)
			{
				...
				Region.mDepth = 1;
				ASSERT_TRUE(Commands.mValue->CopyTextureToStaging(*Staging.mValue, Region, *Texture.mValue, Region));
			}
		}
		ASSERT_TRUE(Commands.mValue->Close());
		ASSERT_TRUE(mDevice->ExecuteCommandList(Commands.mValue));
	...
	TEST_P(FArdaBackendRegressionTest, PersistentPipelineKeyIncludesBindlessLayoutAbi)
	{
		...
		const std::string Path = std::string(ARDA_BACKEND_TEST_SHADER_DIR "/") + "ArdaShaderStructTest" +
		    GetShaderArtifactExtension(GetParam());
		...
		ASSERT_TRUE(Status) << Status.mMessage.c_str();
		EXPECT_NE(Conventional->GetDesc().mPersistentCacheKey, Direct->GetDesc().mPersistentCacheKey);
	}
	...
	INSTANTIATE_TEST_SUITE_P(NativeProviders, FArdaBackendRegressionTest, testing::ValuesIn(NativeBackends));
}
