...
namespace
{
    ...
    struct FArdaVulkanHost
    {
        ...
        bool Initialize()
        {
            ...
            if (!GetProc) { mError = "Vulkan loader unavailable"; return false; }
            ...
            if (!bValidation) { mError = "Vulkan validation unavailable"; return false; }
            ...
            if (mDispatch.vkCreateInstance(&InstanceInfo, nullptr, &mInstance) != VK_SUCCESS) return false;
            ...
            Debug.pfnUserCallback = Diagnostic; Debug.pUserData = this;
            if (mDispatch.vkCreateDebugUtilsMessengerEXT(mInstance, &Debug, nullptr, &mMessenger) != VK_SUCCESS) return false;
            ...
            for (const auto Device : Devices)
            {
                ...
                for (uint32_t I = 0; I < Count; ++I)
                    ...
                    { mPhysicalDevice = Device; mFamily = I; break; }
                if (mPhysicalDevice) break;
            }
            if (!mPhysicalDevice) { mError = "No graphics/compute Vulkan device"; return false; }
            ...
            Timeline.timelineSemaphore = VK_TRUE; Features13.pNext = &Timeline;
            const float Priority = 1.f;
            VkDeviceQueueCreateInfo QueueInfo{VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO};
            QueueInfo.queueFamilyIndex = mFamily; QueueInfo.queueCount = 1; QueueInfo.pQueuePriorities = &Priority;
            VkDeviceCreateInfo DeviceInfo{VK_STRUCTURE_TYPE_DEVICE_CREATE_INFO};
            DeviceInfo.pNext = &Features13; DeviceInfo.queueCreateInfoCount = 1; DeviceInfo.pQueueCreateInfos = &QueueInfo;
            if (mDispatch.vkCreateDevice(mPhysicalDevice, &DeviceInfo, nullptr, &mDevice) != VK_SUCCESS) return false;
        ...
        ~FArdaVulkanHost()
        {
            if (mDevice) { mDispatch.vkDeviceWaitIdle(mDevice); mDispatch.vkDestroyDevice(mDevice, nullptr); }
            if (mMessenger) mDispatch.vkDestroyDebugUtilsMessengerEXT(mInstance, mMessenger, nullptr);
            if (mInstance) mDispatch.vkDestroyInstance(mInstance, nullptr);
    ...
    struct FArdaVulkanHostProvider : IArdaExternalDeviceProvider
    {
        ...
        const char* GetBackendName() const noexcept override { return "native-vulkan"; }
        bool GetExternalDeviceDesc(FArdaExternalDeviceDesc& Out) const override { Out = mDesc; return true; }
        eastl::shared_ptr<void> GetLifetimeToken() const override { return mHost; }
    };
    ...
    protected:
        ...
        void SetUp() override
        {
            ...
            if (!mProvider.mHost->Initialize()) GTEST_SKIP() << mProvider.mHost->mError.c_str();
            auto& D = mProvider.mDesc;
            D.mBackendName = "native-vulkan"; D.mNativeApi = "vulkan";
            ...
            D.mQueues.push_back({EArdaRHIQueueType::Graphics, FArdaNativeObject(mProvider.mHost->mQueue), mProvider.mHost->mFamily, 0});
            D.mProperties = {{"vulkan.api-version", "1.3"}, {"vulkan.validation", "enabled"},
                {"vulkan.enabled-feature", "dynamicRendering"}, {"vulkan.enabled-feature", "synchronization2"},
                {"vulkan.enabled-feature", "timelineSemaphore"}};
            ASSERT_TRUE(RegisterExternalDeviceProvider(mProvider));
            FArdaBackendConfiguration C;
            C.mBackendName = "native-vulkan"; C.mDeviceSource = EArdaDeviceSource::ExternalProvider;
    ...
    TEST_F(ArdaVulkanExternal, ExecutesAndRetainsHostBeyondShutdown)
    {
        ...
        FArdaRHIBufferDesc Desc; Desc.mByteSize = 1024;
        auto Buffer = Device->CreateBuffer(Desc); ASSERT_TRUE(Buffer) << Buffer.mStatus.mMessage.c_str();
        auto Commands = Device->CreateCommandList(EArdaRHIQueueType::Graphics); ASSERT_TRUE(Commands);
        ...
        {
            auto Host = Weak.lock(); ASSERT_TRUE(Host);
            VkFenceCreateInfo Info{VK_STRUCTURE_TYPE_FENCE_CREATE_INFO}; VkFence Fence = VK_NULL_HANDLE;
        ...
        Commands.mValue = nullptr; Buffer.mValue = nullptr; Device = nullptr;
