...
#define INVALID_FIELDS(V, B, S) V(eastl::string, mUnsupported)
	...
	{
		FArdaCudaKernelSignature mSignature{&typeid(FScalarParameters::FCuda),
		    sizeof(FScalarParameters::FCuda),
		    alignof(FScalarParameters::FCuda),
		    true};
	...
	TEST(ArdaCudaVariants, EnforcesExactSignatureAndBuildManifestAtRuntime)
	{
		...
		FRegistry Missing;
		auto V = Variant();
		auto Entry = eastl::make_shared<FFakeEntry>();
	...
	public:
		...
		void BindKernelVariants(FRegistry& R) const override
		{
			++mBindings;
			R.Add(Variant());
		}
	...
	{
		FArdaCudaKernel K;
		K.mEntry = eastl::make_shared<FFakeEntry>();
