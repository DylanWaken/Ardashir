...
namespace arda
{
     ...
     */
    template<class T> bool IsArdaCudaSurfaceElementSupported(EArdaRHIFormat Format)
    {
        const auto Info = GetArdaCudaFormatInfo(Format);
        if (!std::is_trivially_copyable_v<T> || !std::is_standard_layout_v<T> || !Info.mChannels ||
            sizeof(T) != size_t(Info.mChannels) * Info.mBits / 8) return false;
        if constexpr (std::is_arithmetic_v<T>)
        {
            const auto Scalar = std::is_floating_point_v<T> ? EArdaCudaScalarType::Float :
                std::is_signed_v<T> ? EArdaCudaScalarType::SInt : EArdaCudaScalarType::UInt;
    ...
    struct FArdaCudaHostDomain
    {
        ...
        template<class T, EArdaComputeAccess Access> using Buffer = FArdaComputeBufferParameter;
        template<class T, EArdaComputeAccess Access> using Surface = FArdaComputeTextureParameter;
    };
    /** Device representation; unsupported values receive inert storage and fail runtime validation. */
    struct FArdaCudaDeviceDomain
    {
        template<class T> using Value = std::conditional_t<std::is_trivially_copyable_v<T> &&
            std::is_standard_layout_v<T>, T, uint64_t>;
        template<class T, EArdaComputeAccess Access> using Buffer =
            std::conditional_t<Access == EArdaComputeAccess::Read, const T*, T*>;
        template<class T, EArdaComputeAccess Access> using Surface = uint64_t;
    };
    ...
    public:
        ...
        FArdaCudaParameterMetadata(const char* Name, size_t HostSize, size_t HostAlignment,
            FArdaCudaKernelSignature Signature, bool HostLayoutSupported,
        ...
        const FArdaComputeParameterMetadata& GetHostMetadata() const noexcept { return mHost; }
        /** Exact single-argument kernel contract. */
        FArdaCudaKernelSignature GetSignature() const noexcept { return mSignature; }
        /** Paired member locations, in declaration order. */
        const eastl::vector<FArdaCudaParameterMember>& GetMembers() const noexcept { return mMembers; }
...
#define ARDA_INTERNAL_CUDA_VALUE_INFO(T, Name) \
    {#Name, ::arda::EArdaComputeParameterKind::Value, ::arda::EArdaComputeAccess::Read, \
        offsetof(FParameters, Name), offsetof(FCuda, Name), sizeof(T), sizeof(typename FCuda::FDomain::template Value<T>), \
        alignof(T), sizeof(T), alignof(T), ::arda::EArdaRHIFormat::Unknown, \
...
#define ARDA_INTERNAL_CUDA_BUFFER_INFO(T, Name, Access) \
    {#Name, ::arda::EArdaComputeParameterKind::Buffer, Access, offsetof(FParameters, Name), offsetof(FCuda, Name), \
        sizeof(::arda::FArdaComputeBufferParameter), sizeof(uint64_t), alignof(::arda::FArdaComputeBufferParameter), \
        sizeof(T), alignof(T), ::arda::EArdaRHIFormat::Unknown, std::is_trivially_copyable_v<T> && std::is_standard_layout_v<T>},
#define ARDA_INTERNAL_CUDA_SURFACE_INFO(T, Name, Access, Format) \
    {#Name, ::arda::EArdaComputeParameterKind::Texture, Access, offsetof(FParameters, Name), offsetof(FCuda, Name), \
        sizeof(::arda::FArdaComputeTextureParameter), sizeof(uint64_t), alignof(::arda::FArdaComputeTextureParameter), \
        sizeof(T), alignof(T), Format, ::arda::IsArdaCudaSurfaceElementSupported<T>(Format)},
#define ARDA_INTERNAL_CUDA_VISIT_VALUE(T, Name) Visitor.template Value<T>(#Name, offsetof(FSelf, Name));
#define ARDA_INTERNAL_CUDA_VISIT_BUFFER(T, Name, Access) Visitor.template Buffer<T, Access>(#Name, offsetof(FSelf, Name));
#define ARDA_INTERNAL_CUDA_VISIT_SURFACE(T, Name, Access, Format) Visitor.template Surface<T, Access>(#Name, offsetof(FSelf, Name), Format);
...
#define ARDA_CUDA_PARAMETER_STRUCT(Name, Fields) \
    template<class Domain> struct Name##Storage { \
        ...
        template<class OtherDomain> using Rebind = Name##Storage<OtherDomain>; \
        Fields(ARDA_INTERNAL_CUDA_VALUE, ARDA_INTERNAL_CUDA_BUFFER, ARDA_INTERNAL_CUDA_SURFACE) \
        static const ::arda::FArdaCudaParameterMetadata& GetCudaMetadata() { \
            static const ::arda::FArdaCudaParameterMetadata Metadata(#Name, sizeof(FParameters), alignof(FParameters), \
                {&typeid(FCuda), sizeof(FCuda), alignof(FCuda), std::is_trivially_copyable_v<FCuda> && std::is_standard_layout_v<FCuda>}, \
                std::is_standard_layout_v<FParameters>, \
                {Fields(ARDA_INTERNAL_CUDA_VALUE_INFO, ARDA_INTERNAL_CUDA_BUFFER_INFO, ARDA_INTERNAL_CUDA_SURFACE_INFO)}); \
            return Metadata; \
        } \
        static const ::arda::FArdaComputeParameterMetadata& GetStaticMetadata() { return GetCudaMetadata().GetHostMetadata(); } \
        template<class V> static void VisitMembers(V& Visitor) { \
