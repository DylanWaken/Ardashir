...
namespace arda
{
     ...
     */
	[[nodiscard]] FArdaShaderCompilerConfiguration GetShaderCompilerConfiguration();
	...
	[[nodiscard]] FArdaShaderCompileResult EnsureRegisteredShaderArtifacts(const std::filesystem::path& OutputDirectory,
	    const char* BackendName);

	/** Ensures one artifact for an exact registered module. */
	[[nodiscard]] FArdaShaderCompileResult EnsureRegisteredShaderArtifact(const FArdaShaderType& Type,
	    const char* BackendName,
