...
namespace arda
{
	namespace
	{
		...
		eastl::string ProfileForStage(arda::EArdaRHIShaderStage Stage)
		{
			...
			if (Stage == StageType::Pixel)
			{
				return "ps_6_0";
			}
			if (Stage == StageType::Compute)
			{
				return "cs_6_0";
			}
			...
			if (Stage == StageType::Hull)
			{
				return "hs_6_0";
			}
			if (Stage == StageType::Domain)
			{
				return "ds_6_0";
			}
			if (Stage == StageType::Amplification)
			{
				return "as_6_5";
			}
			if (Stage == StageType::Mesh)
			{
				return "ms_6_5";
			}
			if (Stage == StageType::WorkGraph)
			{
				return "lib_6_8";
			}
...
#endif
			return Base.string() + ".arda-" + Kind + "-" + std::to_string(ProcessId) + "-" +
	    ...
	    const eastl::vector<FArdaShaderTarget>& Targets)
	{
		...
		const std::filesystem::path StagingDirectory = TemporaryPath(OutputDirectory, "staging");
		std::error_code Error;
