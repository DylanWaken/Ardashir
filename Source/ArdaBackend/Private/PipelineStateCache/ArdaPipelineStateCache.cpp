...
namespace arda
{
	namespace
	{
		...
		void HashShader(FStablePipelineHasher& Hash, const FArdaRHIShaderRef& Shader) noexcept
		{
			Hash.Add(Shader ? Shader->GetPersistentCacheHash() : 0);
		}

		void HashBindingLayout(FStablePipelineHasher& Hash, const FArdaRHIBindingLayoutRef& Layout) noexcept
		{
			if (!Layout)
			{
				Hash.Add(0);
		...
		void HashInputLayout(FStablePipelineHasher& Hash, const FArdaRHIInputLayoutRef& Layout) noexcept
		{
			if (!Layout)
			{
				Hash.Add(0);
		...
		void HashLayouts(FStablePipelineHasher& Hash, const eastl::vector<FArdaRHIBindingLayoutRef>& Layouts) noexcept
		{
		...
		void HashRasterFixedFunctionState(FStablePipelineHasher& Hash, const PipelineDesc& Desc) noexcept
		{
	...
	struct FArdaPipelineStateCache::FImpl
	{
		...
		using FComputeEntry = TEntry<arda::FArdaRHIComputePipelineDesc, arda::FArdaRHIComputePipelineRef>;
		using FGraphicsEntry = TEntry<arda::FArdaRHIGraphicsPipelineDesc, arda::FArdaRHIGraphicsPipelineRef>;
		using FMeshletEntry = TEntry<arda::FArdaRHIMeshletPipelineDesc, arda::FArdaRHIMeshletPipelineRef>;
		using FRayTracingEntry = TEntry<arda::FArdaRHIRayTracingPipelineDesc, arda::FArdaRHIRayTracingPipelineRef>;
		using FWorkGraphEntry = TEntry<arda::FArdaRHIWorkGraphPipelineDesc, arda::FArdaRHIWorkGraphPipelineRef>;

		explicit FImpl(arda::FArdaRHIDeviceRef InDevice, FArdaPipelineStateCacheConfiguration InConfiguration)
