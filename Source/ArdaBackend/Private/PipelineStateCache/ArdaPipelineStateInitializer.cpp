...
namespace arda
{
	namespace
	{
		void AddShaderLayouts(eastl::vector<arda::FArdaRHIBindingLayoutRef>& Destination,
	...
	FArdaComputePipelineStateInitializer FArdaComputePipelineStateInitializer::FromGlobalShader(
	    const FArdaGlobalShaderInstance& Shader,
	...
	FArdaGraphicsPipelineStateInitializer FArdaGraphicsPipelineStateInitializer::FromGlobalShaders(
	...
	{
		return FromGlobalShaders(VertexShader, nullptr, nullptr, nullptr, PixelShader, InputLayout, FixedState);
	}
	    ...
	    const arda::FArdaRHIGraphicsPipelineDesc& FixedState)
	{
		...
		Result.mDesc.mHullShader = HullShader != nullptr ? HullShader->GetShader() : arda::FArdaRHIShaderRef{};
		Result.mDesc.mDomainShader = DomainShader != nullptr ? DomainShader->GetShader() : arda::FArdaRHIShaderRef{};
		Result.mDesc.mGeometryShader =
		    GeometryShader != nullptr ? GeometryShader->GetShader() : arda::FArdaRHIShaderRef{};
		Result.mDesc.mPixelShader = PixelShader != nullptr ? PixelShader->GetShader() : arda::FArdaRHIShaderRef{};
	...
	FArdaMeshletPipelineStateInitializer FArdaMeshletPipelineStateInitializer::FromGlobalShaders(
	    ...
	    const arda::FArdaRHIMeshletPipelineDesc& FixedState)
	{
		...
		Result.mDesc = FixedState;
		Result.mDesc.mAmplificationShader =
		    AmplificationShader != nullptr ? AmplificationShader->GetShader() : arda::FArdaRHIShaderRef{};
