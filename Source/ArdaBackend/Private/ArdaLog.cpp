...
namespace arda
{
	...
	FArdaLogCategory::FArdaLogCategory(const char* name, EArdaLogVerbosity minimumVerbosity) noexcept
	    : mName(name ? name : ""),
	...
	void FArdaLogCategory::SetMinimumVerbosity(EArdaLogVerbosity verbosity) noexcept
	{
	...
	bool FArdaLogCategory::IsEnabled(EArdaLogVerbosity verbosity) const noexcept
	{
		const auto minimumVerbosity = GetMinimumVerbosity();
		return verbosity != EArdaLogVerbosity::Off && minimumVerbosity != EArdaLogVerbosity::Off &&
		    static_cast<std::uint8_t>(verbosity) >= static_cast<std::uint8_t>(minimumVerbosity);
	}
	    ...
	    ...) noexcept
	{
		...
		const FArdaLogRecord record{category.GetName(),
		    verbosity,
		    message.c_str(),
		    file ? file : "",
		    line,
