...
namespace arda
{
	namespace
	{
		...
		{
		public:
			FResource(EArdaRHIResourceType Type,
			    eastl::string Name,
			    const void* Owner,
		...
		{
		public:
			TNativeResource(Desc Descriptor,
		...
		using FBufferBase = TNativeResource<IArdaRHIBuffer, FArdaRHIBufferDesc, EArdaRHIResourceType::Buffer>;
		...
		using FHeap = TNativeResource<IArdaRHIHeap, FArdaRHIHeapDesc, EArdaRHIResourceType::Heap>;
		using FSampler = TNativeResource<IArdaRHISampler, FArdaRHISamplerDesc, EArdaRHIResourceType::Sampler>;
		using FBindingLayoutBase =
		    TNativeResource<IArdaRHIBindingLayout, FArdaRHIBindingLayoutDesc, EArdaRHIResourceType::BindingLayout>;
		...
		using FBindingSet =
		    TNativeResource<IArdaRHIBindingSet, FArdaRHIBindingSetDesc, EArdaRHIResourceType::BindingSet>;

		class FDescriptorTable final : public FResource, public IArdaRHIDescriptorTable
		{
		public:
			FDescriptorTable(FArdaRHIBindingSetDesc Desc,
			    FArdaProviderObjectRef Native,
			    uint32_t Capacity,
			    uint32_t MaxCapacity,
			    const void* Owner,
			    eastl::shared_ptr<FLifetimeTracker> LifetimeTracker)
			    : FResource(EArdaRHIResourceType::DescriptorTable,
			          Desc.mDebugName,
			          Owner,
		...
		class FResourceCollection final : public FResource, public IArdaRHIResourceCollection
		{
		public:
			FResourceCollection(FArdaRHIResourceCollectionDesc Desc,
		...
		using FFramebuffer =
		    TNativeResource<IArdaRHIFramebuffer, FArdaRHIFramebufferDesc, EArdaRHIResourceType::Framebuffer>;
		...
		{
		public:
			FAccelStruct(FArdaRHIAccelStructDesc Desc,
			    FArdaRHIAccelStructMemoryRequirements Requirements,
			    FArdaProviderObjectRef Native,
			    uint64_t DeviceAddress,
			    const void* Owner,
			    eastl::shared_ptr<FLifetimeTracker> LifetimeTracker)
			    : FResource(EArdaRHIResourceType::AccelStruct, Desc.mDebugName, Owner, eastl::move(LifetimeTracker)),
			      mDesc(eastl::move(Desc)),
		...
		{
		public:
			FShaderTable(FArdaRHIShaderTableDesc Desc,
			    FArdaRHIRayTracingPipelineRef Pipeline,
			    FArdaProviderObjectRef Native,
			    const void* Owner,
			    eastl::shared_ptr<FLifetimeTracker> LifetimeTracker)
			    : FResource(EArdaRHIResourceType::ShaderTable, Desc.mDebugName, Owner, eastl::move(LifetimeTracker)),
			      mDesc(eastl::move(Desc)),
		...
		class FShaderBundle final : public FResource, public IArdaRHIShaderBundle
		{
		public:
			FShaderBundle(FArdaRHIShaderBundleDesc Desc,
			    const void* Owner,
			    eastl::shared_ptr<FLifetimeTracker> LifetimeTracker)
			    : FResource(EArdaRHIResourceType::ShaderBundle, Desc.mDebugName, Owner, eastl::move(LifetimeTracker)),
			      mDesc(eastl::move(Desc))
			{
			}
		...
		class FWorkGraphPipeline final : public FResource, public IArdaRHIWorkGraphPipeline
		{
		public:
		...
		{
		public:
			FSamplerFeedbackTexture(FArdaRHISamplerFeedbackTextureDesc Desc,
		...
		{
		public:
			FTextureReference(FArdaRHITextureRef Texture,
			    const void* Owner,
			    eastl::shared_ptr<FLifetimeTracker> LifetimeTracker)
		...
		public:
			FUniformBuffer(FArdaRHIUniformBufferDesc Desc,
			    ...
			    const void* Owner,
			    eastl::shared_ptr<FLifetimeTracker> LifetimeTracker)
			    : FResource(EArdaRHIResourceType::UniformBuffer, Desc.mDebugName, Owner, eastl::move(LifetimeTracker)),
			      mDesc(eastl::move(Desc)),
		...
		{
		public:
			FStagingTexture(FArdaRHIStagingTextureDesc Desc,
			    FArdaProviderObjectRef Native,
			    const void* Owner,
			    eastl::shared_ptr<FLifetimeTracker> LifetimeTracker)
			    : FResource(EArdaRHIResourceType::StagingTexture, Desc.mDebugName, Owner, eastl::move(LifetimeTracker)),
			      mDesc(eastl::move(Desc)),
			      mNative(eastl::move(Native))
			{
			}
		...
		public:
			FShader(const FArdaRHIShaderDesc& Desc,
			    ...
			    const void* Owner,
			    eastl::shared_ptr<FLifetimeTracker> LifetimeTracker)
			    : FResource(EArdaRHIResourceType::Shader, Desc.mDebugName, Owner, eastl::move(LifetimeTracker)),
			      mStage(Desc.mStage),
		...
		{
		public:
			FShaderLibrary(const void* Bytecode,
			    size_t Size,
			    const char* Name,
			    const void* Owner,
			    eastl::shared_ptr<FLifetimeTracker> LifetimeTracker)
			    : FResource(EArdaRHIResourceType::ShaderLibrary, Name ? Name : "", Owner, eastl::move(LifetimeTracker)),
		...
		{
		public:
			FInputLayout(FArdaRHIInputLayoutDesc Desc,
			    const void* Owner,
			    eastl::shared_ptr<FLifetimeTracker> LifetimeTracker)
			    : FResource(EArdaRHIResourceType::InputLayout, "InputLayout", Owner, eastl::move(LifetimeTracker)),
			      mDesc(eastl::move(Desc))
			{
			}
		...
		{
		public:
			TLogicalState(Desc Descriptor, const void* Owner, eastl::shared_ptr<FLifetimeTracker> LifetimeTracker)
			    : FResource(Type, "CachedState", Owner, eastl::move(LifetimeTracker)),
			      mDesc(eastl::move(Descriptor))
			{
			}
		...
		using FRasterState = TLogicalState<IArdaRHIRasterState, FArdaRHIRasterState, EArdaRHIResourceType::RasterState>;
		using FBlendState = TLogicalState<IArdaRHIBlendState, FArdaRHIBlendState, EArdaRHIResourceType::BlendState>;
		...
		using FEventQuery = TNativeSignal<IArdaRHIEventQuery, EArdaRHIResourceType::EventQuery>;
		using FTimerQuery = TNativeSignal<IArdaRHITimerQuery, EArdaRHIResourceType::TimerQuery>;
		using FGpuFence = TNativeSignal<IArdaRHIGpuFence, EArdaRHIResourceType::GpuFence>;
		...
		FArdaProviderObjectRef GetNativeObject(IArdaRHIResource* Resource)
		{
			...
			if (auto* Buffer = Cast<FBuffer>(Resource))
			{
			...
			if (auto* Sampler = Cast<FSampler>(Resource))
			{
				return Sampler->mNative;
			}
			if (auto* Layout = Cast<FBindingLayout>(Resource))
			{
				return Layout->mNative;
			}
			if (auto* Set = Cast<FBindingSet>(Resource))
			{
				return Set->mNative;
			}
			...
			if (auto* Framebuffer = Cast<FFramebuffer>(Resource))
			{
				return Framebuffer->mNative;
			}
			if (auto* Pipeline = Cast<FGraphicsPipeline>(Resource))
			{
				return Pipeline->mNative;
			}
			if (auto* Pipeline = Cast<FComputePipeline>(Resource))
			{
				return Pipeline->mNative;
			}
			if (auto* Pipeline = Cast<FMeshletPipeline>(Resource))
			{
				return Pipeline->mNative;
			}
			...
			if (auto* Shader = Cast<FShader>(Resource))
			{
		...
		private:
			...
			eastl::vector<EArdaRHIResourceState>& GetFacadeTextureStates(FTexture& Texture) const;
			...
			mutable std::unordered_map<FTexture*, eastl::vector<EArdaRHIResourceState>> mFacadeTextureStates;
			mutable std::unordered_map<FBuffer*, EArdaRHIResourceState> mFacadeBufferStates;
			mutable std::unordered_map<FSamplerFeedbackTexture*, EArdaRHIResourceState> mFacadeSamplerFeedbackStates;
			mutable std::unordered_map<FTexture*, EArdaRHIQueueType> mFacadeTextureQueueOwners;
			mutable std::unordered_map<FBuffer*, EArdaRHIQueueType> mFacadeBufferQueueOwners;
			...
			mutable std::unordered_map<FOpacityMicromap*, FAccelStructTracking> mFacadeOpacityMicromapStates;
			std::unordered_map<FTexture*, eastl::vector<EArdaRHIResourceState>> mExpectedTextureStartStates;
			std::unordered_map<FBuffer*, EArdaRHIResourceState> mExpectedBufferStartStates;
		};
		...
		private:
			TArdaRHIResult<uint64_t> FinishCommandListSubmission(FCommandList& CommandList,
			...
			TDescriptorCache<FArdaRHIRayTracingPipelineDesc, FArdaRHIRayTracingPipelineRef> mRayTracingPipelineCache;
		...
		TArdaRHIResult<FArdaRHITextureRef> FArdaRHIDeviceImpl::CreateTexture(const FArdaRHITextureDesc& Desc)
		{
			...
			return {FArdaRHITextureRef(new FTexture(Desc, eastl::move(Native.mValue), this, mLifetimeTracker)), {}};
		}
		    ...
		    const FArdaRHITextureRef& Texture)
		{
			...
			return {FArdaRHITextureReferenceRef(new FTextureReference(Texture, this, mLifetimeTracker)), {}};
		}
		...
		TArdaRHIResult<FArdaRHIBufferRef> FArdaRHIDeviceImpl::CreateBuffer(const FArdaRHIBufferDesc& Desc)
		{
			...
			return {FArdaRHIBufferRef(new FBuffer(Desc, eastl::move(Native.mValue), this, mLifetimeTracker)), {}};
		}
		    ...
		    uint64_t Offset)
		{
			...
			const FArdaRHIStatus Status =
			    mDevice->BindTextureMemory(NativeTexture->mNative, NativeTexture->mDesc, NativeHeap->mNative, Offset);
			if (Status)
			{
		    ...
		    uint64_t Offset)
		{
			...
			const FArdaRHIStatus Status =
			    mDevice->BindBufferMemory(NativeBuffer->mNative, NativeBuffer->mDesc, NativeHeap->mNative, Offset);
			if (Status)
			{
		    ...
		    const void* InitialData)
		{
			...
			return {FArdaRHIUniformBufferRef(new FUniformBuffer(Desc, Buffer.mValue, this, mLifetimeTracker)), {}};
		}
		    ...
		    const FArdaRHINativeTextureImportDesc& Desc)
		{
			...
			FArdaRHITextureRef Result(new FTexture(eastl::move(TextureDesc),
		    ...
		    const FArdaRHINativeBufferImportDesc& Desc)
		{
			...
			FArdaRHIBufferRef Result(new FBuffer(eastl::move(BufferDesc),
			    eastl::move(Native.mValue),
			    this,
			    mLifetimeTracker,
			    Desc.mLifetimeToken));
		    ...
		    const FArdaRHIStagingTextureDesc& Desc)
		{
			...
			return {FArdaRHIStagingTextureRef(
			            new FStagingTexture(Desc, eastl::move(Native.mValue), this, mLifetimeTracker)),
			    {}};
		}
		    ...
		    const FArdaRHIViewDesc& Desc)
		{
			...
			return {FArdaRHIShaderResourceViewRef(new FShaderResourceView(Resource, Desc, this, mLifetimeTracker)), {}};
		}
		    ...
		    const FArdaRHIViewDesc& Desc)
		{
			...
			return {FArdaRHIUnorderedAccessViewRef(new FUnorderedAccessView(Resource, Desc, this, mLifetimeTracker)),
			    {}};
		}
		...
		TArdaRHIResult<FArdaRHISamplerRef> FArdaRHIDeviceImpl::CreateSampler(const FArdaRHISamplerDesc& Desc)
		{
			...
			FArdaRHISamplerRef Result(new FSampler(Desc, eastl::move(Native.mValue), this, mLifetimeTracker));
		...
		TArdaRHIResult<FArdaRHIShaderRef> FArdaRHIDeviceImpl::CreateShader(const FArdaRHIShaderDesc& Desc)
		{
			...
			return {FArdaRHIShaderRef(new FShader(Desc, eastl::move(Native.mValue), this, mLifetimeTracker)), {}};
		}
		    ...
		    const char* DebugName)
		{
			...
			return {
			    FArdaRHIShaderLibraryRef(new FShaderLibrary(Bytecode, BytecodeSize, DebugName, this, mLifetimeTracker)),
			    {}};
		}
		    ...
		    const eastl::vector<FArdaRHIVertexAttributeDesc>& Attributes)
		{
			...
			FArdaRHIInputLayoutRef Result(new FInputLayout(Desc, this, mLifetimeTracker));
		    ...
		    const FArdaRHIBindingLayoutDesc& Desc)
		{
			...
			FArdaRHIBindingLayoutRef Result(
			    new FBindingLayout(Desc, eastl::move(Native.mValue), this, mLifetimeTracker));
		    ...
		    const FArdaRHIBindlessLayoutDesc& Desc)
		{
			...
			auto* Layout = new FBindingLayout(NativeDesc, eastl::move(Native.mValue), this, mLifetimeTracker);
		    ...
		    const FArdaRHIBindingLayoutRef& LayoutRef)
		{
			...
			return {FArdaRHIDescriptorTableRef(new FDescriptorTable(eastl::move(Desc),
			            eastl::move(Native.mValue),
			            Capacity,
			            Capacity,
			            this,
		    ...
		    const FArdaRHIResourceCollectionDesc& Desc)
		{
			...
			for (uint32_t Index = 0; Index < Desc.mItems.size(); ++Index)
			{
				...
				auto* Resource = Cast<FResource>(Binding.mValue.mResource.Get());
			...
			return {FArdaRHIResourceCollectionRef(
			            new FResourceCollection(Desc, eastl::move(DescriptorTable), this, mLifetimeTracker)),
			    {}};
		}
		    ...
		    const FArdaRHIBindingItem& Item)
		{
			...
			const bool bDeclared = eastl::any_of(Layout->mDesc.mItems.begin(),
			    Layout->mDesc.mItems.end(),
			    [&Item](const FArdaRHIBindingLayoutItem& Declared)
			    {
				    return Declared.mSlot == Item.mSlot && Declared.mType == Item.mType &&
				        Item.mArrayElement < Declared.mArraySize;
			    });
			if (!bDeclared)
			{
				return Invalid("Descriptor table write does not match a bindless register space.");
			}
			FArdaRHIBindingSetDesc NewDesc = Table->mDesc;
		...
		TArdaRHIResult<FArdaRHIBindingSetRef> FArdaRHIDeviceImpl::CreateBindingSet(const FArdaRHIBindingSetDesc& Desc)
		{
			...
			return {FArdaRHIBindingSetRef(new FBindingSet(Desc, eastl::move(Native.mValue), this, mLifetimeTracker)),
			    {}};
		}
		    ...
		    const FArdaRHIFramebufferDesc& Desc)
		{
			...
			return {FArdaRHIFramebufferRef(new FFramebuffer(Desc, eastl::move(Native.mValue), this, mLifetimeTracker)),
			    {}};
		}
		    ...
		    const FArdaRHIGraphicsPipelineDesc& Desc)
		{
			...
			return {FArdaRHIGraphicsPipelineRef(
			            new FGraphicsPipeline(Desc, eastl::move(Native.mValue), this, mLifetimeTracker)),
			    {}};
		}
		    ...
		    const FArdaRHIComputePipelineDesc& Desc)
		{
			...
			return {FArdaRHIComputePipelineRef(
			            new FComputePipeline(Desc, eastl::move(Native.mValue), this, mLifetimeTracker)),
			    {}};
		}
		    ...
		    const FArdaRHIMeshletPipelineDesc& Desc)
		{
			...
			return {FArdaRHIMeshletPipelineRef(
			            new FMeshletPipeline(Desc, eastl::move(Native.mValue), this, mLifetimeTracker)),
			    {}};
		}
		    ...
		    const FArdaRHIWorkGraphPipelineDesc& Desc)
		{
			...
			return {FArdaRHIWorkGraphPipelineRef(
			            new FWorkGraphPipeline(Desc, eastl::move(Native.mValue), BackingSize, this, mLifetimeTracker)),
			    {}};
		}
		    ...
		    const FArdaRHISamplerFeedbackTextureDesc& InputDesc)
		{
			...
			auto* Feedback =
			    new FSamplerFeedbackTexture(Desc, PairedTextureRef, eastl::move(Native.mValue), this, mLifetimeTracker);
		    ...
		    const FArdaRHIRayTracingPipelineDesc& Desc)
		{
			...
			for (const auto& ShaderDesc : Desc.mShaders)
			{
				...
				{
					auto* Layout = Cast<FBindingLayout>(ShaderDesc.mLocalBindingLayout.Get());
					if (!Layout || !Owns(Layout))
					{
						return Failure<FArdaRHIRayTracingPipelineRef>(WrongDevice());
					}
			...
			for (const auto& HitDesc : Desc.mHitGroups)
			{
				...
				{
					auto* Layout = Cast<FBindingLayout>(HitDesc.mLocalBindingLayout.Get());
					if (!Layout || !Owns(Layout))
					{
						return Failure<FArdaRHIRayTracingPipelineRef>(WrongDevice());
					}
			...
			FArdaRHIRayTracingPipelineRef Result(
			    new FRayTracingPipeline(Desc, eastl::move(Native.mValue), this, mLifetimeTracker));
		...
		namespace
		{
			    ...
			    const eastl::vector<FArdaRHIRayTracingGeometryDesc>& Geometries)
			{
				...
				for (const auto& Geometry : Geometries)
				{
					...
					{
						auto* Buffer = Cast<FBuffer>(Geometry.mVertexOrAABBBuffer.Get());
						if (!Buffer || !Device.Owns(Buffer))
						{
							return Failure<eastl::vector<FArdaProviderRayTracingGeometry>>(WrongDevice());
						}
					...
					{
						auto* Buffer = Cast<FBuffer>(Geometry.mOpacityMicromapIndexBuffer.Get());
						if (!Buffer || !Device.Owns(Buffer))
						{
							return Failure<eastl::vector<FArdaProviderRayTracingGeometry>>(WrongDevice());
						}
					...
					{
						auto* Micromap = Cast<FOpacityMicromap>(Geometry.mOpacityMicromap.Get());
		    ...
		    const FArdaRHIAccelStructDesc& Desc)
		{
			...
			return {FArdaRHIAccelStructRef(new FAccelStruct(Desc,
			            Requirements.mValue,
			            eastl::move(Native.mValue),
			            Address,
			            this,
		    ...
		    const FArdaRHIOpacityMicromapDesc& Desc)
		{
			...
			return {FArdaRHIOpacityMicromapRef(
			            new FOpacityMicromap(Desc, eastl::move(Native.mValue), Address, this, mLifetimeTracker)),
			    {}};
		}
		    ...
		    const FArdaRHIShaderBundleDesc& Desc)
		{
			...
			return {FArdaRHIShaderBundleRef(new FShaderBundle(Desc, this, mLifetimeTracker)), {}};
		}
		    ...
		    const eastl::vector<FArdaRHIShaderBundleRecord>& Records)
		{
			...
			for (const auto& Record : Records)
			{
				...
				auto* Pipeline = bCompute ? Cast<FResource>(Record.mComputePipeline.Get())
		    ...
		    const FArdaRHIShaderTableDesc& Desc)
		{
			...
			return {FArdaRHIShaderTableRef(
			            new FShaderTable(Desc, PipelineRef, eastl::move(Native.mValue), this, mLifetimeTracker)),
			    {}};
		}
		...
		TArdaRHIResult<FArdaRHIRasterStateRef> FArdaRHIDeviceImpl::CreateRasterState(const FArdaRHIRasterState& Desc)
		{
			...
			FArdaRHIRasterStateRef Result(new FRasterState(Desc, this, mLifetimeTracker));
		...
		TArdaRHIResult<FArdaRHIBlendStateRef> FArdaRHIDeviceImpl::CreateBlendState(const FArdaRHIBlendState& Desc)
		{
			...
			FArdaRHIBlendStateRef Result(new FBlendState(Desc, this, mLifetimeTracker));
		    ...
		    const FArdaRHIDepthStencilState& Desc)
		{
			...
			FArdaRHIDepthStencilStateRef Result(new FDepthStencilState(Desc, this, mLifetimeTracker));
		...
		TArdaRHIResult<FArdaRHIEventQueryRef> FArdaRHIDeviceImpl::CreateEventQuery()
		{
			...
			return {FArdaRHIEventQueryRef(
			            new FEventQuery("EventQuery", eastl::move(Native.mValue), this, mLifetimeTracker)),
			    {}};
		}
		...
		TArdaRHIResult<FArdaRHITimerQueryRef> FArdaRHIDeviceImpl::CreateTimerQuery()
		{
			...
			return {FArdaRHITimerQueryRef(
			            new FTimerQuery("TimerQuery", eastl::move(Native.mValue), this, mLifetimeTracker)),
			    {}};
		}
		...
		TArdaRHIResult<FArdaRHIGpuFenceRef> FArdaRHIDeviceImpl::CreateGpuFence()
		{
			...
			return {FArdaRHIGpuFenceRef(new FGpuFence("GpuFence", eastl::move(Native.mValue), this, mLifetimeTracker)),
			    {}};
		}
		    ...
		    bool bImmediateExecution)
		{
			...
			return {FArdaRHICommandListRef(new FCommandList(this, Queue, eastl::move(Native.mValue), mLifetimeTracker)),
			    {}};
		}

		TArdaRHIResult<uint64_t> FArdaRHIDeviceImpl::FinishCommandListSubmission(FCommandList& CommandList,
		    TArdaRHIResult<uint64_t> Submitted)
		{
		...
		FCommandList::FCommandList(FArdaRHIDeviceImpl* Device,
		    EArdaRHIQueueType Queue,
		    eastl::unique_ptr<IArdaProviderCommandList> Native,
		    eastl::shared_ptr<FLifetimeTracker> LifetimeTracker)
		    : FResource(EArdaRHIResourceType::CommandList, "CommandList", Device, eastl::move(LifetimeTracker)),
		      mDevice(Device),
		      mQueue(Queue),
		      mNative(eastl::move(Native))
		{
		...
		IArdaRHIDevice* FCommandList::GetDevice() const noexcept
		{
			return mDevice;
		}
		...
		eastl::vector<EArdaRHIResourceState>& FCommandList::GetFacadeTextureStates(FTexture& Texture) const
		{
		...
		FArdaRHIStatus FCommandList::WriteBuffer(IArdaRHIBuffer& Buffer, const void* Data, size_t Size, uint64_t Offset)
		{
		...
		FArdaRHIStatus FCommandList::CopyBufferHostToDevice(IArdaRHIBuffer& Destination,
		...
		FArdaRHIStatus FCommandList::CopyBufferHostToDeviceAsync(IArdaRHIBuffer& Destination,
		    const void* SourceData,
		    size_t Size,
		...
		FArdaRHIStatus FCommandList::QueueBufferReadback(IArdaRHIBuffer& Source,
		    uint64_t SourceOffset,
		    uint64_t Size,
		    FPendingBufferCopyCompletion Completion)
		{
		...
		FArdaRHIStatus FCommandList::CopyBufferDeviceToHost(IArdaRHIBuffer& Source,
		...
		FArdaRHIStatus FCommandList::CopyBufferDeviceToHostAsync(IArdaRHIBuffer& Source,
		...
		FArdaRHIStatus FCommandList::CopyBuffer(IArdaRHIBuffer& Destination,
		    uint64_t DestinationOffset,
		...
		FArdaRHIStatus FCommandList::CopyTexture(IArdaRHITexture& Destination,
		...
		FArdaRHIStatus FCommandList::CopyTextureToStaging(IArdaRHIStagingTexture& Destination,
		    const FArdaRHITextureSlice& DestinationSlice,
		    IArdaRHITexture& Source,
		    const FArdaRHITextureSlice& SourceSlice)
		{
		...
		FArdaRHIStatus FCommandList::CopyTextureFromStaging(IArdaRHITexture& Destination,
		    const FArdaRHITextureSlice& DestinationSlice,
		...
		FArdaRHIStatus FCommandList::ClearTexture(IArdaRHITexture& Texture,
		...
		FArdaRHIStatus FCommandList::DispatchCuda(const FArdaCudaDispatch& Dispatch)
		{
			...
			for (const auto& B : Dispatch.mBindings)
			{
				...
				else
				{
					...
					Status = SetTextureState(*Cast<FTexture>(B.mResource.Get()),
					    Range,
					    EArdaRHIResourceState::UnorderedAccess);
				}
				if (!Status)
		...
		FArdaRHIStatus FCommandList::SetBufferState(IArdaRHIBuffer& Buffer, EArdaRHIResourceState State)
		{
			auto* Native = Cast<FBuffer>(&Buffer);
			if (!Native || !RetainOwned(Native))
			{
		...
		FArdaRHIStatus FCommandList::TransitionTexture(IArdaRHITexture& Texture,
		...
		FArdaRHIStatus FCommandList::TransitionBuffer(IArdaRHIBuffer& Buffer,
		...
		FArdaRHIStatus FCommandList::SetAccelStructState(IArdaRHIAccelStruct& Resource, EArdaRHIResourceState State)
		{
			...
			if (Status)
			{
				...
				if (Existing == mFacadeAccelStructStates.end())
				{
					...
					Existing = mFacadeAccelStructStates
					               .emplace(AccelStruct, FAccelStructTracking{State, AccelStruct->mBuildState})
					               .first;
				}
				else
				{
		...
		TArdaRHIResult<FArdaRHIResourceStateSnapshot> FCommandList::QueryAccelStructState(
		...
		FArdaRHIStatus FCommandList::BeginTrackingTextureState(IArdaRHITexture& Texture,
		    const FArdaRHITextureSubresourceRange& Range,
		    EArdaRHIResourceState State)
		{
			auto* Native = Cast<FTexture>(&Texture);
		...
		FArdaRHIStatus FCommandList::BeginTrackingBufferState(IArdaRHIBuffer& Buffer, EArdaRHIResourceState State)
		{
			auto* Native = Cast<FBuffer>(&Buffer);
			if (!Native || !RetainOwned(Native))
			{
		...
		TArdaRHIResult<FArdaRHIResourceStateSnapshot> FCommandList::QueryTextureState(IArdaRHITexture& Texture,
		...
		TArdaRHIResult<FArdaRHIResourceStateSnapshot> FCommandList::QueryBufferState(IArdaRHIBuffer& Buffer) const
		{
			auto* Native = Cast<FBuffer>(&Buffer);
			if (!Native || !RetainOwned(Native))
			{
		...
		TArdaRHIResult<FArdaRHIResourceStateSnapshot> FCommandList::QuerySamplerFeedbackTextureState(
		...
		FArdaRHIStatus FCommandList::SetUAVBarriersForTexture(IArdaRHITexture& Texture, bool bEnabled)
		{
			auto* Native = Cast<FTexture>(&Texture);
			if (!Native || !RetainOwned(Native))
			{
		...
		FArdaRHIStatus FCommandList::SetUAVBarriersForBuffer(IArdaRHIBuffer& Buffer, bool bEnabled)
		{
			auto* Native = Cast<FBuffer>(&Buffer);
			if (!Native || !RetainOwned(Native))
			{
		...
		FArdaRHIStatus FCommandList::AliasingBarrier(IArdaRHIResource* ResourceBefore, IArdaRHIResource* ResourceAfter)
		{
		...
		FArdaRHIStatus FCommandList::ClearTextureUInt(IArdaRHITexture& Texture,
		    const FArdaRHITextureSubresourceRange& Range,
		    uint32_t Value)
		{
			auto* Native = Cast<FTexture>(&Texture);
		...
		FArdaRHIStatus FCommandList::ClearDepthStencilTexture(IArdaRHITexture& Texture,
		    const FArdaRHITextureSubresourceRange& Range,
		    bool bClearDepth,
		    float Depth,
		    bool bClearStencil,
		...
		FArdaRHIStatus FCommandList::ResolveBindings(const eastl::vector<FArdaRHIBindingSetRef>& Bindings,
		    eastl::vector<FArdaProviderObjectRef>& OutBindings) const
		{
		...
		FArdaRHIStatus FCommandList::DrawIndirect(IArdaRHIBuffer& Arguments,
		    uint64_t Offset,
		    uint32_t DrawCount,
		    uint32_t Stride)
		{
		...
		FArdaRHIStatus FCommandList::DrawIndexedIndirect(IArdaRHIBuffer& Arguments,
		    uint64_t Offset,
		    uint32_t DrawCount,
		    uint32_t Stride)
		{
		...
		FArdaRHIStatus FCommandList::DispatchIndirect(IArdaRHIBuffer& Arguments, uint64_t Offset)
		{
			auto* Buffer = Cast<FBuffer>(&Arguments);
		...
		FArdaRHIStatus FCommandList::SetMeshletState(const FArdaRHIMeshletState& State)
		{
			if (mDevice->GetCapabilities().mMeshShaderTier == EArdaRHIMeshShaderTier::None)
			{
		...
		FArdaRHIStatus FCommandList::DispatchMesh(uint32_t GroupsX, uint32_t GroupsY, uint32_t GroupsZ)
		{
			if (mDevice->GetCapabilities().mMeshShaderTier == EArdaRHIMeshShaderTier::None)
			{
				return Unsupported("Mesh shaders are unsupported by this device.");
		...
		FArdaRHIStatus FCommandList::SetRayTracingState(const FArdaRHIRayTracingState& State)
		{
			if (!mDevice->GetCapabilities().mRayTracing.mbPipelineShaders)
			{
		...
		FArdaRHIStatus FCommandList::DispatchRays(uint32_t Width, uint32_t Height, uint32_t Depth)
		{
			if (!mDevice->GetCapabilities().mRayTracing.mbPipelineShaders)
			{
				return Unsupported("Ray tracing is unsupported by this device.");
		...
		FArdaRHIStatus FCommandList::DispatchRaysIndirect(IArdaRHIBuffer& Arguments, uint64_t Offset)
		{
		...
		FArdaRHIStatus FCommandList::BuildBottomLevelAccelStruct(IArdaRHIAccelStruct& Resource,
		    const eastl::vector<FArdaRHIRayTracingGeometryDesc>& Geometries,
		...
		FArdaRHIStatus FCommandList::BuildTopLevelAccelStruct(IArdaRHIAccelStruct& Resource,
		    ...
		    EArdaRHIAccelStructBuildFlags Flags)
		{
			...
			{
				auto* BottomLevel = Cast<FAccelStruct>(Instance.mBottomLevelAccelStruct.Get());
		...
		FArdaRHIStatus FCommandList::BuildTopLevelAccelStructFromBuffer(IArdaRHIAccelStruct& Resource,
		...
		FArdaRHIStatus FCommandList::CompactAccelStruct(IArdaRHIAccelStruct& DestinationResource,
		...
		FArdaRHIStatus FCommandList::BuildOpacityMicromap(IArdaRHIOpacityMicromap& Resource)
		{
		...
		FArdaRHIStatus FCommandList::CompactOpacityMicromap(IArdaRHIOpacityMicromap& DestinationResource,
		...
		TArdaRHIResult<FArdaRHIResourceStateSnapshot> FCommandList::QueryOpacityMicromapState(
		...
		FArdaRHIStatus FCommandList::DispatchShaderBundle(IArdaRHIShaderBundle& Resource)
		{
		...
		FArdaRHIStatus FCommandList::DispatchWorkGraph(IArdaRHIWorkGraphPipeline& Resource,
		    const void* Records,
		    ...
		    const eastl::vector<FArdaRHIBindingSetRef>& BindingRefs)
		{
			...
			return mNative->DispatchWorkGraph(Pipeline->mNative, Records, RecordCount, RecordStride, Bindings);
		}

		FArdaRHIStatus FCommandList::ClearSamplerFeedbackTexture(IArdaRHISamplerFeedbackTexture& Resource)
		{
		...
		FArdaRHIStatus FCommandList::DecodeSamplerFeedbackTexture(IArdaRHITexture& DestinationResource,
		    IArdaRHISamplerFeedbackTexture& FeedbackResource,
		    EArdaRHIFormat Format)
		{
			auto* Destination = Cast<FTexture>(&DestinationResource);
			auto* Feedback = Cast<FSamplerFeedbackTexture>(&FeedbackResource);
		...
		FArdaRHIStatus FCommandList::SetSamplerFeedbackTextureState(IArdaRHISamplerFeedbackTexture& Resource,
		    EArdaRHIResourceState State)
		{
			auto* Feedback = Cast<FSamplerFeedbackTexture>(&Resource);
			if (!Feedback || !RetainOwned(Feedback))
	...
	{
		return Device ? FArdaRHIDeviceRef(new FArdaRHIDeviceImpl(eastl::move(Device))) : FArdaRHIDeviceRef{};
	}
}
