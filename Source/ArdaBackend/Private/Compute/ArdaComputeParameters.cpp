...
namespace arda
{
    FArdaComputeParameterMetadata::FArdaComputeParameterMetadata(
        const char* Name, size_t Size, size_t Alignment, eastl::vector<FArdaComputeParameterMember> Members)
        : mName(Name), mSize(Size), mAlignment(Alignment), mMembers(eastl::move(Members))
    {
        ...
        { Invalid("Invalid compute parameter struct layout."); return; }
        for (size_t I = 0; I < mMembers.size(); ++I)
        {
            ...
            if (!M.mName || !*M.mName || !M.mCppType || !*M.mCppType ||
                ...
                M.mOffset % M.mAlignment || !M.mElementCount || !M.mElementStride ||
                M.mElementStride % M.mAlignment || M.mElementCount > SIZE_MAX / M.mElementStride ||
                M.mSize != M.mElementCount * M.mElementStride || M.mOffset > Size || M.mSize > Size - M.mOffset)
            { Invalid("Invalid compute parameter member layout."); return; }
            for (size_t J = 0; J < I; ++J)
            {
                ...
                { Invalid("Compute parameter members overlap or duplicate a name."); return; }
            }
            ...
            if (M.mKind == EArdaComputeParameterKind::NestedStruct)
            {
                ...
                { Invalid("Invalid nested compute parameter metadata."); return; }
            ...
            else
            {
                ...
                { Invalid("Leaf compute parameters cannot carry nested metadata."); return; }
                if (M.mKind == EArdaComputeParameterKind::Buffer)
                { ExpectedSize = sizeof(FArdaComputeBufferParameter); ExpectedAlignment = alignof(FArdaComputeBufferParameter); }
                if (M.mKind == EArdaComputeParameterKind::Texture)
                { ExpectedSize = sizeof(FArdaComputeTextureParameter); ExpectedAlignment = alignof(FArdaComputeTextureParameter); }
            }
            if (M.mElementStride != ExpectedSize || M.mAlignment != ExpectedAlignment)
            { Invalid("Compute parameter semantic kind does not match its storage layout."); return; }
        }
    }

    const FArdaComputeParameterMember* FArdaComputeParameterMetadata::FindMember(const char* Name) const noexcept
    {
        if (Name) for (const auto& Member : mMembers)
            if (Member.mName && !std::strcmp(Member.mName, Name)) return &Member;
        return nullptr;
    }

    void FArdaComputeParameterMetadata::EnumerateInternal(const uint8_t* Root, size_t Offset,
        const eastl::string& Prefix, const FArdaComputeParameterVisitor& Visitor) const
    {
        ...
        const FArdaComputeParameterVisitor& Visitor) const
    {
        ...
        if (!Parameters || reinterpret_cast<uintptr_t>(Parameters) % mAlignment || !Visitor)
            return FArdaRHIStatus::Error(EArdaRHIResult::InvalidArgument, "Compute parameter instance or visitor is invalid.");
        ...
        eastl::vector<FArdaComputeResourceAccess>& OutAccesses) const
    {
        ...
        auto Status = Enumerate(Parameters, [&](const FArdaComputeParameter& Parameter)
        {
            const auto Kind = Parameter.mMember->mKind;
            if (Kind != EArdaComputeParameterKind::Buffer && Kind != EArdaComputeParameterKind::Texture) return;
        ...
        if (Status) OutAccesses = eastl::move(Accesses);
        return Status;
    }
}
