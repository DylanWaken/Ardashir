...
namespace arda
{
    ...
    FArdaCudaParameterMetadata::FArdaCudaParameterMetadata(const char* Name, size_t HostSize,
        size_t HostAlignment, FArdaCudaKernelSignature Signature, bool HostLayoutSupported,
        eastl::vector<FArdaCudaParameterMember> Members)
        : mSignature(Signature), mMembers(eastl::move(Members)), mHost(Name, HostSize, HostAlignment, HostMembers(mMembers))
    {
        mStatus = mHost.GetStatus();
        if (!mStatus) return;
        const auto Invalid = [&](const char* Reason) { mStatus = FArdaRHIStatus::Error(EArdaRHIResult::InvalidArgument, Reason); };
        ...
        { Invalid("CUDA parameter schema requires supported host layout and plain CUDA argument storage."); return; }
        for (size_t I = 0; I < mMembers.size(); ++I)
        {
            const auto& M = mMembers[I];
            if (!M.mbSupported || !M.mCudaSize || M.mCudaOffset > Signature.mSize ||
                M.mCudaSize > Signature.mSize - M.mCudaOffset || !M.mElementSize ||
                !M.mElementAlignment || (M.mElementAlignment & (M.mElementAlignment - 1)))
            { Invalid("CUDA parameter member has an unsupported type or invalid layout."); return; }
            if (M.mKind == EArdaComputeParameterKind::Value && M.mHostSize != M.mCudaSize)
            { Invalid("CUDA scalar value does not match its device representation."); return; }
            if (M.mKind != EArdaComputeParameterKind::Value && M.mCudaSize != sizeof(uint64_t))
            { Invalid("CUDA resource representation must occupy 64 bits."); return; }
            if (M.mKind == EArdaComputeParameterKind::Texture)
            {
                ...
                { Invalid("CUDA surface element does not match a supported storage format."); return; }
            }
    ...
    FArdaRHIStatus FArdaCudaParameterMetadata::Prepare(const void* Parameters, FArdaCudaDispatch& Output) const
    {
        ...
        if (!Parameters || reinterpret_cast<uintptr_t>(Parameters) % mHost.GetAlignment())
            return FArdaRHIStatus::Error(EArdaRHIResult::InvalidArgument, "CUDA host parameter storage is null or misaligned.");
        ...
        const auto Fail = [](const char* Reason) { return FArdaRHIStatus::Error(EArdaRHIResult::InvalidArgument, Reason); };
        for (const auto& M : mMembers)
        {
            if (M.mKind == EArdaComputeParameterKind::Value)
            { std::memcpy(K.mParameters.data() + M.mCudaOffset, Bytes + M.mHostOffset, M.mCudaSize); continue; }
            ...
            if (M.mKind == EArdaComputeParameterKind::Buffer)
            {
                ...
                if (!P.mBuffer) return Fail("CUDA buffer parameter is unbound.");
                const auto& D = P.mBuffer->GetDesc();
                const auto& R = P.mRange;
                if (R.mByteOffset >= D.mByteSize || R.mByteOffset % M.mElementAlignment ||
                    (R.mByteSize != ArdaRHIWholeBuffer && (!R.mByteSize || R.mByteSize > D.mByteSize - R.mByteOffset)) ||
            ...
            else if (M.mKind == EArdaComputeParameterKind::Texture)
            {
                ...
                if (!P.mTexture) return Fail("CUDA surface parameter is unbound.");
                ...
                if (Raw.mBaseMipLevel >= D.mMipLevels || Raw.mBaseArraySlice >= D.mArraySize || Raw.mBasePlane != 0 ||
                    (Raw.mMipLevelCount != ArdaRHIAllSubresources && (!Raw.mMipLevelCount || Raw.mMipLevelCount > D.mMipLevels - Raw.mBaseMipLevel)) ||
                    (Raw.mArraySliceCount != ArdaRHIAllSubresources && (!Raw.mArraySliceCount || Raw.mArraySliceCount > D.mArraySize - Raw.mBaseArraySlice)) ||
            ...
            else return Fail("Unsupported CUDA parameter resource kind.");
            ...
            if (!Binding.mResource->GetCudaResourceInfo().mbSharingEnabled)
                return FArdaRHIStatus::Error(EArdaRHIResult::Unsupported, "CUDA parameter resource was not created with CUDA sharing.");
            K.mPatches.push_back({static_cast<uint32_t>(Prepared.mBindings.size()), M.mCudaOffset,
