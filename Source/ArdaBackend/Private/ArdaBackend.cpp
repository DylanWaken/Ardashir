...
namespace arda
{
	...
	namespace
	{
		...
		EArdaInitializeResult CreateConfiguredDevice(FArdaBackendState& State,
		    ...
		    IArdaWindowSurface* WindowSurface)
		{
			const IArdaExternalDeviceProvider* ExternalProvider =
			    Configuration.mDeviceSource == EArdaDeviceSource::ExternalProvider ? State.mExternalDeviceProvider
			                                                                       : nullptr;
			FArdaBackendDeviceCreateResult Result = Module.CreateDevice(Configuration, WindowSurface, ExternalProvider);
			if (!Result)
			{
			...
			State.mDevice = arda::CreateArdaRHIDevice(eastl::move(Result.mProviderDevice));
			if (!State.mDevice)
			{
	    ...
	    eastl::unique_ptr<IArdaSwapChain>& OutSwapChain)
	{
		...
		const EArdaInitializeResult result =
		    CreateConfiguredDevice(state, runtimeConfiguration, *Module, &WindowSurface);
		...
		FArdaSwapChainCreateResult SwapChainResult =
		    state.mBackendRuntime->CreateSwapChain(Width, Height, state.mDevice);
		if (!SwapChainResult)
		{
