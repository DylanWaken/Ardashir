...
TEST(ArdaTrace, RecordsNestedScopesCountersAndMarkers)
{
	...
	EXPECT_STREQ(Session.mThreads.at(Session.mScopes.front().mThreadId).c_str(), "Test Thread");
	...
	EXPECT_STREQ(Session.mNames.at(Session.mCounters.front().mNameId).c_str(), "Visible Objects");
	EXPECT_DOUBLE_EQ(Session.mCounters.front().mValue, 42.0);
	EXPECT_STREQ(Session.mNames.at(Session.mMarkers.front().mNameId).c_str(), "Dispatch");
