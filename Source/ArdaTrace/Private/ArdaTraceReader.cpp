...
namespace arda
{
	namespace
	{
		...
		bool ReadValue(std::ifstream& Stream, ValueType& OutValue)
		{
			return static_cast<bool>(
			    Stream.read(reinterpret_cast<char*>(&OutValue), static_cast<std::streamsize>(sizeof(ValueType))));
		}
		...
		bool ReadString(std::ifstream& Stream, eastl::string& OutValue)
		{
			...
			return Length == 0 || static_cast<bool>(Stream.read(OutValue.data(), Length));
		}
		...
		bool ValidateReferences(const FArdaTraceSession& Session, eastl::string& OutError)
		{
			...
			{
				if (Session.mNames.count(Scope.mNameId) == 0 || Session.mThreads.count(Scope.mThreadId) == 0)
				{
				...
				if (Scope.mScopeId == 0 || !ScopesById.emplace(Scope.mScopeId, &Scope).second)
				{
			...
			for (const FArdaTraceScope& Scope : Session.mScopes)
			{
				...
				while (CurrentScope->mParentScopeId != 0)
				{
					...
					if (Parent == ScopesById.end() || Parent->second->mThreadId != Scope.mThreadId ||
			...
			{
				if (Session.mNames.count(Counter.mNameId) == 0 || Session.mThreads.count(Counter.mThreadId) == 0 ||
			...
			{
				if (Session.mNames.count(Marker.mNameId) == 0 || Session.mThreads.count(Marker.mThreadId) == 0)
				{
	...
	bool ReadTraceCapture(const std::filesystem::path& FilePath, FArdaTraceSession& OutSession, eastl::string& OutError)
	{
		...
		if (!Stream.read(Magic.data(), static_cast<std::streamsize>(Magic.size())) || !ReadValue(Stream, Version) ||
		    !ReadValue(Stream, EndianMarker) || !ReadValue(Stream, Session.mOriginNanoseconds))
		{
		...
		while (!bFoundCaptureEnd)
		{
			...
			case arda::EArdaTraceRecordType::Scope:
			{
				...
				if (!ReadValue(Stream, Scope.mThreadId) || !ReadValue(Stream, Scope.mNameId) ||
				    !ReadValue(Stream, Scope.mScopeId) || !ReadValue(Stream, Scope.mParentScopeId) ||
				    !ReadValue(Stream, Scope.mStartNanoseconds) || !ReadValue(Stream, Scope.mEndNanoseconds))
				{
			...
			case arda::EArdaTraceRecordType::Counter:
			{
				...
				if (!ReadValue(Stream, Counter.mThreadId) || !ReadValue(Stream, Counter.mNameId) ||
				    !ReadValue(Stream, Counter.mTimestampNanoseconds) || !ReadValue(Stream, Counter.mValue))
				{
			...
			case arda::EArdaTraceRecordType::Marker:
			{
				...
				if (!ReadValue(Stream, Marker.mThreadId) || !ReadValue(Stream, Marker.mNameId) ||
