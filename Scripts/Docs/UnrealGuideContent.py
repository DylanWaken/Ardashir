...
STAGES = [
    ...
    stage("views", "Views, relevance & draw setup", "setup", 1,
        ...
        [("deferred", "BeginInitViews("), ("visibility", "TaskDatas.VisibilityTaskData->StartGatherDynamicMeshElements();", 5950)],
...
FAQ = [
    ...
    ("What coverage does this guide promise?", "The scene page lists semantic input requirements, major geometry/material families and explicit conversion choices for an independent renderer. It is not a complete plugin export ABI or a guarantee that arbitrary Unreal material graphs can be reduced to a few textures. Unsupported producers and material behavior need a named policy and coverage report. The older declaration appendix is a scoped lexical index, not a compiler AST. Stage function lists are selected source references; operation flowcharts summarize prerequisites rather than every call or runtime branch."),
