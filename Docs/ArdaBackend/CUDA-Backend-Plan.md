...
The compiled example is split between `ArdaTestComputeOperand.h/.cpp`, `ArdaCudaOperand.cu`, `ArdaCudaOperandFinish.cu`, `ArdaCudaOperandMath.cuh` and `ArdaCudaOperandValue.h` in `Source/ArdaBackend/Tests`. `FArdaAddOperand` explicitly records +7 then +11, choosing a block size inside DispatchDeferred from typed shape vectors. An optional graphics callback implements the same semantics. Immediate Dispatch in this sample chooses to create/submit its own RHI list and exposes its submission ID. Its mutable diagnostics require caller serialization.
