...
- CUDA-enabled backend/public-header/RDG targets and CUDA-disabled backend/public-header targets build successfully. After the provider ABI version update, the focused CUDA/operand/provider suite passes 26 tests with one D3D12 CiG surface capability skip; the CUDA-disabled suite passes all 11 tests.
